...
----
+
. In the `/src/main/resources/mule-app.properties` folder, create a file named `OAuth_Authorization_Code_Config` that contains the following variables:
+
----
...
. In the *Name* field, enter `Microsoft_Teams_Client_Credentials_Config`.
...
. In the *Name* field, enter `OAuth_Authorization_Code_Config`.
. Enter the required field values and click *OK*.
