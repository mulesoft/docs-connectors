...
Create a third flow to retrieve information about members of the new team. Use the *List team members* operation in this flow:

.Use a flow like this one to retrieve the members of the new team.
...
. After the app deploys, open a web browser and initiate a flow by entering the associated URL. If the URL has query parameters, make sure that you include values for the parameters:
+
[%header%autowidth.spread]
|===
| Flow | URL
