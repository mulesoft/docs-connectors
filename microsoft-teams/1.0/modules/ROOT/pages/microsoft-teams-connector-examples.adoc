...
* <<retrieve-team,Retrieve team members>>
...
. In the *Mule Palette* view, search for *teams* and select the Microsoft Teams *Get team* operation:
...
Create a fifth flow to onboard a new hire to the team. It adds a new member to the existing team, creates a new channel in the team, adds the member to the created channel, and posts a `Welcome` message in the channel:
...
. After the app deploys, open a web browser and initiate a flow by entering the associated URL.
+
[%header%autowidth.spread]
|===
| Flow | URL
