...
. Create a file named `OAuth_Authorization_Code_Config` like the one that follows. Overlay the italicized text with the correct values for your OAuth configuration:
+
----
config.oauth.authorization.code.consumerKey=_consumerKey_
config.oauth.authorization.code.consumerSecret=_consumerSecret_
config.oauth.authorization.code.authorizationUrl=_authorizationCode
config.oauth.authorization.code.accessTokenUrl=_accessTokenUrl_
----
+
. Place the `OAuth_Authorization_Code_Config` file in the `/src/main/resources/mule-app.properties` folder.
. Create a file named `OAuth_Client_Credentials_Config` like the one that follows. Overlay the italicized text with the correct values for your OAuth configuration:
+
----
config.oauth.client.credentials.clientId=_clientId_
config.oauth.client.credentials.clientSecret=_clientSecret_
config.oauth.client.credentials.tokenUrl=_tokenUrl_
----
+
