...
----
+
. In the `mule-app.properties` file, create a set of properties for the `Client Credentials Grant Type` like the one that follows. Overlay the bracketed text (including the brackets) with the correct values for your OAuth configuration:
+
----
...
=== Create a Global Element for the Properties File

Create a global element for the `mule-app.properties` file:
