= Amazon Lambda Connector 1.2 Reference
...
| Custom STS Endpoint a| String |  Sets a custom STS endpoint. Useful when a non-standard service endpoint is required, such as a VPC endpoint. The custom STS endpoint might not work if `DefaultAWSCredentialsProviderChain` is used. Instead, use AWS environment variables such as `AWS_STS_REGIONAL_ENDPOINTS`, `AWS_REGION`, and `AWS_ENDPOINT_URL`. |  |
