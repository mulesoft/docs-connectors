...
| Receive Buffer Size a| Number |  Size of the buffer used when receiving data, in bytes. This is set on the socket, itself. |  |
...
| Reuse Address a| Boolean |  If `true`, SO_REUSEADDRESS is set on the socket before binding. This helps reduce `address already in use` errors when a socket is reused. | true |
| Send TCP With No Delay a| Boolean |  If `true`, transmitted data is sent immediately instead of collected together for greater efficiency. This value defaults to `true` even though the socket default is false because optimizing to reduce amount of network traffic over latency is not usually concern today. | true |
...
| Type a| String | The type of keystore. |  |
...
| Peer Certificates a| Array of Any | SSL peer certificates. If the socket is not an SSL sockets, this value is null. |  |
| Port a| Number |  |  |
|===
...
|===
| Field | Type | Description | Default Value | Required
| Max Active a| Number | Controls the maximum number of Mule components that are borrowed from a session at one time. When set to a negative value, there is no limit to the number of components that are active at one time. When this value is exceeded, the pool is considered exhausted. |  |
...
+
If you supply a postive *Max Wait* value, it blocks for at most that many milliseconds, after which a `NoSuchElementException` is thrown. If *Max Wait* is a negative value, it blocks indefinitely. |  |
