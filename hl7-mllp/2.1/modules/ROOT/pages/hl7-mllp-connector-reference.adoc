...
|Connection Count |Integer | Number of connections accepted at one time. The value can be an integer or DataWeave expression. An empty value, negative value, or value of `0` indicates that the connector can accept an unlimited number of connections. | |
