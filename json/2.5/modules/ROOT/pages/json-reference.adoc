...
[NOTE] 
====
Draft 06 and later are handled by a different validation engine than Draft 03 and Draft 04. When validation fails, the Draft 03 or Draft 04 engine collects every constraint violation across the entire document into the error report, so validating a large payload that fails can produce a very large report and cause high transient memory usage.

If you validate large payloads and your schema doesn't require Draft 03 or Draft 04 specific behavior, declare a Draft 06 or later `$schema` (for example, `"$schema": "\http://json-schema.org/draft-07/schema#"`) so that validation uses the more memory-efficient engine.

Schemas generated from RAML typically omit `$schema` and therefore default to the Draft 04 path.
====
