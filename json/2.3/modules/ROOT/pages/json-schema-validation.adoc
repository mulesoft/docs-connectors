...
In the following example, you configure the *Schema Content* field of the the *Validate schema* operation. The *Schema Contents* field defines a schema content to validate. 

. In Studio, select the *Validate Schema* operation from your flow.
. In the *Validate Schema* operation configuration screen, set *Schema* to your JSON schema file, for example `schemaJson.json`.
. Set *Schema Content* to *Edit inline*.

image::json-schema-content-1.png[Schema content field set to Edit inline in the Validate schema configuration screen]
[start=3]
...
----
image::json-schema-content-2.png[Schema content window with Schema name and Schema text fields]
