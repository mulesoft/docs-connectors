...
*Reviewers, should "the client" in the previous sentence be changed to "the producer"?*
{sp} +
{sp} +
...
| Maximum in flight requests a| Number |  The maximum number of unacknowledged requests the client sends on a single connection before blocking. If this setting is greater than `1` and there are failed sends, there is a risk of message re-ordering due to retries (for example, if retries are enabled). *Reviewers, should "the client" in the previous sentence be changed to "the producer"?*|  5 |
