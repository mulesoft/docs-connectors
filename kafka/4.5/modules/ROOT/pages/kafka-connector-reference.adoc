...
== Notes: When a Message Listener or a Batch Message Listener is used in Acknowledgement mode MANUAL, it is necessery whenever a flow finishes successfully (either because no error happened, or because of going into On error continue) the user has to Commit in the end of the flow
