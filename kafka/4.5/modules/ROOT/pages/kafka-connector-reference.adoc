...
[NOTE]
When a Message Listener or a Batch Message Listener is used in MANUAL mode in the Acknowledgement, you must commit at the end of the flow when a flow finishes successfully (either because no error happened or because there is an On Error Continue component in the flow).
