...
| Maximum polling interval a| Number |  The maximum delay between invocations of poll() when using consumer group management. This places an upper bound on the amount of time that the consumer can be idle before fetching more records. If poll() is not called before expiration of this timeout, then the consumer is considered failed and the group rebalances in order to reassign the partitions to another member. For consumers using a non-null `group.instance.id` which reach this timeout, partitions are not be immediately reassigned. Instead, the consumer stops sending heartbeats and partitions are reassigned after expiration of `session.timeout.ms`. |  `300` |
