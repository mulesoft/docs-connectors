...
* Configuring the Confluent Schema Registry in the Kakfa Connector connection using the *Additional Properties* field for the *Consumer* operation.
...
. Set *Key* to `key.deserializer` and  *Value* to `com.mulesoft.connectors.kafka.internal.model.serializer.MuleKafkaAvroDeserializer`.
. Repeat step 9 to add all the required Confluent Schema Registry properties.

Optionally, you can configure these system properties `maxCacheSize`, `maxCacheTTL` for the key `mule.kafka.muleKafkaAvroDeserializer`:

[%header,cols="30s,50a,20a"]
|===
| Property | Description | Default
| `mule.kafka.muleKafkaAvroDeserializer.maxCacheSize`| The maximum number of schemas to store in cache. | 200
| `mule.kafka.muleKafkaAvroDeserializer.maxCacheTTL`| The time-to-live (TTL) in minutes that a schema remains in cache when not being used. | 30 minutes
|===
...
<kafka:additional-properties >
  <kafka:additional-property key="key.deserializer" value="com.mulesoft.connectors.kafka.internal.model.serializer.MuleKafkaAvroDeserializer" />
  <kafka:additional-property key="value.deserializer" value="com.mulesoft.connectors.kafka.internal.model.serializer.MuleKafkaAvroDeserializer" />
