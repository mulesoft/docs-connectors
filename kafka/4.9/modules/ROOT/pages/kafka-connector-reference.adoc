...
* For serialization, set *Key* to `key.serializer` and *Value* to `io.confluent.kafka.serializers.KafkaAvroSerializer`.
* For custom serialization, set *Key* to `value.serializer` and *Value* to `com.mulesoft.connectors.kafka.internal.model.serializer.MuleKafkaAvroSerializer`.
* For deserialization, set *Key* to `key.deserializer` and *Value* to `io.confluent.kafka.serializers.KafkaAvroDeserializer`.
