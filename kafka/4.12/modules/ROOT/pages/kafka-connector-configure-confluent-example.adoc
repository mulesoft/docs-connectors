...
Optionally, you can configure these system properties `maxCacheSize`, `maxCacheTTL` for the key `mule.kafka.muleKafkaAvoDeserializer`:

[%header,cols="30s,50a,20a"]
|===
| Property | Description | Default
| `mule.kafka.muleKafkaAvoDeserializer.maxCacheSize`| The maximum number of schemas to store in cache. | 200
| `mule.kafka.muleKafkaAvoDeserializer.maxCacheTTL`| The time-to-live (TTL) in minutes that a schema remains in cache when not being used. | 30 minutes
|===
