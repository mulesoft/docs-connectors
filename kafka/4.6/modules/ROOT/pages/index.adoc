...
* Configuring the offset value to a set value or the beginning or the end of the topic.
...
* Ensuring there is no data loss when the downstream application crashes and manually configuring commits to have more control.
* Storing a commit value in a local store or in the Kafka storage and blocking until the store is successful.
* Storing an offset in a local store or in the Kafka storage asynchronously.
