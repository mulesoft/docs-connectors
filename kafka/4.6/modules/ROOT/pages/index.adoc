...
* Users publishing binary data to the Kafka server and the underlying topic of their choice.
* Integration specialists working with large volumes of data who want to set the batch size of the data to publish to Kafka.
...
* Users focusing on optimal performance who want to configure the message size that they send.
...
* Users ensuring successful data transmission and want to block further commits on a configurable level of acknowledgment for the messages published to the Kafka server.
...
* Users configuring how data is serialized during a publish.
* Users securing messages sent during Mutual TLS.
...
* Users subscribing to the Kafka server and the underlying group of their choice to read and consume messages for downstream processing.
* Users subscribing to a specific Kafka partition to process messages.
...
* Users, on restart, wanting to read the offset value from an alternate storage system.
...
* Users who sometimes run into their downstream application crashing and want to ensure there is no data loss. To have more control, they want to manually configure their commits.
* Users who want to store their commit value in a local store or in the Kafka storage and block until the store is successful.
* Users who want to store their offset in a local store or in the Kafka storage asynchronously.
* Users who have multiple applications reading from the same group ID and want to use Kafka's in-built capability of load-balancing.
* Users who want to configure the number of records they process in a single call.
