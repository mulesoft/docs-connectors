...
Since Kafka Connector version 3.0.0, the default value for the clientDNSLookUp field was removed to keep the connector backwards compatible. Kafka Connector keeps the same values as the previous version but now each value corresponds to the following properties:
...
== Troubleshooting Publishing New Values
