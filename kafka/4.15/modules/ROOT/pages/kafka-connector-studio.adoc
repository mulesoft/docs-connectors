...
|Consumer SASL/AWS IAM | AWS Region | AWS Region where the Amazon MSK cluster is located
...
|Producer SASL/AWS IAM | AWS Region | AWS Region where the Amazon MSK cluster is located
...
. Drag the *Bulk Publish* operation to the Studio canvas.
. Configure the *Bulk Publish* operation in the *General* tab:
+
[%header,cols="20s,20a,35a,20a,5a"]
|===
| Name | Type | Description | Default Value | Required
| Configuration | String | The name of the configuration to use. | | x
| Messages | Array of KafkaMessage | The list of messages to publish. | `#[payload]` |
|===
+
. Configure the following settings in the *Advanced* tab:
+
...
For an example that builds the required `KafkaMessage` array and publishes multiple messages, see xref:kafka-connector-bulk-publish-example.adoc[Publish Multiple Messages with Bulk Publish].
