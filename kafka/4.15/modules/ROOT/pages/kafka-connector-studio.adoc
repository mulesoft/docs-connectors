= Using Anypoint Studio to Configure Apache Kafka 4.15
...
|Consumer SASL/AWS IAM | Access Key ID | AWS access key ID of the principal that can assume the IAM role
| | Secret Access Key | AWS secret access key of the principal that can assume the IAM role
| | Session Token | AWS session token for temporary caller credentials
| | Role ARN | ARN of the IAM role to assume for Amazon MSK access through AWS Security Token Service
| | Role Session Name | Name attached to the assumed-role session
| | AWS Region | AWS Region where the Amazon MSK cluster is located
| | Try Default AWS Credentials Provider Chain | Use the AWS default credentials provider chain instead of an explicit access key and secret access key
...
|Producer SASL/AWS IAM | Access Key ID | AWS access key ID of the principal that can assume the IAM role
| | Secret Access Key | AWS secret access key of the principal that can assume the IAM role
| | Session Token | AWS session token for temporary caller credentials
| | Role ARN | ARN of the IAM role to assume for Amazon MSK access through AWS Security Token Service
| | Role Session Name | Name attached to the assumed-role session
