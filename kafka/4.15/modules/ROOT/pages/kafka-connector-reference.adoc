= Apache Kafka Connector 4.15 Reference
...
* <<consumer-sasl-aws-iam, Consumer SASL/AWS IAM Connection>>
...
[[consumer-sasl-aws-iam]]
=== Consumer SASL/AWS IAM Connection Type

Use AWS Identity and Access Management (IAM) authentication to connect to an Amazon MSK cluster.

[%header,cols="20s,25a,30a,15a,10a"]
|===
| Name | Type | Description | Default Value | Required
| Bootstrap Server URLs a| Array of String | List of servers to bootstrap the connection with the Kafka cluster. | | x
| Endpoint identification algorithm | String | Endpoint identification algorithm used by clients to validate the server hostname. The default value is an empty string, which means the endpoint identification algorithm is disabled. | |
| Group ID a| String | Default group ID for all Kafka consumers that use this configuration. | |
| Consumer Amount a| Number | Number of consumers that the connection initially creates. | `1` |
| Maximum polling interval a| Number | Maximum delay between invocations of `poll()` when using consumer group management. This places an upper bound on the time that a consumer can be idle before fetching more records. If `poll()` isn't called before this timeout expires, the consumer is considered failed and the group rebalances to reassign the partitions to another member. For consumers using a non-null `group.instance.id` that reach this timeout, partitions aren't immediately reassigned. Instead, the consumer stops sending heartbeats and partitions are reassigned after `session.timeout.ms` expires. | `60` |
| Maximum Polling Interval Time Unit a| Enumeration, one of: `NANOSECONDS`, `MICROSECONDS`, `MILLISECONDS`, `SECONDS`, `MINUTES`, `HOURS`, `DAYS` | Time unit for the *Maximum polling interval* field. You can override this parameter at the source level. | `SECONDS` |
...
* `READ_COMMITTED`
+
`consumer.poll()` returns only committed transactional messages.

* `READ_UNCOMMITTED`
+
`consumer.poll()` returns all messages, including transactional messages that were aborted.

Non-transactional messages are returned unconditionally in either mode. Messages are returned in offset order. In `READ_COMMITTED` mode, `consumer.poll()` returns messages only up to the last stable offset (LSO), which is one less than the offset of the first open transaction.

Messages after those belonging to ongoing transactions are withheld until the relevant transaction completes. As a result, `READ_COMMITTED` consumers can't read to the high watermark when transactions are in flight. In `READ_COMMITTED` mode, the `seekToEnd` method returns the LSO. | `READ_UNCOMMITTED` |
...
| Retry Backoff Timeout Time Unit a| Enumeration, one of: `NANOSECONDS`, `MICROSECONDS`, `MILLISECONDS`, `SECONDS`, `MINUTES`, `HOURS`, `DAYS` | Time unit for the *Retry Backoff Timeout* field. | `MILLISECONDS` |
...
| Default receive buffer size unit a| Enumeration, one of: `BYTE`, `KB`, `MB`, `GB` | Unit of measure for the *Default receive buffer size* field. | `KB` |
...
| Default send buffer size unit a| Enumeration, one of: `BYTE`, `KB`, `MB`, `GB` | Unit of measure for the *Default send buffer size* field. | `KB` |
| Request Timeout a| Number | Maximum amount of time the client waits for the response of a request. If the response isn't received before the timeout elapses, the client resends the request if necessary, or fails the request if the retries are exhausted. You can override this parameter at the source level. | `30` |
| Request Timeout Time Unit a| Enumeration, one of: `NANOSECONDS`, `MICROSECONDS`, `MILLISECONDS`, `SECONDS`, `MINUTES`, `HOURS`, `DAYS` | Time unit for the *Request Timeout* field. | `SECONDS` |
...
| DNS Lookups a| Enumeration, one of: `DEFAULT`, `USE_ALL_DNS_IPS`, `RESOLVE_CANONICAL_BOOTSTRAP_SERVERS_ONLY` | Controls how the client uses DNS lookups. If set to `USE_ALL_DNS_IPS`, when the lookup returns multiple IP addresses for a hostname, a connection is attempted to all of the IP addresses before the connection fails. This applies to both bootstrap and advertised servers. If the value is `RESOLVE_CANONICAL_BOOTSTRAP_SERVERS_ONLY`, each entry is resolved and expanded into a list of canonical names. | `USE_ALL_DNS_IPS` |
| Heartbeat interval a| Number | Expected time between heartbeats to the consumer coordinator when using Kafka's group management facilities. Heartbeats ensure that the consumer's session stays active and facilitate rebalancing when consumers join or leave the group. The value must be lower than `session.timeout.ms`, but typically should be no higher than 1/3 of that value. You can adjust it lower to control the expected time for normal rebalances. | `3` |
| Heartbeat Interval Time Unit a| Enumeration, one of: `NANOSECONDS`, `MICROSECONDS`, `MILLISECONDS`, `SECONDS`, `MINUTES`, `HOURS`, `DAYS` | Time unit for the *Heartbeat interval* field. | `SECONDS` |
| Session timeout a| Number | Timeout used to detect consumer failures when using Kafka's group management facility. The consumer sends periodic heartbeats to indicate that it is active to the broker. If no heartbeats are received before the session timeout expires, the broker removes the consumer from the group and initiates a rebalance. The value must be in the allowable range configured by `group.min.session.timeout.ms` and `group.max.session.timeout.ms`. | `10` |
| Session timeout time unit a| Enumeration, one of: `NANOSECONDS`, `MICROSECONDS`, `MILLISECONDS`, `SECONDS`, `MINUTES`, `HOURS`, `DAYS` | Time unit for the *Session timeout* field. | `SECONDS` |
| Connection maximum idle time a| Number | Close idle connections after the specified number of milliseconds. | `540` |
| Connection maximum idle time time unit a| Enumeration, one of: `NANOSECONDS`, `MICROSECONDS`, `MILLISECONDS`, `SECONDS`, `MINUTES`, `HOURS`, `DAYS` | Time unit for the *Connection maximum idle time* field. | `SECONDS` |
| Additional Properties a| Object | Additional properties used to customize the Kafka connection. | |
| TLS Configuration a| <<Tls>> | Defines a TLS configuration that both clients and servers can use to secure communication for the Mule app. The connector automatically sets the `security.protocol` used for communication. Valid values are `PLAINTEXT`, `SSL`, `SASL_PLAINTEXT`, and `SASL_SSL`. The default is `PLAINTEXT`, or `SASL_PLAINTEXT` for SASL authentication. If the broker uses `SSL`, configure at least the keystore in the `tls:context` child element. The connector then automatically uses `SSL`, or `SASL_SSL` for SASL authentication, as the `security.protocol`. | |
...
| Assignments a| Array of <<TopicPartition>> | List of topic-partition pairs to assign. Consumers aren't automatically rebalanced. | |
...
| Fetch Minimum Size Unit a| Enumeration, one of: `BYTE`, `KB`, `MB`, `GB` | Unit of measure for the *Default fetch minimum size* field. | `BYTE` |
| Default fetch maximum size a| Number | Maximum amount of data the server returns for a fetch request. Records are fetched in batches by the consumer. If the first record batch in the first non-empty partition of the fetch is larger than this value, the record batch is still returned to ensure that the consumer can make progress. As such, this isn't an absolute maximum. The maximum record batch size accepted by the broker is defined using `message.max.bytes` (broker configuration) or `max.message.bytes` (topic configuration). The consumer performs multiple fetches in parallel. You can override this parameter at the source level. | `1` |
| Default maximum fetch size unit a| Enumeration, one of: `BYTE`, `KB`, `MB`, `GB` | Unit of measure for the *Default fetch maximum size* field. | `MB` |
...
| Default maximum partition fetch unit a| Enumeration, one of: `BYTE`, `KB`, `MB`, `GB` | Unit of measure for the *Default maximum partition fetch size* field. | `MB` |
...
| Fetch Maximum Wait Timeout Unit a| Enumeration, one of: `NANOSECONDS`, `MICROSECONDS`, `MILLISECONDS`, `SECONDS`, `MINUTES`, `HOURS`, `DAYS` | Time unit for the *Fetch Maximum Wait Timeout* field. | `MILLISECONDS` |
| Access Key ID a| String | AWS access key ID of the principal that can assume the IAM role. Leave this field empty to use the default credentials provider chain. | |
| Secret Access Key a| String | AWS secret access key of the principal that can assume the IAM role. Leave this field empty to use the default credentials provider chain. | |
| Session Token a| String | AWS session token. Specify this field only when the caller credentials are temporary, such as federated credentials, AWS SSO credentials, or credentials from a previous STS call. | |
| Role ARN a| String | ARN of the IAM role to assume for MSK access through STS `AssumeRole`. This field is optional when using the default credentials provider chain without assuming a role. | |
| Role Session Name a| String | Name attached to the assumed-role session. This name appears in AWS CloudTrail audit logs. | `mule-kafka-connector` |
| AWS Region a| String | AWS Region where the MSK cluster is located. This field also determines STS endpoint routing. | | x
| Try Default AWS Credentials Provider Chain a| Boolean | Use the AWS default credentials provider chain instead of an explicit access key and secret access key. This works only when an AWS identity is attached to the runtime. | `false` |
...
| Additional Properties a| Object a|  Additional properties used to customize the Kafka connection.
...
For more information, refer to the https://docs.confluent.io/platform/current/security/authentication/sasl/scram/overview.html[Confluent] documentation.
...
For more information, refer to the https://docs.confluent.io/platform/current/security/authentication/sasl/plain/overview.html[Confluent] documentation.
...
* <<producer-sasl-aws-iam, Producer SASL/AWS IAM Connection>>
...
[[producer-sasl-aws-iam]]
=== Producer SASL/AWS IAM Connection Type
...
| Bootstrap Server URLs a| Array of String | List of servers to bootstrap the connection with the Kafka cluster. | | x
| Endpoint identification algorithm | String | Endpoint identification algorithm used by clients to validate the server hostname. The default value is an empty string, which means the endpoint identification algorithm is disabled. | |
| Batch size a| Number | Producer attempts to batch records together into fewer requests whenever multiple records are sent to the same partition. | `16` |
| The batch size unit of measure. a| Enumeration, one of: `BYTE`, `KB`, `MB`, `GB` | Unit of measure for the *Batch size* field. | `KB` |
| Buffer size a| Number | Total bytes of memory the producer uses to buffer records waiting to send to the server. | `1000` |
| The buffer memory size unit of measure. a| Enumeration, one of: `BYTE`, `KB`, `MB`, `GB` | Unit of measure for the *Buffer size* field. | `KB` |
| DNS Lookups a| Enumeration, one of: `DEFAULT`, `USE_ALL_DNS_IPS`, `RESOLVE_CANONICAL_BOOTSTRAP_SERVERS_ONLY` | Controls how the client uses DNS lookups. | `USE_ALL_DNS_IPS` |
| Compression type a| Enumeration, one of: `NONE`, `GZIP`, `SNAPPY`, `LZ4`, `ZSTD` | Compression type for all data generated by the producer. | `NONE` |
| Connections maximum idle time a| Number | Close idle connections after the specified time. | `540` |
| Connections maximum idle time unit a| Enumeration, one of: `NANOSECONDS`, `MICROSECONDS`, `MILLISECONDS`, `SECONDS`, `MINUTES`, `HOURS`, `DAYS` | Time unit for the *Connections maximum idle time* field. | `SECONDS` |
| Delivery Timeout a| Number | Upper limit on the time to report success or failure after a call to `send()` returns. | `120` |
| Delivery Timeout Time Unit a| Enumeration, one of: `NANOSECONDS`, `MICROSECONDS`, `MILLISECONDS`, `SECONDS`, `MINUTES`, `HOURS`, `DAYS` | Time unit for the *Delivery Timeout* field. | `SECONDS` |
| Enable idempotence a| Boolean | Ensures that exactly one copy of each message is written in the stream. | `false` |
| Linger time a| Number | Amount of time to wait for additional records before sending a batch. | `0` |
| Linger Time Unit a| Enumeration, one of: `NANOSECONDS`, `MICROSECONDS`, `MILLISECONDS`, `SECONDS`, `MINUTES`, `HOURS`, `DAYS` | Time unit for the *Linger time* field. | `SECONDS` |
| Maximum block time a| Number | Time for which `KafkaProducer.send()` and `KafkaProducer.partitionsFor()` can block. | `60` |
| Maximum block time unit a| Enumeration, one of: `NANOSECONDS`, `MICROSECONDS`, `MILLISECONDS`, `SECONDS`, `MINUTES`, `HOURS`, `DAYS` | Time unit for the *Maximum block time* field. | `SECONDS` |
| Maximum in flight requests a| Number | Maximum number of unacknowledged requests the client sends on a single connection before blocking. | `5` |
| Maximum request size a| Number | Maximum size of a request in bytes. | `1` |
| Maximum request size unit a| Enumeration, one of: `BYTE`, `KB`, `MB`, `GB` | Unit of measure for the *Maximum request size* field. | `MB` |
| Producer Acknowledge Mode a| Enumeration, one of: `NONE`, `LEADER_ONLY`, `ALL` | Number of acknowledgments the producer requires before considering a request complete. | `NONE` |
| Default receive buffer size a| Number | Size of the TCP receive buffer (SO_RCVBUF) to use when reading data. | `64` |
| Default receive buffer size unit a| Enumeration, one of: `BYTE`, `KB`, `MB`, `GB` | Unit of measure for the *Default receive buffer size* field. | `KB` |
| Retries amount a| Number | Number of times the client resends a record whose send fails with a potentially transient error. | `1` |
| Retry Backoff Timeout Time Unit a| Enumeration, one of: `NANOSECONDS`, `MICROSECONDS`, `MILLISECONDS`, `SECONDS`, `MINUTES`, `HOURS`, `DAYS` | Time unit for the *Retry Backoff Timeout* field. | `MILLISECONDS` |
| Retry backoff timeout a| Number | Amount of time to wait before attempting to retry a failed request to a given topic partition. | `100` |
| Default send buffer size a| Number | Size of the TCP send buffer (SO_SNDBUF) to use when sending data. | `128` |
| Default send buffer size unit a| Enumeration, one of: `BYTE`, `KB`, `MB`, `GB` | Unit of measure for the *Default send buffer size* field. | `KB` |
| Default request timeout time unit a| Enumeration, one of: `NANOSECONDS`, `MICROSECONDS`, `MILLISECONDS`, `SECONDS`, `MINUTES`, `HOURS`, `DAYS` | Time unit for the *Default request timeout* field. | `SECONDS` |
| Default request timeout a| Number | Maximum amount of time the client waits for the response to a request. | `30` |
| Partitioner a| Enumeration, one of: `NONE`, `DEFAULT`, `ROUND_ROBIN`, `UNIFORM_STICKY` | Controls the partitioning strategy. | `DEFAULT` |
| Additional Properties a| Object | Additional properties used to customize the Kafka connection. | |
| TLS Configuration a| <<Tls>> | Defines a TLS configuration used to secure communication for the Mule app. | |
| Access Key ID a| String | AWS access key ID of the principal that can assume the IAM role. Leave this field empty to use the default credentials provider chain. | |
| Secret Access Key a| String | AWS secret access key of the principal that can assume the IAM role. Leave this field empty to use the default credentials provider chain. | |
| Session Token a| String | AWS session token. Specify this field only when the caller credentials are temporary, such as federated credentials, AWS SSO credentials, or credentials from a previous STS call. | |
| Role ARN a| String | ARN of the IAM role to assume for MSK access through STS `AssumeRole`. This field is optional when using the default credentials provider chain without assuming a role. | |
...
[%autowidth.spread]
|===
| Type| Array of <<KafkaMessageMetadata>>
|===
...
|===
| Field | Type | Description | Default Value | Required
| Attributes a| <<KafkaMessageAttributes>> | Attributes associated with the Kafka message. |  | 
| Payload a| Binary | Payload of the Kafka message, represented as an InputStream. |  | 
|===
...
|===
| Field | Type | Description | Default Value | Required
| Headers a| Object | Headers of the Kafka message, represented as a map in which the key is a string and the value is an InputStream. |  | 
| Key a| Binary | Key of the Kafka message, represented as an InputStream. |  | 
| Partition a| Number | Partition of the topic where the Kafka message is sent. |  | 
| Topic a| String | Topic to which the Kafka message belongs to. |  | 
|===
