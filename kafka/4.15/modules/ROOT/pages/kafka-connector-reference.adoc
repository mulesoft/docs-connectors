...
| Maximum polling interval a| Number | Maximum delay between invocations of `poll()` when using consumer group management. | `60` |
...
| Exclude internal topics a| Boolean | Determines whether internal topics matching a subscribed pattern are excluded from the subscription. | `true` |
| Auto offset reset a| Enumeration, one of: `EARLIEST`, `LATEST`, `ERROR` | Determines what to do when there is no initial offset in Kafka or if the current offset no longer exists on the server. | `LATEST` |
...
| Check CRC a| Boolean | Automatically check the CRC32 of the records consumed. | `true` |
...
| Request Timeout a| Number | Maximum amount of time the client waits for the response of a request. | `30` |
...
| Default record limit a| Number | Maximum number of records returned on a poll call to the Kafka cluster. | `500` |
...
| Heartbeat interval a| Number | Expected time between heartbeats to the consumer coordinator. | `3` |
...
| Session timeout a| Number | Timeout used to detect consumer failures when using Kafka group management. | `10` |
...
| Connection maximum idle time a| Number | Close idle connections after the specified time. | `540` |
...
| Topic Subscription Patterns a| Array of String | List of subscription regular expressions to which to subscribe. | |
| Assignments a| Array of <<TopicPartition>> | List of topic-partition pairs to assign. Consumers are not automatically rebalanced. | |
| Default fetch minimum size a| Number | Minimum amount of data the server returns for a fetch request. | `1` |
| Fetch Minimum Size Unit a| Enumeration, one of: `BYTE`, `KB`, `MB`, `GB` | Unit of measure for the *Default fetch minimum size* field. | `BYTE` |
| Default fetch maximum size a| Number | Maximum amount of data the server returns for a fetch request. | `1` |
...
| Default maximum partition fetch size a| Number | Maximum amount of data per partition that the server returns. | `1` |
...
| Fetch Maximum Wait Timeout a| Number | Maximum amount of time the server blocks before answering a fetch request. | `500` |
