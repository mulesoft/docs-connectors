...
|===
| Field | Type | Description | Default Value | Required
| Host a| String | Host that the proxy requests are sent to. |  | 
| Port a| Number | Port that the proxy requests are sent to. |  | 
...
| Send Tcp No Delay a| Boolean | If set, transmitted data is not collected together for greater efficiency but sent immediately.
...
| Keep Alive a| Boolean | Enables SO_KEEPALIVE behavior on open sockets. This automatically checks socket connections that are open but unused for long
 ...
 alive before they are recycled. | false | 
| Connection Timeout a| Number | Number of milliseconds to wait until an outbound connection to a remote server is successfully created,
