= Microsoft Dynamics NAV Connector 3.0 Reference - Mule 4
...
* Microsoft Dynamics NAV Connector isn't supported in CloudHub 2.0 because the underlying CXF library requires a JDK to generate schema classes. To support Microsoft Dynamics NAV Connector in CloudHub 2.0, the client must provide SOAP requests in raw XML format instead of using Plain Old Java Objects (POJOs). For more information, refer to xref:ms-dynamics-nav-connector-upgrade-migrate.adoc[Upgrading and Migrating Microsoft Dynamics NAV Connector to Version 3.0].
