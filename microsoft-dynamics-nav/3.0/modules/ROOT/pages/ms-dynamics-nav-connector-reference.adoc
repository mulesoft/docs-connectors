...
* Microsoft Dynamics NAV Connector isn't supported in CloudHub 2.0 because the underlying CXF library requires a JDK to generate schema classes. To support Microsoft Dynamics NAV Connector in CloudHub 2.0, the client must provide SOAP requests in raw XML format instead of using plain old java object.
