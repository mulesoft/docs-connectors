= IBM MQ Connector 2.0 Reference 
...
| Connection a| * <<config_connection, IBM MQ Connection>>
* <<config_xa_connection, IBM MQ XA Connection>>
...
** IBM_CIPHER_SUITES
** NONE |  Defines which CipherSuite mapping to use: Oracle JDK or IBM JDK. Configure this parameter depending on the host JDK. For more information, see https://www.ibm.com/docs/en/ibm-mq/8.0.0?topic=SSFKSJ_8.0.0/com.ibm.mq.dev.doc/q113220_.html[SSL/TLS CipherSpecs and CipherSuites]. |  `ORACLE_CIPHER_SUITES` |
...
[[config_xa_connection]]
===== IBM MQ XA Connection

Supports creating and joining XA transactions, which enables the connector to participate in a transaction with multiple resources.

[NOTE]
IBM MQ Connector does not support nested XA transactions due to IBM MQ driver limitations.
...
NOTE: When using multiple MUnit test suites with the *Publish* operation, set the `avoid.dispose.mq.threads` system property to `true`. This property prevents the connector from cleaning up IBM MQ threads that are needed for the next test suite to run.
...
| Inactivity Timeout Minutes a| Number a| Inactivity timeout in minutes. If no message is received within this period, the listener triggers a reconnection.

NOTE: Configure the inactivity timeout based on the expected message frequency. Set the value higher than the maximum expected interval between messages to avoid unnecessary listener reconnections. Avoid very low timeout values. For example, if the listener receives at least one message every 5 hours, configure the inactivity timeout to approximately 6 hours.
|  `0` |
| Output Mime Type a| String |  The MIME type of the payload that this operation outputs. |  |
| Output Encoding a| String |  The encoding of the payload that this operation outputs. |  |
| Transactional Action a| Enumeration, one of:
...
| Message Identifier a| <<RedeliveryPolicyMessageIdentifier>> | One or more expressions that determine if a message was redelivered. You can set this property only if the *Use Secure Hash* field is `false`. |  |
...
Representation of a destination that contains its identifier name and the kind of destination.
...
IBM MQ implementation of the JMSX properties.
...
Maps an error thrown by this component to a custom error type.
...
** CONNECTIVITY
** RETRY_EXHAUSTED
** TIMEOUT |  The error type to map from. |  |
| Target a| String |  The custom error type to map to. |  | x
|===
...
Connection mode in which the IBM MQ client automatically connects to the IBM MQ server running on the same system.
...
Connection mode in which the IBM MQ JMS client connects to the queue manager over TCP/IP. You can use this connection mode to connect to brokers on the same machine or on remote machines.
...
Caching strategy that reuses the logic of the default JMS caching strategy.
