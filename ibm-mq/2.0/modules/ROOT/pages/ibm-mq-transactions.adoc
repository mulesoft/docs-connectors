...
[NOTE]
In version 2.0, the `enableXa` attribute is removed. Use the `<ibm-mq:ibm-mq-xa-connection>` element instead. Applications that still specify `enableXa` fail to start with an unknown-attribute error. See xref:ibm-mq-connector-upgrade-migrate.adoc[Upgrading and Migrating IBM MQ Connector].

To enable XA transactions:

. Create an XA connection by using the `<ibm-mq:ibm-mq-xa-connection>` element. In Studio, select *IBM MQ XA Connection* in the *Connection* field of the global element configuration:
+
[source,xml,linenums]
----
...
----
+
Optionally, configure the XA connection pool by setting the *Min Pool Size* (`minPoolSize`), *Max Pool Size* (`maxPoolSize`), and *Max Idle Seconds* (`maxIdleSeconds`) parameters, which default to `4`, `32`, and `60`. For details, see xref:ibm-mq-xml-ref.adoc#config_xa_connection[IBM MQ XA Connection].
+
