...
. Create an XA connection by using the `<ibm-mq:ibm-mq-xa-connection>` element:
+
[source,xml,linenums]
----
...
----
+
Optionally, configure the XA connection pool by setting the `minPoolSize`, `maxPoolSize`, and `maxIdleSeconds` parameters. For details, see xref:ibm-mq-xml-ref.adoc#config_xa_connection[IBM MQ XA Connection].
+
