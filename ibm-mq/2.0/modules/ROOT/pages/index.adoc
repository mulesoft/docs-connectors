...
IBM MQ Connector 2.0 uses the Jakarta Messaging (`jakarta.jms`) namespace instead of `javax.jms`, and it replaces the `enableXa` attribute with a separate XA connection element. If you are upgrading from version 1.x, see xref:ibm-mq-connector-upgrade-migrate.adoc[Upgrading and Migrating IBM MQ Connector].
