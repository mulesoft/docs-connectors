...
IBM MQ Connector 2.0 requires:

* Mule runtime engine (Mule) 4.10.2 or later. Mule 4.4, 4.6, and 4.9 are not supported.
* Java 17. Java 8 and Java 11 are not supported.
* Anypoint Studio 7.21 or later, which includes Mule 4.10.2.

Version 2.0 uses the Jakarta Messaging (`jakarta.jms`) namespace instead of `javax.jms`, and it replaces the `enableXa` attribute with a separate XA connection element. If you are upgrading from version 1.x, see xref:ibm-mq-connector-upgrade-migrate.adoc[Upgrading and Migrating IBM MQ Connector].
...
Use this connection type to create or join XA transactions that span multiple resources. See xref:ibm-mq-transactions.adoc[Handling Transactions in IBM MQ].

Both connection types accept the same connection parameters. The *IBM MQ XA Connection* type adds the *Min Pool Size*, *Max Pool Size*, and *Max Idle Seconds* XA connection pool parameters. For a full list of parameters for each connection type, see xref:ibm-mq-xml-ref.adoc#config_connection[IBM MQ Connection] and xref:ibm-mq-xml-ref.adoc#config_xa_connection[IBM MQ XA Connection].

== Connection Mode

When creating the IBM configuration, choose
is the https://www.ibm.com/docs/en/ibm-mq/9.0.x?topic=SSFKSJ_9.0.0/com.ibm.mq.dev.doc/q031720_.html[IBM connection mode] to use.
...
|Client Reconnect Options |FALSE |RECONNECT |Controls the reconnection behavior for the IBM MQ client. Applies when *Connection Name List* or a CCDT is configured.
|Client Reconnect Timeout |FALSE |20 |The maximum time, in seconds, that the client spends attempting to reconnect. Applies when *Connection Name List* is configured with multiple hosts.
|===

For descriptions of all client connection mode parameters, see xref:ibm-mq-xml-ref.adoc#client[Client].
...
image::ibm-mq-binding-connection-mode.png[Binding Connection]
...
In Studio, configure these values in the *Caching strategy* field of the connection. For parameter descriptions, see xref:ibm-mq-xml-ref.adoc#default-caching[Default Caching].
...
TIP: Use the IBM MQ Jakarta client library.
...
* xref:ibm-mq-connector-upgrade-migrate.adoc[Upgrading and Migrating IBM MQ Connector]
