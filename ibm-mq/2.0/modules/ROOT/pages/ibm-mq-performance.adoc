...
:page-aliases: connectors::ibm/ibm-mq-performance.adoc
...
For resilience and stability, use a dedicated configuration for the listener (`ibm-mq:listener`) separate from configurations used for Publish or Consume operations. This isolates the listener from connection failures in other operations and enables clearer monitoring of inbound listener health. See xref:ibm-mq-troubleshooting.adoc#isolate-listener-configuration-for-resilience[Isolate Listener Configuration for Resilience] in the troubleshooting guide.
