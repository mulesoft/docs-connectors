...
* The `CORRELATION_ID` pattern value implies that the *Publish Consume* operation, after publishing a message to the destination queue, consumes a message using a selector that expects a message with the same correlation ID used to publish the request message.

* The `MESSAGE_ID` pattern value implies that the *Publish Consume* operation, after publishing a message to the destination queue, consumes a message using a selector expecting a message with a correlation ID of the same value as the message ID of the request message. | CORRELATION_ID |
|===
