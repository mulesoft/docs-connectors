...
| External ID a| String | A unique identifier that may be required when you assume a role in another account. If the administrator of the account to which the role belongs provides an external ID, then provide that value in this field. |  |
| Connection Timeout a| Number |  The amount of time to wait (in milliseconds) when initially establishing a connection before the connector gives up and times out. A value of 0 means infinity and is not recommended. |  50 |
| Connection Timeout Unit a| Enumeration, one of:
...
| Configuration | String | The name of the configuration to use. | | x
| Shard Id a| String |  |  | x
| Shard Iterator Type a| String | Determines how the shard iterator is used. |  | x
