...
* Subscribe groups
  Subscribes a WebSocket to one or more groups.
+
The app can invoke this operation on the same WebSocket multiple times, with each invocation being additive over the previous ones. Repeated groups are ignored.
+
* Unsubscribe groups
  Unsubscribes a WebSocket from one or more groups.
+
