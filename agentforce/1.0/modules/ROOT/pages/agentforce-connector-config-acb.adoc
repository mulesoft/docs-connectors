...
[source,xml]
----
<ms-agentforce:config llmType="#[]" configType="#[]" filePath="#[]" modelName="#[]" name="name" />
----
. Add the attributes to complete the configuration, for example:
+
[source,xml]
----
<ms-agentforce:config configType="Environment Variables" filePath="-" llmType="OPENAI" modelName="gpt-4o" name="Einstein_AI_Config">
...
----
 <http:listener-config basePath="/path" name="HTTP-Listener-config">
  ...
  <ms-agentforce:config configType="Environment Variables" filePath="-" llmType="OPENAI" modelName="gpt-4o" name="AGENTFORCE_Config"></ms-agentforce:config>
