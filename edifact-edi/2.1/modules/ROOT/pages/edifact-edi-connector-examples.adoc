...
. Go to the properties view, select the connector configuration you created and select the `Read` operation. This reads any Byte stream into the structure described by your EDIFACT schemas.
...
Sending generated functional acknowledgments (CONTRL messages) is the same as writing any other type of EDI message except that transactions can be set to the acknowledgments that were generated during the read operation.
...
The generated functional acknowledgments have interchange data set up for sending back to the sender of the original message, so no need to change anything in the transactions to perform the send.
