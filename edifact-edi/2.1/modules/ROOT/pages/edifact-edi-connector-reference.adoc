= EDIFACT EDI Connector Reference
...
| Enforce length limits | Boolean |  Enforce minimum and maximum lengths for receive values. If true, a message with values too long or too short is rejected. If set to false, uses the values and the transaction is not rejected. In either case the error is logged and reported in CONTRL functional acknowledgments. |  true |
...
|===
| Field | Type | Description | Default Value | Required
| Initial Buffer Size | Number | This is the amount of memory to allocate to consume the stream and provide random access to it. If the stream contains more data than can fit into thr buffer, it expands according to the `bufferSizeIncrement` attribute, with an upper limit of `maxInMemorySize`. |  |
