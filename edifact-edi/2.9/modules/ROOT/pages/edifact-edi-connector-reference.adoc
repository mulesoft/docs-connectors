...
** When the app is deployed to stand-alone Mule runtime, Runtime Fabric, or CloudHub 2.0, the connector uses the https://docs.mulesoft.com/mule-runtime/latest/distributed-locking[runtime's distributed locking].
+
* `RUNTIME_LOCK_MANAGER`
+
...
* `OBJECT_STORE`
+
The connector always uses an alternative mechanism in environments in which the runtime's distributed locking mechanism doesn't work. (Recommended when the environment has more than one worker.)| AUTO |
|===
