= EDIFACT EDI Connector 2.9 Reference - Mule 4
...
** OBJECT_STORE a| Determine the mechanism to manage locks for control number sequencing.

* `AUTO`: It will choose between `RUNTIME_LOCK_MANAGER` and `OBJECT_STORE` based on the environment the mule app is running.
** When the app is deployed to CloudHub 1.0, it uses the Object Store mechanism for lock management.
** When the app is deployed to stand-alone Mule runtime or Runtime Fabric or CloudHub 2.0, it uses the https://docs.mulesoft.com/mule-runtime/latest/distributed-locking[runtime's distributed locking].
* `RUNTIME_LOCK_MANAGER`: It will always use the https://docs.mulesoft.com/mule-runtime/latest/distributed-locking[runtime's distributed locking]. +
(Recommended when the environment only has one worker)
* `OBJECT_STORE`: It will always use an alternative mechanism in environments where the https://docs.mulesoft.com/mule-runtime/latest/distributed-locking[runtime's distributed locking] does not work. +
(Recommended when the environment has more than one worker)| AUTO |
|===
