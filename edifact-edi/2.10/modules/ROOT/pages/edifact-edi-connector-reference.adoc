= EDIFACT EDI Connector 2.10 Reference 
...
** Hybrid standalone deployments, including clustered Mule runtimes
| AUTO |
|Acknowledge every transaction | Boolean a| Whether to include UCM Segment in acknowledgement of every transaction even if the EDIFACT message is rejected:

* true
+
The connector uses the configured *ackAllSets* value to include UCM Segment for every transaction even if the EDIFACT message is rejected.
+
* false
+
The connector only includes UCM Segment when its rejected and not for every transaction.| false |
|===
