= EDIFACT EDI Connector 2.3 Reference
...
| Truncate values exceeding the length limits | Boolean | Works when enforce length values is `true`. If `true`, values exceeding maximum length will be trimmed. If `false`, the values are written as is. | false |
|===
