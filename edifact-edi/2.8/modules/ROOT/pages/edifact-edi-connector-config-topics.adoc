...
|requireUniqueGroups="false" |Enforce globally unique Group Control Numbers (UNG.5) for received functional groups. By default, group numbers must be unique only within an interchange. Setting to `true` requires group numbers to be unique across all interchanges received from the same partner and application, as determined by the interchange sender and recipient identification.
|requireUniqueMessages="false" |Enforce globally unique message reference numbers (UNH.1) for received messages. By default, message reference numbers must be unique only within a particular interchange. Setting to `true` requires message reference numbers to be unique across all interchanges received from the same partner and application, as determined by the interchange sender and recipient identification.
...
|writeInterchangeControlNumberKey="" |Interchange number key for an object store. If a key is specified, it is used for subsequent operations use the object store for interchange control numbers.
|writeGroupControlNumberKey="" |Group control number key for the object store. If a key is specified, it is used for subsequent operations that use the object store for the group control numbers.
|writeMessageControlNumberKey="" |Message control number key for the object store. If a key is specified, it is used for subsequent operations that use the object store for the message control number.
...
|batchGroupNumberKey="" |Group control number key for the object store. If a key is specified, it is used for subsequent operations involving the object store for the group control numbers.
