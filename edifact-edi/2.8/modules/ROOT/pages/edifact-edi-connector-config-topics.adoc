= EDIFACT EDI Connector 2.8 Configuring Schemas - Mule 4
...
|requireUniqueGroups="true" |Enforce globally unique Group Control Numbers (UNG.5) for received functional groups. By default, group numbers only need to be unique with an interchange. If `true`, this instead requires group numbers to be unique across all interchanges received from the same partner and application (as determined by the interchange sender and recipient identification).
...
|outputEdiMimeType="APPLICATION_PLAIN"/"APPLICATION_EDIFACT" |Output MIME type to be set for the message.
|writeInterchangeControlNumberKey="" |Interchange number key for object store. If a key is specified, then that key is used for subsequent operations involving the object store for interchange control numbers.
|writeGroupControlNumberKey="" |Group control number key for object store. If a key is specified, then that key is used for subsequent operations involving the object store for the group control numbers.
|writeMessageControlNumberKey="" |Message control number key for object store. If a key is specified, then that key is used for subsequent operations involving the object store for the message control number.
...
|useBatchFunctionalGroups="false" |Setting to `true` the operation classifies each message into the correspondent Functional Group. By default, messages are written one below others outside groups.
|batchSendUniqueGroupNumbers="false" |Send unique Group Reference Numbers (UNG.1). By default, message reference numbers are assigned sequentially within each interchange and are reused in different interchange. If `true`, this instead assigns unique message numbers across all interchanges sent to the same partner (as determined by the interchange sender and recipient identification).
...
|batchGroupNumberKey="" |Group control number key for object store. If a key is specified, then that key is used for subsequent operations involving the object store for the group control numbers.
