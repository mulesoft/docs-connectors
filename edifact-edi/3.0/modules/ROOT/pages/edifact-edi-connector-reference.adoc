= EDIFACT EDI Connector 3.0 Reference 
...
** Hybrid standalone deployments, including clustered Mule runtimes
| AUTO |
|Acknowledge every transaction | Boolean a| Whether to include the UCM Segment in the acknowledgement of every transaction:

* true
+
The connector uses the configured *ackAllSets* value to include the UCM Segment for every transaction, even if the EDIFACT message is not rejected.
+
* false
+
The connector only includes the UCM segment when the EDIFACT message is rejected, not for every transaction.| false |
| Send UNA segment a| Enumeration, one of:

** ALWAYS
** DEPEND_ON_SEPARATORS
** NEVER a| Determines whether the generated EDIFACT interchange begins with a UNA segment. This configuration behavior is dependent on the **Always send UNA** configuration . Valid values are:

* `ALWAYS`
+
Includes UNA at the start of every interchange, even when the separators match the defaults for the selected syntax version and character encoding. Existing configuration **Always send UNA** has same behavior.

* `NEVER`
+
Omits UNA, including when custom separators are used. The interchange begins with UNB. A configuration error is thrown if this option is combined with **Always send UNA**.

* `DEPEND_ON_SEPARATORS`
+
Includes UNA when the resolved separators differ from the defaults for the selected syntax version and character encoding, otherwise omits it. When **Always send UNA** configuration is true, the setting overrides this value and forces UNA to be included.
| DEPEND_ON_SEPARATORS |
|===
