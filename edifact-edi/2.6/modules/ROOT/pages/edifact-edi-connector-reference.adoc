= EDIFACT EDI Connector 2.6 Reference - Mule 4
...
| Enforce segment repeats | Boolean |  Enforce segment repetition count limits in a message. If true, a message with a segment repeated too many times is rejected. If false, the message is not rejected. In either case the error is reported in CONTRL functional acknowledgments. |  true |
