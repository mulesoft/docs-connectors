 ...
 * On the send side, the connector uses value as the UNB interchange recipient identification unless it is overridden in either the send configuration or the message parameters. |  |
| Partner ID qualifier | String a|  Identification qualifier for partner in the exchange (UNB segment). This value is used in combination with the value of the *Partner Interchange ID* field, and if one field is set, the other must also be set. 
{sp}+
{sp}+
If set, this value affects both send and receive message handling.
...
+
Use specified delimiters as defaults for both reads and writes. |  USE_SPECIFIED_FOR_ALL |
...
* false
+
The connector uses the values and does not reject the transaction. 
...
* true
+
The connector rejects a message that has values repeated too many or too few times.
+
* false
+
The connector uses the values and does not reject the message. 
...
* true
+
The connector rejects a received message that contains characters that the syntax level does not allow.
+
* false
+
If a substitution character is set, the connector uses it in place of the disallowed characters. Otherwise, the connector passes the characters through. |  true |
| Allow unknown segments | Boolean a|  Allow unknown segments in a message:

* true 
+
The connector ignores the unknown segments and does not reject the message.
+
* false
+
...
In either case, the error is reported in CONTRL functional acknowledgments. |  true |
| Allow unused segments | Boolean a|  Allow segments marked as Unused in a message:
...
In either case, the error is reported in CONTRL functional acknowledgments. |  true |
| Character encoding override | String |  Character encoding override for receive messages. Normally, character encoding is determined by the syntax identifier in the UNB header segment, but the connector uses this value instead, if it is set. The value must be a Java character encoding name. |  |
...
* false
+
Message reference numbers must be unique with a particular interchange.
+
* true
+
...
|  false |
| Store time-to-live | Number |  Minimum number of days to store interchange and message numbers for checking uniqueness. This value is used only when the configuration requires unique interchange, message numbers, or both. The default persistent object store from the Mule configuration is always used for storing the received identifier numbers. This field sets the entry time-to-live for these stored values. |  30 |
...
* true
+
The connector rejects a received message that contains characters not allowed by the syntax level.
+
* false
+
The connector uses a substitution character if one is set, or it passes the character through. |  true |
...
| Request acknowledgments | Boolean a|  Request acknowledgments for Send interchanges using the Acknowledgment Request field (UNB.9).

* true
+
CONTRL acknowledgments are requested in all sent interchanges
+
* false
+
...
| Truncate values exceeding the length limits | Boolean a| Specifies whether to truncate values that exceed the length limits. 

* true
+
If the *Enforce length values* field is also true, the connector trims values that exceed the maximum length.
+
* false
+
The connector writes the values are written as-is. | false |
|Enforce code set validations | Boolean a| Specifies whether to enforce code set validations:

* true
+
Enforces code set validations, as defined in the EDIFACT schemas.
+
* false
+
Does not enforce code set validations| false |
|Use Version Association Code | Boolean | If `true`, uses UNH0205 (Version Association Code).| false |
|Use segment line ending in the last line | Boolean | If `true`, uses the configured *Segment line ending* value at the end of the last line in the segment.| false |
|===
...
KB
MB
GB | Unit for the Max In Memor Size field. |  |
|===
