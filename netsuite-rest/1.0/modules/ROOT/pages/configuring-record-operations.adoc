...
<ms-netsuite-rest:initialize-record
    ...
    sourceRecordType="salesOrder"
    sourceInternalId="1234"
...
+
The type and internal ID (or `eid:EXTERNAL_ID`) of the record being attached (for example, `contact`, `employee`, `file`, or `invoice`).

* *Attach Body*
+
Optional JSON body for a contact role (for example, `{"role":{"id":"-10"}}`). You can omit it for other record types.
...
<ms-netsuite-rest:attach-record
    ...
    attachRecordId="5678">
    <ms-netsuite-rest:attach-body>#[{"role": {"id": "-10"}}]</ms-netsuite-rest:attach-body>
</ms-netsuite-rest:attach-record>
----
