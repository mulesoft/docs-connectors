...
Each source polls NetSuite on a schedule, maintains a watermark in an object store, and emits an event for each record found since the last poll. In a cluster, only the primary node polls by default, and the shared watermark lets a failover resume from the persisted boundary day with at-least-once delivery. The three sources differ only in the NetSuite date column they filter on. For full parameter, output, and error details, see xref:netsuite-rest-connector-reference.adoc[NetSuite REST Connector Reference].
