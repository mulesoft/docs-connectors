...
Anypoint Connector for FTP (FTP Connector) can read a file at any point in the flow. The *Read* operation reads the file in the given path and returns a Mule message with the following attributes:
...
For more information repeatable streams, refer to the xref:mule-runtime::streaming-about.adoc[streaming in Mule 4] documentation.
...
. In the *MIME Type* tab, set *MIME Type* to `application/xml` and *Encoding* to `ISO 10646/Unicode(UTF-8)`.
