...
* Although the information posted through the HTTP Listener source is written to each FTP site multiple times, the repeatable streams feature prevents the overhead of consuming the stream multiple times.
