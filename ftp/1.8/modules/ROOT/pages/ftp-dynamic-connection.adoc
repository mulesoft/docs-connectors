= Connect to FTP Servers Dynamically Example 
...
* Although the information posted through the HTTP Listener source is written to each FTP site multiple times, the repeatable streams feature prevents the overhead from consuming the stream multiple times.

* The For Each component automatically goes through each line of the CSV file without needing to first transform it into a Java structure.
+
