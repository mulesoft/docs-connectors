...
:page-aliases: 3.9@mule-runtime::ftp-connector.adoc
...
. Decide where you want to place the FTP endpoint within your Mule flow, then drag it from the palette to the appropriate place in the sequence of processors that make up your Mule flow.
...
.. If you place the FTP processor in the middle or at the end of the flow, it serves as an outbound endpoint, transferring files out of the flow to the connected file system (such as, the FTP server).
...
* See the xref:3.9@mule-runtime::ftp-transport-reference.adoc[FTP Transport Reference] for details on setting the properties for an FTP endpoint using an XML editor.
