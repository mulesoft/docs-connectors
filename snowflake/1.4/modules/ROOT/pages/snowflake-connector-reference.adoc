= Snowflake Connector 1.4 Reference
...
| Connection a| * <<key-pair-connection, Key-pair Connection>> 
* <<snowflake-config_snowflake-connection, Basic Snowflake Connection (To be deprecated)>> 
...
For further details on how to configure key-pair authentication, refer to https://docs.snowflake.com/en/user-guide/key-pair-auth#configuring-key-pair-authentication[configure key-pair authentication] Snowflake documentation. 

=== Parameters
[%header,cols="20s,20a,35a,20a,5a"]
|===
| Name | Type | Description | Default Value | Required
| Pooling Profile a| <<SnowflakeDbPoolingProfile>> |  Configures database connection pooling. |  |
| Account Name a| String |  Account name provided when registering for the Snowflake account. The account name is in the `<OrgId>.<AccountId>` format. Specify the account name with a cloud region or cloud name, for example, `<OrgId>.<AccountId>.<CloudRegion>` or `<OrgId>.<AccountId>.<CloudRegion>.<CloudName>`.|  | x
...
| Private Key File Path | String | Private key path used to authenticate the user. |  | x
| Private Key Password | String | Password to decrypt an encrypted private key. Only for encrypted private keys.|  | 
| Role a| String | Role assigned to the user. |  |
| Reconnection a| <<Reconnection>> |  When the application is deployed, a connectivity test is performed on all connectors. If set to true, deployment fails if the test doesn't pass after exhausting the associated reconnection strategy. |  |
|===

[[snowflake-config_snowflake-connection]]
== Basic Snowflake Connection (To be deprecated)
...
| Account Name a| String |  Account name that was provided when registering for the Snowflake account. The account name is in the `<OrgId>.<AccountId>` format. You can also specify the account name with a cloud region or cloud name, for example, `<OrgId>.<AccountId>.<CloudRegion>` or `<OrgId>.<AccountId>.<CloudRegion>.<CloudName>`.|  | x
| Warehouse a| String |  Name of the Snowflake warehouse to use. |  | x
| Database a| String |  Snowflake database to connect to. |  | x
| Schema a| String |  Snowflake database schema to use. |  | x
| User a| String |  Snowflake database username used to initialize the session. |  |
...
| Input Parameters a| Array of Object | A list of maps in which every list item represents a row to insert. The map contains the parameter names as keys and the value the parameter is bound to. |  #[payload] |
| Transactional Action a| Enumeration, one of:
...
| Name | Type | Description | Default Value | Required
| Configuration | String | Name of the configuration to use. | | x
| Input Parameters a| Array of Object | A list of maps in which every list item represents a row to update. The map contains the parameter names as keys and the value the parameter is bound to. |  #[payload] |
| Transactional Action a| Enumeration, one of:
...
| Max Idle Time a| Number | Determines how many seconds a Connection can remain pooled but unused before being discarded.
 Zero means idle connections never expire. | 10800 |
