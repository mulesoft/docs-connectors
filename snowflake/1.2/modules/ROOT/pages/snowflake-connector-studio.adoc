...
. Open the `AnypointStudio.ini` file and identify where the Java VM is stored. For further details, refer to  https://help.salesforce.com/s/articleView?id=001115951&type=1[How to find the Anypoint Studio .ini file]. Use the `-vm` command to set up the path. Consider the following path `JAVA_VM_FOLDER`.
