...
Note that if the JVM doesn't contain the `jre/lib/ext directory`, use the directory shown by the `java.hom`e property which is printed at the beginning of a Mule deployment.

For example:
----
- java.home =                           
/Applications/AnypointStudio.app/Contents/Eclipse/plugins/org.mule 
.tooling.jdk.v8.macosx.x86_64_1.1.3/Contents/Home/jre
----

Because Snowflake Connector depends on the Bouncy Castle provider to do this type of authentication, the connector requires `bcpkix-jdk18on` as the external library. To configure this library:

* For Java 8, add the library to the `jre/lib/ext` JVM’s folder
* For Java 9 +, use a runtime standalone in the `lib/boot/` folder, directly from Anypoint Studio or when deploying this is not possible.
* For Java 11 +, follow these steps:
...
. Append this flag to the run configuration in VM arguments: 
+
----
-Dmule.security.provider=BC
----
...
For further details about the configuration, refer to https://help.salesforce.com/s/articleView?id=001120389&type=1 [setting up Bouncy Castle security provider] knowledge base article.
