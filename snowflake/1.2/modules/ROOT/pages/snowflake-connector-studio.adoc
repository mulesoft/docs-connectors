= Using Anypoint Studio to Configure Snowflake Connector 1.2 
...
Note that if the JVM doesn't contain the `jre/lib/ext` directory, use the directory shown by the `java.home` property which is printed at the beginning of a Mule deployment.
...
* For Java 9 and 11 +, follow these steps:
+
. Add the library in the `lib/boot/` folder. 
...
. Append this flag to run the configuration in VM arguments: 
+
----
