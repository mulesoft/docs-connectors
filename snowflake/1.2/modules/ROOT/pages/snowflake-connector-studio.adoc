...
If you are using Key-pair connection[insert link] with an encrypted private key you will be needed to configure Bouncy Castle Security provider.

Open `AnypointStudio.ini` file and identify where Java VM is stored [INSERT THIS LINK https://help.salesforce.com/s/articleView?id=001115951&type=1]. The path to VM is set up by the `-vm` command. Consider the following path `JAVA_VM_FOLDER`.
...
. Set the Bouncy Castle security provider at the end of the list of the security providers inserting this line with the correspondent number N.

`security.provider.N=org.bouncycastle.jce.provider.BouncyCastleProvider`

=== Configure BCPKIX Cryptography module

On the *Snowflake Config* window, when Key-pair authentication is selected, for *Required Libraries*, add the required Bouncy Castle Bcpkix jar by selecting *Configure > Add recommended libraries* or *Configure > Add Maven dependency* and adding the artifact ID `snowflake-jdbc`.

In the case you are running in FIPS mode add the bcpkix-fips library.

=== Enable Bouncy Castle Security Provider for CloudHub and Runtime Fabric

To be able to use encrypted private keys in Key-pair authentication in Cloudhub and Runtime Fabric, set the `mule.security.provider` property to `BC`, for example:
...
If you are running in FIPS environment set `mule.security.provider` property to `BCFIPS` 
