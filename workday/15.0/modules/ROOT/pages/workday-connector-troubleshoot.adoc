...
The reconnection process is handled by Mule runtime and comes with some exceptions, for example, if a `CONNECTIVITY` error is thrown during the connection creation process, then the reconnection is not triggered even if reconnection is configured. This is because an error occurs while connecting to the third-party system, so the error (usually caused by expired credentials or misconfiguration) is likely to occur every time, which retries the reconnection as many times as the reconnection is configured. It is the user's responsibility to handle these errors if they are thrown during connection creation.
