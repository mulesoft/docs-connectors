...
|Test Connection |To validate connectivity using the *Test Connection* button, Workday Connector uses the `Identity_Management` WSDL service with the `Get_Workday_Account_Signons` operation. You must ensure that your service account has the required permissions to operate on that service.
|===
