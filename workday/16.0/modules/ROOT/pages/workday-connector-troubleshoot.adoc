= Troubleshooting Workday Connector 16.0 - Mule 4
...
To validate connectivity using the *Test Connection* button, Workday Connector uses a `Performance_Management` service with the `Get_Degrees` operation. To resolve this error, you must ensure that your service account has the required permissions to operate on that service.
