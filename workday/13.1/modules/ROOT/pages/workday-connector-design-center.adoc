= Use Flow Designer to Configure Workday Connector 13.1 - Mule 4
...
Workday Connector provides the *On Modified Objects* trigger and *On New Or Updated Records* trigger. Both triggers start your app when an object is created, modified, or deleted in Workday. The difference between these triggers is the output. *On New Or Updated Records* trigger provides full Workday output while *On New Or Updated Records* trigger provides simplified output with several fields used mainly by the Citizen users. Alternatively, you can use:
