...
*Michal, why are we showing this? Is this input sent with the body of the REST request?*:
...
. You will receive an XML response similar to this:
...
This example Create_Position_Request an app project that makes a Workday position request:

. Create a Mule application in Design Center.
. Add and configure HTTP Listener as a trigger for your flow.
. Set *Path* to `/create_position` and use the default configuration for the HTTP Listener connection.
. Add the *Transform* DdataWeavecomponent to your flow.
