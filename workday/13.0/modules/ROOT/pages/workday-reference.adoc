...
| Max An expression that evaluates against the operation's output. The outcome of the expression is stored in the target variable.s a| Number |  Maximum number of outbound connections to keep open concurrently. By default, the number of connections is unlimited. |  -1 | {nbsp}
...
| Insecure a| Boolean | If `true`, no certificate validations are performed, which makeds connections vulnerable to attacks. Use at your own risk. |  |
|===
