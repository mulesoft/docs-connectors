= Using Anypoint Studio to Configure MuleSoft AI Chain Connector 2.0 
...
. In the *Global Elements Properties > General* tab, choose your preferred large language model (LLM) type from *LLM type*:
+
** Anthropic
** Azure OpenAI
** Groq AI
...
----
. In *File Path*, enter `-` if you chose *Environment Variables* as the configuration type. If you chose *Configuration JSON*, enter the path to the JSON file, or the DataWeave expression if you are storing it in the Mule application under the `src/main/resource` folder. A path that contains `..` is rejected and the configuration file is not loaded.
...
. In *LLM timeout*, enter the timeout for the LLM request. The default is `60`, and the default *LLM timeout unit* is `SECONDS`. 
