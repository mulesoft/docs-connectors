...
Uses a client ID, client secret, and cryptography client cache for authentication. Note that the connector configuration does not allow to pass the properties such as client_id and client_secret to be hardcoded.
