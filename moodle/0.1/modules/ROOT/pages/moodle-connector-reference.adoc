...
Enrolls users in courses with the specified roles (manual enrolment and compatible enrolment plugins).
...
Unenrolls a user from a course for a specific enrolment instance (manual or plugin instance ID in the request).
...
Unenrolls multiple users from courses in a single call, following Moodle enrolment rules for each instance.
