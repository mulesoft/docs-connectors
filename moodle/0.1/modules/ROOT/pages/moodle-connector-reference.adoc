...
Adds users to a site or category cohort (Moodle site-wide groups used for bulk enrolment).
...
Creates one or more cohorts (named batches of users for enrolment and administration).
...
Deletes a cohort by ID when Moodle allows removal (for example, no blocking enrolment rules).
...
Enrols users in courses with the specified roles (manual enrolment and compatible enrolment plugins).
...
Unenrols a user from a course for a specific enrolment instance (manual or plugin instance ID in the request).
...
Unenrols multiple users from courses in a single call, following Moodle enrolment rules for each instance.
