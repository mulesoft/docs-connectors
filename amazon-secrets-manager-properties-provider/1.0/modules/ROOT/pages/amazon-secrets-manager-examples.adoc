...
When your JSON secret contains keys with dots in their names, you need to handle them differently because DataWeave treats dots as subtrees in JSON. Ensure that configuration fields are set to `Expression mode` (fx button enabled) when using these syntaxes.
