...
It is recommended to avoid using the '$' special character in the values stored in the secret manager. If the '$' character is necessary, it is advised to use the variable in literal mode, which treats it as a string. After that, you can convert the entire value into JSON and extract the required information.
