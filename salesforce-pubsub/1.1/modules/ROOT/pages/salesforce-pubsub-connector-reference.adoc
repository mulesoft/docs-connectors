...
| Publish Request Timeout a| Number |  Specifies the amount of time the client will wait for a publish request to complete before timing out. Values less than one millisecond will be converted to 0 (no deadline). |  0 | {nbsp}
...
** MINUTES
** HOURS
** DAYS |  Time unit to be used for the Publish Request Timeout parameter. |  SECONDS | {nbsp}
