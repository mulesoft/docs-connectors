...
+
Subscriber receives only events with a replay ID higher than the specified value in object store. If no value is found, it defaults to `Earliest`. Provide values for the *Object Store Name* and the *Object Store Key* fields to connect to the desired object store and retrieve the replay ID value. For more information on how to use `Replay id from object store`, refer to xref:salesforce-pubsub-connector-studio.adoc#store-objectstore[]. |  | x
