...
| Proxy configuration a| <<ProxyConfiguration>> |  Configures a proxy for outbound connections. The requests to the gRPC API (towards the gRPC host and gRPC port) are proxied through the proxy server configured using this parameter. |  |
...
| gRPC host a| String |  Host in which the proxy requests are sent to the gRPC API (towards the gRPC host). The proxy requests to the gRPC host go through the proxy. |  `api.pilot.pubsub.salesforce.com` |
