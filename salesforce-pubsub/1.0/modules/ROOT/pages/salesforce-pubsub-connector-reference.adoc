...
* <<Latest>>
* <<CustomReplayId>>
* <<ReplayIdFromObjectStore>> |  Interface with the following implementations:
...
* `CUSTOM`
+
Subscriber will only receive events with a replay ID higher than the specified value).

* `REPLAY ID FROM OBJECT STORE`
+
Subscriber will only receive events with a replay ID higher than the one specified inside object store). |  | x
