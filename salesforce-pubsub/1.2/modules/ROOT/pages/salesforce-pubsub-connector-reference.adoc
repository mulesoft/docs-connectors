...
| Inactivity Watchdog Scheduler Frequency a| Number | Specifies how often the inactivity watchdog scheduler performs checks. Values less than or equal to 0, including blank entries, disable the inactivity watchdog. |  | 
...
[NOTE]
--
The Inactivity Watchdog Scheduler runs only when both *Inactivity Watchdog Scheduler Frequency* and *Inactivity Watchdog Scheduler Time Unit* are set. Leaving either field unset disables the scheduler.
--
...
. When using OAuth 2.0 authentication, the listener is not initialized during the initial application startup. You must complete the OAuth authorization process and then restart the application to successfully initialize the listener and begin receiving events.
====
