...
. Enter the values to configure either Basic Authentication, OAuth v2.0, OAuth JWT, OAuth Client Credentials, OAuth Username Password, or OAuth SAML. Note that the `Basic Authentication` connection type is only supported for Salesforce API v64.0 and earlier versions.
