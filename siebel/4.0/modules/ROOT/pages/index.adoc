...
+
image::siebel-studio-create-config.png["Plug sign in properties window to create a new configuration"]
+
. Fill in the data in the configuration form.
+
image::siebel-studio-config.png["Global element properties window"]
+
...
+
image::siebel-studio-add-jar.png["Maven dependency window with JAR files added"]
+
...
+
image::siebel-studio-create.png["Create business component operation properties window"]
+
...
+
image::siebel-studio-transform-create.png["Transform Message properties window with input payload configuration"]
+
...
.. Configure the operation's parameters:
+
image::siebel-execute.png["Execute business service operation properties window"]
+
.. Drag the Transform Message connector between the HTTP Listener and the Siebel connector on the flow.
.. Configure the input payload with the desired fields:
+
image::siebel-transform-execute.png["Transform Message properties window with input payload configuration"]
+
. Execute Siebel Adapter (Creating an Integration Object)
+
.. Create a new Mule project in Anypoint Studio.
.. Add a suitable Mule Inbound endpoint, such as the HTTP listener, to begin the flow.
.. Drag the *Execute siebel adapter* operation from the Siebel JDB palette onto the canvas, then select it to open the properties editor.
.. Configure the operation's parameters:
+
image::siebel-integration.png["Execute siebel adapter operation properties window"]
+
.. Drag the Transform Message connector between the HTTP Listener and the Siebel connector on the flow.
.. Configure the input payload with the desired fields:
+
image::siebel-transform-integration.png["Transform Message properties window with input payload configuration"]
