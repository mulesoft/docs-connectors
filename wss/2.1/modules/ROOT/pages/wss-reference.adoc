= WSS Module 2.1 Reference

WSS Module enables you to process and validate an inbound SOAP request against a given Web Service Security (WSS) extension configuration.
...
| Config Ref a| ConfigurationProvider |  The name of the configuration used to execute this component. |  | 
...
** SHA512 | The signature digest algorithm to use.
 See SignatureDigestAlgorithmConstants | SHA256 |
...
** THUMBPRINT
** SKI_KEY_IDENTIFIER
** ENCRYPTED_KEY_SHA1 | The key identifier type to use for encryption. | ISSUER_SERIAL |
...
** SHA256
** SHA384
** SHA512 | The encryption digest algorithm to use with the key transport algorithm. | SHA256 |
