...
| WSS | SECURITY_VALIDATING | n.a. | Thrown when the request isn't valid or no WS-Security information is found
...
This example shows you how to decrypt a request using a keystore:
...
These examples show you how to validate against either an <<ldap-validate,LDAP server>> or a provided <<username-password,username/password credentials pair>>.
...
These examples show you how to validate signatures in your incoming requests against a configured <<validate-trustore-signature,truststore>>, a <<validate-binary-security-token,binary security token>>, or a <<validate-x509-certificate,X.509 certificate>>.
...
This example validates the request signature against a configured truststore to verify that only valid messages from trusted senders are received:
...
This example shows validation of an X.509 certificate issuer by pattern:
...
These examples show you how to validate both <<signed-saml,signed>> and <<unsigned-saml,unsigned>> security assertions in your incoming SOAP requests.
