...
. On the *Advanced* tab, optionally configure the parsed-schema cache and specify reconnection information, including a reconnection strategy.
+
To enable the parsed-schema cache, set *Maximum size* to the maximum number of parsed schemas to retain. To remove unused schemas after an idle period, set *Time to live* and *Time to live time unit*. A *Time to live* value greater than `0` requires a *Maximum size* value greater than `0`.
. Click *Test Connection* to confirm that Mule can connect with the specified server.
. Click *OK*.
