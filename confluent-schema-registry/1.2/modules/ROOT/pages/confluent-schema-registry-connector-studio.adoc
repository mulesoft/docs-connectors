= Using Anypoint Studio to Configure Confluent Schema Registry Connector 1.2
...
. On the *Advanced* tab, optionally configure *Avro Schema Parse Cache* and specify reconnection information, including a reconnection strategy.
+
The *Avro Schema Parse Cache* settings affect only Avro-related operations. To enable this cache, set *Maximum size* to the maximum number of parsed schemas to retain. To remove unused schemas after an idle period, set *Time to live* and *Time to live time unit*. A *Time to live* value greater than `0` requires a *Maximum size* value greater than `0`.
. Click *Test Connection* to confirm that Mule can connect with the specified server.
. Click *OK*.
