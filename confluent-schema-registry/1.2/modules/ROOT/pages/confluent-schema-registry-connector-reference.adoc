= Confluent Schema Registry Connector 1.2 Reference
...
| Time to live a| Number | Time to live for a cached schema in time units. This field is in the *General* section. |  0 |
...
** MINUTES
** HOURS
** DAYS | Time unit for the *Time to live* field in the *General* section. |  SECONDS |
| Persist cache a| Boolean | Defines if the cache is persistent. This field is in the *General* section. |  true |
| Name a| String | ID used to reference this configuration. |  | x
| Expiration Policy a| <<ExpirationPolicy>> | Configures the minimum amount of time that a dynamic configuration instance can remain idle before Mule considers it eligible for expiration. |  |
| Maximum size a| Number |  Part of the optional avro-schema-parse-cache parameter group, which only affects the <<ReplaceIdWithAvroSchema>> operation. Upper bound on the number of parsed schemas retained. 0 (the default) disables the cache entirely; a positive value enables a bounded cache whose retained set never persistently exceeds this value. |  0 |
| Time to live a| Number | Part of the optional avro-schema-parse-cache parameter group, which only affects the <<ReplaceIdWithAvroSchema>> operation. This field is unrelated to this configuration's own *Time to live* (the ObjectStore subject or ID cache). Idle-eviction window for a parsed schema, in the time unit set below. 0 (the default) disables idle eviction (entries are evicted only under size pressure). A positive value requires a positive maximum size. |  0 |
| Time to live time unit a| Enumeration, one of:
...
** MINUTES
** HOURS
** DAYS | Part of the optional avro-schema-parse-cache parameter group. The time unit for this group's own *Time to live* (the parsed-schema idle-eviction window above). This field is unrelated to this configuration's own *Time to live time unit*. |  SECONDS |
|===
