...
| Maximum size a| Number | Maximum number of parsed schemas retained in the cache. A value of `0` disables the parsed-schema cache. |  0 |
| Time to live a| Number | How long an unused parsed schema remains in the cache. A value of `0` disables idle eviction. If this value is greater than `0`, *Maximum size* must also be greater than `0`. |  0 |
| Time to live time unit a| Enumeration, one of:
...
** MINUTES
** HOURS
** DAYS | Time unit for the parsed-schema cache *Time to live* field. |  SECONDS |
|===
