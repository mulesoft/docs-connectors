...
* xref:msmq/index.adoc[MSMQ Connector]
* xref:microsoft-dynamics-crm/index.adoc[Microsoft Dynamics CRM Connector]
...
* To enable TLS 1.2 to be negotiated on servers that run Windows Server 2008 R2, you must create the "DisabledByDefault" entry in the appropriate subkey (Client, Server) and set it to "0". The entry does not appear in the registry and it is set to "1" by default.
...
NOTE: Installation requires Windows 2008 Server, Windows 2008 R2 Server,or Windows 2012 Server, and .NET Framework 4.5 or later.
...
The authentication is done through this unique security token used by both connectors. This token is included on every HTTP request to the Gateway using the Mule scheme. The following example shows how MSMQ uses the Gateway to connect to a specific queue sending the security token as specified in this section (the same authorization header applies for PowerShell):
...
For more information on the MSMQ connector, see the xref:msmq/index.adoc[MSMQ Connector User Guide].
