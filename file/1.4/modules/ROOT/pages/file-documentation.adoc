...
| In Memory Objects a| Number | Defines the maximum number of instances the stream uses to keep data in memory. If more than the specified number of instances are required, then the system starts to buffer the content on disk. The default value is 500.  | 500 |
|===
