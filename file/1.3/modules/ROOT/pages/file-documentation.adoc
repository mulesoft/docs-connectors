...
When a write operation is performed on a file, it might take some time before
the operation has been fully accomplished, and the written-to file is ready
to be read or accessed. In order to prevent an operation from accessing an incomplete
file, the parameter `timeBetweenSizeCheck` was introduced. When this parameter
is present, the file's size will be verified before accessing its contents.
The verification will consist of validating that the file's size does not vary after
waiting for the amount of time specified by the parameters `timeBetweenSizeCheck` and `timeBetweenSizeCheckUnit`.
The `timeBetweenSizeCheck` parameter will then indicate the time to wait between each
verification, which will be performed twice. If the file's size remains the same both times, it will be assumed
that the write operation has been completed and the access operation can be
performed, otherwise an exception will be thrown.
When we say that a wait will be performed, we mean to say that the thread executing
the operation will be sent to sleep for the indicated period of time. This wait is realized
...
For mule 4.1 and 4.2 users:

Because this wait can take place in multiple message processors, this connector
has no control over which thread will be the one performing the wait. This means that any thread
from any thread pool, even a cpu-intensive thread, which is not meant for this kind of task, could be the one performing said wait.
