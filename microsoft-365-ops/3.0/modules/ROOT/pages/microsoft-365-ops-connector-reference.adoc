= Microsoft Dynamics 365 For Operations Connector 3.0 Reference - Mule 4
...
| Connection a| * <<Dynamics365ForOperations_OauthClientCredentials, OAuth Client Credentials>> {nbsp}
...
| Username a| String |  Username used to authenticate against the proxy. Required if a proxy is used. If this property is not set, then no authentication is used against the proxy.  |  | 
...
| Scopes a| String |  OAuth scopes to request during the OAuth dance. If not provided, it defaults to the scopes specified in the annotation. |  | 
...
** MINUTES
** HOURS
** DAYS |  Time unit to be used for the Read Timeout parameter |  SECONDS | {nbsp}
...
** MINUTES
** HOURS
** DAYS |  Time unit to be used for the Connection parameter |  SECONDS | {nbsp}
|===
