= ServiceNow Connector 6.16 Examples - Mule 4
...
To fully benefit from using Anypoint Connector for ServiceNow (ServiceNow Connector), see the following examples.

== Retrieve Records from a ServiceNow Incident Table

This example creates a Mule flow that retrieves records from the `Incident` table. The following screenshot shows the Studio app flow for this example:

.App flow for the Retrieve Records example
image::servicenow-flow-incident-getrecords.png["ServiceNow flow for incident get records"]
...
+
image::servicenow-studio-connection-6.6.0.png["Global element properties window"]
+
image::servicenow-studio-config-transport-tab.png["Transport tab in global element properties window"]
+
...
+
image::servicenow-metadata-without-reference-values.png["ServiceNow metadata without reference values"]
+
...
This example creates a Mule flow that retrieve keys from the Incident table in a ServiceNow instance:
