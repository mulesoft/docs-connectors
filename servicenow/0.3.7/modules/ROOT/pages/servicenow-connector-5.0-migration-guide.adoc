= ServiceNow Connector Migration Guide - Version 4.0 to 5.x
:keywords: anypoint studio, esb, connector, endpoint, servicenow, service now
:page-aliases: 3.7@mule-runtime::servicenow-connector-5.0-migration-guide.adoc

The ServiceNow connector v5 is a complete revamp of the connector, and is not compatible with any version of the connector prior to v5.

The ServiceNow Connector version 5.x is developed to:

* Support all standard ServiceNow operations
* Support customized tables (including the custom fields)

*Guide:* xref:index.adoc[ServiceNow Connector 5.x Guide]

*Release Notes*: xref:release-notes::connector/servicenow-connector-release-notes.adoc[ServiceNow Connector Release Notes]

== Version 5.x vs. Previous Versions

[%header,cols="3*a"]
|===
|  |ServiceNow Connector 5.x |Previous versions of the ServiceNow Connector
...
. *Advanced Config* - Apart from what is available in the simple configuration, the advanced configuration supports a user defined HTTP Request Configuration having proxies, SSL support, and authentication methods.
 ...
 |Previous versions of the connector only have support for username, password, and service address fields for test connectivity. They support 47 tables out of the box and it is not possible to support more tables without upgrading the connector to version 5.x.
...
. Select the **ServiceNow connector 5.x** checkbox and click *Next*
