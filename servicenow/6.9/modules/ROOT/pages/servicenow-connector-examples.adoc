...
[IMPORTANT]
Your ServiceNow account must either have the `soap` role set or be an `admin` account. Admin accounts automatically have access to SOAP and are good for testing. However, for production, create a dedicated API account with the `soap` role and any additional roles to grant minimal necessary access.
