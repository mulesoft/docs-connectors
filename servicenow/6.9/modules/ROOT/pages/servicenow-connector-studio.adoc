...
|*Authorize path* | Path of the local HTTP endpoint that triggers OAuth dance
...
* If the Resource owner id parameter is set in the OAuth configuration, use the following URL. Substitute `<resourceOwnerId>` with the correct value:
+
...
+
You should see a confirmation that the OAuth dance succeeded and the access token was successfully retrieved.
