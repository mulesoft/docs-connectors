= Use Anypoint Studio to Configure ServiceNow Connector 6.9 - Mule 4
...
NOTE: To reach servicenow endpoints using oauth 2.0 configuration, you have to manually perform the Oauth dance after start of your Mule application.

Run in the browser http://localhost:8081/authorize?resourceOwnerId=demo to start the OAuth dance.

NOTE: If Resource owner id parameter is not set in OAuth configuration then simply use the following link http://localhost:8081/authorize to initiate the OAuth dance

ServiceNow will redirect us to the login screen.
Here we will need to enter login details used to access the ServiceNow instance and click *Login* button.

After that we will be redirected to Authorize Screen.
On this screen click *Allow* button, and we will be redirected to the next screen which confirms that OAuth dance was successful, and the access token was retrieved.
