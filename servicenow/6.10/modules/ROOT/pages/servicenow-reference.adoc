= ServiceNow Connector 6.10 Reference - Mule 4
...
Efficiently polls for changes in the selected table periodically and accepts only new records into the flow.
...
| Display system references a| String | Data retrieval operation for reference and choice fields. Retrieves the display value and/or the actual value from the database. The valid values are:
