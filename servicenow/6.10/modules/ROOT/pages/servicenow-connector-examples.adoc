...
+
IMPORTANT: The ServiceNow account you're using has to have the `soap` role set or needs to be an `admin`. Admin accounts have access to SOAP automatically and are a good option for testing, but for production it is recommended you create a dedicated API account with the `soap` role and any additional roles to grant the minimal necessary access.
+
