...
* SAP libraries for IDoc, JCO, and JCO Native. Go to the SAP website and download these libraries to a folder on your computer. Studio 7 can use the library formats directly without conversion. See the xref:release-notes::release-notes::connector/sap-connector-release-notes-mule-4.adoc[SAP Connector Release Notes] for which libraries are compatible with the connector.
