...
|===
+
NOTE: When using the embedded installer, you must reference the CommonCryptoLib library in the SAP Connector Config properties with the proper path.
+
[%header,cols="30a,30,30"]
|===
|Property|Example Value|Description
...
+
For SAP Connector 5.9.14 and later, set *X.509 Certificate* to the path of a PEM-formatted X.509 certificate file. You can also use a file that contains only the Base64 certificate body, without PEM headers.
+
