...
[NOTE]
When using the embedded installer, you will need to reference the library in the SAP Connector Config properties. Example:  - ${app.home}/sec/libsapcrypto.so (connector operations)
jco.server.snc_lib - ${app.home}/sec/libsapcrypto.so (connector sources)
+
[%header,cols="30a,30,30"]
|===
|Property|Example Value|Description
