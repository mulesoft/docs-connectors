...
|`secFilesCopy`|`true`|Triggers the embedded installer.
|`sourceSecFolder`|`/opt/mule/apps/cert-ch2/sec/`|This is where the `resources/sec` folder is located in CloudHub 2.0. Replace the name of the application to match your scenario.
|`targetSecFolder`|`/tmp/sec/`|This value should remain hardcoded.
|`pseFile`|`RFC`|The name of the PSE file, without an extension.
