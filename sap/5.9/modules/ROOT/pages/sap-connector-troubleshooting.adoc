...
[NOTE]
====
To prevent build issues, ensure the versions of the `maven-shade-plugin` and `munit-maven-plugin` are compatible:

. maven-shade-plugin 3.5.x and above requires munit-maven-plugin 2.3.x and above.
. maven-shade-plugin 3.4.x and below requires munit-maven-plugin 2.2.x and below.
====
