= Troubleshooting SAP Connector 5.9 
...
----
			<Loggers>
				...
				<AsyncLogger name="com.mulesoft.connector.{page-connector-name}" level="DEBUG"/>
				...
			</Loggers>
----
[start=6]
...
image::sap-connector-missing-libraries.png["Global element properties window with missing libraries"]
...
[.center.text-center]
image::sap-error-jco-classloader-folder.png["SAP Error JCo classloader folder structure"]
...
+
[.center.text-center]
image::sap-error-jco-classloader-fix.png["SAP Error JCo classloader fixed folder structure"]
...
This is also true for the IDoc library and you might receive this error:
...
To resolve this error, add this `maven-shade-plugin` to your application's `pom.xml` file:
...
[NOTE]
====
When configuring the maven-shade-plugin, ensure its version is compatible with the munit-maven-plugin version used in your project to avoid potential build or testing issues.

. If you are using maven-shade-plugin 3.5.x or above, you must use munit-maven-plugin 2.3.x or above.
. If you are using maven-shade-plugin 3.4.x or below, you must use munit-maven-plugin 2.2.x or below.
====

To determine the correct `groupId` and `artifactId` values, refer to your existing dependencies. An example configuration is shown below (these might vary depending on your setup):
...
In this example, the full artifact coordinates are `com.sap.conn.jco:com.sap.conn.jco.sapjco3` and `com.sap.conn.idoc:com.sap.conn.idoc.sapidoc3`.
 
NOTE: Shading might increase the application size by nearly 2 MB.
