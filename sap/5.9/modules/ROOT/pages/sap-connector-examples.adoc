...
The message server configuration can be used for all SAP Connector operations, including Document Listener, Function Listener, and operations such as Synchronous Remote Function Call and Send IDoc.
...
The first flow uses the message server configuration with the *Document listener* source. To configure this flow:

. In Anypoint Studio, create a new Mule project.
. From the *Mule Palette* view, select *SAP* and drag the *Document listener* source to the canvas.
. Configure the SAP Connector configuration for the *Document listener* source:
.. In the connection configuration, select either *Certificate*, *Kerberos*, or *Simple Connection Provider* as the connection type.
.. Fill in the required fields such as *SAP client ID*, *Username* and *Password* (for Simple Connection Provider), or the appropriate authentication fields for your connection type.
.. Configure the *Message server configuration* section with the following fields:
... *Host*: The host of the message server (for example, `54.160.77.19`)
... *System Id*: The system ID of the SAP system
... *Port*: The port with which the connector logs into the message server
... *Group*: The group of SAP application servers
.. Note: When using *Message server configuration*, the *SAP system number* field is not required. When using *Application server host*, the *SAP system number* field is required.
