...
. Create a Service Principal Name (SPN) `SAPService/sapdev@SSODOMAIN.LAB` for the `adminsap` user in Active Directory.
+
`setspn -A SAPService/sapdev@SSODOMAIN.LAB DOMAIN_SHORT\adminsap`
+
...
+
`ktpass -princ SAPService/sapdev@SSODOMAIN.LAB -mapuser DOMAIN_SHORT\adminsap -crypto AES256-SHA1 -ptype KRB5_NT_PRINCIPAL -mapop set -pass <*password*> -out <SID>aes.keytab`
...
+
The keytab must contain only the entry for the *Principal* (`adminsap`). If you use an existing keytab, you must remove the existing entry first. To remove the existing entry, run the following command:
+
