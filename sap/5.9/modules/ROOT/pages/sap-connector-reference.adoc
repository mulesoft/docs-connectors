...
* The `XSTRING` data format in SAP is a binary blop format. To submit values in an `XSTRING` field, you must use the xref:dataweave::dw-numbers-functions-tohex.adoc[toHex] function to base-32 encode the binary within the XML submitted to the RFC step.
...
| Use SAP Format a| Boolean |  Determines whether to support the legacy format for returning SAP values. Used in the `JCoToBusinessObjectParser` class when parsing a `JCoField`. Use the SAP default format only when working with BAPIFunctions over sRFC.
...
|  `false` |
| Use SAP Format On Function Listener a| Boolean | Determines whether to support the legacy format for returning SAP values. Used to parse a `JCoField`. Use the SAP default format only when working with the BAPI function listener.
...
|===
| Name | Type | Description | Default Value | Required
| SAP system number a| String |  Two-digit system number (sysnr) of the SAP system. Required when using *Application server host*. Not required when using *Message server configuration*. |  |
...
If the function being called must impact changes in SAP, you must add a `try` block because you need an open transaction.
...
[NOTE]
====
The *Synchronous Remote Function Call* operation requires authorization for the `RFC_METADATA_GET` function module. If authorization is not granted, you may encounter this error:

`com.sap.conn.rfc.exceptions.RfcGetException: No RFC authorization for function module RFC_METADATA_GET`.
====
...
You can configure a message server with multiple gateways to distribute the load and improve performance. The message server configuration is used for all SAP Connector operations (Document Listener, Function Listener, and all other operations such as Synchronous Remote Function Call and Send IDoc). For more information, refer to xref:sap-connector-examples.adoc#configure-multiple-gateways[Configure Multiple Gateways].
