...
This example performs two calls on standard BAPIs that handle bank records. The following screenshots show the Studio flows for this example:
...
----
. From the *Mule Palette* view, select *SAP* and drag another *Synchronous Remote Function Call* operation to the right of the *Transform Message* component.
. Create a global element named `SAP_Config` and specify the connection information.
. Click *Test Connection* to confirm that Mule runtime engine can connect with the SAP instance.
