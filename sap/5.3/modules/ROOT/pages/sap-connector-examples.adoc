...
Create a Mule app that establishes a connection to SAP and receives a function using the SAP connector's *Function listener* source, which returns the export parameter to SAP.
...
. Select the connector and click *Add*.
. Click *Finish*.
. Drag the *Function listener* source to the canvas.
. Create a new *SAP_Inbound* global element configuration and specify the X.509 certificate, SAP system number, SAP client ID, and either the application server host or message host.
...
+
The flow now contains a *Function listener* operation in the canvas source area and the *Transform Message* component in the canvas process area:
+
image::sap-flow-response.png[Flow for receiving an SAP function]
+
...
. In Anypoint Studio, create a new Mule project.
. Drag *HTTP > Listener* operation to the canvas.
...
. Create a new *SAP Inbound* global element configuration and specify the X.509 certificate and the environment values.
. Add a *Logger* component next to *Document source* on the canvas. +
...
+
Use the same global element configuration that you used for the *HTTP Listener* operation.
+
. Drag a *Transform Message* component next to Listener* on the canvas.
+
The content of this message is the payload of the BAPI function that creates an IDoc in the SAP system.
...
----
+
. Drag the SAP *Synchronous Remote Function Call* operation next to the *Transform Message* component on the canvas.
+
This operation triggers IDocs that are requested to be sent to the SAP IDoc source.
+
