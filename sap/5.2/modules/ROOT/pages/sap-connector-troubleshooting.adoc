...
You can enable JCo trace from outside of Mule and accepts the following Java startup environment properties:
...
JCo supports trace levels from 0 (default) to 10. The amount of traced data increases with the specified trace level. Each trace level also contains all the trace data from the lower trace levels. If you are using a high JCo trace level with many JCo calls and transferred content, ensure that enough disk space is available.
...
These errors typically occurs when you test the connection or try to deploy the Mule application without loading one or more of the required SAP JCo dependencies:
...
. Browse through and select the appropriate file. +
The missing dependencies are automatically added to the project's classpath.

[TIP]
Right-click the project in the Package Explorer and navigate to *Build Path > Configure Build Path* to see the libraries in the project's build path.
