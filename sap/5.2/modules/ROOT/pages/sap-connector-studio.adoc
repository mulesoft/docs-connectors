...
. In the *Install* menu, click *Browse* to locate the library file.
+
On MacOS, starting with JCO version 3.1.2, the native library extension was changed from `jnilib` to `dylib`. Starting with Anypoint Studio 7.0, there is a limit on selecting the `dylib` file type.
+
