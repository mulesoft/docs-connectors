...
If you intend to employ an SAP endpoint in conjunction with a xref:6@studio::datamapper-user-guide-and-reference.adoc[DataMapper transformer] to map and transform data, you can make use of Anypoint Studio's xref:3.5@mule-runtime::datasense.adoc[DataSense] functionality.

. Follow the detailed xref:3.5@mule-runtime::datasense.adoc[DataSense] instructions to drop the endpoint into your flow, then xref:5@studio::testing-connections.adoc[test the connection] to SAP using the SAP Connector.
