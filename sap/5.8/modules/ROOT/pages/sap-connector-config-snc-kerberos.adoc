...
** *GSS Library Path*
+
Static value `/usr/lib64/libgssapi_krb5.so.2.2` when using CloudHub or static value `/usr/lib/x86_64-linux-gnu/libgssapi_krb5.so.2.2` when using CloudHub 2.0, Runtime Fabric, and Mule Runtime On-Premises on Ubuntu. *GSS Library Path* is compatible with macOS, Windows, HP-UX 11i V3, AIX 7.2, Solaris 11.3, and Red Hat Enterprise Linux 8.1.
** *Principal*
+
