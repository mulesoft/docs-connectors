...
Configure the system variables that depend on the operating system you are using, and then generate the personal security environment (PSE):
...
. Click the download button at the bottom of the screen and enter the target path for the download. *(Reviewers: is this text correct? Also, is the download a button or icon?)*
...
** *Sap system number*
** *Sap client ID*
** *Application server host*. On the *Advanced* tab, configure extended properties.
