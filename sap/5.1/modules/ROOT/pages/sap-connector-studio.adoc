...
starting with version 3.1.2 of JCO, the native library extension was changed from `jnilib` into `dylib`. Anypoint Studio versions 7.0 through 7.6 have a limitation on selecting the `dylib` file type. After installing the library from the local machine, navigate to the bottom of the popup, select `Any`, and select the `dylib` native library. Then, enter values for the *Group ID*, *Artifact ID*, and *Version* fields in the *Install file into local repository* screen.
+
. Specify a unique group ID, artifact ID, and version.
+
