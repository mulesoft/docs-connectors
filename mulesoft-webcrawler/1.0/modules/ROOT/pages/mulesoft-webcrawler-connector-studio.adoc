...
|Base URL | Enter the base URL against which relative paths are resolved.
|Remote Browser URL | Enter the URL of the remote Selenium WebDriver endpoint (for example, Selenium Grid, standalone ChromeDriver, BrowserStack, or a CloudHub-hosted remote-browser app). Chromium-only (Chrome, Chromium, Edge).
|Browser User Agent | Enter the browser user-agent advertised on outbound requests.
