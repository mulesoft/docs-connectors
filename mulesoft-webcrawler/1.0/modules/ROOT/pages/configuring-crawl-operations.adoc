...
The *Get SiteMap* operation returns a sitemap in standard XML format following the https://www.sitemaps.org/protocol.html[sitemap protocol]. The operation first tries to discover URLs from the site's own sitemap and robots.txt, then falls back to a breadth-first crawl up to the specified depth. The sitemap results can be used to customize subsequent crawling of specific pages.
