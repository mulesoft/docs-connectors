...
* *Remote WebDriver*
+
Connects through a remote Selenium WebDriver endpoint with a configurable base URL, browser user agent, browser authentication, proxy, response timeout, TLS, and connection pooling. Users can deploy the remote browser application from Anypoint Exchange to CloudHub, or use an existing remote browser infrastructure already available within their environment. Use for JavaScript-heavy or SPA pages that require browser rendering.
