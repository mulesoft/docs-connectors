...
* A remote browser setup when content must be rendered in a real browser before scraping, such as JavaScript-heavy web applications
...
* *HTTP*
+
Connects via standard HTTP requests with configurable user agent, proxy, response timeout, and TLS settings. Use for static web content that doesn't require JavaScript rendering.
* *Remote WebDriver*
+
Connects through a remote Selenium WebDriver endpoint with configurable user agent, proxy, response timeout, TLS, and connection pooling. Users can deploy the remote browser application from Anypoint Exchange to CloudHub, or use an existing remote browser infrastructure already available within their environment. Use for JavaScript-heavy or SPA pages that require browser rendering.
