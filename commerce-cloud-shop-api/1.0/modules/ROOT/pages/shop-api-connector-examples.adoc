...
This example for Anypoint Connector for Salesforce Commerce Cloud B2C Shop (B2C Shop Connector) example shows you how to:
...
+
. Obtain the required parameters for *Get Customer* operation from Postman.
+
...
+
In the above image the authentication token from *Authorize Customer* is used as the *Authorization* parameter value.
+
