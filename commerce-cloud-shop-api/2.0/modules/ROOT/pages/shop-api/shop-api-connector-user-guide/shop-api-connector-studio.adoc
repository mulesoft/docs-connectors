...
. To avoid dependency errors, update maven settings.xml. Please refer https://docs.mulesoft.com/mule-runtime/4.3/maven-reference[Maven Reference] for more information (Ignore if already done).
...
. On the *TLS* tab, optionally specify the TLS information for the connector. For TLS details and its configuration refer https://docs.mulesoft.com/mule-runtime/4.3/tls-configuration[TLS Configuration].
. On the *Advanced* tab, optionally specify reconnection information, including a reconnection strategy. For Reconnection strategy details and its configuration refer https://docs.mulesoft.com/mule-runtime/4.3/reconnection-strategy-about[Reconnection Strategies].
...
image::shop-api/shop-connector-example/shop-connector-add-operation-to-flow.JPG[]
...
. Go back to Anypoint Studio.
In Mule Application, click on Global Elements and click Create button.
...
|============
|Field Name   |Value
|Base URI    |URL of the service with which the Connector interacts.
