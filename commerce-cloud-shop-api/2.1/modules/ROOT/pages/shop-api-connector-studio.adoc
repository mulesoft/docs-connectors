= Using Anypoint Studio to Configure Commerce Cloud B2C Shop Connector 2.1 
...
+
image::shop-connector-add-operation-to-flow.png["Example of operation dragged to flow"]
...
|============
+
image::shop-connector-shopper-token-config.jpg["Shopper token global elements properties config window on General tab"]
...
+
image::shop-connector-customer-auth-config.jpg["Auth config global elements properties config window on General tab"]
+
