= Commerce Cloud B2C Shop Connector 2.1 Reference 
...
include::connectors::partial$commerce-cloud-b2c-limitation-reference.adoc[]
...
| version a| String |  Version. |  | x
| Organization Id a| String |  Organization ID. |  | x
| Client id a| String | SLAS client ID. |  | x
| Refresh token a| String |  Refresh token given during the access token request. |  | x
| Channel id a| String |  This field is required if the shopper authenticates using the `login` endpoint with ECOM. |  | 
| hint a| String |  Optional field for logging out user sessions. Use `all-sessions` to log out all user sessions. If this field is not used, only the current user session is logged out. |  | 
...
| version a| String |  Version. |  | x
| Organization Id a| String |  Organization ID. |  | x
| Login a| String |  Base64-encoded username and password for HTTP Basic authentication. |  | x
| Content a| Binary |  Content to use. |  `#[payload]` | 
| Config Ref a| ConfigurationProvider |  Name of the configuration to use to execute this component. |  | 
| Custom Query Parameters a| Object | Custom query parameters to include in the request. The specified query parameters are merged with the default query parameters that are specified in the configuration. |  | 
| Custom Headers a| Object | Custom headers to include in the request. The specified custom headers are merged with the default headers that are specified in the configuration. |  |
...
Gets a JWT access token and a refresh token for the shopper or guest. This is the second step of the OAuth 2.0 authorization code flow. The client appplication gets an access token through the back channel (a trusted server) by passing in the client credentials and the authorization code retrieved from the `authorize` endpoint. 
...
| version a| String |  Version. |  | x
| Organization Id a| String |  Organization ID. |  | x
| Authorization a| String |  Base64-encoded string for HTTP Basic authentication. The string is composed of a client ID and a client secret separated by a colon, for example: `clientId:clientSecret`. This field is required unless the grant type is `authorization_code_pkce`. |  | 
| Content a| Binary |  Content to use. |  `#[payload]` | 
| Config Ref a| ConfigurationProvider |  Name of the configuration to use to execute this component. |  | 
| Streaming Strategy a| * <<RepeatableInMemoryStream>>
* <<RepeatableFileStoreStream>>
