...
+
| AUTO |
|Include hierarchical segment details | Boolean a|
Whether to include the Hierarchical Level (HL) segments in the output map and metadata:

* If `true`, all HL segments that have the `Ignored` usage are included in both the output map and metadata.
* If `false`, HL segments that have the `Ignored` usage are ignored in both the output map and metadata. | false |
|===
