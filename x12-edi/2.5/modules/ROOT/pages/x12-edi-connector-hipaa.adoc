...
Anypoint Connector for X12 EDI Connector HIPAA provides support for the HIPAA (Health Insurance Portability and Accountability Act) standards based on the X12 standards and version 005010.
This information provides details about using the X12 Module for HIPAA transaction sets, and how usage differs from the standard X12.
For complete information on X12 standards, refer to xref:index.adoc.adoc[X12 EDI Connector 2.5] and xref:x12-edi-schema-language-reference-x12.adoc[X12 EDI Schema Language Reference].
...
extract the supplied HIPAA schema from inside the `x12-schemas-2.0.0.jar`, that is found in the standard MuleSoft enterprise Maven repositories (under group ID com.mulesoft.connectors).
You can copy a message structure schema from this JAR (that contains both standard X12 and HIPAA schemas) and modify the extracted schema to use
