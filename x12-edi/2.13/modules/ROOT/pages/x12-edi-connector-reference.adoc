= X12 EDI Connector 2.13 Reference - Mule 4
...
* If `true`, `InterchangeTrailer` and `GroupTrailer` and `SetTrailer` details are included in each transaction.
* If `false`, only `SetTrailer` details are included in each transaction. | false |
|===
