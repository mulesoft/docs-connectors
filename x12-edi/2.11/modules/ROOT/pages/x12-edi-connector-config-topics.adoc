= X12 EDI Connector 2.10 Schema and Trading Partner Configuration - Mule 4
...
|setAckSchemaByInboundMessage="false" |If *acknowledgmentSchemaPath* is `EMPTY` and this property is `true` it will use the *Version / Release / Industry Identifier Code (GS08)* field value from the inbound message to set the Functional/Implementation Acknowledgment structure. Setting to `false` it use a hardcoded Functional/Implementation Acknowledgment structure of 005010 version.
