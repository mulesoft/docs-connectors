...
| Enforce conditional rules a| Boolean | Enforce conditional rules are applied in a transaction. If true,  a transaction with values not meeting conditional rules is rejected; If false, the values are used anyway and the transaction is not rejected. | false |
|===
