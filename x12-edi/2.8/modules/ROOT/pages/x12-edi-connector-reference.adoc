= X12 EDI Connector 2.8 Reference - Mule 4
...
|Use supplied values | Boolean a| Indicates whether the connector uses the payload values:

* If `true`, the connector uses the payload values.
* If `false`, the connector uses user-supplied values. The defaults for these values are as follows...
| false
...
==== Default Values for the Use Supplied Values Field

* ISA13
+
The value of the ISA13 segment is a sequence that increases by one with each new transaction.
+
When the first transaction occurs, the default value of ISA13 is the value of the *Initial interchange number* field. If the *Initial interchange number* field has no value, the default value of ISA13 is `1`. If you change the value of the *Interchange number key* field, the sequence resets to the initial value.
+
* GS01
+
The default value depends on the value of the class field in the structure of the schema (`.esl` file) used. For example, for the Health Care Claim structure (837), the default value is `HC`.
+
* GS06
+
The value is a sequence that increases by one with each new transaction.
+
When the first transaction occurs, the initial value of GS06 is the value of the *Initial group number* field, which has a default of `1`. If you change the value of the *Group number key* field, the sequence resets to the initial value.
+
ST01 class
+
The default value depends on the value of the structure ID of the schema (`.esl` file) used. For example, for the Health Care Claim structure (837.esl), the default value is `837`.
+
ST02
+
The value for ST02 is a sequence that increases by one with each new transaction.
+
When the first transaction occurs, the initial value is the value of the *Initial transaction number* field, which has a default value of `1`. If you change the value of the *Transaction number key* field, the sequence resets to the initial value.
+
SE01
+
The default value indicates the number of segments included in the transaction.
+
SE02
+
The default value matches the ST02 control number.
+
GE01
+
The default value indicates the number of transactions in the group.
+
GE02
+
The default value matches the GS076 control number.
+
IEA01
+
The default value indicates the number of groups included in the interchange.
+
IEA02
+
The default value matches the ISA13 control number.
+
HL segment
+
There are two fields or elements in the segment HL that form the parent/child relationship:

* HL01 (Hierarchical ID Number)contains an incremental value that uniquely identifies an HL segment occurrence.
* HL02 (Hierarchical Parent ID Number) contains the value of the HL01 element of its parent.
* HL03 Hierarchical Level Code)Indicates the specific code set associated with the segment. For example, '23'.
* HL04 indicates whether the HL segment has child segments. If it has child segments, the default value is `1`; otherwise, it is `0`.
