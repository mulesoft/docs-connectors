= X12 EDI Connector 2.19 Reference 
...
For receive messages, the invalid characters are reported in 997 functional acknowledgments. |  UNRESTRICTED |
...
| Require unique transactions in group a| Boolean a| Whether to enforce globally unique transaction set control numbers (ST02) for received transaction sets that are scoped within the functional group: 

* If `true`, transaction set control numbers must be unique within the functional groups that are received from the same partner and application. For partners, the uniqueness is determined by the interchange sender and receiver identification. For the application, the uniqueness is determined by the application codes of the functional group sender and receiver. 

* If `false`, a functional group can have duplicate transaction set control numbers, and each instance is treated as a separate transaction set. | true |
...
* If `false`, the connector includes in the acknowledgment only transaction sets that contain errors. Nonincluded transaction sets are implicitly acknowledged. |  false |
 ...
 * If `true`, the connector generates 999 Implementation Acknowledgments for each received interchange instead of 997 Functional Acknowledgment transaction sets. Support for 999 Implementation Acknowledgments does not include CTX segment generation.
 * If `false`, the connector generates 997 Functional Acknowledgment transaction sets for each received interchange.  |  false |
| Report segment details a| Boolean a|  Whether to report segment error details to the sender in 997/999 Functional Acknowledgments:
...
* If `false`, TA1 acknowledgments are not requested.|  false |
| Default usage indicator a| String a|  Default interchange usage indicator (ISA15): 

* `I`: Information
* `P`: Production data
* `T`: Test data |  P |
|Set acknowledgment schema by inbound message a| Boolean a| If the *acknowledgmentSchemaPath* is `EMPTY` and this property is `true`, it uses the *Version / Release / Industry Identifier Code (GS08)* field value from the inbound message to set the Functional/Implementation Acknowledgment structure. When `false`, it uses a hardcoded Functional/Implementation Acknowledgment structure of the 005010 version. | false |
|Acknowledgment schema path a| String a| The path from the file system or the `classpath`. If the path is specified, it overwrites the default acknowledgment path. | |
...
Whether to report non-fatal errors in acknowledgments:
...
* If `false`, all segments with `Ignore` as the usage type are ignored in both the output map and metadata. | false |
|Interchange control version | String a|
The interchange control version (ISA12) from the incoming payload will be applied if present; otherwise, the provided value will be used to override the default setting specified in the X12 schema. Allowed values are 5 characters in length. |  |
|===
