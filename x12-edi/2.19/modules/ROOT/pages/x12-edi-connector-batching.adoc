= X12 EDI Connector 2.19 Batch Outbound X12 Messages 
...
+
Use the control number key on the Write batch operation, or leave the control number key fields empty during Write batch execution so that X12 EDI Connector generates the control number sequence automatically based on the sender and receiver identifier combination:
+
