= Use Anypoint Studio to Configure X12 EDI Connector 2.19 
...
Anypoint Studio 7.x with X12 Connector 2.x, then you'll need to change the memory setting for Studio.
...
Configure this source for use with X12 EDI Connector:
...
If configured, the values are verified when reading an input message and used as defaults when writing an output message (if no output data value is specified).
...
Set the following options to control parser validation of receive messages.
...
|Include Acknowledgment schema |Automatically include a standard 997 or 999 schema that matches the generated acknowledgment in the configuration for received messages
...
|Use supplied values |Whether to use supplied values for control segment information (control numbers, in particular). The default behavior is to always generate control numbers when writing. This option enables use of selected values.
