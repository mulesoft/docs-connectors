= X12 EDI Connector 2.19 Schema and Trading Partner Configuration 
...
Set the types of error conditions that reject receive transaction sets and writer message separator operations.
...
Configure schema locations in the Anypoint Studio XML view. The schema files are loaded when the connector is initialized, so runtime expressions such as flow variables are not supported.
...
|lineEnding="NONE"/"LF"/"CRLF"/"CR" |Line ending to add between segments. The default is `NONE`. Add line endings between segments to improve message text output readability.
...
|batchLineEnding="NONE"/"LF"/"CRLF"/"CR" |Line ending to add between segments. The default is `NONE`. Add line endings between segments to improve message text output readability.
...
After you complete configuring the connector, try
