...
|Include HL segment details | Boolean a|
Whether to include the HL segments in the output map and metadata:

* If `true`, all ignored Hierarchical Level segments (that have the Ignored usage) will be included both in the output map and the metadata.
* If `false`, Hierarchical Level segments with the ignored usage will be ignored in the output map and metadata. | false |
//TODO Add details about the new sync property
|===
