...
** HTTP
** HTTPS |  Protocol to use for communication. When using HTTPS, the HTTP communication is secured using TLS or SSL. If HTTPS is configured as protocol, the user must configure the keystore in the `tls:context` child element of this listener configuration. |  HTTP |
...
| Id a| String |  Value of the primary or secondary key field. Defaults to primary key when not specified. |  | x
...
Creates new records or updates existing records. This operation makes an HTTP POST request to the /ingest/connectors/{connector-name}/{object-name} endpoint.
...
Closes or aborts a job given its ID. This operation makes an HTTP PATCH request to the /api/v1/ingest/jobs/{id} endpoint.
