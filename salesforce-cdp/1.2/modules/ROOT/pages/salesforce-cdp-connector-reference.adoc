...
** TRUE
** FALSE |  Determines whether to get the response data in arrow stream or JSON format. A value of `true` returns the data in arrow stream format. |  FALSE | {nbsp}
...
Query V2 supports SQL query in ANSI standard. The results return as an array of records. The expected input when calling this API is free form SQL. The input objects include data stream, profile and engagement data model objects, and unified data model objects. 
