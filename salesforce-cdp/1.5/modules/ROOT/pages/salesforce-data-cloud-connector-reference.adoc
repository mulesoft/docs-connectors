= Salesforce Data Cloud Connector 1.5 Reference 

Anypoint Connector for Salesforce Data Cloud (Salesforce Data Cloud Connector) provides Mule users with a quick and easy way to integrate with Salesforce Data 360 API endpoints. Users can automate data ingestion from third parties, query the Data 360 Data Lake, search and query customer profile information and calculated insights, and consume data action events to enrich the 360-degree view of their customers in their Data 360 instance.
...
| Connection a| * <<SdcConfig_OauthClientCredentials, OAuth Client Credentials>> 
* <<SdcConfig_OauthJwt, OAuth JWT>>
* <<SdcConfig_OauthUserPass, OAuth Username Password>>
 | Connection types for this configuration. | | x
| Response Timeout a| Number | How long Mule waits for a response to complete before the response times out. |  60 |
...
[[SdcConfig_OauthClientCredentials]]
===== OAuth Client Credentials

Custom connection provider for the OAuth Client Credentials flow.
...
| Token endpoint a| String |  URL pointing to the server responsible for providing the authentication token. |  `\https://domain.my.salesforce.com/services/oauth2/token` | 
| Data Space a| String | Filters results from this data space only. |  default | 
...
Cancels an in-progress query. Returns `204 No Content` on success.
...
| Query ID | String | Query identifier to cancel. | | x
| Workload Name | String | Workload identifier for monitoring and debugging. | |
| External Client Context | String | Additional context information as a JSON string. | |
...
| Custom Query Parameters | Object | Custom query parameters to include in the request. The specified query parameters are merged with the default query parameters that are specified in the configuration. | #[null] |
| Custom Headers | Object | Custom headers to include in the request. The specified custom headers are merged with the default headers that are specified in the configuration. | |
| Response Timeout | Number | How long Mule waits for a response to complete before the response times out. | |
| Response Timeout Unit a| Enumeration, one of:
...
Creates a job that represents the bulk operation for sending data to Salesforce Data 360 for asynchronous processing. This operation makes an HTTP POST request to the /ingest/jobs endpoint.
...
Deletes one or more records from Salesforce Data 360. This operation makes an HTTP DELETE request to the /ingest/sources/{source-name}/{object-name} endpoint.
...
Retrieves the job details for the specified job ID. This operation makes an HTTP GET request to the /ingest/jobs/{id} endpoint.
...
Retrieves all bulk jobs. This operation makes an HTTP GET request to the /ingest/jobs endpoint.
...
| limit a| Number |  Number of items to return. The maximum number of items to return is 4999. | |
...
[[GetQueryChunkV3]]
== Query v3 - Get Chunk
`<sdc:get-query-chunk-v3>`

Retrieves a specific chunk of query results for large result sets. Use this operation when the query status indicates multiple chunks (`chunkCount > 1`).
...
| Name | Type | Description | Default Value | Required
| Configuration | String | Name of the configuration to use. | | x
| Query ID | String | Query identifier. | | x
| Chunk ID | Number | Zero-indexed chunk identifier. | | x
| Omit Schema | Boolean | If `true`, omits metadata and schema from the response. | false |
| Accept Header a| Enumeration, one of:

** APPLICATION_JSON
** ARROW_IPC | Response format: JSON or Apache Arrow stream. | APPLICATION_JSON |
| Workload Name | String | Workload identifier for monitoring and debugging. | |
| External Client Context | String | Additional context information as a JSON string. | |
...
* <<RepeatableFileStoreStream>>
* non-repeatable-stream | Configures how Mule processes streams. The default is to use repeatable streams. | |
| Custom Query Parameters | Object | Custom query parameters to include in the request. | #[null] |
| Custom Headers | Object | Custom headers to include in the request. | |
| Response Timeout | Number | How long Mule waits for a response to complete before the response times out. | |
| Response Timeout Unit a| Enumeration, one of:
...
`<sdc:get-query-metadata-v3>`

Retrieves the schema, including column definitions, of a query result without fetching data.
...
** NEVER | How the connector streams request content to the remote service. | |
| Target Variable | String | Name of the variable that stores the operation's output. | |
| Target Value | String | Expression that evaluates the operation's output. | #[payload] |
| Error Mappings | Array of <<ErrorMapping>> | Set of error mappings. | |
| Reconnection Strategy a| * <<Reconnect>>
* <<ReconnectForever>> | Retry strategy in case of connectivity errors. | |
|===
...
[[GetQueryRowsV3]]
== Query v3 - Get Rows
`<sdc:get-query-rows-v3>`

Retrieves all rows from a completed query result. Unlike chunks, this returns all data at once (for smaller result sets).
...
| Configuration | String | Name of the configuration to use. | | x
| Query ID | String | Query identifier. | | x
| Offset | Number | Row offset from which to start. | | x
| Limit | Number | Maximum number of rows to return. If omitted, the server decides. | |
| Byte Limit | Number | Maximum byte size of the result. If omitted, the server default is `20000000`. | |
| Omit Schema | Boolean | If `true`, omits metadata and schema from the response. | false |
| Accept Header a| Enumeration, one of:
...
[[GetQueryStatusV3]]
== Query v3 - Get Status
`<sdc:get-query-status-v3>`

Retrieves the execution status of a query. Supports long polling through the *Wait Time (ms)* parameter.
...
| Name | Type | Description | Default Value | Required
| Configuration | String | Name of the configuration to use. | | x
| Query ID | String | Query identifier returned by the submit response. | | x
| Wait Time (ms) | Number | Long-polling timeout in milliseconds. The maximum is `10000`. | 10000 |
| Workload Name | String | Workload identifier for monitoring and debugging. | |
| External Client Context | String | Additional context information as a JSON string. | |
| Config Ref | ConfigurationProvider | Name of the configuration used to execute this component. | | x
| Streaming Strategy a| * <<RepeatableInMemoryStream>>
...
Creates new records or updates existing records in Salesforce Data Cloud. This operation makes an HTTP POST request to the `/ingest/sources/{source-name}/{object-name}` endpoint.
...
Executes a SQL query against Data 360 with support for asynchronous and adaptive transfer modes. Results can be returned in JSON or Apache Arrow format based on *Accept Header*.
...
* <<RepeatableFileStoreStream>>
* non-repeatable-stream | Configures how Mule processes streams. The default is to use repeatable streams. | |
| Custom Query Parameters | Object | Custom query parameters to include in the request. | |
| Custom Headers | Object | Custom headers to include in the request. | |
| Response Timeout | Number | How long Mule waits for a response to complete before the response times out. | |
| Response Timeout Unit a| Enumeration, one of:
...
Listener for Salesforce Data 360 Data Action Webhook.
...
| Name | Type | Description | Default Value | Required
| Configuration | String | Name of the configuration to use. | | x
| Path a| String |  Relative path from MuleSoft Salesforce Data 360 Data Action Webhook configuration. |  / |
| Signing Key a| String |  Secret key copied from Salesforce Data 360 Data Action Target. |  | x
| Signing Algorithm a| Enumeration, one of:

** HmacSHA256 |  Signing algorithm copied from Salesforce Data 360 Data Action Target. |  | x
