...
_Note: If a relative path is provided, the system will expect the file to exist in the corresponding location under `/src/main/resources` directory. For example, in  the screenshot below, the file is expected to be in `/src/main/resources/prod/keyStoreFile.jks` directory_
