= JDBC Connector
:keywords: jdbc
:page-aliases: 3.6@mule-runtime::jdbc-connector.adoc
...
http://www.mulesoft.org/docs/site/current3/schemadocs/namespaces/http_www_mulesoft_org_schema_mule_jdbc/namespace-overview.html[Schema]
...
NOTE: The JDBC connector supports multi-resource transactions. Multi-resource transactions are also supported by the xref:3.6@mule-runtime::database-connector.adoc[Database Connector]. Consider also the use of xref:3.6@mule-runtime::xa-transactions.adoc[XA Transactions] to handle this use case.
...
+
[NOTE]
JDBC Inbound Endpoints can only be set to a one-way exchange pattern. JDBC Outbound Endpoints can be set to either a one-way or request-response exchange pattern. For details about exchange patterns, see xref:6@studio::index.adoc[Anypoint Studio Essentials].
...
For details on setting the properties for a JDBC endpoint using an XML editor, consult the xref:3.6@mule-runtime::jdbc-transport-reference.adoc[JDBC Transport Reference].
