...
By default, the WSC consumer runs over a default configuration for the xref:0.3.6@http-request-connector::index.adoc[HTTP Request Connector]. If you need it to instead run over a configuration of the connector that uses HTTPS (or an HTTP configuration that is different from the default) follow the steps below:
