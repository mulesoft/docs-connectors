...
This document assumes that you are familiar with Anypoint Studio; review the chapter on xref:5@studio::index.adoc[Anypoint Studio Essentials] to learn more about developing with Mule's graphical user interface. This document also assumes you are familiar with xref:dataweave.adoc[DataWeave], xref:global-elements.adoc[Global Elements], and xref:5@studio::datasense.adoc[DataSense].
