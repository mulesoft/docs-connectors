= Web Service Consumer Connector 2.2 Reference 
...
| Must Understand a| Boolean |  Value of the mustUnderstand attribute in WS-Security header. |  true | 
...
| Expand XOP Include a| Enumeration, one of:

** TRUE
** FALSE | Configures whether XOP (XML-binary Optimized Packaging) includes are expanded inline within the SOAP message. If set to true, XOP includes are expanded inline. If set to false, XOP includes are not expanded. When not set, WSS4J uses its default behavior. |  | 
|===
