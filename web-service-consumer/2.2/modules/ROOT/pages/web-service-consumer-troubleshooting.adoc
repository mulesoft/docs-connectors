= Troubleshooting Web Service Consumer Connector 
...
To change this value, you have to set the `mule.soap.maxAttributeSize` system property with the accurate number according to your needs. Refer to the Knowledge article https://help.salesforce.com/s/articleView?id=001115098&type=1[How can I set Mule and Java system properties at startup?]). The maximum value allowed is 2147483647 (https://docs.oracle.com/javase/8/docs/api/java/lang/Integer.html#MAX_VALUE[Integer.MAX_VALUE] in Java). If you set 0 or if you don't set the property, the connector takes the default value.

[[disable-xxe-protection]]
=== Re-enable DOCTYPE and External Entity Processing in XML Parsing

Starting with Web Service Consumer Connector 2.2, XML parsing is hardened by default to mitigate XML External Entity (XXE) attacks. DOCTYPE declarations and external general/parameter entities are disabled when parsing SOAP messages.

If your integration legitimately requires DOCTYPE declarations or external entity processing in SOAP messages, you can re-enable these XML features by setting the following system property:

* `mule.soap.xml.disableXxeProtection` (default value = false)

Setting this property to `true` re-enables DOCTYPE and external entity processing. This is *not recommended*: doing so reintroduces exposure to XXE-based attacks. The runtime logs a warning when the property is active.

To set this property, refer to the Knowledge article https://help.salesforce.com/s/articleView?id=001115098&type=1[How can I set Mule and Java system properties at startup?].

[[no-content-type]]
== No Content-Type Boundled in the Message

In Web Service Consumer Connector 1.10.0 and earlier, you might see the `No Content-Type boundled in the message` error. In Web Service Consumer Connector 1.10.1 and later, the `No Content-Type boundled in the message` error is not thrown if the Content-Type header is missing in the response when the status code is not `200`. For more information, refer to the https://www.w3.org/TR/2000/NOTE-SOAP-20000508/#_Toc478383529[SOAP specification] and https://www.w3.org/TR/soap12-part2/#http-respbindprocess[HTTP Response Headers Fields].
