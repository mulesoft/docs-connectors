...
In Web Service Consumer Connector 2.2 and later, XML parsing is hardened by default to mitigate XML External Entity (XXE) attacks. DOCTYPE declarations and external general or parameter entities are disabled when parsing SOAP messages.

If your integration requires DOCTYPE declarations or external entity processing in SOAP messages, you can re-enable these XML features by setting this system property:

`mule.soap.xml.disableXxeProtection` (default value = `false`)

Setting this property to `true` re-enables DOCTYPE and external entity processing. This isn't recommended because doing so reintroduces exposure to XXE-based attacks. The runtime logs a warning when the property is active.

To set this property, refer to https://help.salesforce.com/s/articleView?id=001115098&type=1[How can I set Mule and Java system properties at startup?].
