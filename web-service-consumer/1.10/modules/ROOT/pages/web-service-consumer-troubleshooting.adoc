...
You might see the `No Content-Type boundled in the message` error. In Web Service Consumer Connector 1.10.1 and later, the `IllegalStateException` error is not thrown if the Content-Type header is missing in the response when the status code is not `200`. For more information, refer to the https://www.w3.org/TR/2000/NOTE-SOAP-20000508/#_Toc478383529[SOAP specification] and https://www.w3.org/TR/soap12-part2/#http-respbindprocess[HTTP Response Headers Fields].
