...
If your Mule Web Service Consumer app throws the message `WSC:CANNOT_DISPATCH`, and the third-party server returns `<faultcode>soap:MustUnderstand</faultcode>`, as the exception for the connector caused error, this indicates that the Web Service Consumer connector configuration has the Security setting *Must Understand* field set to the default value of `true`.
...
Subsequently, you can use a *Logger* component in your Mule app to log the entire payload body with the exception, for example, the following logs shows the decompressed payload SOAP body with `<faultcode>soap:MustUnderstand</faultcode>`:
