...
| Address a| String |  The address of the web service. This field is case-sensitive, use lowercase. |  |
...
| Time To Live a| Number a| The time difference between creation and expiry time in the time unit specified in `timeToLiveUnit`.
 After this time the message is invalid. This parameter's value must be greater than or equal to 1 second (or similar in another unit).
...
|===
| Field | Type | Description | Default Value | Required
| Response Timeout a| Number | The time (in milliseconds) to wait for a response.  | 5000 |
|===
