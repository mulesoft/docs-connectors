...
The default value is 524,288 characters and it is a reasonable default value for normal usage, but it may be changed to a bigger value if input documents can legitimately exceed the default limit, or to a lower value if specific attacks are observed or system has lower resource allocation.  
