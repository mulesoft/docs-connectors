= Web Service Consumer Additional Configuration Information 
...
=== Handling SOAP Faults with Custom HTTP Configurations

When using a custom HTTP Request configuration as your transport, the HTTP requester’s default behavior is to treat status codes like `500 Internal Server Error` as failures. Because many SOAP services return `SOAP Faults` with a 500 status code, the custom transport may throw an `HTTP:INTERNAL_SERVER_ERROR` before the Web Service Consumer Connector can parse the SOAP body.

To ensure that the Web Service Consumer Connector correctly receives and processes the SOAP Fault, you must configure the *Response validator* in your HTTP Request configuration to accept a broader range of status codes, typically `100..500`.

The following example illustrates how to capture the SOAP Fault when Web Service Consumer Connector uses custom HTTP Requester configuration:

. In the *HTTP Request configuration* window, navigate to the *Settings* tab.
. In the *Response options* section, set the *Response validator* field to `Success status code validator`.
. In the *Values* field, enter `100..500`.

In Studio, configuring the response validator for custom HTTP transport looks like this:

image::web-service-consumer-configure-response-validator.png[Configure HTTP Response Validator]
