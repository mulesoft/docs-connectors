...
. Enter values in the required fields. For a full list of elements, attributes and default values, consult  xref:3.7@mule-runtime::web-service-consumer-reference.adoc[Web Service Consumer Reference].
...
By default, the WSC consumer runs over a default configuration for the xref:3.6@http-request-connector::index.adoc[HTTP Outbound Endpoint]. If you need it to instead run over a configuration of the connector that uses HTTPS (or an HTTP configuration that is different from the default) follow the steps below:
...
. Add an xref:3.6@http-connector::index.adoc[HTTP Connector] global element in your project, configure it with the necessary security attributes
+
