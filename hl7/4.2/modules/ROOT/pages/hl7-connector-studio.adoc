...
. In the *Add Dependencies to Project* window, type `HL7 EDI` in the search field.
...
You can configure one of these sources to use with HL7 EDI Connector:
...
|Access Key | Enter the access key provided by HL7 EDI.
|Secret Key | Enter the secret key provided by HL7 EDI.
|Try Default AWS Credentials Provider Chain | Set to `true` to obtain credentials from the AWS environment.
|Region Endpoint | Select the region endpoint for the service.
|===
