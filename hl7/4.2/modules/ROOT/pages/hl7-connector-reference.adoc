...
| Disable numeric prefixes for data keys a| Boolean | Uses segment and group IDs directly as keys in the data, rather than prefixing the IDs with position values. |  true |
...
|===
| Field | Type | Description | Default Value | Required
| Frequency a| Number | How often to attempt to reconnect, iin milliseconds. |  |
