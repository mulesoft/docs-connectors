= HL7 EDI Connector 5.0 Reference
...
| Handle empty arrays for fields in message a| Boolean a| When the incoming payload contains empty arrays and the parameter is set to `true`, the connector includes the field separation character for optional repeatable fields.
|  false |

| Enforce Acknowledgement ERR Segment repeatability a| Boolean a| Whether to enforce the repeatability of the ERR segment (count) in the acknowledgement of HL7 messages. This configuration can be used for handling of ERR segments in HL7 messages prior to v2.5.

* If `true`, the connector  collapses all errors into one ERR segment, with multiple errors packed as repeated ERR‑1 (ELD) field occurrences joined by the repeatition terminator.
* If `false`, the connector does not enforce the repeatability of the ERR segment.
