= HL7 EDI Connector 4.3 Reference
...
| Handle empty arrays for fields in message a| Boolean a| Emits the field separation character for optional repeatable fields when the incoming payload contains empty arrays and the `Handle empty arrays for fields in message` parameter is set to *true*.
|  false |
