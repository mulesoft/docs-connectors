...
| Handle empty arrays for fields in message a| Boolean a| When an incoming payload has empty arrays and the parameter is `true`, the connector emits field separation character for optional repeatable fields.
|  false |
