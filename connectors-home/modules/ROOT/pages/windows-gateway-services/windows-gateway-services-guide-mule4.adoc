...
When the Anypoint Gateway service runs on one machine and needs to access a remote queue on another, ensure one of the following settings are in place based on your environment.

Option 1: Verify Machine Grouping

1. Ensure that both machines belong to the same workgroup.
2. If anonymous access is not enabled, grant full access to the "Anonymous" user on the remote queue.
3. Verify that the GET operation works successfully under these updated anonymous access settings.

Option 2: Domain Membership and Permissions

1. Confirm that both machines are members of the same domain.
2. Add the machine running the Anypoint Gateway service to the permission settings of the remote queue.Grant full access to this machine on the remote queue.
3. Ensure that GET operations work correctly under these domain and permission settings.

*Note :* Only one of these configurations is required to enable the Anypoint Gateway service to access the remote queue. Select the setup that best matches your environment configuration.
