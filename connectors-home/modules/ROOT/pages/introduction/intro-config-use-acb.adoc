...
+
If the connector isn't available locally, click the image:anypoint-code-builder::icon-search-exchange.png["Search in Exchange icon",15,15] (*Search in Exchange*) toggle:
+
...
You can update a connector to a newer version in Anypoint Code Builder. For instructions, see xref:anypoint-code-builder::int-versions.adoc#project-versions-connectors[Select Connector Versions for a Project].

Updating to a new major version (for example, from `1.x` to `2.x`) can introduce backward-compatibility changes. Before you upgrade, review the release notes and, if available, the upgrade and migration guide for the connector.
...
|Streaming Strategy a| * xref:http-connector::http-documentation.adoc#repeatable-in-memory-stream[Repeatable in-memory stream]
* xref:http-connector::http-documentation.adoc#repeatable-file-store-stream[Repeatable file store stream]
* Non-repeatable stream

For more information about configuring the streaming strategy, see <<configuring-streaming-strategy>>.
| No
...
|Redelivery policy |Configures the redelivery policy for executing requests that generate errors. You can add a redelivery policy to any source in a flow. For more information about configuring the redelivery policy, see xref:http-connector::http-documentation.adoc#RedeliveryPolicy[Redelivery Policy]. | No
|Reconnection strategy |When an operation in a Mule application fails to connect to an external server, the default behavior is for the operation to fail immediately and return a connectivity error. You can modify the default behavior by configuring the reconnection strategy for the operation. See xref:http-connector::http-documentation.adoc#reconnect[Reconnect], xref:http-connector::http-documentation.adoc#reconnect[Reconnect Forever], and <<configuring-reconnection-strategy>>. | No
| Response |Attributes that contain the metadata associated with the body (or payload) of the message. For more information, see xref:http-connector::http-documentation.adoc#HttpResponseAttributes[HTTP Response Attributes]. | No
...
For details about the Listener configuration attributes, see xref:http-connector::http-documentation.adoc#listenerConfig[Listener Configuration].
...
include::anypoint-code-builder::partial$mule-components/4.4/component-scheduler.adoc[tag="component-xml-attributes-child1"]
...
include::anypoint-code-builder::partial$mule-components/4.4/component-scheduler.adoc[tag="cron-expressions"]
...
|Scheduling strategy |Triggers the flow's execution based on a time-based condition. See <<use-scheduler-trigger>>.| Yes
|===
...
| Redelivery policy | For more information, see xref:salesforce/salesforce-connector-reference.adoc#RedeliveryPolicy[Redelivery Policy] | No
...
* HOURS
* DAYS | No
|Reconnection strategy |See xref:salesforce/salesforce-connector-reference.adoc#reconnect[Reconnect] and xref:salesforce/salesforce-connector-reference.adoc#reconnect-forever[Reconnect Forever], and <<configuring-reconnection-strategy>>. | No
|===
