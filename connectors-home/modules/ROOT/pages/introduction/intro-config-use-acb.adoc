...
A configuration file is a YAML file that can reference different configurations and specify different values for your connector properties. Use configuration files to define specific properties depending on the environment you are deploying your application to.
...
. In your project XML file, create a new line after the `<configuration-properties/>` element, then type `salesforce`, and select *salesforce:sfdc-config*.
...
| Response streaming mode | Defines whether to use streaming when the response is sent. | No
