...
[NOTE]
====
If you are deploying to multiple regions or have some other gateway configured, use the hostname of the load balancer or the gateway in your OAuth provider callback URL and in *External Callback Url*.

If your Salesforce instance does not support HTTP or HTTPS callback URLs with private cloud URLs, you can update the callback URL to use a public endpoint.
====
+
