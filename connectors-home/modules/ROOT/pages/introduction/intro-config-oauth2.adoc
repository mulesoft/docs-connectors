...
Here are the configuration fields typically available for OAuth 2.0 for connectors. For more information, see xref:mule-sdk::oauth.adoc[].
...
| Auth Server Tls Context | TLS Context | TLS context to use for HTTP requests to the authorization server during the OAuth dance. Available in Mule runtime 4.10.0 and later. | |
| Auth Server Proxy Config | Proxy Config | Proxy configuration to use for HTTP requests to the authorization server during the OAuth dance. Available in Mule runtime 4.10.0 and later. | |
|===
