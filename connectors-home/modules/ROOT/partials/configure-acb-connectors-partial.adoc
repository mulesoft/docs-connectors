...
A trigger (source) starts the flow when specified conditions are met. If a connector has its own associated triggers, you can use a connector-specific trigger to initiate the flow. Each connector source has a unique configuration.
