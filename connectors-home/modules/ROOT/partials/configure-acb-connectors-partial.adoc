...
. In the *General* tab, in the *Path* field, enter the relative path for the path set in the HTTP configuration element. See xref:connectors::http-listener-ref.adoc#configure-paths[Configure Paths]. 
. Select the *Advanced* tab to configure the following attributes:
+
[%header%autowidth.spread]
|===
| UI Field | Description 
|Allowed methods |To allow all HTTP methods (default), do not define this attribute. To restrict certain methods, specify all allowed HTTP methods, for example, `GET, POST`, as a comma-separated list.  
| Response streaming mode | Defines whether or not to use streaming when the response is sent.
| Output MIME type | Defines the format of the response.
|Parameters | Adds additional parameters for the *Listener*.
| Output encoding | Selects the encoding to transform the data to.
...
|Primary node only |Select this option to execute the *Listener* trigger on the primary node only when running in a cluster.
|Redelivery policy |This attribute is not currently enabled in the configuration UI. Configure it in the XML editor. See xref:http-documentation.adoc#RedeliveryPolicy[Redelivery Policy].
...
| Response |See xref:http-documentation.adoc#HttpResponseAttributes[HTTP Respoonse Attributes]
| Error Response | HTTP error response returns a status code for the error. 
|Body | Body of the error message.
|Headers |Headers to include in the error message.
|Status code | Status code of the error response, for example `500`.
|Reason phrase | Reason for the error, for example `Gateway Timeout` or `Internal Server Error`.
|===
