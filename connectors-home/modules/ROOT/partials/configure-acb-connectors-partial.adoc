...
+
image:connectors::acb-create-integration.png["Develop an Integration Flow window in Anypoint Code Builder"]
...
+
image:anypoint-code-builder::int-add-connector-operation.png["Add Publish operation from Anypoint MQ Connector"]
+
If the connector is not available locally, click the image:anypoint-code-builder::icon-search-exchange.png["Search in Exchange icon",15,15] (*Search in Exchange*) toggle:
+
...
|Streaming Strategy | Configures the way data streams are consumed in Mule apps.  | No
...
|Reconnection strategy |When an operation in a Mule application fails to connect to an external server, the default behavior is for the operation to fail immediately and return a connectivity error. You can modify the default behavior by configuring the reconnection strategy for the operation.  | No
| Response |See xref:http-connector::http-documentation.adoc#HttpResponseAttributes[HTTP Response Attributes]. | No
...
+
image:connectors::acb-build-flow.png["Anypoint Code Builder canvas with starting cards with the options to start from scratch, build a flow, or build a subflow"]
+
