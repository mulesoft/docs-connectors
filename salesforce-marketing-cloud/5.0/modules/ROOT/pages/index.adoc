= Salesforce Marketing Cloud Connector 5.0
...
Salesforce Marketing Cloud Connector 5.0 is a major release that requires Java 17 and Mule runtime engine 4.9.0 or later. The same operations from the 4.x release are preserved. Key changes in this release include:

* The Spring Framework is upgraded to version 6.x, which requires Java 17 as a minimum.
* The connector no longer depends on FuelSDK. Instead, it uses CXF-generated classes from the connector WSDL to perform SOAP requests directly.
* The deprecated `omitInternalIdFromCustomFields` parameter is removed.
* Multiple libraries are upgraded to address security vulnerabilities. For a full list, see xref:salesforce-mktg-connector-upgrade-migrate.adoc[Upgrading and Migrating Salesforce Marketing Cloud Connector to 5.x].
...
+
image::salesforce-mktg-3-user-pass-config.png["Global element properties window for Basic Authentication"]
+
...
+
image::salesforce-mktg-3-oauth-client-credentials-config.png["Global element properties window for OAuth Client Credentials"]
...
+
image::salesforce-mktg-3-http-listener-config.png["HTTP Listener component properties window"]
+
...
. Double-click *Create*.
+
image::salesforce-mktg-3-smc-create-config.png["Create entities properties window"]
+
...
+
image::salesforce-mktg-3-transform-before-config.png["Window of Transform message before configuration"]
+
. Double-click *Transform Message* (to the right of *Create* in the flow) and configure as shown below:
+
image::salesforce-mktg-3-transform-after-config.png["Window of Transform message after configuration"]
+
. Deploy the app.
...
image::salesforce-mktg-3-proxy.png["Proxy fields configuration"]
...
image::salesforce-mktg-3-schedule-start-automation.png["Schedule Start Automation configuration window"]
...
image::salesforce-mktg-3-create-automation-config.png["Create options field with value specified"]
...
image::salesforce-mktg-as-create-automation-options.png["Create options output"]
...
* https://developer.salesforce.com/docs/marketing/marketing-cloud/guide/getting_started_developers_and_the_exacttarget_api.html[Salesforce Get Started with the SOAP Web Services API]
* https://developer.salesforce.com/docs/marketing/marketing-cloud/guide/api-integration.html[Salesforce API Integration].
