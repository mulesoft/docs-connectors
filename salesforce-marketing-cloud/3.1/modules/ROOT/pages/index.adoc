...
An important thing to mention is that our connector does not send the SQL query to the Salesforce server as it is. Instead, the query is parsed and transformed into an XML object that is sent to the server. Because of this, any other filter that is not included in the table above is not supported as well as any other SQL syntax features as: LIMIT, OFFSET, ALIAS, JOINs and so on. For more details please see the https://developer.salesforce.com/docs/marketing/marketing-cloud/guide/retrieverequest.html#properties[*Salesforce API Guide*].
