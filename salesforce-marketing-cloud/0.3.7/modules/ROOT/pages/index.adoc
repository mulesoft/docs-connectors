...
This document assumes that you are familiar with Mule, the xref:5.x@studio::index.adoc[Anypoint Studio interface], xref:3.7@mule-runtime::global-elements.adoc[Global Elements], xref:5.x@studio::datasense.adoc[DataSense], and xref:3.7@mule-runtime::batch-processing.adoc[batch processing] in Mule. Further, it assumes you are familiar with Salesforce Marketing Cloud.
