= Using Anypoint Studio to Configure Amazon Redshift Connector 1.1 - Mule 4
...
. On the *General* tab, configure the fields depending on the connection type.
+
* Basic Connection
+
[%header,cols="30s,70a"]
...
|Region Endpoint | Region Endpoint (optional)
...
|Database | Database name
|User | Database username
|Access key | Access key provided by Amazon for retrieving temporary database credentials
|Secret key | Secret key provided by Amazon for retrieving temporary database credentials
|Role | Role configuration (optional)
|Custom STS Endpoint | Sets a custom STS endpoint. Useful when a non-standard service endpoint is required, such as a VPC or FIPS endpoint (optional and on Advanced tab).
|===
+
