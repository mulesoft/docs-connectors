...
This example shows you how to use Anypoint Connector for Amazon Redshift (Amazon Redshift Connector) to load data from Amazon S3 to Redshift.
...
. Open the `redshift-demo.xml` file in `src/main/mule` folder and follow the instructions for xref:amazon-redshift-connector-studio.adoc#add-connector-to-project[how to add the Amazon Redshift Connector to the project]
. Repeat the previous step to add Amazon S3 Connector and File Connector to the Mule project.

== XML Code for This Example

Overwrite the content of `redshift-demo.xml` file with the following XML snippet:
...
[NOTES]
---
* S3 credentials are essential in order to create and delete buckets and to create an object.
* Redshift credentials are required to establish a connection to the database.
* This example is using a `COPY` command to load a table in parallel from a data file on Amazon S3. To use this `COPY` command, you must authenticate using your IAM user credentials with the correct policies attached to it. It is best to use the Amazon S3 Read only policy for the `copy.access_key` and `copy.secret_key` properties. 
* Alternatively, it is possible to use an IAM Role instead of the Access and Secret key pair. To do this, you must ensure the role has the correct policies attached to it. It is best to use the Amazon S3 Read only policy for this.
** To create an IAM role to allow your Redshift cluster to communicate with the Amazon S3 service on your behalf, follow the steps in this https://docs.aws.amazon.com/redshift/latest/mgmt/authorizing-redshift-service.html[tutorial].

The following images show the S3 and Redshift configurations:

image::s3-configuration.png[S3 configuration]

image::redshift-configuration.png[Redshift configuration]

[[prepare-data-file]]
=== Prepare the Data File

Prepare a data file, which will be uploaded to S3 and then used as the data set in the `COPY` command for the Redshift table:
...
+
image::create-flow.png[Create the Redshift table S3 bucket and S3 object flow]
+
...
+
* This flow deletes the Redshift table and S3 bucket:
+
image:delete-flow.png[Delete the Redshift table and S3 bucket flow]
...
. Verify that the new S3 bucket `username-bucket` was created in your S3 instance.
. Verify that the new Redshift table `username` was created in your Redshift instance.
...
. Verify that both the `username` Redshift table and `username-bucket` S3 bucket were deleted.
