...
** Optionally, you can specify *ARN* field value in the *Role* field if the connector should first assume an IAM role for a set of temporary security IAM credentials and use those to retrieve the password.
...
Or alternatively, you can use sources such as HTTP Listener, or Scheduler.
...
. Select the created connector configuration or click the plus sign (*+*) next to the *Connector _configuration_* field to configure a new global element.
