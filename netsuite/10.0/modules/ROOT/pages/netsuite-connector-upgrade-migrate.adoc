...
The following operations are now primary content parameters:
...
Additionally, since it is now a content parameter, instead of using it like this: +
`<netsuite:async-search config-ref="..." criteria="something">`, you now need to assign it like this: +
----
...
If you are upgrading from an earlier version of the connector, you can click the `Updates Available` pop-up message in the bottom right corner of Anypoint Studio to upgrade your connector and follow the prompts. 
