...
You will notice that the `On Modified Object` listener triggers new objects aswell, that's because Netsuite populates the field `lastModifiedDate` at creation, which is the field we use to poll objects, so when new objects are created, the `lastModifiedDate` is populated and the object will be polled.
