...
. On the *General* tab, configure the authentication mechanism:
* <<token-authentication,Token Authentication>>, which authenticates by using either a consumer key and token ID or secret pairs for authentication (*Reviewers, is this correct? A:Yes*)
+
...
With token authentication, you authenticate by using either a consumer key and token ID or by using secret pairs, which are established within the NetSuite environment.(*Reviewers, is this description correct? A: Yes*)
...
After configuring Studio, see the xref:netsuite-examples.adoc[Search Examples] topic for more configuration ideas.
