...
* <<request-based-authentication,Request-Based Authentication>>, which authenticates by using the SuiteTalk login API call on the first request that the connector sends (*Reviewers, is this text correct? Is this actually OAuth 2.0? If yes, should we mention that in the docs? Studio does not call this OAuth 2.0 in the v10.0 release.*)
...
Use request-based authentication to send user credentials in the SOAP header of each request instead of authenticating to NetSuite using the login ID and password. [*Reviewers, is this description correct?*]
...
This token-based authentication method uses either a consumer key and token ID or secret pairs, which are established within the NetSuite environment:
