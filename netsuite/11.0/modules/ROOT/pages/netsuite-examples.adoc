...
Creating this example involves creating a new Mule project and configuring `HTTP Listener`, the NetSuite Connector `Search` operation, one for-each, and two `Transform Message` components.
...
Creating this example involves creating a new Mule project and configuring `HTTP Listener`, the NetSuite `Search` operation, two `Transform Message` components, and a `For-Each` scope.
...
. Drag a *Transform Message* component inside the *For Each* box, rename the component to "Response to JSON" and then replace the DataWeave codewith this code:
+
[source,dataweave,linenums]
----
%dw 2.0
