...
IMPORTANT: This content describes features that are pre-GA status and is intended for testing only.
...
|===
| Operation |Changes
| Change Email | Returns a NetSuiteSoapResult object in the output metadata
| Change Password | Returns a NetSuiteSoapResult object in the output metadata
| Get Account Governance Info | Returns a GetAccountGovernanceInfoResult object in the output metadata
| Get Data Center URLs | Returns a NetSuiteSoapResult object in the output metadata.
| Get Integration Governance Info | Returns a GetIntegrationGovernanceInfoResult object in the output metadata
| Get Server Time | Returns a NetSuiteSoapResult object in the output metadata
|===

=== Async Operations

Each async operation now returns an asynGetListResponse object in the output metadta. Other changes to the async operations include the following:
...
Each item operation now return an NetSuiteSoapResult object in the XML metadata. Other changes to the invitee operations include the following:
...
|===
| Operation |Changes
| Add File | Removed this operation. For equivalent functionality, use the Add Record operation and specify `FILE` as the record type in the Message field of the input metadata.
| Add List a|
...
* Returns a NetsuiteSoapResult object in the output metadata.
...
*  Returns a NetsuiteSoapResult object in the output metadata with an attached object as the payload..
| Delete a|
...
* Returns a NetsuiteSoapResult object in the output metdata.
...
| Detach Record a| * Removed the Source and Destination fields. You now specify the associated values in the Message field of the input metatada.
* Returns a NetsuiteSoapResult object in the output metdata.
...
| Get Records | Returns a NetsuiteSoapResult object in the output metadata
...
* Returns a NetsuiteSoapResult object in the output metadata.
| Initialize List a| * Removed the Initialize Records field. You now specify the associated values in the Message field of the input metatada.
* Returns a NetsuiteSoapResult object in the output metadata.
