...
This document assumes that you are familiar with Mule, the xref:6.x@studio::index.adoc[Anypoint Studio interface], and xref:3.6@mule-runtime::global-elements.adoc[Global Elements]. Further, it is assumed that you are familiar with the Microsoft .NET Framework.
...
* Anypoint Studio, the graphical interface to Mule. See xref:5.x@studio::index.adoc[Download and Launch Anypoint Studio] for details.
...
|*xref:5.x@studio::datamapper-concepts.adoc[DataMapper]* |The DataMapper is a Mule transformer that delivers simple, yet powerful, visual design of complex data transformations for use in Mule flows
|===
