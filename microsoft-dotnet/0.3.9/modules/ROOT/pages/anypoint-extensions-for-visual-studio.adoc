...
This document assumes that you are familiar with Mule, the xref:studio::index.adoc[], and xref:3.9@mule-runtime::global-elements.adoc[Global Elements]. Further, it is assumed that you are familiar with the Microsoft .NET Framework.
...
* Anypoint Studio, the graphical interface to Mule. See xref:studio::to-download-and-install-studio.adoc[]
