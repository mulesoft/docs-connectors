...
+
If the *Declared methods only* field is set to `true`,  the connector returns only the declared methods from the .NET assembly.
* *Include auto generated code*
+
If the *Include auto generated code* field is set to `false` (the default value), the connector does not contain the code generated by the compiler
+
. On the *Advanced* tab, optionally specify reconnection information, including a reconnection strategy.
