...
When `errorsThreshold` is reached, the circuit trips and stops polling for new messages for the duration specified by `tripTimeout`.

Anypoint MQ:

* Marks all messages as not acknowledged (NACK).
* Doesn't poll or acknowledge (ACK) any messages.
* Halts all operations.

With multiple workers (multi-threaded), each thread has its own circuit breaker and acts based on the values of `errorsThreshold` and `tripTimeout`.

After `tripTimeout` elapses, clients can consume messages on the next poll.
