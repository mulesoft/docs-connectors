...
* All messages are not acknowledged (NACK).
* No messages are polled or acknowledged (ACK).
* All operations are halted.

With multiple workers (multi-threaded), each thread has its own circuit breaker and acts based on `errorsThreshold` and `tripTimeout`.

Messages are consumed again on the next poll after `tripTimeout`` elapses.
