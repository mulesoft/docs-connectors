...
When using the `prefetch` mode, the `acknowledgementTimeout` timer begins from the moment the message is taken from the queue. The time the message remains in the local buffer doesn't count as part of the timeout.

Don't use `prefetch` mode for applications with long processing times or that require messages to be processed only once.
