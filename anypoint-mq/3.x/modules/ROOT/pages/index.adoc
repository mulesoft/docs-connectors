...
. Set the `cross.region.failover.feature.enabled` system property to `true`. (where?)
...
* To specify the interval that the connector checks the fallback queue for messages,
set the `fallback.messages.status.check.interval.ms` property.
* To specify the interval that the connector checks to see if the primary queue is back up,
set the `primary.region.status.check.interval.ms` property.

Set the property either in *Run Configurations* in Anypoint Studio or as an app property in Runtime Manager. 
For more information about setting properties, see:

* xref:mule-runtime::mule-app-properties-system.adoc[] 
