...
| Acknowledgment Timeout a| Number |  Duration that a message is held by a broker waiting for an acknowledgment or not acknowledgment before being made available to any subscriber. Specifies the default `acknowledgmentTimeout` value for all Subscriber sources, unless it is overridden. 
The maximum value of this parameter is 12 hours. | If `acknowledgementTimeout` is not set (the default), the connector uses the TTL value of at least two minutes. |
| Acknowledgment Timeout Unit a| Enumeration, one of:
