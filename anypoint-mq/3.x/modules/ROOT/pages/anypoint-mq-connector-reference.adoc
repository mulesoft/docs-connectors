...
| Object Store a| Object Store | Object Store created by the Mule default `ObjectStoreManager`: non-persistent, `entryTtl` value of 300 seconds and an `expirationInterval` value of 6 seconds.| Object Store in which the redelivery counter for each message is stored. You can configure the Object Store as a reference or an inner element.

For information, see xref:object-store-connector::index.adoc[Object Store Connector]. |
|===
