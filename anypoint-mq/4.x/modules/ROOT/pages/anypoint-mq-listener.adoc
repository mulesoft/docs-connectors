...
| Backpressure disabled (default) | Subscriber behaves exactly as before this release. | None.
| Backpressure enabled, `maxConcurrency` not set | Subscriber behaves exactly as before this release. | None.
| Backpressure enabled, `maxConcurrency` set | The polling strategy honors `maxConcurrency` as a true in-flight limit. Under heavy load, fetching paces itself to the flow instead of running ahead. | Under sustained load, message intake is throttled to the flow's capacity. Acknowledgment and redelivery behavior is identical.
|===
