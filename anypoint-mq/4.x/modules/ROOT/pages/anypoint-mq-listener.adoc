...
The behavior depends on whether backpressure is enabled and whether `maxConcurrency` is set on the flow:
...
| Feature enabled, `maxConcurrency` not set | Subscriber behaves exactly as before this release. | None.
...
. If you need strict ordering, use the polling strategy with `maxConcurrency = 1`, and expect a higher API request volume.
