...
[NOTE]
With failover enabled, the app can't consume directly from the fallback queue.
...
To process any messages that remain in the fallback queue after disabling failover, <<disable-failover-feature,disable failover on the app>> to allow the app to consume messages directly from the fallback queue.  
