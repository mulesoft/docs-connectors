...
+
image::mq-set-properties-failover-enabled.png["Properties table with cross.region.failover.feature.enabled set to true"]
...
+
image::mq-publish-fallback-queue-subscribe.png["Subscriber configuration with the queue name, myDemoQueue_fb"]
...
+
image::mq-set-properties-failover-disabled.png["Properties table with cross.region.failover.feature.enabled set to false"]
+
Alternatively, set the property in *Run Configurations* in Anypoint Studio.
For more information, see xref:mule-runtime::mule-app-properties-system.adoc[System Properties in Mule Apps].
...
+
image::mq-set-properties-failover.png["Properties table with intervals for primary and fallback status check intervals"]
