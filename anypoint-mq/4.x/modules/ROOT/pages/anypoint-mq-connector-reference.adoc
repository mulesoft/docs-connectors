...
| Acknowledgment Timeout a| Number |  Duration that a message is held by a broker waiting for an acknowledgment or not acknowledgment before being made available to any subscriber. Specifies the default `acknowledgmentTimeout` value for all Subscriber sources, unless it is overridden. If `acknowledgementTimeout`` is not set (the default), MQ Connector uses the value of 2 minutes.|  |
| Acknowledgment Timeout Unit a| Enumeration, one of:
