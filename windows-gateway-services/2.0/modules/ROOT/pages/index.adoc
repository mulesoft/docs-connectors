...
When you configure the connector running in Mule runtime, set the authorization token value using the Gateway Access Token configuration setting (`accessToken` attribute in the connector's XML configuration).

[NOTE]
The installer for the Windows Gateway service automatically generates a cryptographically secure token for use by callers upon first installation. This token is displayed and placed upon the clipboard during installation for easy copying into a Mule application.
...
The following table shows the configuration settings for MSMQ Connector:
...
For more information about the MSMQ Connector settings, see the xref:msmq/msmq-connector-reference.adoc[MSMQ Connector Reference].

=== About Impersonating a Windows User From a Remote Queue

When your queue is marked to require authentication then you can impersonate the caller user as previously specified (in the same general section). In addition to this, if you work with a remote queue the connector has a particular header to override this behavior.
...
*Note*: When running in a load-balanced configuration, the Gateway Services must be configured to run as 'Administrator' when the nodes involved (MSMQ, gateways) are under a WORKGROUP but not joined to a DOMAIN. When joined to the same DOMAIN the permissions for each of the nodes and objects involved (queues) must be correctly set by the domain's administrator.

== Windows Gateway Services Troubleshooting
...
The Windows Gateway Services is configured to log Information events. This is configured under the `<switches>` element. If you want to log everything, you should use the Verbose level, by changing it in the configuration element.
...
A useful way to troubleshoot issues is to enable the console listener, which is enabled by default. If it's not enabled, add it to the listeners section and run the Windows Gateway Services from the command line.
...
. To enable the console listener, add it to the listeners collection:
+
[source,xml,linenums]
----
<sources>
...
. When you are done troubleshooting, close the console and restart the Windows service.
