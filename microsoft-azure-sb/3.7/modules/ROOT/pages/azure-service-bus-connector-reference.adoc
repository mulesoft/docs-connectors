= Azure Service Bus Connector 3.7 Reference 
...
| Security a| TLS |  TLS enabled by default, using the default JVM certificates to secure communication. |  | x
...
Sends the message associated with the Lock Token to the entity's native dead-letter sub-queue, recording an optional reason and description on the dead-lettered message. This operation can be used only from inside a flow that also includes a *Message Listener Source* operation.
...
** IMMEDIATE
** MANUAL | Acknowledgment mode to use when consuming messages. IMMEDIATE acknowledges messages automatically upon receipt (uses ReceiveAndDelete mode). MANUAL requires explicit acknowledgment via the Complete or Abandon operations (uses PeekLock mode). |  | x
...
| Destination Name a| String | Name of the destination from which messages are received |  | x
| Subscription Name a| String | Name of the subscription from which messages are received |  NONE |
| Consume From Dead Letter Queue a| Boolean |  When enabled, the listener consumes messages from the entity's native dead-letter sub-queue (`$deadletterqueue`) instead of the main entity. Cannot be combined with a Message Session Id, since dead-letter queue messages are session-less. |  false |
