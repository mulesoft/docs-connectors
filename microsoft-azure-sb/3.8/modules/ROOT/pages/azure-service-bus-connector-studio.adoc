= Using Anypoint Studio to Configure Azure Service Bus Connector 3.8 
...
. From *Ack Mode*, select from these options:
* *Auto* +
...
. (Optional) To receive messages from session IDs that are generated dynamically and not known in advance, select the *Advanced* tab and select *Enable Dynamic Sessions*. When this option is enabled, each consumer continuously accepts the next available Service Bus session, drains it, and rolls to the next one, instead of locking a fixed *Message Session Id*.
* Use *Number Of Consumers* to control how many sessions are processed concurrently.
* Use *Session Idle Timeout* (an ISO-8601 duration such as `PT30S`) to control how long a consumer waits for the next message in the currently locked session before releasing it and rolling to the next available session.
+
*Enable Dynamic Sessions* cannot be combined with a fixed *Message Session Id* or with *Consume From Dead Letter Queue*. For details about these fields, see the *Message Listener* section of the xref:azure-service-bus-connector-reference.adoc#message-listener[Azure Service Bus Connector Reference].
...
image::azure-service-bus-sas-connection.png["Authentication configuration of fields on the General tab"]
...
image::azure-service-bus-aad-connection.png["Authentication configuration of fields on the General tab"]
