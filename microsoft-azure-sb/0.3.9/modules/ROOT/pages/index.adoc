...
=======
The difference between maximumRetryCount and reconnect is that the first is part of the RetryPolicy and is used at the library level. When there is a connection problem, Azure uses these parameters to try to re-establish
the connection internally. If this does not happen, the library sends an Exception to the connector and the connector then tries to re-establish the connection
>>>>>>> a7f8626866de6fa8158bcbe20551be66d7468fad
