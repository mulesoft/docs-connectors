= Azure Service Bus Connector 2 Reference - Mule 4
...
Azure AD integration for Service Bus resources provides Azure role-based access control (RBAC) for fine-grained control over a client’s access to resources.

Through the use of https://docs.microsoft.com/azure/service-bus-messaging/authenticate-application[Azure AD app registrations] users can generate a Client Id and Client Secret to authenticate into the tenant where the Azure Service Bus instance is running.
...
| Name | Type | Description | Default Value | Required
| Service namespace a| String |  The namespace contains the resources to which the connector can establish a connection. |  | x
| Tenant Id a| String |  Tenant where the app registration and the Azure Service Bus instance is running |  | x
