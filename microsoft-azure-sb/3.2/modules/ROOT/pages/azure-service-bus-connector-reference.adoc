= Azure Service Bus Connector 3.2 Reference - Mule 4
...
Due to a limitation on the SDK that Azure Service Bus Connector uses provided by Azure, in some cases, the *Receive Batch* operation can receive fewer messages than the ones that are available. To receive all the remaining messages, call the operation again.
