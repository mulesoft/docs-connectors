...
When using the Message Listener, the Azure Service Bus library will emit a log message with INFO level every 30 seconds approximately, indicating that no new messages were received. To avoid this excess log, the `log4j.xml` file must be edited by adding the following AsyncLogger:
