= Use Studio to Configure Azure Service Bus Connector 3.1 - Mule 4
...
When using the Message Listener source, the Azure Service Bus library emits a log message with INFO level approximately every 30 seconds, indicating that no new messages were received. To avoid this excessive logging, add the following AsyncLogger element to the `log4j.xml` file:
