= Azure Service Bus Connector 3.1 Reference - Mule 4
...
| Buffered Mode a| Boolean |  +++Buffered is the default mode: the consumer(s) send the message to the flow and immediately receive the next one. If it is disabled, the consumer(s) will not receive the next message until the current one exits the flow.+++ |  +++true+++ |
| Output Mime Type a| String | MIME type of the payload that this operation outputs |  | 
| Output Encoding a| String |  Encoding of the payload that this operation outputs |  | 
