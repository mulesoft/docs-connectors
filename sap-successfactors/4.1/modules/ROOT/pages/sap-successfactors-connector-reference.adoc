...
* `SNAPSHOT`
+
Is a server-side pagination that works by keeping a list of all business keys of the data set on the server. The `SNAPSHOT` pagination type does not support `$top` or `$skip`.
 | `CLIENT` |
| Config Ref a| ConfigurationProvider |  The name of the configuration to be used to execute this component |  | *x*
