...
** CLIENT
** CURSOR
** SNAPSHOT |  Pagination type used by the *Query* operation.

The `CURSOR` pagination type does not support `$orderby`. The `SNAPSHOT` pagination type does not support `$top` and `$skip`. | CLIENT | 
| Config Ref a| ConfigurationProvider |  The name of the configuration to be used to execute this component |  | *x*
