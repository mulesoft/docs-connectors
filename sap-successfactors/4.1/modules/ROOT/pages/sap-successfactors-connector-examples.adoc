= SAP SuccessFactors Connector 4.1 - Examples 
...
+
image::sap-successfactors-basic-config.png["Basic authentication in global element properties window"]
+
. Click *OK*.
...
http://www.mulesoft.org/schema/mule/successfactors http://www.mulesoft.org/schema/mule/successfactors/current/mule-successfactors.xsd">
    ...
    <set-variable value='#[now() as Date {format: "yyyyMMdd"}]' doc:name="Set current date" variableName="currDate" />
