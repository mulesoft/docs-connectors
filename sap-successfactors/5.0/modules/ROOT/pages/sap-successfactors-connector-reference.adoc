= SAP SuccessFactors Connector 5.0 Reference 
...
| Connection a| * <<Config_Basic, Basic Authentication - Deprecated>>
...
This connection type is deprecated. Use <<Config_Oauth, OAuth SAML Bearer Assertion Authentication>> instead.
...
| Default query parameters a| Array of <<DefaultQueryParam>> a|  Default Query parameters to include in the request. Here is how the expression should be defined:

----
"defaultQueryParams": [
  { "key": "fromDate", "value": "2025-01-01" },
  { 'key': 'asOfDate', 'value': vars.currDate as String },
  { 'key': 'balanceAsOfDate', 'value': attributes.queryParams.effective_date as String },
  { "key": "toDate", "value": payload.toDate as String }
]
----
 |  |
...
| The token url a| String |  The token URL that is passed to the identity provider to generate the OAuth Bearer token. | `\https://api4.successfactors.com/oauth/token`  |
| Key Store a| String | Allows to select the keystore file. |  | x
| Store Password a| String | Corresponding keystore password. |  | x
| Type a| String |  Type of the keystore. |  `jks` | 
| Certificate Alias a| String | Alias name for the certificate stored in the keystore. |  | 
...
|======================
| Field | Type | Description | Default Value | Required
| Key a| String | The key. For example, `balanceAsOfDate`. |  | x
| Value a| String | The value. For example, `vars.effectiveDate` (as a String).|  | x
|======================
