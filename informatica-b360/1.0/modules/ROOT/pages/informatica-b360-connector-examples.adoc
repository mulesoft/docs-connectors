...
This example performs a full-text search with filters and iterates over paginated results.
...
<flow name="search-flow">
    ...
    <informatica-b360:search
        ...
        <informatica-b360:filters><![CDATA[#[output application/json ---
        [
            {
                comparator: "EQUALS",
                fieldName: "lastName",
...
== Discover Available Business Objects

This example retrieves all available business entity types from the B360 tenant.

=== XML Configuration

[source,xml,linenums]
----
<flow name="list-objects-flow">
    <http:listener config-ref="HTTP_Listener_Config" path="/list-objects" />

    <informatica-b360:list-objects config-ref="InfaB360_Config" />

    <!-- Returns a list of BusinessObject with guid, name, and description -->
    <foreach>
        <logger level="INFO"
            message="Object: #[payload.name] (GUID: #[payload.guid])" />
    </foreach>
</flow>
----
