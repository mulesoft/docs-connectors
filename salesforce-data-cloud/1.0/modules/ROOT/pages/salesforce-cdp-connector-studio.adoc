= Using Anypoint Studio to Configure Salesforce CDP Connector 1.0 - Mule 4
...
NOTE: If a relative path is provided, then the file must exist in the corresponding location under the `/src/main/resources` directory. For example, in the following screenshot, the file is expected to be in the `/src/main/resources/prod/keyStoreFile.jks` directory.
