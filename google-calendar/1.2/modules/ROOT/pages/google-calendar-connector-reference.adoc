...
| Access Type a| String |  Indicates whether your application can refresh access tokens when the user is not present at the browser. Valid values are `online` and `offline`. If `offline` is set, a refresh token is retrieved. |  `offline` |
| Prompt a| String |  Valid values are `consent`, `none`, or `select_account`. Must be set to `consent` to retrieve a refresh token. |  `consent` | 
