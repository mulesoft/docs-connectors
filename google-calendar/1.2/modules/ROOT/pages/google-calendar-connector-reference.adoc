= Google Calendar Connector 1.2 Reference
...
| Access Type a| String |  Indicates whether your application can refresh access tokens when the user is not using the browser to access the application. Valid values are `online` and `offline`. If `offline` is set, a refresh token is retrieved. |  `offline` |
| Prompt a| String |  Indicates whether to retrieve an access token or not. Valid values are `consent`, `none`, or `select_account`. Must be set to `consent` to retrieve a refresh token. |  `consent` | 
