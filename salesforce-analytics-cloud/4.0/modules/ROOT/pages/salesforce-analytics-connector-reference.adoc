= Salesforce Einstein Analytics Connector 4.0 Reference 
...
* OAuth 2.0 SAML Bearer
 | The connection types to provide to this configuration. Note that the `Username Password` connection type is only supported for Salesforce API v64.0 and earlier versions.| |x
...
NOTE: The `Username Password` connection type is only supported for Salesforce API v64.0 and earlier versions.
...
[[salesforce-analytics_oauth-client-credentials]]
=== OAuth Client Credentials

Use the OAuth Client Credentials connection type for automated, server-to-server integrations that don't require user interaction.
...
| Token Url a| String |  The service provider's token endpoint URL. |  `+https://{domain}.my.salesforce.com/services/oauth2/token+` |
| Scopes a| String |  The OAuth scopes to request during the dance. If not provided, it defaults to those in the annotation. |  |
...
Deletes the specified dataset (InsightsExternalData record) from the Salesforce Analytics Cloud system. For more information about limitations, refer to https://developer.salesforce.com/docs/atlas.en-us.bi_dev_guide_ext_data.meta/bi_dev_guide_ext_data/bi_ext_data_object_externaldata.htm[The InsightsExternalData Object^]. 
...
| Name | Type | Description | Default Value | Required
| Configuration | String | The name of the configuration to use | |x
| Data Set Id a| String |  Identifier of the dataset (InsightsExternalData ID) to delete. |  |x
| Config Ref a| ConfigurationProvider |  The name of the configuration to use to execute this component. |  |x
| Error Mappings a| Array of <<ErrorMapping>> |  Set of error mappings. |  |
| Reconnection Strategy a| * <<reconnect>>
* <<reconnect-forever>> |  A retry strategy in case of connectivity errors |  |
...
| Name | Type | Description | Default Value | Required
| Configuration | String | The name of the configuration to use | |x
| Data Set Id a| String | Identifier of the dataset (InsightsExternalData ID) to process. |  |x
| Config Ref a| ConfigurationProvider |  The name of the configuration to use to execute this component. |  |x
| Error Mappings a| Array of <<ErrorMapping>> |  Set of error mappings. |  |
| Reconnection Strategy a| * <<reconnect>>
* <<reconnect-forever>> |  A retry strategy in case of connectivity errors |  |
...
| Name | Type | Description | Default Value | Required
| Configuration | String | The name of the configuration to use. | |x
| Data Set Id a| String | Identifier of a dataset (InsightsExternalData ID) within the Salesforce Analytics Cloud system. |  |x
