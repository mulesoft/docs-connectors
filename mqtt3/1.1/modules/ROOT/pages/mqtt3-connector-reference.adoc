...
* MQTT3:UNAUTHORIZED
+
The client is not authorized to publish a message.
...
| Name | Type | Description | Default Value | Required
| Configuration | String | Name of the configuration to use. | | x
| Topics a| Array of <<Topic>> |  List of topics that the listener subscribes to. You can configure this field by using either *Edit inline* or *Bean reference*. For source configuration examples, see xref:mqtt3-connector-listener.adoc[Listen for New Messages with MQTT Connector]. |  | x
| Config Ref a| ConfigurationProvider |  Name of the configuration to execute the component. |  | x
| Primary Node Only a| Boolean |  Determines whether to execute the source only on the primary node when running Mule instances in a cluster. |  `true` |
