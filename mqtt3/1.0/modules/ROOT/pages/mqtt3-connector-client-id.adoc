...
. In the pom file of your connector project, add MQTT Connector as a dependency, for example:
+
[source,xml,linenums]
----
...
+
MQTT Connector exposes the `ClientIDGenerator` interface, which only requires that you implement the method `generateClientID()`. At this point, you must import this interface using the `@Import` annotation, which is then exported by MQTT Connector.
+
[source,java,linenums]
----
@Xml(prefix = "mqtt3-extended")
...
+
To do so, make the previously defined implementation available to the MQTT Connector by defining your custom implementation as a `ClientIDGenerator` subtype in your connector class. Also, export your `MyClientIdGenerator` class using the `@Export` annotation.
+
[source,java,linenums]
----
@Xml(prefix = "mqtt3-extended")
...
----
+
After you set up your custom connector project, you are ready to access the new implementation from your application using the MQTT Connector.
