...
If you need to reuse topic subscriptions across multiple listeners, in *Topics*, select *Bean reference* instead of *Edit inline*. This option is useful when you want to centralize the list of subscriptions and avoid manually duplicating topic filter and QoS values in each listener.
