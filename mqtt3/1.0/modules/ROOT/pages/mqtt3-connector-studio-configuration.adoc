...
image::mqtt3-operations.png[Select any of the MQTT Connector o from the Mule Palette view on the right side of Anypoint Studio]
...
. On the *Advanced* tab, optionally specify a advanced connection options and reconnection strategy.
...
. In Studio, navigate to the *Global Elements* tab.
. Select your *MQTT3 Config* and click *Edit*.
. In the *MQTT3 Config* window, on the *Advance* tab for *Connection*, select *MQTT3 URL Connection*.
. Set the following fields:
+
* *Clean session*: `False`
[start=7]
