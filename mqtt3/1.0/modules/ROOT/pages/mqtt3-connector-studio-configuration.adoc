...
If you are new to configuring connectors in Studio, see xref:connectors::introduction/intro-config-use-studio.adoc[Use Studio to Configure a Connector]. If, after reading this topic, you need additional information about the connector fields, see the xref:mqtt3-connector-reference.adoc[MQTT 3 Connector Reference].
...
You can configure one of these input sources to use with MQTT 3 Connector:
...
image::mqtt3-operations.png[Select any of the MQTT 3 Connector o from the Mule Palette view on the right side of Anypoint Studio]
...
image::mqtt3-failover-serverlist.png[MQTT3 fail-Over Server List configuration in Global Element Properties window]
...
[WARNING]
If you set a dynamic client ID, MQTT 3 Connector cannot recover the persisted files in the event of a crash. You must set a client ID that does not change in the event of an application restart after a crash.
