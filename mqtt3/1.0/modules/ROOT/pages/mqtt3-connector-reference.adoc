...
| Name | Type | Description | Default Value | Required
| Configuration | String | Name of the configuration to use. | | x
| Topics a| Array of <<Topic>> |  List of topics that the listener subscribes to. In Studio, you can configure this field by using either *Edit inline* or *Bean reference*. For source configuration examples, see xref:mqtt3-connector-listener.adoc[Listen for New Messages with MQTT Connector]. |  | x
| Config Ref a| ConfigurationProvider |  Name of the configuration to execute the component. |  | x
