...
Inside the `<version>` tags, put a version number, the word `RELEASE` for the latest release, or `SNAPSHOT` for the latest available version. See https://www.mulesoft.com/exchange/org.mule.modules/mule-module-ms-dynamics-gp-connector/[Anypoint Exchange] for versions.
