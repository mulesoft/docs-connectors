...
| Connection Timeout a| Number |  +++The amount of time to wait (in milliseconds) when initially establishing a connection before the connector gives up and times out. A value of 0 means infinity and is not recommended.+++ |  +++50+++ | {nbsp}
...
** MINUTES
** HOURS
** DAYS |  +++Time unit used in the connection timeout configurations.+++ |  +++SECONDS+++ | {nbsp}
| Max Connections a| Number |  +++Sets the maximum number of allowed open HTTP connections.+++ |  | {nbsp}
| Socket Timeout a| Number |  +++The amount of time to wait (in milliseconds) for data to be transferred over an established, open connection before the connection times out. A value of 0 means infinity and is not recommended.+++ |  +++50+++ | {nbsp}
...
** MINUTES
** HOURS
** DAYS |  +++Time unit used in the socket timeout configurations.+++ |  +++SECONDS+++ | {nbsp}
...
| Host a| String |  +++The optional proxy host having protocol information. e.g. https://localhost or http://localhost.+++ |  | {nbsp}
| Port a| Number |  +++The optional proxy port which should not be null when proxy host is present+++ |  | {nbsp}
...
| External ID a| String |  +++A unique identifier that might be required when you assume a role in another account. If the administrator of the account to which the role belongs provides an external ID, then provide that value in this field.+++ |  | {nbsp}
...
| Name | Type | Description | Default Value | Required
| Configuration | String | The name of the configuration to use. | | *x*{nbsp}
| Name a| String |  +++Application name must be made up of only uppercase and lowercase ASCII letters, numbers, underscores, hyphens, and periods, and must be between 1 and 256 characters long.+++ |  | *x*{nbsp}
...
| Name | Type | Description | Default Value | Required
| Configuration | String | The name of the configuration to use. | | *x*{nbsp}
| Platform Application Arn a| String |  +++PlatformApplicationArn returned from CreatePlatformApplication is used to create an endpoint.+++ |  | *x*{nbsp}
...
| Name | Type | Description | Default Value | Required
| Configuration | String | The name of the configuration to use. | | *x*{nbsp}
| Topic Name a| String |  +++The name of the topic. Maximum 256 characters. Can include alphanumeric characters, hyphens (-) and underscores (_). FIFO topic names must end with `.fifo`.+++ |  | *x*{nbsp}
| Content Based Deduplication a| Boolean |  +++Enables content-based deduplication for FIFO topics. If you create a FIFO topic and this attribute is false, you must specify a value for the MessageDeduplicationID parameter for the Publish operation. When you set this parameter as true, Amazon SNS uses an SHA-256 hash to generate the MessageDeduplicationID using the body of the message (but not the attributes of the message).+++ |  +++true+++ | {nbsp}
| Target Variable a| String |  +++The name of a variable on which the operation's output will be placed+++ |  | {nbsp}
| Target Value a| String |  +++An expression that will be evaluated against the operation's output and the outcome of that expression will be stored in the target variable+++ |  +++#[payload]+++ | {nbsp}
| Reconnection Strategy a| * <<reconnect>>
* <<reconnect-forever>> |  +++A retry strategy in case of connectivity errors+++ |  | {nbsp}
...
| Name | Type | Description | Default Value | Required
| Configuration | String | The name of the configuration to use. | | *x*{nbsp}
| FIFO Parameters a| <<FifoParameters>> |  +++Parameters related to the FIFO topic.+++ |  | {nbsp}
| Topic Arn a| String |  +++The Amazon Resource Name (ARN) of the topic you want to publish to.+++ |  | {nbsp}
| Target Arn a| String |  +++The Amazon Resource Name (ARN) of the platform endpoint you want to publish to.+++ |  | {nbsp}
| Phone Number a| String |  +++The Phone Number of the device you want to publish to.+++ |  | {nbsp}
| Message a| String |  +++Message body.+++ |  +++#[payload]+++ | {nbsp}
| Subject a| String |  +++Subject line when the message is delivered to email endpoints.+++ |  | {nbsp}
| Message structure a| String |  +++Message structure that can be different for each protocol. Set the value to <code>json</code> if you want to send a different message for each protocol.+++ |  | {nbsp}
| Message Attributes a| Object |  +++Message attributes that provide structured metadata items for the message, such as timestamps, geospatial data, signatures, and identifiers.+++ |  | {nbsp}
| Target Variable a| String |  +++The name of a variable on which the operation's output will be placed+++ |  | {nbsp}
| Target Value a| String |  +++An expression that will be evaluated against the operation's output and the outcome of that expression will be stored in the target variable+++ |  +++#[payload]+++ | {nbsp}
| Reconnection Strategy a| * <<reconnect>>
* <<reconnect-forever>> |  +++A retry strategy in case of connectivity errors+++ |  | {nbsp}
...
** SQS
** APPLICATION
** LAMBDA |  +++The protocol you want to use. If it is a FIFO topic, then only Amazon SQS FIFO queues can be subscribers.+++ |  | *x*{nbsp}
| Endpoint a| String |  +++The endpoint that you want to receive notifications. Endpoints vary by protocol:+++ |  | *x*{nbsp}
| Target Variable a| String |  +++The name of a variable on which the operation's output will be placed+++ |  | {nbsp}
| Target Value a| String |  +++An expression that will be evaluated against the operation's output and the outcome of that expression will be stored in the target variable+++ |  +++#[payload]+++ | {nbsp}
| Reconnection Strategy a| * <<reconnect>>
* <<reconnect-forever>> |  +++A retry strategy in case of connectivity errors+++ |  | {nbsp}
...
|======================
| Field | Type | Description | Default Value | Required
| Message Group Id a| String |  |  | x
| Message Deduplication Id a| String |  |  | 
|======================
