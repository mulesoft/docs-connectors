= Amazon SNS 4.5 Connector Examples
...
When you subscribe an endpoint to an Amazon SNS topic, you can publish a notification to the topic. Amazon SNS then sends an HTTP POST request that delivers the contents of the notification to the subscribed endpoint.
...
2+|*General section*
|*Topic arn* | Amazon Resource Name (ARN) of the topic you want to publish to.
...
----
+
The `default` JSON key has a special meaning for Amazon SQS. The key's value is sent as a message to every subscriber, regardless of the protocol used. You must specify this key for the Publish Message operation. You can optionally specify other top-level keys that define the message to send to a specific transport protocol, such as `HTTP` or `SQS`.

For more information, see `https://docs.aws.amazon.com/sns/latest/api/API_Publish.html#API_Publish_RequestParameters[Publish]` in the Amazon Simple Notification Service documentation.
