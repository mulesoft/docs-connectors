...
The connector behaves differently depending what flow processing strategy is used.

- For the sync scenario, the connector will acknowledge a message only when the flow finishes with success.
In case of an error, the message is not acknowledged, and will be consumed until success.
- For the async scenario, the message is never auto-acknowledged by the connector sources, so it will be re-consumed no matter if the flow fails or is successful.
In order for the message to be acknowledged, the *Acknowledge Message* processor must be used in the flow. This processor receives an *AmpqMessage* as input and acknowledges it.
So, to be acknowledged, the *AmpqMessage* received from the source must be given as input to an "Acknowledge Message" processor to be acknowledged.
- Below is an example of an asynchronous flow that uses the *Acknowledge Message* processor.
...
In this example, the *AmqpMessage* is saved in a flow variable called "test" that is passed as input to the *Acknowledge Message* processor.
...
*Max Connections*: (Optional) Maximum number of connections to keep in pool in order to be reused by producer. If set to "-1" it will create a new connection everytime.
