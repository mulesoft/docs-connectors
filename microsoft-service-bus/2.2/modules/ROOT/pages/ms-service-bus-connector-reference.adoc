= Microsoft Service Bus Connector 2.2 Reference 
:page-aliases: connectors::ms-service-bus/ms-service-bus-connector-reference.adoc, connectors::ms-service-bus-connector-reference.adoc
...
| Port a| String | Port of the proxy server. |  |
...
|===
| Name | Type | Description | Default Value | Required
| Read Timeout a| Number |  How long a client waits for a response before timing out. `0` specifies that the client waits indefinitely. |  60000 |
...
| Connection Timeout a| Number |  How long the connector waits before timing out when establishing a connection to the remote service. |  60000 |
