...
=== Failed Connectivity Test - General SSLEngine problem
If when performing a connectivity test or when starting the mule app you get this error:
 `SSL certificate error` or this error: `General SSLEngine problem`, you must configure the connection trust-store in insecure mode by following these steps:

- Open the Connection Configuration.
- Go to Security tab.
- In the combobox, select `Edit inline`.
- Inside the `Trust Store Configuration`, check the `Insecure` item.
- Save changes.

image::ms-service-bus-trust-store-insecure.png[Insecure Trust Store]

For more details about these problems, go to:
https://docs.mulesoft.com/microsoft-service-bus-connector/2.2/service-bus-troubleshooting#failed-connectivity-test[Troubleshooting - Failed Connectivity Test]
