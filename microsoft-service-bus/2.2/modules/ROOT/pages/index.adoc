...
You must configure the truststore in insecure mode if you get one of the following errors when performing a connectivity test or starting a Mule app:
...
The first item shows the where to select `Edit inline` and the second item shows where to select the *Insecure* checkbox.

For more information about failed connectivity tests, see
