...
+
In either single or cluster runtime instance mode, when using persistent queues, the data you send must be serializable. Although you can enable Kryo serialization to get a wider range of serializable values, Kryo does have some limitations. Refer to xref:mule-runtime::configure-custom-serializers.adoc#considerations-when-using-serialization[] for details.
+
