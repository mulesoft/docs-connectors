= SharePoint Connector 3.10 Reference 
...
* <<sharepoint-online_oauth-client-credentials, OAuth Client Credentials>> 
* <<sharepoint-online_okta, Okta>>
* <<sharepoint-online_online, Online>>
* <<sharepoint-online_security-token, (DEPRECATED) Security Token>>
...
[[sharepoint-online_oauth-client-credentials]]
...
|===
| Name | Type | Description | Default Value | Required
| Disable SSL certificate validation a| Boolean |  Disables the CN (Common Name) check so that the server won't throw an exception if the HTTPS certificate is not signed by a trusted partner. To prevent this, you can disable the CN (Common Name) check. Avoid disabling this for production environments. |  `false` |
| Site Url a| String |  URL for the target SharePoint instance. |  | x
| Client ID a| String |  Client ID to use to call the API. |  | x
| Token URL a| String |  URL used to get an access token. |  | x
| Scopes a| String |  Defined scopes for the client ID.|  | 
| Keystore alias a| String |  Alias used to get the correct certificate from the keystore. |  | x
| Path to keystore a| String | Path to the client's keystore with the private key and certificate. |  | x
| Keystore password a| String |  Password used to access the private key from the keystore. |  | x
| Keystore type a| Enumeration, one of:

** JCEKS
** JKS
** PKCS12
** BCFKS  |  Keystore type.  |  | x
| Key password a| String | Password used to protect the private key. |  | 
| Object Store a| Object Store | Configures the object store that stores data for each resource owner. If not configured, Mule uses the default object store. |  | 
| Host a| String |  Hostname for the proxy server. The connector also uses this hostname for OAuth token requests to the token endpoint. |  | 
| Port a| Number |  Port number for the proxy server. |  | 
| Username a| String |  Username to use to authenticate against the proxy server. |  |
| Password a| String |  Password to use to authenticate against the proxy server. |  |
| NTLM Domain a| String |  Domain to use to authenticate against the proxy server. |  |
| TLS Context a| <<Tls>> |  TLS configuration used when requesting the OAuth token. Configure this field when the proxy or token endpoint presents a certificate that the JVM default truststore doesn't trust. |  | 
...
* <<sharepoint-onpremise_security-token, (DEPRECATED) Security Token>>
 | The connection types to provide to this configuration. | | x
| Charset a| Enumeration, one of:
