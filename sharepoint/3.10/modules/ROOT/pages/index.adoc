= Microsoft SharePoint Connector 3.10
:page-aliases: connectors::sharepoint/sharepoint-connector.adoc


Anypoint Connector for Microsoft SharePoint (Microsoft SharePoint Connector) integrates with SharePoint 2013, SharePoint Online, and SharePoint Server Subscription Edition through the REST API for cloud and on-premises deployments. You can manage documents and content, access intranet and extranet portals, collaborate across your organization, and search enterprise-wide resources. The connector supports OAuth 2.0 authentication and provides operations for lists, files, folders, and site management.
...
* TLS Context: TLS configuration used when requesting the OAuth token. In Studio, this field appears on the *TLS* tab. Configure it when a TLS-inspecting proxy or the token endpoint presents a certificate that the JVM default truststore doesn't trust.
+
The connector sends OAuth token requests through the configured proxy (*Host*, *Port*, and related proxy fields). The *Disable SSL certificate validation* field applies to SharePoint API calls only. It doesn't apply to OAuth token requests. Use *TLS Context* to trust a proxy or token-endpoint certificate.
. Click *Test Connection* to confirm that Mule can connect with the specified server.
. Click *OK*.

== Okta Authentication

image::sharepoint-okta-config.png["Okta authentication configuration in global element properties window"]
...
image::sharepoint-security-token-config.png["Security token authentication configuration in global element properties window"]
...
+
image::sharepoint-global-types.png["List of connection types"]
+
. Click *Test Connection* to ensure the connection works correctly:
+
image::sharepoint-global-element-props.png["Global element properties window"]
...
+
image::sharepoint-studio-flow.png["Studio flow with File Connector"]
+
...
Set these fields:
+
image::sharepoint-visual-editor-fields.png["File add operation properties window"]
+
...
+
To learn more about building queries, see the SharePoint documentation page https://learn.microsoft.com/en-us/sharepoint/dev/sp-add-ins/use-odata-query-operations-in-sharepoint-rest-requests[Use OData query operations in SharePoint REST requests].
+
