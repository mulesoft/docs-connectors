...
. Create the flow and add the keystore and the credentials file into `resources`. Add the required fields for the connector configuration. Configuring the connector to use OAuth Client Credentials with Certificate authentication requires the following information:
