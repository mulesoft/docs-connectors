= Microsoft SharePoint Connector 3.5 - Mule 4
...
* Site Url: The URL for the target SharePoint instance.
...
Configuring the connector to use OAuth Client Credentials with Certificate authentication requires the following information:

* Site Url: The URL for the target SharePoint instance.
* Client ID: The client ID to call the API. 
* Token URL: The URL used to get an access token.
* Keystore alias: The alias used to get the correct certificate from the keystore. 
* Path to keystore: The path to the client's keystore with the private key and the certificate. 
* Keystore password: The password used to access the private key from the keystore. 
