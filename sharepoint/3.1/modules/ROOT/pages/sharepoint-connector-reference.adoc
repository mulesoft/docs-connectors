ID of the= SharePoint Connector 3.1 Reference - Mule 4
...
| Disable SSL certificate validation a| Boolean |  Disables the CN (Common Name) check so the server won't throw an exception if the HTTPs certificate is not signed by a trusted partner.
...
| Host a| String |  Hostname for the proxy server. |  |
| Port a| Number |  Port number for the proxy server. |  |
| Username a| String |  Username for authenticating against the proxy server. |  |
| Password a| String |  Password for authenticating against the proxy server. |  |
| NTLM Domain a| String |  Domain for authenticating against the proxy server. |  |
| Reconnection a| <<Reconnection>> |  When the application is deployed, a connectivity test is performed on all connectors. If set to true, deployment fails if the test doesn't pass after exhausting the associated reconnection strategy. |  |
|===
...
|===
| Name | Type | Description | Default Value | Required
| Claims Username a| String |  Username used to log in with. Do not include the domain name. |  | x
| Claims Password a| String |  Password used to log in with. |  | x
| Site Url a| String |  URL for the SharePoint instance that the connector is going to interact with. |  | x
| Sts Url a| String |  Security Token Service URL to use for claims authentication |  | x
| Scope a| String |  Application Identifier configured for this Sharepoint Site in the Sts (`Relying Party Identifier`, `Client Identifier`, `Scope` or `Realm`). |  | x
| Disable SSL certificate validation a| Boolean |  Disables the CN (Common Name) check so the server wont throw an exception if the HTTPs certificate is not signed by a trusted partner.

NOTE: Disabling the CN check is not recommended for production environments. |  false |
| Host a| String |  Hostname for the proxy server. |  |
| Port a| Number |  Port number for the proxy server. |  |
| Username a| String |  Username used for authenticating against the proxy server. |  |
| Password a| String |  Password used for authenticating against the proxy server. |  |
| NTLM Domain a| String |  The domain used for authenticating against the proxy. |  |
| Reconnection a| <<Reconnection>> |  When the application is deployed, a connectivity test is performed on all connectors. If set to true, deployment fails if the test doesn't pass after exhausting the associated reconnection strategy. |  |
|===
...
| Login Properties File Path a| String |  (Optional) Path to a customized login properties file. When not specified, the default values usually work for most cases. |  |
...
| Username a| String |  Username to use for authenticating against the proxy server. |  |
| Password a| String |  Password to use for authenticating against the proxy server. |  |
| NTLM Domain a| String |  Domain to use for authenticating against the proxy ServerRelativeUrl. |  |
| Reconnection a| <<Reconnection>> |  When the application is deployed, a connectivity test is performed on all connectors. If set to true, deployment fails if the test doesn't pass after exhausting the associated reconnection strategy. |  |
|===
...
| Port a| Number |  Port number for the proxy server. |  |
| Username a| String |  Username to use for authenticating against the proxy server. |  |
| Password to use for authenticating against the proxy server.  a| String |  Password to use for authenticating against the proxy server |  |
| NTLM Domain a| String |  The domain to authenticate against the proxy. |  |
| Reconnection a| <<Reconnection>> |  When the application is deployed, a connectivity test is performed on all connectors. If set to true, deployment fails if the test doesn't pass after exhausting the associated reconnection strategy. |  |
|===
...
Deprecated the SecurityTokenConnectionStrategy connection provider because the BasicConnectionProvider already obtains the security token
...
| Path a| String |  Optional parameter that specifies the base path for the FOLDER and FILE object types|  / |
| Primary Node Only a| Boolean |  Whether this source should only be executed on the primary node when running in Cluster mode|  |
...
| Name | Type | Description | Default Value | Required
| Configuration | String | The name of the configuration to use. | | x
| Url a| String |  Server relative URL of the folder to delete. For example, `/Shared Documents/new folder`, deletes `new folder` from `Shared Documents` list |  | x
| Reconnection Strategy a| * <<reconnect>>
* <<reconnect-forever>> |  A retry strategy in case of connectivity errors |  |
|===
...
| Name | Type | Description | Default Value | Required
| Configuration | String | The name of the configuration to use. | | x
| Query a| String |  OData query in the format `listDocumentName&#63;queryString`, for example, `Shared Documents&#63;&#36;select=Name&#38;&#36`;filter Name , for example `folderName` |  | x
| Starting Folder Path a| String |  The starting path of the folder from where to begin the query, relative to the document library selected as part of the query below. For example, `/myfolder/level2` searches for folders inside `/document library/myfolder/level2`. NOTE: you must specify the document library as part of the query below. |  / |
