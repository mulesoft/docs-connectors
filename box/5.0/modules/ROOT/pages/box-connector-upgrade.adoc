...
|Get Files Id Content | Returns the contents of a file in binary format.
...
|Get Files Upload Sessions Id | Return information about an upload session.
|Get Files Upload Sessions Id Parts | Return a list of the chunks uploaded to the upload session so far.
|Post Files Id Upload Sessions | Creates an upload session for an existing file.
|Post Files Upload Sessions Id Commit | Close an upload session and create a file from the uploaded chunks.
|Put Files Upload Sessions Id | Updates a chunk of an upload session for a file.
|Delete Files Upload Sessions Id | Abort an upload session and discard all data uploaded.
|===
