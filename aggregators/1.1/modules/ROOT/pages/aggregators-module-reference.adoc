= Aggregators Module Reference 1.1 
...
+
NOTE: Aggregators are not recommended for large payloads. Because the aggregator state is stored in the object store and polled every second, large payloads can significantly increase object store throughput consumption and impact performance.
...
| Timeout | Number |  The maximum time to wait for the aggregation of a group to complete. If the timeout is reached before the total number of elements in the group equals the group’s size, the aggregator triggers `notifyListenerOnTimeout`. This allows timed-out aggregations to be picked up by the Aggregator Listener if `IncludeTimedOutGroups` is set to `true`, without marking them as complete. Additionally, the `IncludeTimedOutGroups` parameter determines whether these timed-out groups are passed to the listener without marking the aggregation as complete. To avoid constant group timeouts, a value of `0` is not supported. | -1(UNLIMITED) |
| Timeout unit | Time Unit | The time unit in which to measure the timeout. |  SECONDS |
...
| `mule.aggregatorsSchedulingPeriod` | Configures the scheduling period (in milliseconds) to determine if Size-Based or Time-Based aggregators reached the configured size. The module triggers a GET call every 1 second. | | `-M-Dmule.aggregatorsSchedulingPeriod=1`
|===
