...
The Aggregators module *Size based aggregator* scope enables you to aggregate elements until a pre-defined size number of elements completes the aggregation. The following example shows how to configure this scope to aggregate up to `3` HTTP request calls. Then, the application uses DataWeave to set the payload of each call with a random ID number. When the aggregation reaches `3` calls, the application logs all the aggregated random ID number elements.
