...
[IMPORTANT]
Aggregators won't work with multiple workers and a persistent Object Store because clustering is not available in CloudHub, see xref:runtime-manager::cloudhub-fabric.adoc[CloudHub Fabric] for details on how workers can be shared or doubled to scale your application and provide high availability.
