...
[IMPORTANT]
Aggregators don't work with multiple workers and a persistent Object Store because clustering is not available in CloudHub, see the xref:runtime-manager::cloudhub-fabric.adoc[CloudHub Fabric] documentation for details on how workers can be shared or doubled to scale your application and provide high availability.
