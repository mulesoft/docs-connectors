...
* *Persistent*: If a cluster is not running, object store data is stored on disk. If a cluster is running, data is stored in memory. If Mule runtime is restarted, the object store content persists.
