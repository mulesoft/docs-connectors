...
* *Rolling TTL*: The expiration timer extends each time the entry is read or written. Applies only when `entryTtl` is omitted from the configuration (Mule 4.2.1 and later). Entries persist as long as the application accesses them at least once a week.
* *Static TTL*: The expiration timer is fixed when the entry is written. Reads do not extend the timer. The entry is evicted at `creation_time + entryTtl`.
