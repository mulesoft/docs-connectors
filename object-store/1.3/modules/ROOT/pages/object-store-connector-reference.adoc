...
| Lock Timeout a| Number |  Timeout for acquiring ObjectStore locks. If not specified, operations wait indefinitely. |  | 
| Lock Timeout Unit a| Enumeration, one of:
...
** MINUTES
** HOURS
** DAYS |  Time unit for the lock timeout parameter. |  SECONDS | 
|===
...
Running the same application with STORE operations across multiple nodes without clustering enabled might lead to inconsistent results due to the absence of coordinated locking.
...
| Entry ttl a| Number | The entry timeout. If absent, then the described object store has no time boundaries. If `entryTtl` is set to a value lower than or equal to zero, then the object store never expires. |  |
...
Object Store v2 supports two TTL behaviors:

* *Rolling TTL*: Each entry access (read or write) extends the expiration timer. This behavior applies only when you omit `entryTtl` from the configuration (Mule 4.2.1 and later). Entries persist as long as the application accesses them at least once a week.
* *Static TTL*: The system fixes the expiration timer at the moment of the write operation. Reads don't extend this timer. The system evicts the entry at `creation_time + entryTtl`.
...
+
TTL is static and set to the maximum value (2592000 seconds/30 days). Reads do not extend the entry's lifetime.
* Positive number
+
...
IMPORTANT: Specifying any `entryTtl` value, including `0`, configures the store with static TTL. To use rolling TTL, omit the `entryTtl` field entirely. Adding `entryTtl="0"` to a configuration that previously omitted the field changes the eviction behavior from rolling to static (30 days from entry creation, regardless of read activity).
...
| Expiration interval a| Positive number a| How frequently the expiration thread runs.

The `expirationInterval` field requires that the `entryTtl` field is set; otherwise, it is ignored.

* If `expirationInterval` is set to a positive value less than the value of `entryTtl`, the entry expires based on the `entryTtl` configuration.
...
|===
| Field | Type | Description | Default Value | Required
| Alias a| String | A friendly name to refer to this store in the management user interface. Provide this alias if you would like it to be easier to identify this store. If not provided, Mule auto generates an ID. Use an alias for consistent OAuth token refresh behavior. Without an alias, each Mule app node creates its own partition. This can lead to issues with OAuth token refresh, as each node might try to refresh the token independently.

By providing an alias, you make sure that the Mule app uses the same partition across restarts. As long as the cached key or value is not expired, the caching scope is hit, and the Mule app continues to use the same token. This prevents unnecessary token refreshes and improves application efficiency. |  |
| Persistent a| Boolean | Whether the store is persistent or transient. | true |
