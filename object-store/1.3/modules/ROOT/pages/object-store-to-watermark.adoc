= Set Up Watermarks with an Object Store Example 
...
For an explanation of what watermarks are, see https://blogs.mulesoft.com/dev-guides/how-to-tutorials/data-synchronizing-made-easy-with-mule-watermarks/[this blog post].

Limitations of the earlier approach:

* Although the `<watermark/>` component allowed setting a custom `ObjectStore`, there was no easy way to create it. This issue is resolved in current implementations.
* The `<watermark/>` element required the `<poll/>` component (replaced by  `<scheduler/>`) to return an iterable payload, and you had to fully iterate over that payload to obtain the new watermark value. Failure to do so resulted in the value being wrongfully updated or not updated at all.
...
This example shows how to perform the use case described in the blog.
