...
Use Retrieve when to read previously stored data in a flow, for example, to resume state, reuse cached values, or pass stored data to the next operation. For all parameters (including `objectStore`, `target`, and `targetValue`), see the xref:object-store-connector-reference.adoc#retrieve[Retrieve] operation in the connector reference.
