...
+
Because the object store is referenced by name, more than one component can use it. Suppose you have a two instances of Salesforce Connector that are configured slightly differently. You can use the same token for both Salesforce Connector instances to avoid the need to reauthenticate.
