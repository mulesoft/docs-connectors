= Redis Connector 5.2 Studio Configuration - Mule 4
...
* `Scheduler`, which initiates a flow when a time-based condition is me
...
. On the *Connection* tab, select whether the connection is a clustered or non-clustered configuration:
...
|Cluster Hosts| Redis cluster hosts, separated by commas and with the format `host:port`. Use this field to access a Redis cluster through the connector.
...
|Connection Timeout| Connection timeout, in milliseconds.
|Entry Ttl| The default time to live to set on keys stored through the connector.
