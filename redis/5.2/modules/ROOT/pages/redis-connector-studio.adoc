...
The following screenshot shows an example of configuring a global element for a Redis cluster mode configuration:
...
* *Connection*
+
`Clustered` for a clustered Redis configuration
+
* *Cluster hosts*
+
Redis cluster hosts, separated by commas and with the format `host:port`
+
* *Connection timeout*
+
...
* *Entry Ttl*
+
Default time to live to set on keys stored through the connector
+
* *Password*
+
* Redis password
...
* *Connection*
+
`NonClustered` for the non-clustered Redis configurations
+
* *Host*
+
Host for a Redis server that is not part of a cluster
+
* *Port*
+
Port on which a non-clustered server is running
+
* *Connection Timeout*
+
Connection timeout, in milliseconds
...
After completing configuration, see the xref:redis-connector-examples.adoc[Examples].
