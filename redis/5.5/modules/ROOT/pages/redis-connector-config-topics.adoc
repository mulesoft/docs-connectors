= Redis Connector 5.5 Additional Configuration Information
...
image::redis-scenario-1.png["Shows two separate applications, App A and App B, each connecting to its own cloud service and dedicated database to store unique access and refresh tokens."]
...
image::redis-scenario-2.png["Shows App A and App B sharing a single database that stores multiple sets of access and refresh tokens."]
...
image::redis-scenario-3.png["shows App A and App B both connecting to a centralized Redis database while also maintaining individual connections to Salesforce and ServiceNow."]
