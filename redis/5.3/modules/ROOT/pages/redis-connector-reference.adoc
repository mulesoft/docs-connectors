...
| Password a| String |  Redis password. Set this in case that the server requires authentication. |  |
| Entry TTL a| Number |  The default time to live to be set on keys stored through connector. |  |
| Sentinels Hosts a| String |  Set of sentinel addresses that are monitoring the main host to which the connector will connect. |  | *x*
| Main Name a| String |  Redis main name. The name of the main for which the sentinels will return the current address. |  |
