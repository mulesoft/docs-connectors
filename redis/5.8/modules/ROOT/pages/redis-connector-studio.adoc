= Using Anypoint Studio to Configure Redis Connector 5.8 
:page-aliases: connectors::redis/redis-connector-studio.adoc, connectors::redis/redis-connector-design-center.adoc
...
* *Username* +
Optional Redis ACL username for Redis 6 and later ACL-based authentication. When set, the connector authenticates using the `AUTH <username> <password>` form. When empty, the connector falls back to the legacy `AUTH <password>` form.
* *Password* +
Redis password for server authentication
* *Entry TTL* +
Optional default time-to-live value to set on keys stored through the connector
. Optionally, on the *Pooling* tab, configure pool sizing such as *Max Total* and *Max Wait Millis*. Starting with Redis Connector 5.8, these values are applied to the cluster client. For field descriptions, see xref:redis-connector-reference.adoc#redis_clustered[Clustered].
. Optionally, on the *Advanced* tab, configure <<socket-timeout-config,timeout fields>>.
...
. In *Connection*, select *Sentinel* to use the connector with a Sentinel Redis configuration.
. Enter values for the following fields:
* *Sentinels Hosts* +
...
* *Sentinel Username* +
Optional Redis ACL username for authenticating to the Sentinel processes (Redis 6.2 and later). When set, the connector uses `AUTH <username> <password>` to authenticate with each Sentinel. When empty, the connector falls back to the legacy `AUTH <sentinelPassword>` form.
* *Sentinel Password* +
Password for Sentinel server authentication
* *Index* +
Index of the database that the pool uses
* *Username* +
Optional Redis ACL username for Redis 6 and later ACL-based authentication on the master. When set, the connector authenticates using the `AUTH <username> <password>` form. When empty, the connector falls back to the legacy `AUTH <password>` form.
* *Password* +
...
On the *Advanced* tab for *Clustered*, *NonClustered*, and *Sentinel* connections, configure these timeout fields:

* *Connection Timeout* +
Optional TCP connect timeout. The default is `2000`.
* *Connection Time Unit* +
Time unit for *Connection Timeout*. The default is milliseconds.
* *Socket Timeout* +
Optional read timeout for Redis command responses. This value bounds how long a single blocking command waits for the server to reply, independently of the TCP connect timeout. Leave empty to reuse *Connection Timeout* (the 5.7 and earlier single-timeout behavior).
* *Socket Time Unit* +
Time unit for *Socket Timeout*. The default is milliseconds. This field applies only when *Socket Timeout* is set.

For *Sentinel* connections, *Socket Timeout* applies to the Redis master channel. Sentinel discovery uses *Sentinel Connection Timeout* for both connect and read.

For field descriptions, see xref:redis-connector-reference.adoc#redis_clustered[Clustered], xref:redis-connector-reference.adoc#redis_nonclustered[NonClustered], and xref:redis-connector-reference.adoc#redis_sentinel[Sentinel].
