...
. Optionally, on the *Advanced* tab, configure <<socket-timeout-config,Socket Timeout>>.
...
* *Sentinel username* +
Optional Redis ACL username for authenticating to the Sentinel processes (Redis 6.2 and later). When set, the connector uses `AUTH <sentinelUsername> <sentinelPassword>` to authenticate with each Sentinel. When empty, falls back to the legacy `AUTH <sentinelPassword>` form.
* *Index* +
...
Starting with Redis Connector 5.8, you can set an independent socket (read) timeout on the *Advanced* tab for *Clustered*, *NonClustered*, and *Sentinel* connections:

* *Socket Timeout* +
Optional read timeout for Redis command responses. Leave empty to reuse *Connection Timeout* (the 5.7 and earlier single-timeout behavior).
* *Socket Time Unit* +
