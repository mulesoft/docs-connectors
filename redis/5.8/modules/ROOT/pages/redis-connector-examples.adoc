...
image::redis-set-raw-flow.png["Studio flow with Listener, Set, and Set Payload"]
...
+
image::redis-set-config.png["Set operation properties window on General tab"]
+
When *If Not Exists* is `true` and the key already exists, the *Set* operation returns null and does not change the existing value or TTL. For details, see xref:redis-connector-reference.adoc#set[Set].
+
...
+
image::redis-set-response-config.png["Set Payload properties window on General tab"]
+
