...
[NOTE]
Starting with Redis Connector 5.8, the *Clustered*, *NonClustered*, and *Sentinel* connection types include optional *Socket Timeout* and *Socket Time Unit* fields. Leave *Socket Timeout* unset to preserve the legacy behavior where *Connection Timeout* also served as the read timeout.
...
[NOTE]
Starting with Redis Connector 5.8, pooling settings such as *Max Total* and *Max Wait Millis* on the *Clustered* connection are applied to the cluster client. In Redis Connector 5.4.6 through 5.7.0, those values were ignored and the client used Jedis defaults.
...
| Socket Timeout a| Number | Socket (read) timeout applied to Redis command responses. This bounds how long a single blocking command waits for the server to reply, independently of the TCP connect timeout. Leave unset to preserve the legacy behavior where *Connection Timeout* also served as the read timeout. Existing configurations that leave this field empty keep the previous behavior. |  |
| Socket Time Unit a| Enumeration, one of:
...
** MINUTES
** HOURS
** DAYS | Time unit for the socket (read) timeout. This field applies only when *Socket Timeout* is set. |  MILLISECONDS |
...
Set the key to hold the payload. If *If Not Exists* is `false` and the key already holds a value, the value is overwritten regardless of its type. If *If Not Exists* is `true` and the key already exists, the operation returns null, leaves the existing value unchanged, and does not update the key's TTL. TTL is applied only when the write succeeds.

[NOTE]
Starting with Redis Connector 5.8, a *Set* with *If Not Exists* set to `true` returns null when the key already exists. Redis Connector 5.7 and earlier raised `REDIS:UNKNOWN` for that no-op. Overwrite (`If Not Exists` = `false`) still raises `REDIS:UNKNOWN` when the server returns a genuine null response.
...
| If Not Exists a| Boolean |  If `true`, execute SETNX on the Redis server. If the key already exists, the operation returns null and does not change the existing value or TTL. If `false`, execute SET. |  false |
| Config Ref a| ConfigurationProvider |  Name of the configuration to use to execute this component. |  |
| Target Variable a| String |  Name of a variable that stores the operation's output |  |
| Target Value a| String |  Expression that evaluates against the operation’s output. The expression outcome is stored in the target variable. |  `#[payload]` |
| Error Mappings a| Array of <<ErrorMapping>> |  Set of error mappings |  |
