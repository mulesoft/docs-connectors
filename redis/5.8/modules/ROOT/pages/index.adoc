...
[NOTE]
Starting with Redis Connector 5.8, you can configure an independent *Socket Timeout* on the connection *Advanced* tab so that Redis command responses can fail faster than the TCP connect timeout. Leave *Socket Timeout* empty to keep the previous behavior that reuses *Connection Timeout* as the read timeout.
