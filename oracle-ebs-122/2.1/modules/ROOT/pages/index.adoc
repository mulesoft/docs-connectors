= Oracle E-Business Suite 12.2 Connector 2.1 - Mule 4
...
+
WADLs corresponding to deployed PL/SQLs must have their internal names in lower case as an alias. For example, if you want to deploy `PLSQL Interface : Absence Attendance Type`, which has `HR_ABSENCE_TYPE_API` as the internal name, its service alias must be `hr_absence_type_api`.
