...
This implementation lets you feed the Agent with a live list of vendors and their assigned categories, while keeping the LLM decoupled from Concur and all its nuances like authentication, custom field setup, and so on.

In this example, MCP tools are Remote Procedure Calls (RPC) that take a list of input arguments and produce a result.

=== Create the MCP Server Configuration

This configuration implements an MCP server using the SSE transport on top of an existing `<http:listener-config>`. This configuration not only provides all the configuration options of the HTTP Connector, you can also write applications that serve both MCP and traditional APIs in the same port.
...
In this example, each vendor is assigned with a product category that's mapped in a custom field called `Custom19`:
...
You can then create an MCP Client configuration to consume it by using the *Call Tool* operation. The *Call Tool* operation automatically fetches all the metadata from the MCP server and displays the existing tools, while also providing Datasense auto-completion on the arguments.
