...
Anypoint Connector for MCP (MCP Connector) uses the Model Context Protocol (MCP) to enable AI clients that support MCP to invoke and interact with APIs, connectors, and apps. Use MCP Connector to connect APIs with LLM applications. You can expose APIs with complex endpoints as simpler, more atomic tools or resources that are better suited for agentic clients. The key is to decide on what is straightforward, repeatable, and optimal for agent actions, and start there.
