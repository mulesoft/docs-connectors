= Upgrading MCP Connector to Version 1.5
...
+
Define tool parameter and output schemas using either inline JSON or external files, fully supported by robust design-time schema validation.
* The connector now includes the new `MCP:SCHEMA_VALIDATION_FAILED` error.
