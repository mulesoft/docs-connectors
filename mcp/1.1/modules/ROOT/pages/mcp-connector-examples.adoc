...
This example demonstrates how to configure an MCP server within your Mule application using the StreamableHttp transport. When acting as an MCP client, MuleSoft facilitates the creation of integrations and orchestrations where AI Agents are integrated as just another system in the workflow.
...
<mcp:server-config name="MCP_Server" serverName="Mule MCP Server" serverVersion="1.1.0"> <<2>>
	<mcp:streamable-http-server-connection 
		listenerConfig="HTTP_Listener_config" 
		mcpEndpointPath="/mcp">
		<mcp:custom-headers> <<3>>
			<mcp:custom-header key="X-API-Version" value="1.1.0"/>
			<mcp:custom-header key="X-Server-ID" value="${server.id}"/>
		</mcp:custom-headers>
	</mcp:streamable-http-server-connection>
</mcp:server-config>

<configuration-properties file="mule-artifact.properties"/> <<4>>
----

[calloutlist]
.. The HTTP listener configuration defines the endpoint where the MCP server is exposed.
.. The MCP server configuration uses the StreamableHttp transport.
.. Custom headers are added to all responses for version tracking and server identification.
...
.. A cache configuration is added to improve performance with a 1-hour TTL.
...
<mcp:client-config name="Google_Maps_Client" clientName="Mule MCP Maps Client" clientVersion="1.1.0"> <<2>>
...
<mcp:server-config name="Weather_MCP_Server" serverName="Weather MCP Server" serverVersion="1.1.0"> <<3>>
...
.. The Google Maps MCP client configuration includes reconnection settings.
.. The Weather MCP server configuration exposes the weather service endpoints.
