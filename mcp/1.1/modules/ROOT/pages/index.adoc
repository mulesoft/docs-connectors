...
The Streamable HTTP connection type allows deployment of multiple replicas of the same SSE server without the client being aware of each replica's URL, which provides better horizontal scalability. This connection type is designed for modern cloud environments and containerized deployments. 

The Streamable HTTP connection type has these limitations:

* No support for message batching
* No support for calling the GET `/mcp` endpoint for getting notifications
* No support for Resumability and Redelivery
