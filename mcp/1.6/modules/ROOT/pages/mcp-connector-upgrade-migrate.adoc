...
* MCP Connector now supports elicitation workflows through the *MCP Server - Create Elicitation* operation and the *MCP Client - Elicitation Listener* source.
* Server connection configuration now includes *Elicitations Object Store* and *Elicitation Polling Seconds* to route elicitation responses across replicas.
* In multi-replica deployments, configure a distributed object store for elicitation routing. The default in-memory object store is single-replica only.
* The SSE server transport remains single-replica only. For multi-replica deployments, use Streamable HTTP server transport.
