...
== Configure Distributed Tracing for MCP Operations

This example demonstrates how to implement comprehensive distributed tracing for MCP operations to monitor performance, track request flows, and troubleshoot issues across distributed systems.

=== MCP Server with Full Tracing

This example shows how to configure an MCP server with distributed tracing capabilities:

[source,xml,linenums]
----
<mcp:config name="MCP_Server_Traced" serverName="Traced MCP Server" serverVersion="1.2.0"> <1>
    <mcp:streamable-http-server-connection listenerConfig="HTTP_Listener_config" mcpEndpointPath="/mcp">
        <mcp:default-response-headers>
            <mcp:default-response-header key="X-Trace-Enabled" value="true"/>
            <mcp:default-response-header key="X-Server-Version" value="1.2.0"/>
        </mcp:default-response-headers>
    </mcp:streamable-http-server-connection>
</mcp:config>

<flow name="traced-vendor-lookup"> <2>
    <mcp:tool-listener config-ref="MCP_Server_Traced" name="get-vendors-traced">
        <mcp:description>Get vendor list with comprehensive tracing</mcp:description>
        <mcp:parameters-schema><![CDATA[{
            "$schema": "http://json-schema.org/draft-07/schema#",
            "type": "object",
            "properties": {
    ...
    <!-- Initialize tracing context --> <3>
    <tracing:set-logging-variable variableName="mcp.tool.name" value="#[payload.name]"/>
    <tracing:set-logging-variable variableName="mcp.tool.args" value="#[write(payload.arguments, 'application/json')]"/>
    <tracing:set-logging-variable variableName="mcp.session.id" value="#[vars.sessionId]"/>
    <tracing:set-logging-variable variableName="mcp.operation.id" value="#[uuid()]"/>
    
    <!-- Performance tracking --> <4>
    <set-variable variableName="operationStartTime" value="#[now()]"/>
    <tracing:set-logging-variable variableName="mcp.performance.start" value="#[vars.operationStartTime]"/>
    
    <logger level="INFO" message="Starting MCP vendor lookup operation"/>
    
    <!-- Business logic with nested tracing --> <5>
    <try>
        <tracing:set-logging-variable variableName="mcp.vendor.lookup.status" value="started"/>
        
        <!-- Call external system -->
        <sap-concur:get-vendors config-ref="SAP_Concur_Config"/>
        
        <tracing:set-logging-variable variableName="mcp.vendor.count" value="#[sizeOf(payload)]"/>
        <tracing:set-logging-variable variableName="mcp.vendor.lookup.status" value="completed"/>
        
        <!-- Filter based on vendor type if specified -->
        <choice>
            <when expression="#[payload.arguments.vendorType != null]">
                <tracing:set-logging-variable variableName="mcp.filter.applied" value="#[payload.arguments.vendorType]"/>
                <ee:transform>
                    <ee:message>
                        <ee:set-payload><![CDATA[%dw 2.0
                        output application/json
                        ---
                        payload filter ($.vendorType == vars.filterType)]]></ee:set-payload>
                    </ee:message>
                </ee:transform>
                <tracing:set-logging-variable variableName="mcp.vendor.filtered.count" value="#[sizeOf(payload)]"/>
            </when>
        </choice>
        
        <error-handler> <6>
            <on-error-continue type="SAP-CONCUR:CONNECTIVITY">
                <tracing:set-logging-variable variableName="mcp.error.type" value="connectivity"/>
                <tracing:set-logging-variable variableName="mcp.error.description" value="#[error.description]"/>
                <logger level="ERROR" message="SAP Concur connectivity error in MCP operation"/>
                <set-payload value="#[{error: 'Connection to vendor system failed', details: error.description}]"/>
            </on-error-continue>
            <on-error-continue type="ANY">
                <tracing:set-logging-variable variableName="mcp.error.type" value="general"/>
                <tracing:set-logging-variable variableName="mcp.error.description" value="#[error.description]"/>
                <logger level="ERROR" message="Unexpected error in MCP vendor lookup"/>
                <set-payload value="#[{error: 'Internal server error', details: error.description}]"/>
            </on-error-continue>
        </error-handler>
    </try>
    
    <!-- Calculate final performance metrics --> <7>
    <set-variable variableName="operationDuration" value="#[(now() - vars.operationStartTime) as Number]"/>
    <tracing:set-logging-variable variableName="mcp.performance.duration.ms" value="#[vars.operationDuration]"/>
    <tracing:set-logging-variable variableName="mcp.operation.status" value="completed"/>
    
    <!-- Performance alerting -->
    <choice>
        <when expression="#[vars.operationDuration > 10000]">
            <tracing:set-logging-variable variableName="mcp.performance.alert" value="slow-operation"/>
            <logger level="WARN" message="Slow MCP operation detected: #[vars.operationDuration]ms"/>
        </when>
        <when expression="#[vars.operationDuration > 30000]">
            <tracing:set-logging-variable variableName="mcp.performance.alert" value="critical-slow"/>
            <logger level="ERROR" message="Critical slow MCP operation: #[vars.operationDuration]ms"/>
        </when>
    </choice>
    
    <logger level="INFO" message="MCP vendor lookup completed successfully"/>
</flow>
----

[calloutlist]
.. The MCP server configuration includes trace-enabled headers.
.. The tool listener is configured with comprehensive tracing capabilities.
.. Tracing context is established at the beginning of the operation.
.. Performance tracking measures operation timing.
.. Business logic includes nested tracing for detailed monitoring.
.. Error handling includes tracing for troubleshooting.
.. Final performance metrics and alerting are calculated and logged.

=== Cross-Service Tracing with MCP Client

This example demonstrates how to maintain tracing context when using MCP Connector as a client:

[source,xml,linenums]
----
<flow name="cross-service-mcp-client"> <1>
    <http:listener config-ref="HTTP_Listener_config" path="/orchestrate"/>
    
    <!-- Create correlation ID for cross-service tracing --> <2>
    <tracing:with-correlation-id correlationId="#[uuid()]">
        <tracing:set-logging-variable variableName="orchestration.id" value="#[correlationId]"/>
        <tracing:set-logging-variable variableName="orchestration.type" value="multi-service"/>
        <tracing:set-logging-variable variableName="request.source" value="external-client"/>
        
        <logger level="INFO" message="Starting cross-service orchestration"/>
        
        <!-- First MCP call with tracing --> <3>
        <tracing:set-logging-variable variableName="mcp.call.sequence" value="1"/>
        <tracing:set-logging-variable variableName="mcp.target.service" value="vendor-service"/>
        
        <mcp:call-tool config-ref="Vendor_MCP_Client" toolName="get-vendors">
            <mcp:arguments><![CDATA[#[output application/java
            ---
            {
                vendorType: payload.vendorType
            }]]></mcp:arguments>
            <mcp:additional-properties><![CDATA[#[output application/java <4>
            ---
            {
                "X-Correlation-ID": correlationId,
                "X-Request-Source": "mule-orchestrator",
                "X-Trace-Parent": "mule-" ++ correlationId
            }]]></mcp:additional-properties>
        </mcp:call-tool>
        
        <tracing:set-logging-variable variableName="mcp.vendors.received" value="#[sizeOf(payload)]"/>
        
        <!-- Second MCP call with context --> <5>
        <tracing:set-logging-variable variableName="mcp.call.sequence" value="2"/>
        <tracing:set-logging-variable variableName="mcp.target.service" value="pricing-service"/>
        
        <mcp:call-tool config-ref="Pricing_MCP_Client" toolName="get-pricing">
            <mcp:arguments><![CDATA[#[output application/java
            ---
            {
                vendors: payload map $.vendorId
            }]]></mcp:arguments>
            <mcp:additional-properties><![CDATA[#[output application/java
            ---
            {
                "X-Correlation-ID": correlationId,
                "X-Request-Source": "mule-orchestrator",
                "X-Trace-Parent": "mule-" ++ correlationId,
                "X-Previous-Call": "vendor-service"
            }]]></mcp:additional-properties>
        </mcp:call-tool>
        
        <tracing:set-logging-variable variableName="mcp.pricing.received" value="#[sizeOf(payload)]"/>
        <tracing:set-logging-variable variableName="orchestration.status" value="completed"/>
        
        <logger level="INFO" message="Cross-service orchestration completed"/>
    </tracing:with-correlation-id>
</flow>
----

[calloutlist]
.. The flow initiates cross-service orchestration.
.. A correlation ID is created for tracking across services.
.. First MCP call includes tracing context.
.. Additional properties carry tracing headers to the remote service.
.. Second MCP call maintains the correlation context with additional metadata.

=== Session Lifecycle Tracing

Monitor MCP session establishment and lifecycle:

[source,xml,linenums]
----
<flow name="mcp-session-lifecycle-tracing"> <1>
    <mcp:on-new-session-listener config-ref="MCP_Server_Traced"/>
    
    <!-- Session establishment tracing --> <2>
    <tracing:set-logging-variable variableName="mcp.session.event" value="established"/>
    <tracing:set-logging-variable variableName="mcp.session.id" value="#[payload.sessionId]"/>
    <tracing:set-logging-variable variableName="mcp.session.timestamp" value="#[now()]"/>
    <tracing:set-logging-variable variableName="mcp.client.ip" value="#[payload.additionalProperties.'x-forwarded-for' default payload.additionalProperties.host default 'unknown']"/>
    <tracing:set-logging-variable variableName="mcp.client.user-agent" value="#[payload.additionalProperties.'user-agent' default 'unknown']"/>
    
    <!-- Authentication context if available --> <3>
    <choice>
        <when expression="#[payload.additionalProperties.authorization != null]">
            <tracing:set-logging-variable variableName="mcp.auth.present" value="true"/>
            <tracing:set-logging-variable variableName="mcp.auth.type" value="#[if(payload.additionalProperties.authorization startsWith 'Bearer') 'bearer' else 'other']"/>
        </when>
        <otherwise>
            <tracing:set-logging-variable variableName="mcp.auth.present" value="false"/>
        </otherwise>
    </choice>
    
    <!-- Geographic context if available --> <4>
    <choice>
        <when expression="#[payload.additionalProperties.'cf-ipcountry' != null]">
            <tracing:set-logging-variable variableName="mcp.client.country" value="#[payload.additionalProperties.'cf-ipcountry']"/>
        </when>
    </choice>
    
    <logger level="INFO" message="New MCP session established with comprehensive tracing"/>
</flow>
----

[calloutlist]
.. Session lifecycle monitoring flow.
.. Basic session information is captured for tracing.
.. Authentication context is recorded when available.
.. Geographic information is captured for enhanced analytics.
