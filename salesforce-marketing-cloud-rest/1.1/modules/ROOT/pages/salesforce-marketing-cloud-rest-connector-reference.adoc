= Salesforce Marketing Cloud REST Connector 1.1 Reference
...
| Is Cdn Delete a| Boolean |  Permanently deletes the file and its URL in the CDN when the associated file is deleted in Content Builder. A value of `1` permanently deletes the file. An unspecified value or a value of `0` does not permanently delete the file. |  false |
| Read Timeout a| Number |  Read timeout value. Overrides the read timeout values defined in the connector configuration. |  |
| Read Timeout Time Unit a| Enumeration, one of:
