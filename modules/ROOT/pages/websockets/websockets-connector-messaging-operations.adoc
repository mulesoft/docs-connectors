...
== About WebSockets Operations

=== Send Operation

The send operation allows the user to send a message through WebSocket identified by its socketId, once a connection has been established. This operation can be used from both ends of the connection, by Inbound and Outbound WebSockets.


[source,xml,linenums]
----
    <flow name="sayHello">
      <websocket:open-outbound-socket path="/chat/hello" config-ref="ws"/>
...
* xref:http/websocket-connector-managing-groups.adoc[About WebSocket Group Management]
