...
WebSockets Version 1.0.0

Anypoint Connector for WebSockets (WebSockets Connector) provides communication through the WebSockets protocol. This connector allows for external systems to open connections to the server (INBOUND sockets), or allows connections to external systems (OUTBOUND socket). In both cases, full duplex traffic is supported.
...
Sends the given content to all the active WebSockets that match the path, socketType and groups criteria. In the case of the path, the configured base path will not be considered, as you can target both inbound and outbound sockets. 

The message will be sent to all the matching sockets, even though it's possible that delivery to some of them fail. The operation will return a list of BroadcastFailure containing details of each of the failures. If none fail, an empty list is returned.
...
This operation sends the given content through the specified socket.
...
Exposes an endpoint through which external systems can connect through a WebSocket. This source will trigger the owning flow each time one of those WebSockets sends information to this server. Additionally, any `websocket:on-inbound-connection` component with a matching path will be triggered when that socket connection is established.
...
This operation triggers when an inbound WebSocket connection is established.
...
This operation triggers when a WebSocket is closed.
...
This operation triggers when an outbound socket receives a message.
...
| Connection Idle Timeout a| Number | A timeout for idle connections to be closed. If not specified, there will be no timeout. |  | 
...
* xref:websockets/websockets-connector.adoc[WebSocket Connector]
