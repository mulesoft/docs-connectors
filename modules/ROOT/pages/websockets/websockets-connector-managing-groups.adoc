...
The WebSocket Connector uses socket groups, which are String identifiers that WebSocket clients can optionally subscribe to. This is useful for what we call message segregation, which enables your app to broadcast a single message to a group of different WebSocket clients. 
...
* xref:websocket/websocket-connector-messaging-operations.adoc[Operations]
