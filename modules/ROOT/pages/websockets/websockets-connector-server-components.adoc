...
Unlike the `<http:listener>` element which automatically sends a response when the flow ends, this component  never automatically sends a response. This is because the web sockets protocol is full duplex and therefore provides no guarantee that you actually want or need to send a response at all. Depending on your use case, you might want to simply process the messages without sending a response, or only send responses on certain cases.
