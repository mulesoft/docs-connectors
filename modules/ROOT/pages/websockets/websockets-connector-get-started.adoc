...
One of the use cases for this connector is broadcasting, which is the ability to send messages to a set of all active WebSockets. In order to provide an easy way to route each message to the appropriate set of WebSockets, we introduced the concept of `groups`. Groups are String identifiers that all Websockets can be optionally subscribed to.
