...
However, in this case a path cannot be indicated because the `URL` and `path` parameters are mutually exclusive. This is because the URL indicates a full resource locator, while the path indicates a path relative to the host, port, and basePath.
...
Now that you have completed configuration, you can try out the WebSockets xref:websockets/websockets-connector-examples.adoc[Examples].
