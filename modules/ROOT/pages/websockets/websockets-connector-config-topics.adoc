...
If your app exposes a WebSocket inbound endpoint, you need to set that up using the `<ws:server-settings>` element.
...
Notice the WebSocket server settings reference the HTTP listener configuration in the `listenerConfig` attribute. In this example, the WebSocket inbound listener listens for incoming messages on `localhost` and port 80, which are set in the HTTP listener configuration. This also sets basePath to `/ws` to override the HTTP basePath setting in the WebSocket configuration. To sum up, the WebSocket inbound listener listens for incoming messages at `+ws://localhost:80/ws/chat+`.
...
WebSocket connections are persistent and stateful. Therefore, it's a good practice to set a timeout to automatically close connections that have been idle.
...
If your application needs to connect to external WebSocket services, provide the client settings element:
...
In the configuration above, we chose to specify a host, port, and basePath to build the WS request URI. The host parameter is optional, if not set, it is set to `localhost`. The port parameter is optional, if not specified, the default value is set according to the protocol scheme: the default for WS is 80, the default for WSS is 443. The basePath parameter is optional, if URI is not set, it indicates the base path for all client requests relative to the indicated host and port, if not set the default value will be set to `/`.
...
However, in this case we cannot indicate a path because the URL and Path parameters are mutually exclusive, this is because URL indicates a full resource locator, while the Path indicates a path relative to the host, port, and basePath.
...
In the case of the WebSocket server, TLS configuration is inherited from the referenced HTTP listener configuration, for example, to configure TLS on a WebSocket listener, point to `<http:listener-config>` with TLS configured.

You first need to set up your trust store if your application connects to external services, a key store if your application listens for incoming connections, or you can set up both.

In the following example, the WebSocket server inherits its TLS configuration from the HTTP listener, which in turn references  `<tls:context>` to configure a key store:
...
In case of a WebSocket client, you can set up a trust store by directly referencing the <tls:context> in the `tlsConfig` property.
...
The protocol parameter is optional and defaults to WS (insecure). If set to `WSS`, then SSL is enabled in the connection. If  `<tls:content>` was specified, then that configuration is used to create the SSL session. Otherwise, default settings are used.

== Next

Now that you have completed configuration, you can try out the WebSocket xref:websocket/websocket-connector-examples.adoc[Examples].

== See Also

* https://help.mulesoft.com[MuleSoft Help Center]
* xref:4.1@mule-runtime::tls-configuration.adoc[Configure TLS with Key Stores and Trust Stores]
* xref:websocket/websocket-connector-get-started.adoc[About WebSocket Connector]
* xref:websocket/websocket-connector-cloudhub.adoc[WebSocket Connector in VPC]
* xref:websocket/websocket-connector-xml-maven.adoc[WebSocket XML and Maven]
