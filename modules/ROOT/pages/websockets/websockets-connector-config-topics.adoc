...
If your application exposes a WebSocket endpoint that only accepts incoming connections, but doesn't connect to any external services, you will have to specify your connector's configuration under the <server-settings> tag, because you are configuring a WebSocket Server.

WebSocket server settings require an underlying HTTP Listener Configuration, from which several connection settings will be inherited, however these settings can be overriden in the WebSocket configuration or at the operation level.
...
If your application will only connect to external services then you will need to provide Client Settings.
...
TLS configuration will be inherited from the referenced HTTP Listener config. So for example, in order to configure TLS on a WebSocket listener, just point to a <http:listener-config> with TLS configured.
