...
A WebSocket is a bidirectional, TCP connection between a client and server that is kept open until the app closes it. When using WeSockets, an app uses HTTP to make an initial connection. The connection then gets upgraded to a TCP socket-based connection.
...
Traditional HTTP connections require a lot of overhead because the client app is blocked until it receives a response from the server. Also, with this type of connection, the client must always initiate the request. Therefore, the server cannot push notifications to multiple apps at one time.
...
== Common Use Cases for the WebSockets Connector

The WebSocket Connector is useful for real-time apps that must avoid the overhead caused by the HTTP request-response message exchange pattern.
...
Before creating an app, you must have access to the WebSockets target resource and Anypoint Platform. You must also understand how to create a Mule app using Design Center or Anypoint Studio.

== Basic WebSockets Connector Concepts
...
* Inbound +
WebSockets created when the Mule app listens for incoming connections.
+
In this scenario, an external system connects to an endpoint exposed by the Mule app.
+
* Outbound +
WebSockets created when the Mule app connects to an external system.
+
...
Subscriptions only exist in the context of each instance of a WebSockets Connector configuration. Therefore, when you establish WebSockets between a Mule app and a remote system, subscribing the WebSockets on Mule's end of the connection to a group doesn't affect the WebSockets on the remote system's end.
...
* `outbound-listener` exposes an endpoint that initiates a flow when outbound WebSockets receive a message from an external system.
* `on_connection-closed` initiates the containing flow when inbound or outbound WebSockets close.
