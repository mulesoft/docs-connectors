...
Subscriptions only exist in the context of each instance of a WebSockets Connector configuration. Therefore, when you establish WebSockets between a Mule app and a remote system, subscribing theWebSockets on Mule's end of the connection to a group doesn't affect the WebSockets on the remote system's end.
...
+
The app can invoke this operation on the same WebSocket multiple times. each invocation being additive over the previous ones. Repeated groups are ignored.
+
