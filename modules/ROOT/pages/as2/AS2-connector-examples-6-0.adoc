...
|===
|Key| Description
|Self config - AS2 Partner Name| Sender’s AS2 system identifier. `AS2 Partner Name` is the field name that displays in Anypoint Studio. `as2Name` is the field name as a DataWeave transformation. Used for AS2-From and AS2-To headers.
...
|Partner config - AS2 ID| Receiver’s AS2 system identifier. `AS2 Partner Name` is the field name that displays in Anypoint Studio. `as2Name` is the field name as a DataWeave transformation. Used for AS2-From and AS2-To headers.
...
|Requester config - MDN MIC Signature Algorithm| The algorithm with which MIC is calculated for the MDN signature. Request partner to respond with a signed MDN using this algorithm for calculating MIC. The message is `UNSIGNED` by default.
