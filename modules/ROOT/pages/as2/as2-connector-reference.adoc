...
| AS2 Partner Name a| String |  +++AS2 system identifier (used for AS2-From and AS2-To headers)+++ |  | x
| x509 Alias a| String |  +++Certificate and/or private key alias in keystore (required if using signing or encryption)+++ |  |
| email a| String |  +++Email address used as HTTP 'From' header when sending+++ |  | x
| AS2 Partner Name a| String |  +++AS2 system identifier (used for AS2-From and AS2-To headers)+++ |  | x
| x509 Alias a| String |  +++Certificate and/or private key alias in keystore (required if using signing or encryption)+++ |  |
| email a| String |  +++Email address used as HTTP 'From' header when sending+++ |  | x
...
| Receipt Delivery URL a| String |  +++The A-Synchronous MDN delivery address which the mdn will be returned to+++ |  |
| Keystore Password a| String |  +++The password of the provided keystore.+++ |  | x
| Keystore Path a| String |  +++The Absolute or relative path to the keystore containing certificates and private keys for self(this application) and the partner.+++ |  | x
| Private Key Password a| String |  +++If the private key to use for signing/encryption is password protected, please give that value here.+++ |  |
|===
...
| AS2 MIME Type a| String |  +++The content type of the Document being sent via AS2 (i.e. application/EDI-X12).+++ |  |
...
| Require Processed a| Boolean |  +++Require Processed response in MDN to continue flow (error otherwise)+++ |  +++true+++ |
...
| Mime Type a| String | Mime type, ex. application/xml etc |  | x
