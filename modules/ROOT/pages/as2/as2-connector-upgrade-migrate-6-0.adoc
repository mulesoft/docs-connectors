...
For `Send with Sync MDN` and `Send with Async MDN` operations, an attribute object of type `com.mulesoft.connector.as2.api.AS2MdnAttributes` has been changed for v6.0.0.

Amend mule flows that are using these attributes are accordingly.

Note: The field `com.mulesoft.connector.as2.api.AS2SendAttributes.as2MdnAttributes` is of type `com.mulesoft.connector.as2.api.AS2MdnAttributes`)
