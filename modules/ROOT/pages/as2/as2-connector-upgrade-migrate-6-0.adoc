...
* The MultiMap `dispositionContent` has been changed into an object called `DispositionNotification` which now has each individual field of the message `disposition-notification` part of the mdn. The `DispositionNotification` can be found in the `DispositionNotificationReport` object.
