...
|Partner config - AS2 Partner Name| Receiver’s AS2 system identifier. Used for AS2-From and AS2-To headers.
...
|Requester config - Receipt delivery URL| Asynchronous receipt delivery URL that points to the MDN listener endpoint.
...
|Requester config - MDN MIC Signature Algorithm| Request partner to respond with a signed MDN using this algorithm for calculating MIC.
...
|===
|Key| Description
|Self config - AS2 Partner Name| Receiver's AS2 system identifier. Used for AS2-From and AS2-To headers.
...
|Partner config - AS2 Partner Name| Sender's AS2 system identifier. Used for AS2-From and AS2-To headers.
