...
* The `Async MDN` is now handled as a separate flow. You need to add a new flow (with an endpoint) to handle async MDN.
...
. Verify that the connector is working by making sure there are no exceptions on startup and testing against the AS2 partner.
