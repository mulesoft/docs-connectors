...
** `listener-config` now used by the `as2-listener` source
** `mdn-listener-config` now used by the `as2-mdn-listener` source
...
. Download the AS2 version 5 connector from Exchange.
...
These configurations are for a "receive endpoint" from a "send endpoint" since they are mutually exclusive.
