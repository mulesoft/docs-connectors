...
This example demonstrates how to upload a file to Box.
...
|===
+
. Transform this payload to multipart/form-data type of request. Specify a folder to be uploaded by setting the ID of the parent folder. Drag a DataWeave component next to the Read File component and use the following script.
+
[source,dataweave,linenums]
----
%dw 2.0
...
|===
+
. Next create an Authorization header in the HTTP Request. Click the Headers tab next to the Body in the HTTP request, then click the plus sign and configure the following parameter:
+
[%header%autowidth.spread]
|===
|Name |Value
...
|===
+
. Finally drag the DataWeave component to set the payload to display the result of the flow to the usert and use the following script.
+
[source,dataweave,linenums]
----
%dw 2.0
