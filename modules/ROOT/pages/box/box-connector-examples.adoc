...
This example demonstrates how to upload files to use in other box operations.

. Create a new Mule Project in Anypoint Studio.

. Create a folder called `input` in `src/main/resources directory`.

. Copy the file you want to upload to `input directory`.
...
. In the *Choose Global Type* wizard, expand the Connector Configuration, select HTTP Listener and click *Ok*.
...
. From the *Choose Global Type* wizard, expand Connector Configuration, select *File Config* and click *Ok*.
...
. In the Choose Global Type wizard, expand Connector Configuration and select HTTP Request Configuration and click *Ok*.
...
|===
+
. Now drag an HTTP request onto canvas and configure the following parameters:
+
[%header%autowidth.spread]
|===
|Parameter |Value
|Display Name| Request - Box Upload File
|Configuration| Select the HTTP request configuration that has been previously created in Global Elements
|Method|POST
|Path|
...
. Click the Headers tab next to the Body in the HTTP request, then click the plus sign and configure the following parameter:
...
|===
+
. Finally drag the DataWeave component to set the payload to display the result of the flow to the user and use the following script.
+
[source,dataweave,linenums]
----
%dw 2.0
...
Here is the XML example you can use.
