...
|`accessTokenUrl`|Specifies the URL for requesting for an access token.
|===
+
...
NOTE: For simpler maintenance and better re-usability of your project, Mule recommends that you use a configuration file.
Keeping these values in a separate file is useful if you need to deploy to different environments such as: production, development, and QA (where your access credentials are different).
+
. Click *OK*.
