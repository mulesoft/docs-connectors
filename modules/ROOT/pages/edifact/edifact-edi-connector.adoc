...
NOTE: The EDIFACT EDI Connector supports files up to 15MB. The EDIFACT EDI connector's memory requirement is roughly 40 to 1. For example a 1MB file will require up to 40MB of memory to process. This is not an exact figure; the value may vary according to the complexity of the mapping instructions. It is important to consider this memory requirement in conjunction with your TPS needs for large EDIFACT files.
...
You can also define your own schemas from scratch, or copy and edit a base EDIFACT schema definition to suit your data. See the xref:partner-manager::edi-schema-language-reference.adoc[EDI Schema Language Reference] for more details.
