...
----
+
. Add a os:config element to your project, then configure it to contain a redis connection type(clustered or nonclusterd):
This is only possible in Studio at the moment and it has to be configured through xml(no support using graphic configuration)
+
[source,xml,linenums]
----
...
----
+
. Add a os:object-store element to your project and configure it to refference the previously created config.
+
[source,xml,linenums]
----
<os:object-store name="Object_store" config-ref="ObjectStore_Config" maxEntries="1" entryTtl="60" expirationInterval="10" expirationIntervalUnit="SECONDS"/>
----
+
. Add a `http:listener-config` element to your project, and configure its attributes:
+
...
----
+
. Within the flow element, add a `os:store` after the `http:listener` and configure it to use the previously created Object Store
+
[source,xml,linenums]
----
<os:store key="#[attributes.queryParams.key]" objectStore="Object_store" failIfPresent="true" failOnNullValue="false">
...
http://www.mulesoft.org/schema/mule/http http://www.mulesoft.org/schema/mule/http/current/mule-http.xsd">
	...
	<flow name="StoreFlow" doc:id="2c8d4c96-b1d9-4a79-a70e-2cec288cb16a" >
        <http:listener doc:name="Listener" doc:id="0b2c5018-b4f7-4091-8bc8-09531553ec4f" config-ref="HTTP_Listener_config" path="/store"/>
        <os:store doc:name="Store" key="#[attributes.queryParams.key]" objectStore="Object_store" failIfPresent="true" failOnNullValue="false">
		<os:value ><![CDATA[#[attributes.queryParams.value]]]></os:value>
	</os:store>
    	</flow>
