...
To avoid multiple executions of the same logic, the `flow-ref` followed by the `aggregator-listener` should be wrapped in a `choice` component, evaluating if the aggregation is not complete by checking the `aggregationComplete` attribute. That way, the `flow-ref` will only be called
when a timeout triggered that flow.
