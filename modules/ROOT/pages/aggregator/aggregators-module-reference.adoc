...
---
image::aggregator/size-based-aggregator.png[Size based Aggregator]
. Input Data
...
---
image::aggregator/time-based-aggregator.png[Time based Aggregator]
. Input Data
. Aggregated elements (Not complete)
. Aggregated elements, one of:
...
Lastly, when elements that reach group-based aggregators come from a sequence that was splitted (by a xref:4.2@mule-runtime::index.adoc[ForEach] component for example), each will have assigned a different `sequenceNumber`. In that case, they are sorted in increasing order prior to the aggregation release.
...
---
image::aggregator/group-based-aggregator.png[Group based Aggregator]
. Input Data
. Aggregated elements (Not complete)
. Aggregated elements (Complete)
. Input Data (same as 1) + New Variables
