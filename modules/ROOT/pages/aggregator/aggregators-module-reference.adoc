...
Using an aggregator listener creates a new flow. The context of the flow where the aggregator exists is not accessible from the new flow. Therefore, the new flow cannot access variables from the aggregrator's flow.
