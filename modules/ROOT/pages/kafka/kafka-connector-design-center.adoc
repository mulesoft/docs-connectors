...
. If you are using the Anypoint Connector for Apache Kafka (Apache Kafka Connector) as a trigger, search for the connector; otherwise, search for HTTP or Scheduler.
. Select the source. +
Apache Kafka Connector provides the *Batch message listener* and *Message listener* as sources.
...
** MANUAL
** IMMEDIATE
** DUPS_OK a|  Defines the way that the Apache Kafka broker instance is notified of the consumption of messages.
...
|===
| Name | Type | Description | Default Value | Required
| Bootstrap Server URLs a| Array of String |  The list of servers to bootstrap the connection with the Apache Kafka cluster. This can be a partial list of the available servers. |  | x
| Group ID a| String |  Default group ID for all the Apache Kafka consumers that use this configuration. |  |
...
| Default Record Limit a| Number |  The maximum number of records returned on a poll call to the Apache Kafka cluster. This parameter can be overridden at the source level. |  `500` |
