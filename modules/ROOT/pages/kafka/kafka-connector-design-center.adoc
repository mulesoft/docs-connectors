= Apache Kafka Design Center Configuration - Mule 4
...
. If you are using the Anypoint Connector for Kafka (Kafka Connector) as a trigger, search for the connector; otherwise, search for HTTP or Scheduler.
. Select the source. +
