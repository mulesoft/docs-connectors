...
To follow this example, construct a flow as follows:
...
.. In the *Bootstrap server URLs* field, select *Edit inline* and then *+*. .. Enter the value `${config.basic.bootstrapServers}` and click *Finish*.
...
. For the next flow, drag the Apache Kafka *Consume Message* input source to the Anypoint Studio canvas.
