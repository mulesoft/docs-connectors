...
** Keytab +
Path to the keytab file associated with the `principal`.
** Service Name +
...
+
** Default acknowledgment mode +
Declares the kind of acknowledgment mode supported
** Default poll timeout + 
The amount of time to block
** Default poll timeout time unit +
...
| Partition Offsets a| Array of Offset - List of offsets representing the partitions offsets configuration. For each element in the list you must specify a partition index and offset.
...
| Acknowledgment mode | Declares the kind of supported acknowledgment mode
