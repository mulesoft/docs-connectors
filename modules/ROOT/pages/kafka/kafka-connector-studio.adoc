= Use Studio to Configure Apache Kafka - Mule 4
ifndef::env-site,env-github[]
include::_attributes.adoc[]
endif::[]

In Anypoint Studio, add Anypoint Connector for Apache Kafka (Apache Kafka Connector) to a Mule project, configure the connection to the Apache Kafka cluster, and configure an input source for the connector.
...
. Select Apache Kafka Connector and click *Add to project*.
...
List of host-port pairs used for establishing the initial connection to the Apache Kafka cluster. This is the same as the `bootstrap.servers` value you must provide to Apache Kafka clients (producer or consumer).
* Bean Reference (Optional) +
The URLs that the consumer can use to connect to the Apache Kafka cluster.
* Group ID +
Default group ID for all the Apache Kafka consumers that use this configuration.
...
List of host-port pairs used for establishing the initial connection to the Apache Kafka cluster. This is the same as the `bootstrap.servers` value you must provide to Apache Kafka clients (producer or consumer).
* Bean Reference (Optional) +
The URLs that the producer can use to connect to the Apache Kafka cluster.
+
...
The *Batch Message Listener* operation can also be used as an input source in Apache Kafka Connector:
