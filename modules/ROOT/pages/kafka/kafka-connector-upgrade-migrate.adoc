...
Upgrade Anypoint Connector for Kafka (Kafka Connector) to version 4.2.x.
...
| Consume | Enables receiving messages from one or more Kafka topics. It works very similarly to the Message Listener source, so all the operations that apply to the Message Listener source also apply to this operation. +
[NOTE]
