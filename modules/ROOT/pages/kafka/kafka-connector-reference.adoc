...
| Additional Properties a| Object |  Additional properties such as a key and value that you need for your connection. |  | 
...
| Additional JAAS Properties a| Object |  Additional properties such as a key and value that you need to set in `sasl.jaas.config` and that you usually include in the JAAS configuration file. |  | 
|===
...
| Buffer Size Increment a| Number | This is by how much the buffer size expands if it exceeds its initial size. Setting a value of zero or lower means that the buffer should not expand, and to raise a STREAM_MAXIMUM_SIZE_EXCEEDED error if the buffer gets full. |  | 
