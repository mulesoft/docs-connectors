...
This document assumes that you are familiar with Roostify, Mule concepts, Anypoint connectors,
...
To use the Roostify connector with Maven, find dependency information on the *Dependency Snippets* tab of Blend connector's page on the Anypoint Exchange.
...
== Install Roostify Connector in Anypoint Studio
...
* Schema Location: `+http://www.mulesoft.org/schema/mule/roostify/current/mule-blend.xsd+`
...
** *x-blend-special-instance-id*: Your special deployment instance ID, if required.
+
TIP: An example configuration file for Roostify
...
----
blend:
  baseurl: "<url-to-access-roostify-api>"
  username: "<username-to-access-roostify-api>"
...
. From the *Mule Palette*, search and drag *Set Variable* operation after the Roostify operation, to capture the loan ID returned from Blend into a variable. In the *Settings* section of the *Set Variable* operation, set the following:
.. *Name*: `loanId`
.. *Value*: `#[payload.id]`
