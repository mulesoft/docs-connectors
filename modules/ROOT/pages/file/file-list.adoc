...
`config-ref` parameter. To see an example of creating a configuration check
...
This will output an Array of Mule Messages, where the `payload` is the file's contents (or null if it is
a folder) and `attributes` corresponds to the file's (or folder's) attributes such as name, creation time,
size, xref:file/file-documentation.adoc#LocalFileAttributes[among others]. In Studio, DataSense will show
us the metadata for the operation's output:
...
xref:file/file-read.adoc[`read` operation] after a `list` to access the file's content. See below
for an <<Read files returned by List,example>> of this.
========

== Read files returned by List

In this example, we list all files and folders in the `directoryPath` and then, in a
https://docs.mulesoft.com/mule-runtime/4.1/for-each-scope-concept[For Each loop], we
check if each element is a file or a folder with a
https://docs.mulesoft.com/mule-runtime/4.1/choice-router-concept[choice component]. If it is a file,
then a https://docs.mulesoft.com/mule-runtime/4.1/logger-component-reference[logger] outputs it's
path and contents. If it is a directory, we send it to another flow for processing.
...
To list files or folders within any sub-folders of the `directoryPath`, i.e. doing a recursive list, you
...
alongside the `TimeBetweenSizeCheckUnit` parameter). When enabled, Mule performs two size checks: one
...
consume the output stream in a https://docs.mulesoft.com/mule-runtime/4.1/logger-component-reference[`logger` component].
...
Note that the 15 seconds check will happen when the `logger` component is executed, not at the read operation. is executed, not at the read operation.
