...
The only required parameter for the `Read` operation is the path to the file to be read. This can be either 
the full path to the file or a relative path to the working directory value defined in the configuration
referenced by the `config-ref` parameter. In Studio 7, you can set the path in the *File path* field. In XML, the *path* attribute in the `<file:read` statement specifies the path.
...
In Studio, click the 3-dot browse button to locate the file, for example, `/Users/me/myfile`

In XML, set the *path* attribute to the file's path, for example, `<file:read path="/Users/me/myfile" />`
