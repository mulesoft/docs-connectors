...
file or a relative path to the working directory value defined in the configuration
...
* The file's metadata in the message attributes, metadata (such as such as name, creation time, size, and so on) in the message attributes.
...
share part of their path, their base path can be extracted into a xref:file/file-connector.adoc#connection_settings[file configuration] for better reuse.
...
It is set to `false` by default, but when the parameter is set to `true`, the connector asks the operating system to lock the file, which prevents any other process from accessing that file while the
