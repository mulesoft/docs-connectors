...
If the target file already exists, then it will be overwritten if the overwrite argument is true. Otherwise, FILE:FILE_ALREADY_EXISTS error is thrown. 

As for the sourcePath, it can either be a file or a directory. If it points to a directory, then it's copied recursively.
