...
* Set the path to a Functional Acknowledgment schema to be used for generated acknowledgments such as `/hipaa/005010X231A1/999.esl` which uses the standard HIPAA 005010X231A1 acknowledgment.
 Using this means the schema type (997 or 999) must match the Generate 999 Functional Acknowledgments setting (false for 997, true for 999).
 If not set, the parser uses a hardcoded X12 997 or 999 schema, as appropriate.
