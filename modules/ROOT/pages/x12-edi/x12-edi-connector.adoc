...
NOTE: This connector effectively supports files up to 20MB. As you process files beyond 20MB, consider that the connector has a 40x memory requirement corresponding to the EDI file size. 
For example, a 30MB X12 (or EDIFACT) file needs 1.4GB of memory. It's important to consider this memory requirement in conjunction with your TPS needs for large files.
