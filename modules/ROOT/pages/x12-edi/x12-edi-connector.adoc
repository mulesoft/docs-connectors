...
NOTE: X12 EDI Connector supports files up to 20MB. If you process files larger than 20MB, consider that the connector has a 40x memory requirement that corresponds to the EDI file size. 
For example, a 30MB X12 (or `EDIFACT`) file needs 1.4GB of memory, so it's important to consider this memory requirement in conjunction with your TPS needs for large files.
