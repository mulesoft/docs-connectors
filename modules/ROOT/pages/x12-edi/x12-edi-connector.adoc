...
NOTE: X12 EDI Connector supports files up to 15MB. The X12 EDI connector's memory requirement is roughly 40 to 1. For example a 1MB file will require up to 40MB of memory to process. This is not an exact figure; the value may vary according to the complexity of the mapping instructions. It is important to consider this memory requirement in conjunction with your TPS needs for large X12 files.
