...
. Define a flow with a `<vm:listener>` as a message source.

The listener flow will receive the published message from a different flow, for example:
...
----
[start=2]
. Publish messages into the newly created `queueName="myQueue"`.
