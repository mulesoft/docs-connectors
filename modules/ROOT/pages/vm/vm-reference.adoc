...
** MINUTES
** HOURS
** DAYS |  A TimeUnit which qualifies the #timeoutUnit |  SECONDS | 
...
The temporal reply queue is automatically disposed of after a response is received or the timeout expires.
...
A source that creates and listens on VM queues. VM queues are created by placing listeners on them, which is why this listener contains parameters for a queue's behavior, such as whether or not it is persistent, the max capacity, and so on.
...
** LOCAL
** XA |  The type of transaction to create. Availability will depend on the Mule version. |  LOCAL | 
