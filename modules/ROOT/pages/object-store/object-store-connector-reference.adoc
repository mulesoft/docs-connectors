...
NOTE: These settings do not apply to xref:object-store::index.adoc[Object Store v2]. These are only for use with an on-premises object store.
...
The `Expiration Interval` field requires that the `Max Entries` or `Entry Ttl` fields also be set. If neither is set, then putting a value in `Expiration Interval` has no effect and the expiration thread does not run.  | 1 |
| Expiration Interval Unit a| Enumeration, one of:
