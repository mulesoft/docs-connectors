...
If the parameter was not provided or resolved to a null value, then a OS:KEY_NOT_FOUND error is thrown. Otherwise, the defaultValue is returned, but that value is not stored.
...
| Value a| Any |  +++The value to be stored. Should not be null if `failOnNullValue` is set to true.+++ |  `#[payload]` | 
...
| Max Entries a| Number | The max number of entries allowed. Exceeding entries are removed when expiration thread runs. If absent, then the described Object Store has no size boundaries. |  | 
...
** MINUTES
** HOURS
** DAYS | An entry's time to live (`#entryTtl`). | SECONDS | 
| Expiration Interval a| Number | How frequently the expiration thread should run.
 
If `#entryTtl` is set, then the expiration thread does not run despite the value set here.

If set to a value lower than or equal to zero, then there is no expiration. | 1 | 
...
** MINUTES
** HOURS
** DAYS | A #expirationInterval | MINUTES | 
| Config Ref a| <<ObjectStoreConnector>> | A reference to an os:config element used to declare this Object Store.
...
|===
| Field | Type | Description | Default Value | Required
| Alias a| String | A friendly name to refer to this store in the management user interface. Provide this alias if you would like it
 to be easier to identify this store. If not provided, Mule auto generates an ID. |  | 
| Persistent a| Boolean | Whether the store is persistent or transient. | true | 
| Max Entries a| Number | The maximum number of entries allowed. Exceeding entries are removed when the expiration thread runs. If absent, then the
 described Object Store has no size boundaries. |  | 
| Entry Ttl a| Number | The entry timeout. If absent, then the described Object Store has no time boundaries. |  | 
| Entry Ttl Unit a| Enumeration, one of:
...
| Expiration Interval a| Number | How frequently the expiration thread should run.
 ...
 If #entryTtl is set, then the expiration thread does not run despite the value set there.
...
** HOURS
** DAYS | A #expirationInterval | MINUTES | 
| Config Ref a| <<ObjectStoreConnector>> | A reference to an os:config element which is used to declare this Object Store.
