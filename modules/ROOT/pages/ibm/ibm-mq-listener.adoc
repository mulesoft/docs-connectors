...
For more information regarding a message ACK, see xref:ibm/ibm-mq-ack.adoc[Handling Message Acknowledgement].
...
The automatic reply may be disabled using the `ignoreReplyTo` attribute of the `ibm-mq:response` element:
...
For details on how to configure the topic subscriptions see xref:ibm/ibm-mq-topic-subscription.adoc[Using Topic Subscriptions].
