...
. Click a trigger to be the input source, such as HTTP Listener or Scheduler trigger. The IBM CTG Connector does not provide a trigger operation.
+
...
To enable logging of the interactions of the connector with IBM CTG, a logger must be configured in the log4j2.xml file of the Mule app as follows. Put this statement in the `Loggers` block in the log4j2.xml file:

[source,xml,linenums]
----
<AsyncLogger name="org.mule.modules.ibmctg.internal.service.CTGServiceImpl" level="DEBUG"/>
----
...
|===
+
. Add a Transform Message between the HTTP and the IBM CTG operation. The connector does not provide dynamic or static metadata, but allows users to define custom types using the Metadata Type tool.
+
...
. Add a Logger at the end of the flow.
. Save the changes and deploy the Mule app. Open a browser and make a request to `+http://localhost:8081/ec03+`. The result should be similar to:
+
[source,json,linenums]
----
{
...
The IBM CTG connector does not provide dynamic or static metadata out-of-the-box, but allows users to define custom types using the Metadata Type tool. Define input and output metadata as follows:
...
. This populates a drop-down with a list of available data segments. Choose the one you need and click *Select* to save the configuration.
...
+
A metadata definition is not a required step to use the IBM CTG connector, but it is essential to improve the usability of the connector. See xref:7.2@studio::create-metadata-class-task.adoc[Create Metadata].
...
* xref:7.2@studio::create-metadata-class-task.adoc[Create Metadata]
