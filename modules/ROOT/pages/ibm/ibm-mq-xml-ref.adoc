...
| Max Idle Seconds a| Number |  +++How many seconds can an XA transaction remain idle before its eligible for eviction.+++ |  +++60+++ |
| IBM MQ CipherSpec Mapping a| Enumeration, one of:

** ORACLE_CIPHER_SUITES
** IBM_CIPHER_SUITES
** NONE |  Defines which Cipher Suite Mapping to use. Oracle JDK or IBM JDK. Configure depending on the Host JDK. https://www.ibm.com/support/knowledgecenter/en/SSFKSJ_8.0.0/com.ibm.mq.dev.doc/q113220_.htm[More information]. |  ORACLE_CIPHER_SUITES | 
