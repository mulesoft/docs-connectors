...
Environment:: The CloudHub environment to use (by default this is the environment defined as the default in Anypoint Platform)
...
If you create your app outside Anypoint Studio, update your POM file as follows:
...
Once the username and password are configured correctly, you can
...
To specify the environment to which to deploy the app, use the <<environment.id-sysprop,`environment.id`>> system property.
...
* Limit
+
Specifies the number of notifications to retrieve, by default `-1`, which indicates all available notifications
* Status
+
Filters the notifications to retrieve (unread, read, or all), by default, only retrieves notifications marked as unread
* Search
+
...
Once a notification is consumed, mark a notification as read so that the next time notifications are read, only the unread ones are retrieved.
...
Using the connector, you can retrieve all available applications in the given environment. There is nothing required to configure.
