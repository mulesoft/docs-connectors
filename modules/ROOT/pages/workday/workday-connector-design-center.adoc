...
. If you are using the Workday connector as the input source trigger, search for the connector;
otherwise, you can use HTTP or Scheduler.
. Complete these fields when using *New Object Trigger*:
+
...
|Start delay|Delay time in time units that Scheduler needs to wait before starting.|No
...
The target component enables your app to process data received from the trigger. The Workday connector 
provides operations that cover tasks for educational organizations, human resources, professional services,
