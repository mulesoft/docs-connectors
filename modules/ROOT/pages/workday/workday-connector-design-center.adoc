...
A trigger which starts your Mule app. Workday Connector provides the Modified Object Trigger operation that starts your app when a object is created in Workday. With the experiences we've gained it turned out that the released version of trigger had significantly poor performance when Workday instance contained thousand of records shared between numerous workers. Based on this finding we've improved the performance of trigger Object Type - WORKERS. For now, it was the only possible operation where this issue could have been fixed. When choosing the Object type - **WORKERS** you can also specify the field `updatedFrom` which will give you all of the created or updated Worker objects from given time. Alternatively, you can use the HTTP Listener operation to start your app from a browser or with a command such as `curl`. Another alternative is to use Scheduler to start your app on a timed schedule.
