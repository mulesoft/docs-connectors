...
* On Modified Objects +
Start your app using this trigger when an object is created in Workday. With the experiences we've gained it turned out that the released version of trigger had significantly poor performance when Workday instance contained thousand of records shared between numerous application workers. Based on this finding we've improved the performance of trigger Object Type - WORKERS. When choosing the Object type - **WORKERS** you can also specify the field `updatedFrom` which will give you all of the created or updated Worker objects from given time. For now, it's the only possible Object Type operation where this issue can be fixed.
* HTTP Listener +
...
The required fields for the On Modified Objects input source are:
