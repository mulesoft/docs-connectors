...
| Connection Idle Timeout a| Number |  How long a connection can remain idle before it is closed, based on the unit specified in the Connection Idle Timeout Unit field. The value of this attribute is only used when persistent connections are enabled. |  30 |
...
| Connection Timeout a| Number |  How long a connection can remain idle before it is closed, based on the unit specified in the Connection Timeout Unit field. The value of this attribute is only used when persistent connections are enabled. |  30 | {nbsp}
...
| TLS Configuration a| <<Tls>> | Protocol to use for communication. Valid values are HTTP and HTTPS. Default value is HTTP. When using HTTPS, the HTTP communication is secured using TLS / SSL. In this case, the user needs to configure at least the keystore in the tls:context child element of this listener-config. |  |
...
| Reconnection Strategy a| * <<reconnect>>
* <<reconnect-forever>> |  A retry strategy for handling connectivity errors. |  |
|===
...
Invoke the Compensation Review web service, which contains operations that exposes the Workday human capital management business services compensation review related data.
