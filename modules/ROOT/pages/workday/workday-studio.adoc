...
IMPORTANT: To validate connectivity using the Test Connection button, the Workday connector uses an `Identity_Management` WSDL service with the `Get_Workday_Account_Signons_Request` operation. Ensure that your service account has permissions to operate on that service. If not, using the Test Connection fails.
