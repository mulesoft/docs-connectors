...
* Replaced the `On New Object` input source with the `On Modified Object` input source, which polls data more efficiently. For more information, see <<on-modified-objects,On Modified Objects>>.
...
The following example shows how to use the `On Modified Objects` input source. This example creates a new position and hires an employee when a new Worker instance is added to Workday. The `On Modified Objects Listener` input source listens for events that are created when a Worker instance is modified. When the listener finds a modified Worker instance, it logs the new instance to the Anypoint Studio console.
