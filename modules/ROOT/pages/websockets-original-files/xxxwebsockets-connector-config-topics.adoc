...
If your application exposes a WebSocket inbound endpoint, you need to set that up using the `<ws:server-settings>` element.

Because WebSocket connections are initiated with an HTTP request, exposing a WebSocket's inbound endpoint requires pointing to an underlying HTTP listener configuration. This not only allows you to reuse all the available features and configurations of an HTTP listener (such as host, port, TLS, and so on), it also allows your application to expose both WebSockets and regular HTTP endpoints using the same port.
...
Notice the WebSocket server settings reference the HTTP listener config in the `listenerConfig` attribute. In this example, WebSocket Inbound Listener will listen for incoming messages on localhost and port 80, which are set at the HTTP listener configuration. The basePath is set to to "/ws", since it overrides the HTTP basePath setting in the WebSocket configuration. To summarize, the WebSocket Inbound Listener will be listening for incoming messages at "ws://localhost:80/ws/chat".

You can see that since the HTTP listener has no reference to a TLS configuration component, the connection will be opened using WS protocol by default.
...
In this example, an Outbound Socket is initialized, which references the WebSocket client configuration ("wsClient") in the `config-ref` attribute. In this case, there is no need to reference an HTTP configuration.
...
The `port` parameter is optional; if not specified the default value will be set according to the protocol scheme. If WS is used, the default value is set to `80`. If WSS is used, the default is set to `443`. 

The `basePath` parameter is optional; if the URI is not set, it will indicate the base path for all client requests relative to the indicated host and port. If the `basePath` parameter is not set, the default value is set to "/".
...
In the following example, the WebSocket server will inherit TLS configuration from the HTTP Listener, which will in turn reference a `<tls:context>` to configure a key-store.
