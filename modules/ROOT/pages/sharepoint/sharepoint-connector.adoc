...
+
For the `List Item Query` operation the query should have this format: `LIST_ID?$select=FIELDS_TO_SELECT&filter=FILTER_PART`.
+
For example: `Vessels?$select=Title,FLAG,SEGMENT,ID,VesselName&$filter=Title eq 1`.
+
The same format also applies to the other query operations.
+
For a better understanding of how the queries can be built, see the SharePoint documentation page https://docs.microsoft.com/en-us/sharepoint/dev/sp-add-ins/use-odata-query-operations-in-sharepoint-rest-requests[Use OData query operations in SharePoint REST requests].
