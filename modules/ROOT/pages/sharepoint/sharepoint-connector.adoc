...
+
* How do I build a query for the query operations?
+
The query operations(Folder Query, File Query and List Item Query) only support OData queries. 
+
For example, for the List Item Query operation the query should have the following format `LIST_ID?$select=FIELDS_TO_SELECT&filter=FILTER_PART` I.E. `Vessels?$select=Title,FLAG,SEGMENT,ID,VesselName&$filter=Title eq 1`. 
+
The same format applies for the other query operations too. For a better understanding of how the queries can be built check out this https://docs.microsoft.com/en-us/sharepoint/dev/sp-add-ins/use-odata-query-operations-in-sharepoint-rest-requests[page]
