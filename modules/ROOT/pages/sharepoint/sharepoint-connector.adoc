...
Anypoint Connector for Microsoft SharePoint (Microsoft SharePoint Connector) version 3.1

Release Notes: xref:release-notes::connector/sharepoint-connector-release-notes-mule-4.adoc[Microsoft SharePoint Connector Release Notes - Mule 4]

Microsoft SharePoint Connector provides support for both SharePoint 2013 and SharePoint Online for use in the cloud and on-premises. This connector enables content and document management, access to intranet and extranet portals and websites, collaboration, and enterprise search. This connector also enables integration with SharePoint through its REST API.

== Prerequisites

This document assumes that you are familiar with Microsoft SharePoint, Mule, Anypoint Connectors, Anypoint Studio, Mule concepts, elements in a Mule flow, and Global Elements.
...
To use this connector with Apache Maven, view the pom.xml dependency information in
the Dependency Snippets in https://www.mulesoft.com/exchange/com.mulesoft.connectors/mule-sharepoint-connector/[Anypoint Exchange].

== What's New in this Connector

In Mule 4 SharePoint online and SharePoint on-premises were merged into one connector that includes all functionalities from both previous connectors.
...
For information on Flow Designer field values, see the xref:connectors::sharepoint/sharepoint-connector-reference.adoc[Microsoft SharePoint Connector Reference].
...
+
Microsoft SharePoint Connector provides trigger operations for:

* Created Objects
* Modified Objects
* Deleted Objects
+
image::sharepoint/sharepoint-global-config.png[]
+
. Provide these required fields (differs by operation):
...
You can use a SAML security token to log into SharePoint.
...
. Import the demo project into your workspace using Anypoint Exchange or using the *File* > *Import command.
...
You can use the selection menu from `+http://localhost:8081+` to test the flows or you can POST JSONs using a tool like curl, or any other tool (Chrome/Mozilla Firefox extensions) that lets you post an HTTP body when calling the URL.
...
+
For SharePoint Online, authentication using standard SharePoint online user credentials is supported, secure authentication,  as well as Okta authentication.
+
