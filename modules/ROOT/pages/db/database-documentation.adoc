...
| Parameter Types a| Array of <<ParameterType>> |  Allows to optionally specify the type of one or more of the parameters in the query. If provided, you're not required to reference all of the parameters, but you cannot reference a parameter not present in the input values. |  | 
| Input Parameters a| Object |  A Map which keys are the name of an input parameter to be set on the JDBC prepared statement. Each parameter should be referenced in the SQL text using a colon prefix (for example, where `ID = :myParamName`).  The map's values contain the actual assignment for each parameter. |  | 
| Input - Output Parameters a| Object |  A Map which keys are the name of a parameter to be set on the JDBC prepared statement which is both input and output.  Each parameter should be referenced in the SQL text using a colon prefix (for example, where `ID = :myParamName`).  The map's values will contain the actual assignment for each parameter. |  | 
