...
[NOTE]
====
*Using Providers' Custom Types*:
When using a provider's custom type in a Data Source Reference Connection, define the type inside the "Column types" form of the Advanced section in the Database config. +
For example, for Oracle DB checkout their Constant Field Values page (section oracle.jdbc.OracleTypes) for the correct "id/type name" combination. 
====
