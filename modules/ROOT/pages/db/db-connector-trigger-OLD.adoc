= Trigger a Flow for Each Row in a Table - Mule 4
...
<1> Configure the `<db:listener>` to check for rows in the `EVENTS` table once every minute. Runs the `SELECT * FROM EVENTS` query.
<2> Executes per each row, and in this example, processed through a flow ref.
<3> The processed row is deleted to make sure it's not picked up again in the next poll.
...
To help you with this, the component allows you to specify an ID column. That column will be considered as a unique identifier for the row and the listener will make sure that the row is not picked up again if it has already been found AND processing of it hasn't finished yet. For example:
...
DataSense is available for the listeners's output. Studio and Flow Designer automatically hint the table structure for you, making it easy to write expressions and mappings.
