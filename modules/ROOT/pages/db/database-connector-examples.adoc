...
Database Connector supports the use of callable statements with the format ``{ call procedureName(:param1, :param2, ..., :paramN) }`` where parameters matches positionally, that is to say, `:paramN` matches the `Nth` parameter in the stored procedure declaration. Suppose you have declared a stored procedure which receives two parameters: `myFirstParameter` and `mySecondParameter`, in that order. The connector does not support named parameters in callable statements, so attempting to match parameters by name ( `{ call myStoredProcedure(mySecondParameter => :param1, myFirstParameter => :param2) }`` ) does not work.

[[use_dates_stored_procedures]]
== Use dates on Stored Procedures
Database Connector does not support the use of engine specific embedded functions, for example, if you want to change a date to a specific format before calling a procedure in an Oracle database,the following approach does not work:  `{ call myStoredProcedure(paramName => to_date(:param1, 'YYYY-MM-DD HH:mm:ss')) }.` +
Use DataWeave for all data transformations before the invocation to the callable statement, for the previous example we perform the following transformation:
