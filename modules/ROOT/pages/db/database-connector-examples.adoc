...
If an error arises while executing one of the operations (for example, if bulk insert fails to insert 1 out of 100 rows), a single exception will be thrown.

It may happen that while some statements in the Bulk operation can be successfully executed, some may result in an error. When this happens, it will be up to the driver to either:

    1) Stop execution immediately and ignore all remaining operations, or
    2) Continue to execute the remaining statements.

In both cases, whenever an error occurs you can examine your application logs to see which caused the failure. When this happens a single exception will be thrown describing what went wrong.
