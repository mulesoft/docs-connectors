...
If an error arises while executing one of the operations (for example, if bulk insert fails to insert 1 out of 100 rows), an exception will be thrown, but no alterations will have taken effect on the database.

Bulk operations will stop and fail as soon as an error is found, ignoring all records that haven't been processed yet.

When this happens a single exception will be thrown describing what went wrong. There is no specific way of telling which record caused the failure, other than examining the logs carefully and retrying the operation.
