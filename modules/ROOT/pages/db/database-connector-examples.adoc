...
Now the connector accepts SQL Casting PostgreSQL and Snowflake syntax's, through the double colon (::) expression without the neeed to escape each colon (this feature does not change the existing behavior for the connector). For example:
