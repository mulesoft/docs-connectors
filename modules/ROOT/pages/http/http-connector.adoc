...
In this example, the HTTP Listener configuration is named `serverConfig` and listens on a computer's localhost address
...
In this next example, we use the incoming HTTP request query parameters and customize the body of our HTTP responses:
...
HTTP clients allow sending requests to HTTP services and handling their responses. For
...
In this example, the HTTP request configuration is named `clientConfig` and will send requests to
localhost and port 8081, the server we previously configured.

To then perform requests, each operation you use only needs to feature the path where it should go.
...
In the previous examples we consume the service we exposed before by sending requests to `greet` and `exit`. When our flow
...
The requests can be customized and the responses data is shared with the flow.

In this next example, we customize the request we send with query parameters and use the received HTTP response status:
