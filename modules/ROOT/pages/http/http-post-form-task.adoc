...
Learn how to post a form using Anypoint Connector for HTTP (HTTP Connector) Listener operation. The following example shows how to perform this operation both in Anypoint Studio and XML. Additionally, review how to send the HTTP request that contains form parameters using an API tool such as Postman or Curl.

== Studio

. In Studio, create a Mule project.
. To create a new flow, add an HTTP Listener Connector into the canvas, set *Path* to `submitform`.
. Go to the *Global Elements* tab, and click *Create*. Set *Host* to `localhost` and *Port* as the default value of `8081`.
...
== Post Form Requests with a x-www-form-urlencoded Type Body - Full XML Code
