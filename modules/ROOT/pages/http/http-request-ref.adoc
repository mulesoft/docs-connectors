...
This is helpful and convenient but has some important caveats to consider. Using expressions in a configuration element constitutes a dynamic configuration. The lifecycle of a dynamic configuration is explained here, but in a nutsheel, all expressions in the config are evaluated each time the connector is used and for each set of distinct values a new configuration instance is created and initialised. 

For the particular case of the HTTP connector, this means that it's only a good idea to default headers with expressions when you have certainty that the universe of possible values that those expressions can take is small. If on the contrary, changes are that every single evaluation is going to yield a different value, you'll end up creating several instances of the http client. That will affect performance (due to the extra work that initializing a client takes) and it will be resource consuming. For such cases, the recommendation is not to use default headers but to do it on the request operation itself.
...
image::http/http-mule4-uriparams.png[]
