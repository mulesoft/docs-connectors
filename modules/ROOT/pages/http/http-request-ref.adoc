...
For the particular case of the HTTP connector, this means that it's only a good idea to use default headers with expressions when you are certain that the universe of all possible values that those expressions can take is small. If on the contrary, it is likely that every single evaluation is going to yield a different value, you'll end up creating several instances of the HTTP client. That will affect performance (due to the extra work that initializing a client takes), and it will be resource-consuming. For such cases, the recommendation is not to use default headers but to specify them on the request operation itself:
