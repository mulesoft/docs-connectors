...
By default, the token manager uses an in-memory object store to store the credentials. You can customize the token manager object store by using the `objectStore` attribute. See also xref:object-store/object-store-to-define-a-new-os.adoc[Configure a custom object store].
