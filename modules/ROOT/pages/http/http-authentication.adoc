...
. In Connector Configuration, click the green plus icon (image::http/add-16x16.png[]).
+
...
=== Configure an HTTP Request Connector
...
. In the properties editor for *Connector Configuration*, click the green plus icon (image::http/add-16x16.png[]).
+
...
----
+
. In the properties editor for *Connector Configuration*, click the green plus icon (image::http/add-16x16.png[]) (or click the Edit icon if you have already created a configuration).
. Set the *Authentication* field to *Authorization code grant type*.
. Set the following required fields:
+
** *External callback url*: `+http://localhost:8082/callback+`
+
...
=== Send Custom Parameters to the Authorization URL
...
You can customize when Mule performs one of these requests to obtain a new access token using a xref:4.1@mule-runtime::dataweave.adoc[DataWeave expression]. The expression is evaluated against the response of the HTTP Request call.
...
image::http/oauth-danceposta-simple.png[]
...
Once you have a token manager associated with the authorization grant type (in the example below, with authorization code) you can use the operations provided by the OAuth module anywhere in the flow to access information from an OAuth authorization.
...
|Custom parameter extracted from the token URL response for resource owner `Peter` accessible through `vars.customParam` from DataWeav.
|===
...
=== Customize the Token Manager Object Store
