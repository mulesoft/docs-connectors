...
When the response is in JSON format, the parameters are automatically extracted and you can use xref:4.1@mule4-runtime::dataweave.adoc[DataWeave expressions] to reference these values in the response to the request to the token URL, as shown in the previous Github example.
...
You can customize when Mule performs one of these requests to obtain a new access token using a xref:4.1@mule4-runtime::dataweave.adoc[DataWeave expression]. The expression is evaluated against the response of the HTTP Request Connector call.
