...
In this procedure, you configure an HTTP Listener to specify a host name, a port, and a path. To listen on a specific port for every network interface on the server, set the host to `0.0.0.0`. Using `localhost` listens for incoming requests generated inside the server. Start a browser and submit a URL. The result appears in the Anypoint Studio Console.

. In Studio, create a new project from *File* > *New* > *Mule Project*.
. From the Mule Palette, drag an HTTP Listener to a new flow.
. Configure the Listener:
* Click plus (+) and ensure that *Host* is `0.0.0.0` and *Port* is `8081`, with no base path.
