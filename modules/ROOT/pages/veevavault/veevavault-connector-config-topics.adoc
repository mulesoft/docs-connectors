= Veeva Vault Connector Additional Configuration Information - Mule 4
...
image::veevavault/veevavault-connector-basic-configuration.png[]

Follow the instructions in xref:veevavault/veevavault-connector-reference.adoc#veevavault-connector-connection-config[Connector Configuration]
...
|===
|Field |Description
|Metadata Format |Select the CSV or JSON option to accept document metadata in the specified format.
|Document Metadata |Document Metadata payload with required metadata either in CSV or JSON format. Ensure the payload is in the format specified in the Metadata Format field.
|Input a| The following shows the document metadata required to create documents on Vault. Ensure the payload has the mandatory metadata per Vault.
+
...
+
Note: It’s important to check the response status of the Success and Failure of operations in the section link:#connector-operations-success-and-failure-response[Connector Operations SUCCESS and FAILURE Response].
...
Note: The output of the operation is in pagination mechanism. See link:#streaming-and-pagination[Streaming and Pagination] for more details.
...
image::veevavault/veevavault-connector-get-documents.png[image,width=601,height=355]

[%header%autowidth.spread]
|===
|Fields |Description
|Type (Optional) |Click on icon image::veevavault/veevavault-connector-refresh-icon.png[image,width=22,height=22] to retrieve Document Types from vault. Select a document type from the list to retrieve document properties.
|Subtype (Optional) |Click on icon image::veevavault/veevavault-connector-refresh-icon.png[image,width=22,height=22] to retrieve Document Subtypes from vault. Select a document subtype from the list to retrieve document properties.
|Classification (Optional) |Click on icon image::veevavault/veevavault-connector-refresh-icon.png[image,width=22,height=22] to retrieve Document Classification from vault. Select a document classification from the list to retrieve document properties.
...
|Input |N/A
|Output a|
You can find a list of queryable document properties retrieved from Vault. If a document properties list is provided, then the specified properties detail is retrieved and results in operation output. The default is a list of all queryable properties.
+
...
|Output a| You can find delete documents operation output status or response from vault.
+
image::veevavault/veevavault-connector-delete-documents-output.png[image,width=300,height=147]
+
*Note:* It’s important to check the response status of the Success and Failure of operations in link:#connector-operations-success-and-failure-response[Connector Operations SUCCESS and FAILURE Response].
...
Follow the instructions in link::veevavault/veevavault-connector-reference.adoc#veevavault-connector-connection-config[Connector Configuration]

==== Operation Configuration

image::veevavault/veevavault-connector-update-documents.png[image,width=601,height=95]
...
The exported documents are stored on the FTP Staging area in the folder, with the hierarchy structure: `\{root}/\{userId}/\{jobId}/\{documentIds}/\{versions}`. 
...
* `user_id__v` +
Vault system user ID of a user whose credentials are configured in the connector. The folder is created on the Vault FTP server staging area root location and under which the exported file is located. The folder is created with the name of the user ID prefixed with ‘u’, for example: `/uXXXXXXX/{jobId}/{documentId}/{major-minor-version}/{filename}\)`.
* `id` +
...
Follow the instructions mentioned in link:#configuration[]

=== Operation Configuration

image::veevavault/veevavault-connector-export-documents.png[image,width=601,height=221]

[%header%autowidth.spread]
|===
|*Field* |*Description*
|*Metadata Format* |Select the CSV or JSON option to accept metadata for the specified document ID in the specified format.
|*Document Metadata* |Document Metadata payload contains document IDs in either CSV or JSON format. The payload should be in the format specified in Metadata format field.
...
|*Polling Interval* |(Optional) To poll the vault at specified intervals (in seconds) until the job is successful. Default is 30 seconds.
|*Input* a| You can find following input document metadata required to export documents from vault to FTP staging server. Make sure the payload has the mandatory metadata as per the vault.
+
image::veevavault/veevavault-connector-export-documents-input.png[]

|*Output* a|
You can find export documents job status response under the *Output* tab:
+
...
|*Input* a| You can find the following list of document metadata required in payload under the *Input* tab:
+
image::veevavault/veevavault-connector-create-document-renditions-input.png[]

|*Output* a| You can find the following operation output or response under the *Output* tab:
+
...
Follow the instructions in link:#configuration[*Connector Configuration*]

==== Operation Configuration

image::veevavault/veevavault-connector-get-document-renditions-types.png[image,width=601,height=61]
...
image:veevavault/veevavault-connector-get-document-renditions-types-input.png[image,width=293,height=38]

|*Output* a|The following operation output or response is under the *Output* tab:
+
image:veevavault/veevavault-connector-get-document-renditions-types-output.png[image,width=300,height=145]
+
*Note* : It’s important to check response status of Success and Failure of operation in section link:#connector-operations-success-and-failure-response[*Connector Operations SUCCESS and FAILURE Response*].
|===
...
image::veevavault/veevavault-connector-delete-document-renditions.png[image,width=601,height=90]
...
|===
|*Field* |*Description*
|*Object Name* |Click on the VeevaVault refresh icon (image:veevavault/veevavault-connector-refresh-icon.png[VeevaVault refresh icon, 5%, 5%]) to retrieve vault object list. Select the object name from the list to create records for selected object.
|*Metadata Format* |Select the CSV or JSON option to accept object metadata in specified format.
...
|===
|*Fields* |*Description*
|*Vault Object* |Click the VeevaVault refresh icon (image:veevavault/veevavault-connector-refresh-icon.png[VeevaVault refresh icon, 5%, 5%]) to retrieve a vault objects list from vault. Select an object from the list to retrieve object records detail.
...
|*WHERE Clause* |Insert a https://developer.veevavault.com/vql/#introduction-to-vault-queries[VQL] `WHERE` clause (without using the keyword `WHERE`) conditions as per your business requirement and it will be appended to dynamically to build VQL query to execute on vault. For example: `study_country__v = ‘0SC000000000101’`
...
|*Input* |N/A
|*Output* a|The list of object records fields to refer to add into Object fields list. If object fields list is provided, then specified fields detail returns as output. Default would be list of all fields.
...
image:veevavault/veevavault-connector-get-picklists.png[image,width=601,height=60]
...
image:veevavault/veevavault-connector-get-picklists-output.png[image,width=284,height=91]
...
image:veevavault/veevavault-connector-get-audit-details.png[image,width=600,height=179]
...
image:veevavault/veevavault-connector-streaming-pagination.png[image,width=601,height=124]
