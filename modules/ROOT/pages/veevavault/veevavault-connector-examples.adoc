...
== Create Documents on Vault

This example creates documents in bulk with document metadata fetched from an external source (database). Before creating documents on Vault the document files are uploaded using Vault FTP server. The documents' `created` status is inserted into the database. 
...
* Drag the *Flow Reference* element to reference Sub Flow `upload-files-on-vault-ftp-server`.
...
* Configure the Veeva Vault connection configuration with the username, password, Vault URL, Vault version, and Vault client ID.
* Drag the *Database Bulk Insert* operation to insert the document created status data into the database table.
* Drag the *Logger* element to log information in the console or log file.
* Create a Sub-Flow with Logger to log information.
* Drag the *For-Each* scope containing the `Read file` operation and FTPS `Write` operation to write the file to the Vault FTP server.
* Configure the FTPS Connector configuration with the credentials for Vault FTP server.
...
The following example XML code shows the Create Documents operations flow:
...
The following example XML code shows the upload files on Vault FTP server: 
...
* Configure the Veeva Vault connection with the username, password, Vault URL, Vault version, and Vault client ID.
...
* Drag the *For-Each* element after Get Documents operation.
...
== Fetch Document or Object data from Vault using VQL query.
...
== Validating Spark Message triggered from Vault.
