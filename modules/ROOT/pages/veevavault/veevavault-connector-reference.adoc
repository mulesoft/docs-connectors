...
Exchange: https://www.anypoint.mulesoft.com/exchange/org.mule.extension/mule-veevavault-connector/[Veeva Vault Connector]
...
|Client Id |String |The Client ID to send with each request header to the Veeva Vault instance in the format: `\{company}-\{organization}-\{server}-\{component}`; for example: `veeva-clinical-mulesoft-connector` | |X
|===
...
User the Query operation to execute the specified VQL query on the vault and retrieve results in pagination. Pagination is based on Veeva Vault's API.
