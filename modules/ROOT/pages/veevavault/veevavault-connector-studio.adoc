...
Install Anypoint Studio, then add VeevaVault Connector to your Mule project in Studio, or by using Exchange. Anypoint Studio provides two ways to add the connector to your Studio project: 
...
Before you install Studio, install the following: 
...
== Add VeevaVault Connector Using Exchange
...
== Add VeevaVault Connector in Studio
...
|===
+
In the previous image, the placeholder values refer to a configuration file placed in the folder (src/main/resources) of your project. You can either hardcode your credentials into the global configuration properties or reference a configuration file that contains these values. For simpler maintenance and better re-usability of your project, Mule recommends that you use a configuration file. Keeping these values in a separate file is useful if you need to deploy to different environments, such as production, development, and QA, where your access credentials differ. 

The following is an example snippet of `configuration.yaml` file under the `src/main/resources/config` folder:
+
...
----
+
The Connection Configuration performs Basic Authentication, which accepts the Vault URL, Username, and password required parameters to connect to the Veeva Vault instance to retrieve the session. The retrieved session is used for each operation configured in the Mule application for sending requests.


. In the VeevaVault Config window, *Advanced* tab.
+
