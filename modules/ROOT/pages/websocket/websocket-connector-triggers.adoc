...
WebSocket Connector supports the following triggers for inbound (server end) and outbound (client end) WebSockets:
...
* <<on-websocket-close-trigger, On WebSocket Close>>
|Outbound a|
...
== On WebSocket Close Trigger [[on-websocket-close-trigger]]
