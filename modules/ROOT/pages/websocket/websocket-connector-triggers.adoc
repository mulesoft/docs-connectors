...
[[inbound-listener-trigger]]
== Inbound Listener

Use the `<websocket:inbound-listener>` element to expose an inbound WebSocket endpoint on a server. External clients can establish an inbound WebSocket when they connect to this endpoint. Once the WebSocket is established, the flow is triggered each time the client sends a message through the generated WebSocket.
...
[[outbound-listener-trigger]]
== Outbound Listener

Use the `<websocket:outbound-listener>` trigger to invoke an app flow when a message is received through an outbound WebSocket. You can use the associated flow to perform tasks such as sending a return message to the server or processing the information sent by the server.
