...
Use `<websocket:close>` to close an established inbound or outbound WebSocket. This operation is associated with the On Socket Close trigger, as described in xxx.
...
The following example shows how to close a WebSocket that has the socket ID represented by the `#[attributes.socketId] variable`.
...
To specify default groups for an inbound WebSocket, use the <<inbound-listener-trigger,Inbound Listener Trigger>> or the <<open-outbound-socket-operation,Open Outbound Socket>>.
...
The following example subscribes the WebSocket identified by the #[attributes.socketId] variable to the `Science` and `Humanities` groups:
...
The following example unsubscribes the WebSocket identified by the #[attributes.socketId] variable from the `Science` and `Humanities` groups.
