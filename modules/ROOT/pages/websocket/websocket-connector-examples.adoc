...
Let's see an example of how to use the WebSockets connector in a complex use case. In this example, an integration system is built to obtain and broadcast stock quotes through WebSockets. 

This system consists of three apps:

* <<quote_producer,Quote producer>>
* <<quote_aggregator,Quote aggregator>>
* <<quote_client,Quote client>>

[[quote_producer]]
== Quote Producer Application

This application generates continuous random stock quotes like the following example:
...
The type `Quote` is called.

This application needs to generate around 50 of these, using at least five different tickers (CRM, MELI, GOOG, NFLX, AAPL). The prices are random (no need to go get the real ones for this example).

This application also exposes the endpoint `wss://localhost:8080/feed`. This TLS secure endpoint accepts one and only one connection. The first client to connect needs to do so successfully. A WSS message with the text "Sorry, spot taken" is sent to the following clients, after which the connection is dropped.

Each produced `Quote` will be sent to the connected feed client.

[[quote_aggregator]]
== Quote Aggregator Application

This application connects to the feed endpoint in the Quote Producer application and receives all of the Quotes. It splits them by ticker and places them in time-based aggregators, which flush every five seconds.

When the aggregators flush, they output an `Array<Quote>` that needs to be reduced to one single `QuoteSnapshot` type. `QuoteSnapshot` looks like the following:
...
The produced snapshots are broadcasted to a dynamic list of subscribers. Subscriptions are done through the `wss://localhost:8082/quotes` endpoint that this application exposes. Clients can connect to this endpoint, using a query parameter to indicate which stock quotes they want to follow. For example: `wss://localhost:8082/quotes?ticker=CRM&ticker=MELI`

The above request should establish a WebSocket which gets the snapshots for the `CRM` and `MELI` tickers. This should be done by subscribing the resulting sockets to the proper socket groups.

[[quote_client]]
== Quote Client Application

Finally, the client application should open at least three different WebSockets to the quotes endpoint in the aggregator application. Each of those sockets should be listening for a different set of tickers.
...
* xref:websocket/websocket-connector-configuration.adoc[WebSocket Configuration]
* xref:websocket/websocket-connector-triggers.adoc[WebSocket Triggers]
* xref:websocket/websockets-connector-operations.adoc[WebSocket Operations]
