...
You can use the WebSocket connector in a complex use case that builds an integration system that obtains and broadcasts stock quotes through WebSocket. 
...
The request should establish a web socket that gets the snapshots for the `CRM` and `MELI` tickers. This should be done by subscribing the resulting sockets to the proper socket groups.

== Quote Client App

Finally, the client app should open at least three different web sockets to the quotes endpoint in the aggregator app. Each of those sockets should be listening for a different set of tickers.
...
----    
    ...
    <flow name="producer-app-websocket-close-flow">
        ...
        <choice>
            ...
            <otherwise >
                <logger level="INFO" message="There is no web socket to close"/>
            </otherwise>
        </choice>
    </flow>
...
* xref:websockets/websockets-connector-config-topics.adoc[WebSockets Configuration Topics]
* xref:websockets/websockets-connector-client-components.adoc[WebSockets Client Components]
* xref:websockets/websockets-connector-server-components.adoc[WebSockets Server Components]
