...
Anypoint Connector for WebSockets (WebSocket Connector) enables you to create Mule apps that...<this will be filled in when I get this information from Esteban. I might make this its own section.>>
...
A chat application sends messages to multiple clients in a group. If a Mule app uses HTTP to send messages, the clients must constantly poll the server for available response messages, which can create performance issues. If a Mule app uses WebSockets, the server can push the available messages to all subscribed users without the need for client polling.
...
To see the code for a stock quotes app that uses WebSockets Connector, see xxx.
...
Before creating a Mule app, you must have access to the WebSocket target resource and Anypoint Platform. You must also understand how to create a Mule app using Anypoint Studio or another XML editor.
...
All WebSockets, both inbound and outbound, have a unique socket ID that identifies them. The Mule app must reference these IDs to operate on individual WebSockets.

=== Groups [[groups]]

WebSocket Connector introduces the concept of groups, which are string identifiers to which individual WebSockets can subscribe. An app can use groups to broadcast one message to a group of different WebSockets. Groups facilitate the process of identifying which WebSockets should participate in each broadcast.

Subscriptions only exist in the context of each instance of a WebSocket Connector configuration. Therefore, when you establish a web socket between a Mule app and a remote system, subscribing the WebSocket on Mule's end of the connection to a group doesn't affect the WebSockets on the remote system's end.
...
+
The app can invoke this operation on the same WebSocket multiple times. . Repeated groups are ignored.
+
...
WebSockets Connector supports securing the client and server endpoints through TLS using the WebSocket (WS) or WebSocket Secure (WSS) protocols:

* The WS protocol provides connection security, but does does not provide data encryption.
+
To use WS, preface the WebSocket server name and port with `ws`, for example: `ws://localhost:80`.
+
* The WSS protocol provides both connection security and data encryption.
+
To use WSS, preface the WebSocket server name and port with `wss`, for example: `wss://localhost:433`.

For information about configuring TLS for the WebSocket connector, see <<websocket/websocket-connector-configuration#tls-configuration,TLS Configuration.
