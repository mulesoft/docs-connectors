...
* `inbound-listenerPort` exposes an endpoint to which external systems can connect and initiates the associated app flow when the endpoint receives a message.
...
`ws://localhost:80`
+
* WSS identifies a WebSocket server and resource name in a URI. In addition, it uses SSL for authentication and TLS on inbound and outbound endpoints to encrypt the communication. To use WSS, specify `wss` in the URI:
+
`wss://localhost:433`.
