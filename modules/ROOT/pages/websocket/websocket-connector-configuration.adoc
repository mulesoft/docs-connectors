...
The following example shows a sample WebSocket client configuration called `wsClient` that builds a URI from the host, port, and basePath values. This example also initializes an outbound socket that references the WebSocket client configuration (`wsClient`) in the config-ref attribute:
...
+
* For the WS protocol, the default is `80`.
* For WSS, the default is `443`.
+
...
You can specify default headers and query parameters that are included in the connection request message. For example:
...
You can also add headers and query parameters to your connect operation to have them sent along with the default headers and parameters in the request to establish the connection. For example:
...
By default, outbound sockets have no idle timeout although the remote service to which they connect probably has one. Either way, it's a good practice to set a timeout in the connector to automatically close idle outbound connections.
