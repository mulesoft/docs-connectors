...
When you configure HTTP Listener, you can specify base connection settings such as settings for the host, port, and base path. Alternatively, you can configure a TLS-based connection (WSS), as described in <xxx>.
...
In the following example, the WebSocket server inherits TLS configuration from the HTTP Listener. The HTTP Listener references a <tls:context> to configure a key-store.
...
To set up TLS on a WebSocket Client, set up a Truststore by directly referencing the <tls:context> in the tlsConfig property.
