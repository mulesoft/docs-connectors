...
If your app needs to connect to external WebSocket services, use the `websocket:client-settings` element to configure the outbound endpoint. You can either specify a URI or you can build a URI by specifying a host, port, and basePath. The second option is only supported when you specify `WS` as the protocol scheme.
...
* WS protocol: `80`
* WSS protocol: `443`
* What is the default port when not using TLS?
...
The following example shows a sample WebSocket client configuration called `wsClient` that:
