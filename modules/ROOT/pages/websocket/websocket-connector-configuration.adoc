...
If your app needs to connect to external WebSocket services, use the `websocket:client-settings` element to configure the outbound endpoint. You can either specify a URI, or you can one by specifying a host, port, and basePath. The second option is only supported when you specify `WS` as the protocol scheme.
...
* WS protocol : `80`
* WSS protocol: `443`
* Protocol when not using TLS: `80`
...
== Combined WebSocket Client/Server Configuration [[combined-configuration]]
...
**Reviewers: I don't see the path '/chat' in the example**
...
**Reviewers: Are these default TLS settings?**
