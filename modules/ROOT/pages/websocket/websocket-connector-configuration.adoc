...
* <<combined-configuration>,Configure a WebSocket server and client>> that listen for incoming connection requests and request external services.
...
When you configure HTTP Listener, you can specify base connection settings such as settings for the host, port, and base path. Alternatively, you can configure a TLS-based connection (WSS), as described in <<tls-configuration,TLS Configuration>>.
...
Because the HTTP Listener does not reference a TLS Configuration component, the connection uses the Websocket (WS) protocol by default.
...
If your app needs to connect to external WebSocket services, configure the `websocket:client-settings` element. You can either specify a URI or you can build a URI by specifying a host, port, and basePath. The second option is only supported when you specify `WS` as the protocol scheme.

=== Sample Client Configuration that Builds a URI

The following example shows a sample WebSocket client configuration called `wsClient` that builds a URI from the host, port, and basePath attribute values. This example also initializes an outbound socket that references the WebSocket client configuration (`wsClient`) in the config-ref attribute:
...
Use the following parameters to build a URI:

[%header,cols=“33s,33a,33a”]
|===
|Parameter | Description | Default Value
|host | Host name for the external service| `localhost`
|port | Port for the external service| Depends on the protocol scheme:
...
////
Can the value for host be an IP address?
Where is the protocol specified in the example text coming from?
Should we also show the <flow> and <logger> components for the second example? Are the examples in the correct order?
////

== Sample Client Configuration that Specifies a URI

The following example shows a sample WebSocket client configuration called `WsClient` that specifies the URI `ws://www.mulesoft.com`. This example also initializes an outbound socket that references the WebSocket client configuration(`wsClient`) in the config-ref attribute::
...
This example specifies default headers and query parameters in a connection request (`<websocket:connection>`):
...
This example specifies default headers and query parameters in the `websocket:open-outbound-socket` operation:
...
TLS (Transport Layer Security) enables client/server applications to communicate through a secure channel and prevents attackers from eavesdropping or tampering with the transmitted data.

TLS configuration for the WebSocket server is inherited from the referenced HTTP Listener configuration. To configure TLS on a WebSocket server:
. Set up your Truststore.
. Reference an HTTP Listener that has TLS configured.

In the following example, the WebSocket server inherits the TLS configuration from the HTTP Listener named `listenerTlsConfig`. The HTTP Listener references a TLS context configuration named `listnerTlsContext`, which configures the keystore:
...
For more information on configuring TLS for a Mule app, see xref:4.1@mule-runtime::tls-configuration.adoc[Configure TLS with Keystores and Truststores].
