...
Request and Reply is a very common pattern between applications, and AMQP Connector provides an operation specially designed for that, called `publish-consume`. The `publish-consume` operation allows you to publish a message to any exchange, and then wait for a reply on a different destination.
