...
You can set the `createFallbackExchange` global config to prevent changes to the AMQP topography resulting from the definition of fallback exchanges, see xref:amqp/amqp-topography.adoc[How to Avoid Changing the AMQP Topography].
...
xref:amqp/amqp-publish-consume.adoc[To Listen For A Reply]