...
The Publish operation allows you to create a new AMQP message and send it to the specified exchange. You can then configure not only the content of the message, but also all the headers, the properties and the envelope in the AMQP message.
...
You can set the `createFallbackExchange` global config to prevent changes to the AMQP topography resulting from the definition of fallback exchanges, see xref:amqp/amqp-topography.adoc[Avoid Changing the AMQP Topography].
