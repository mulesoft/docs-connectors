...
The `consume` operation in AMQP Connector provides the ability to consume a message at any given time of the flow, from any given AMQP queue.
...
This operation consumes the first available message in the queue identified by the `queueName`, and converts it to an `AmqpMessage`, which results in the following structure:

* The message's content as payload
* The message's metadata in the message attributes

Once received, the message is immediately ACKed by default. If you want to control the ACK of the message after some processing, then set the `ackMode` to `MANUAL`.
For more information about message ACKs, see xref:amqp/amqp-ack.adoc[How to handle Acknowledgment].
...
To wait indefinitely for a Message to arrive, the `maximumWait` value must be set to `-1`. In this case, no `TIMEOUT` error will be thrown.

== MIME Types and Encoding

AMQP Connector does its best to auto determine a message’s MIME type (`contentType`) based on the `contentType` property of the message. However, there are cases in which that best guess is not enough, and you need firsthand knowledge of the message’s content.

In such cases, you can force the content type to a particular value by using the `contentType` parameter.

The same process works for encoding. By default, the connector assumes that the Mule's default encoding matches the one in the message if no other information is provided. You can set this by using the `encoding` parameter.
