...
In this case, every `consume` operation or `listener` associated to this config uses an acknowledge mode of the type `IMMEDIATE`, while every time we do a `publish` or `publish-consume`, the outgoing Message requests a confirm from the broker.
