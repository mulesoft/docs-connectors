...
Anypoint Connector for AMQP (AMQP Connector) v1.5
...
Out of the box, the AMQP connector comes with a fine tuned set of default values, both for publishing and consuming messages. This means that the only requirement is for you to configure which connection to use. 
...
Mule converts `RELEASE` to the current version. To specify a version, view 
https://www.mulesoft.com/exchange/com.mulesoft.connectors/mule-amqp-connector/[AMQP Connector] in
Anypoint Exchange and click *Dependency Snippets*.

=== Define Global Defaults

The AMQP `config` allows you to define multiple parameters to use by default while consuming or publishing messages. This way, you can define a global default behavior for all the operations associated with the config and then override each parameter only in the operations that require a custom behavior.
...
In the above example, every `consume` operation or `listener` associated to this config uses an acknowledge mode of the type `IMMEDIATE`, while every time we do a `publish` or `publish-consume`, the outgoing message requests a confirm from the broker.
