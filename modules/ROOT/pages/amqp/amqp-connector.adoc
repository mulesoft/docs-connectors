...
Using the parameter `useSNI` you can indicate that the amqp client should add the hostname as an extension in the ClientHello message during the SSL handshake. In case the broker supports SNI, it will be available to verify the hostname and present different certificates for the same ip address.
