= NetSuite Connector Reference - Mule 4
...
| Customizations Separator a| String |   Separator used to generate the keys required to provided a better support for custom fields and records. If the script ID of the customization that defines a custom field or record contains the separator default character, set up a new one and regenerate the metadata.  |  __ |
|===
...
`https://abcd123456.suitetalk.api.netsuite.com/services/tstNetSuitePort_2019_1` +
If the call to the specified endpoint does not complete, the connector uses the remote URL, which it obtains automatically by using the getDataCenterUrl operation. *(Do we have to say what operation is used to obtain the remote URL, or is that irrelevant?)*
|  |
...
| Token Secret a| String |  The respective token secret for the user and integration pair. |  | x
| Account a| String |  NetSuite SuiteTalk WebService account ID. |  | x
...
* All available jobs for each polling period are processed contiguously.
* There is no enforced waiting period for a job that is available. Once a job is initiated,
a job ID is returned in the Web services response. Your client application can then check
the status and result of the request by referencing the job ID.
* Asynchronous request JobIDs are valid for 30 days.
====
...
An asynchronous request equivalent to `RecordOperations#search(NetSuiteSoapConfig, String, SearchRecord, boolean, boolean, int)`. Searches for all records that match the given criteria, asynchronously. In asynchronous requests, your client application sends a request to the SuiteTalk Platform where it is placed in a processing queue and handled asynchronously with other requests.

[NOTE]
====
Notes:

* Due to the nature of the operation, this operation does not support paging. This means that only the first page of results would be returned. If no criteria is specified, all records of the given type are retrieved.
* All available jobs for each polling period are processed contiguously.
* There is no enforced waiting period for a job that is available. Once a job is initiated,
a job ID is returned in the Web services response. Your client application can then check
the status and result of the request by referencing the job ID.
...
Not all record types are supported as source, destination or contact.

For more information, see the NetSuite documentation.
...
Returns a list of deleted records for a given record type and date period.
...
Retrieves a list of all records of a specified type. Records that support the getAll operation are listed in the GetAllRecordType.
...
Retrieves a list of existing saved searches for a record type.
...
Retrieves valid values for a <<RecordRef>> field where the referenced record type is not yet exposed in the web services API, or when the logged in role does not have access to the instances of the record type.
...
Automates the mapping between external applications credentials and NetSuite credentials for a user. This operation provides a mapping, not a login to NetSuite.

This operation does NOT provide the ability to log in using a single sign-on token, cannot provision a partner with the inbound single sign-on feature, or aid with public and private key exchange.
...
| Name | Type | Description | Default Value | Required
| Configuration | String | The name of the configuration to use. | | x
| Credentials a| <<SsoCredentials>> |  The SSO credentials to map. |  `#[payload]` |
| Target Variable a| String |  The name of a variable to store the operation's output. |  |
...
Sets a new invitation status for an event.
...
Sets the status of one or more invitations for an event.
...
NOTE: This call cannot be used with data mapping or DataSense, it was added to allow users to add a batch of records of any type in one call.
...
Adds or updates one or more instances of a record type in NetSuite. The upsertList operation is similar to both the addList and updateList operations, but upsert can be run without first determining whether records exist in NetSuite.
...
Because an external ID is mandatory for this operation, *Upsert List* is supported only for records that support the external ID field. Also, this operation prohibits the passing of internal ID values.
...
Adds a new instance or updates an instance of a record in NetSuite. The upsert operation is similar to both the add and update operations, but upsert can be run without first determining whether a record exists in NetSuite.
...
|===
| Field | Type | Description | Default Value | Required
| Max Active a| Number | Controls the maximum number of Mule components that can be borrowed from a session at one time. When set to a negative value, there is no limit to the number of components that may be active at one time. When *Max Active* is exceeded, the pool is said to be exhausted. |  |
...
* WHEN_EXHAUSTED_GROW - Creates a new Mule instance and returns it, essentially making *Max Active* meaningless. If a positive *Max Wait* value is supplied, this action blocks for at most that many milliseconds, after which a NoSuchElementException is thrown. If maxThreadWait is a negative value, it will block indefinitely. |  |
