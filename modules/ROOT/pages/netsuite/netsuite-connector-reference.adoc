...
* There is no enforced waiting period for a job that is available. 
* Once a job is initiated, a job ID is returned in the Web services response. 
* Your client application can then check on the status and result of the request by referencing the job ID. 
