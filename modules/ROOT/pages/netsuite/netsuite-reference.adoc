...
NetSuite Connector synchronizes data and automates business processes between NetSuite and third party applications, either on-premise or in the cloud. The connector makes use of SuiteTalk WSDL to provide SOAP-based integration to generate NetSuite business objects, make use of different authentication levels, and support error handling.
...
| Send runServerSuiteScript tag in Preferences header a| Boolean |  |  true | 
| Customizations Separator a| String |   Separator used to generate the keys required to provided a better support for custom fields/records. If the script ID of the customization that defines a custom field or record contains the separator default character, set up a new one and regenerate the metadata.  |  `__` | 
|===
...
|===
| Name | Type | Description | Default Value | Required
| Read Timeout a| Number |  Specifies the amount of time, in seconds, that the client waits for a response before it times out. A value of `0` specifies that the client waits indefinitely. |  10000 | 
| Connection Timeout a| Number |  Specifies the amount of time, in seconds, that the client will attempt to establish a connection before it times out. A value of `0` specifies that the client will continue to attempt to open a connection indefinitely. |  10000 | 
...
| Role ID a| String |  The ID of the role used to log into SuiteTalk, which determines the processor privileges |  | x
...
| Username a| String |  The username which should be supplied to the HTTP proxy on every request to NetSuite. This field is optional, since a user might want to pass through an unauthenticated HTTP proxy. |  | 
| Password a| String |  The password which would be supplied to the HTTP proxy on every request to NetSuite. This field is optional, since a user might want to pass through an unauthenticated HTTP proxy. |  | 
| E-mail a| String |  The login email of both the NetSuite UI and SuiteTalk |  | x
| Password a| String |  The login password of both the NetSuite UI and SuiteTalk |  | x
| Account a| String |  NetSuite SuiteTalk WebService account ID |  | x
| Role ID a| String |  The ID of the role used to log into SuiteTalk, which determines the processor privileges |  | x
| Application ID a| String |  The ID related to the Integration record to use. Integration records are set up on the NetSuite environment. |  | x
...
| Content a| Any |  The content of the file record to add, which can be of type String, Byte Array, File or InputStream. |  `#[payload]` | 
...
All available jobs for each polling period are processed contiguously. There is no enforced waiting period for a job that is available. Once a job is initiated, a job ID is returned in the web services response. Your client application can then check job status and the result of the request by referencing the job ID. 
...
An asynchronous request equivalent to `RecordOperations#deleteList(NetSuiteSoapConfig, NetSuiteSoapConnection, List)`. In asynchronous requests, your client application sends a request to the SuiteTalk Platform where it is placed in a processing queue and handled asynchronously with other requests. 
...
An asynchronous request equivalent to `RecordOperations#deleteRecordsList(NetSuiteSoapConfig, NetSuiteSoapConnection, List)`. In asynchronous requests, your client application sends a request to the SuiteTalk Platform where it is placed in a processing queue and handled asynchronously with other requests. 
...
An asynchronous request equivalent to `RecordOperations#getList(NetSuiteSoapConfig, NetSuiteSoapConnection, List)`. In asynchronous requests, your client application sends a request to the SuiteTalk Platform where it is placed in a processing queue and handled asynchronously with other requests. 
...
An asynchronous request equivalent to `RecordOperations#initializeList(NetSuiteSoapConfig, NetSuiteSoapConnection, List)`. In asynchronous requests, your client application sends a request to the SuiteTalk Platform where it is placed in a processing queue and handled asynchronously with other requests. 
...
This operation does not support paging, meaning that only the first page of results can be returned. If no criteria is specified, all records of a given type are retrieved. In asynchronous requests, your client application sends a request to the SuiteTalk Platform where it is placed in a processing queue and handled asynchronously with other requests. 
...
An asynchronous request equivalent to `RecordOperations#updateList(NetSuiteSoapConfig, NetSuiteSoapConnection, String, List)`. In asynchronous requests, your client application sends a request to the SuiteTalk platform where it is placed in a processing queue and handled asynchronously with other requests. 
...
| Configuration | String | The name of the configuration to use. | | x
| Record Type a| String |  The target record type. |  | x
| Record Attributes a| Array of Object |  A list of `Map<String,Object>` containing the attributes of the records to update. |  `#[payload]` | 
| Target Variable a| String |  The name of a variable to store the operation's output. |  | 
| Target Value a| String |  An expression to evaluate against the operation's output and store the expression outcome in the target variable |  `#[payload]` | 
| Reconnection Strategy a| * <<reconnect>>
* <<reconnect-forever>> |  A retry strategy in case of connectivity errors |  | 
...
An asynchronous request equivalent to `RecordOperations#upsertList(NetSuiteSoapConfig, NetSuiteSoapConnection, String, List)`. In asynchronous requests, your client application sends a request to the SuiteTalk Platform where it is placed in a processing queue and handled asynchronously with other requests. 
...
Attaches a source or contact record, that is, the attachment, to another destination one. Not all record types are supported as source, destination, or contact. See the NetSuite documentation.
...
Deletes a record. Not all records can be deleted. See the NetSuite documentation.
...
Retrieves a record by providing the unique ID that identifies a record.
...
Gets and filters all data related to the budget exchange rates table. This table maintain exchange rates between the root-parent and child subsidiaries for use in the budgeting process.
...
Retrieves a custom record by providing the unique ID that identifies a record.
...
Obtains the NetSuite data center URL for an account.
...
Retrieves a summary of the actual data in an account. This operation allows you to retrieve a summary of the actual data posted to the general ledger in an account. You can use available filters or fields to generate reports that are similar to what you see when you run financial reports such as for a trial balance, balance sheet, or an income statement.
...
Retrieves valid values for a `recordRef` field where the referenced record type is not yet exposed in the web services API, or when the logged into a role that does not have access to instances of the record type.
...
Populates fields on transaction line items with values from a related record, in a similar way that empty text boxes are pre-populated within the NetSuite UI.
...
Emulates the UI workflow by pre-populating fields in transaction line items with values from a related record.
...
Automates the mapping between external applications credentials and NetSuites credentials for a user. This operation provides a mapping, not a login to NetSuite. 

NOTE: This operation does not provide the ability to log in using a single sign-on token, cannot provision a partner with the inbound single sign-on feature, or aid with public or private key exchange. 
...
Searches for a specific record type based on a set of criteria. This processor has been enhanced with a paging mechanism. Searches can be Basic, Advanced, or Join. See <<SearchRecord>> for all available search options.
...
Sets multiple new invitation status values for an event.
...
Updates one or more existing records in the system by providing new values for the fields to be updated for each record. The records to be updated are identified through the provided unique identifiers.
...
Updates an existing record. The attributes can either be the POJOs corresponding to the field or a map that represents a field.
...
NOTE: This call cannot be used with DataWeave or DataSense.

This operation allows users to add a batch of records of any type in a single call.
...
Adds or updates one or more instances of a record type in NetSuite. This operation is similar to both the addList and updateList operations, but upsert can be run without first determining whether records exist in NetSuite. 

Records are identified by their external ID and their record type. If a record of the specified type with a matching external ID exists in the system, it is updated. If it does not exist, a new record is created. 
...
Adds a new instance or to update an instance of a record in NetSuite. The upsert operation is similar to both the add and update operations, but upsert can be run without first determining whether a record exists in NetSuite. 

A record is identified by its external ID and its record type. If a record of the specified type with a matching external ID exists in the system, it is updated. If it does not exist, a new record is created. 
...
| Name | Type | Description | Default Value | Required
| Configuration | String | The name of the configuration to use. | | x
| Page Size a| Number |  |  1 | 
| Since a| String |  Specify a date in the `YYYY-MM-DD hh:mm:ss` format, such as `2019-03-17 16:30:40`. If this field is empty, this operation retrieves the selected objects from the time the Mule app is started. |  | 
...
* WHEN_EXHAUSTED_GROW - Creates a new Mule instance and returns it, essentially making maxActive meaningless. If a positive maxWait value is supplied, it will block for at most that many milliseconds, after which a NoSuchElementException will be thrown. If maxThreadWait is a negative value, it will block indefinitely. |  | 
