...
This example requires that at least one employee in the NetSuite instance meets the search conditions.
...
This examples requires that at least one customer meets the search conditions in the NetSuite instance.
...
The For-Each scope processes each record on the list returned by the Search operation individually so that the records can be displayed on the console.
