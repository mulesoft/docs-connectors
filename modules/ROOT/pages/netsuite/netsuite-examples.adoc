...
////
Is "items" the correct terminology in the above paragraph or should we say "records"?
////
...
. Start the Mule app.
. Call `+http://localhost:8081/account?internalId=5+` to retrieve the Account Basic items with internal IDs that are less than 5.
...
//Same question as before about the term "items"
...
. Start the mule app.
. Call `+http://localhost:8081/employee?isInactive=false&lastName=A+` to retrieve the inactive Employee Basic items that have last names beginning with `A`.
...
// Same question about the term "items"
