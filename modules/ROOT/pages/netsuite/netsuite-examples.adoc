...
This example shows how to use the NetSuite Search operation to obtain the Account Basic items with internal IDs that are less than the value of the `query` parameter.
...
Creating this example involves creating a new Mule project and configuring HTTP Listener, the NetSuite Search operation, and two Transform Message components.
...
. Accept the defaults.
. In the Listener properties window, set the *Path* field value to `/account`.

=== Add the NetSuite Search Operation
...
Paste this code into Studio to quickly load the flow for the Account Basic Search example into your Mule app. If needed, change the values to reflect your environment.
...
This example shows how to use the `Search` operation to obtain the inactive Employee Basic items that have last names beginning with a specified letter.
...
. Select an existing global element, or create a new one for *HTTP Listener* and keep the defaults.
...
Paste this code into your XML editor to quickly load the flow for the Employee Basic Search example into your Mule app. If needed, change the values to reflect your environment.
...
This example shows how to use the `Search` operation to obtain the Customer items that were created after 2015 and that are in the "LEAD" stage.
...
Creating this example involves creating a new Mule project and configuring HTTP Listener, the NetSuite Search operation, three Transform Message components, and a For Each
...
The For Each component processes each item on the list returned by the Search operation individually so that the items can be displayed on the console.
...
Paste this code into your XML editor to quickly load the flow for the Customer Advanced Search Flow example into your Mule app. If needed, change the values to reflect your environment.
