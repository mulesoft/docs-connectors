...
. You need to set the *Servers* property which is composed of a `host:port` comma-separated list (in case you have a replica set). This attribute is optional, and if left empty, it defaults to `localhost:27017`.
...
. In the *Advanced* tab, you may adjust the values for the *Connect Timeout*, the *Socket Timeout* for requests that are sent to MongoDB and *Pooling Settings*.  The default value for *Connect Timeout* is 30 seconds. Any connection requests that take longer than 30 seconds to be sent and received  throw an exception. You can override this behavior by setting their values to 0, meaning that the MongoDB connector waits indefinitely until a request is successfully sent and a response is received.
...
MongoDB Connector v6.0.0 supports the use of SSL. To set up the Global Configuration:

* In your MongoDB connection, select the `TLS context` property field in the *Security* tab.
...
You can use the Object Listener operation for the connector in the *Source* area of the Studio canvas.

This operation invoke your application when MongoDB has a new object in a certain collection.
