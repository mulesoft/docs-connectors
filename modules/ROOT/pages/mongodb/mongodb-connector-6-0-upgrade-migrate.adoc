...
** Maximum staleness seconds: Specifies how stale a secondary can be before the client stops using it for read operations.
+
...
+
Set this field to 0 for no compression or from 1 through 9 for compression, with 1 being the lowest compression level and 9 the highest. High compression levels require more processing time than lower compression levels.
