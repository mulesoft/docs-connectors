...
| Insert document | Return type is now a entire JSON object with the `_id` object of the created document.
...
| Create File
| N/A
a| A GridFS JSON object with the following attributes:
...
|Find one file | Returns the first file that matches the given query | Find files | Specifying a specific file to query
|===
