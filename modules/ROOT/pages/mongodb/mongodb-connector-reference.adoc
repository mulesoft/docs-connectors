...
|===
| Name | Type | Description | Default Value | Required
| Servers (host:port) a| String a|  A list of MongoDB instances, with the format `host:port`, separated by commas. `Example: 127.0.0.1:27017, 192.168.1.2:27017` |  +++localhost:27017+++ | 
...
Creates a new MuleGridFSFile in the database, saving the given content, filename, contentType, and extraData, and answers. 
...
| Name | Type | Description | Default Value | Required
| Configuration | String | The name of the configuration to use. | | x
| File Id a| <<MuleBsonObjectId>> |  +++The MuleBsonObjectId of the file to be deleted+++ a|  `#[payload]` | 
| Streaming Strategy a| * <<repeatable-in-memory-stream>>
* <<repeatable-file-store-stream>>
* non-repeatable-stream |  +++Configure to use repeatable streams.+++ |  | 
| Target Variable a| String |  +++The name of a variable to store the operation's output.+++ |  | 
...
| Document a| Binary |  +++A String instance.+++ a|  `#[payload]` | 
| Target Variable a| String |  +++The name of a variable to store the operation's output.+++ |  | 
| Target Value a| String |  +++An expression to evaluate against the operation's output and store the expression outcome in the target variable+++ a|  `#[payload]` | 
| Reconnection Strategy a| * <<reconnect>>
* <<reconnect-forever>> |  +++A retry strategy in case of connectivity errors.+++ |  | 
...
Transforms a collection into a collection of aggregated groups, by applying a supplied element-mapping function 
to each element, that transforms each one into a key-value pair, grouping the resulting pairs by key, and 
finally reducing values in each group applying a supplying `reduce` function. 

Each supplied function is coded in JavaScript. Note that the correct way of writing those functions may not 
be obvious; consult MongoDB documentation to write them.
...
+++
Removes all the documents that match a given optional query. If query is not specified, all documents are removed. However, notice that this is normally less performant that dropping the collection and creating it and its indices again.
+++
...
| Find Query a| Binary |  +++The optional String query object. Documents that match it will be removed.+++ a|  `#[payload]` | 
| Reconnection Strategy a| * <<reconnect>>
* <<reconnect-forever>> |  +++A retry strategy in case of connectivity errors.+++ |  | 
|===
...
| Name | Type | Description | Default Value | Required
| Configuration | String | The name of the configuration to use. | | x
| File Id a| <<MuleBsonObjectId>> a|  The `MuleBsonObjectId` of the file to delete. a|  `#[payload]` | 
| Reconnection Strategy a| * <<reconnect>>
* <<reconnect-forever>> |  +++A retry strategy in case of connectivity errors.+++ |  | 
|===
...
+++
Takes the output from the dump and restores it. Indexes are created on a restore. It only does inserts with the data to restore, and if existing data is there, it is not replaced.
+++
...
| Name | Type | Description | Default Value | Required
| Configuration | String | The name of the configuration to use. | | x
| Collection Name a| String |  +++The collection in which to insert the object.+++ |  | x
| Document a| Binary |  +++The mandatory String document to insert.+++ a|  `#[payload]` | 
| Reconnection Strategy a| * <<reconnect>>
* <<reconnect-forever>> |  +++A retry strategy in case of connectivity errors.+++ |  | 
|===
...
+++
Updates documents that match the given query. If query is not specified, all documents are retrieved. If parameter `multi` is set to false, only the first document matching it is updated. Otherwise, all the documents matching it are updated.
+++
...
| Field Name a| String a|  The field that will queried to detect the objects that match the trigger condition. For New Objects trigger, if it is empty the `_id` field will be used. |  | 
| Since a| String a|  Specify an initial value from where to start querying objects. If this field is empty, this operation retrieves the selected objects from when the Mule app started. For most object types, the `Object#toString()` representation is sufficient. For Date objects, use `Date#getTime()`. |  | 
...
| Name | Type | Description | Default Value | Required
| Configuration | String | The name of the configuration to use. | | x
| Collection Name a| String |  +++The collection that is watched to detect the matching documents.+++ |  | x
| Field Name a| String a|  The field that is queried to detect the objects that match the trigger condition. For New Objects trigger, if it is empty the `_id` field will be used. |  | 
| Since a| String a| Specify an initial value from where to start querying objects. If this field is empty, this operation retrieves the selected objects from when the Mule app started. For most object types, the `Object#toString()` representation is sufficient. For Date objects, use `Date#getTime()`. |  | 
| Primary Node Only a| Boolean |  +++Whether this source should only be executed on the primary node when running in Cluster+++ |  | 
| Scheduling Strategy a| scheduling-strategy |  +++Configures the scheduler that triggers the polling+++ |  | x
| Streaming Strategy a| * <<repeatable-in-memory-stream>>
...
| Configuration | String | The name of the configuration to use. | | x
| Collection Name a| String |  +++The collection that is watched to detect the matching documents.+++ |  | x
| Field Name a| String a|  The field that is queried to detect the objects that match the trigger condition. For New Objects trigger, if it is empty, the `_id` field is used. |  | 
| Since a| String a|  Specify an initial value from where to start querying objects. If this field is empty, this operation retrieves the selected objects from when the Mule app started. For most object types, the `Object#toString()` representation is sufficient. For Date objects use `Date#getTime()`. |  | 
| Primary Node Only a| Boolean |  +++Whether this source should only be executed on the primary node when running in Cluster+++ |  | 
| Scheduling Strategy a| scheduling-strategy |  +++Configures the scheduler that triggers the polling+++ |  | x
| Streaming Strategy a| * <<repeatable-in-memory-stream>>
...
|===
| Field | Type | Description | Default Value | Required
| Path a| String | The location of the trust store. If possible, this is resolved relative to the current classpath and file system. |  | 
...
|===
| Field | Type | Description | Default Value | Required
| Path a| String | The location of the key store. If possible, this is resolved relative to the current classpath and file system. |  | 
| Type a| String | The type of store used. |  | 
