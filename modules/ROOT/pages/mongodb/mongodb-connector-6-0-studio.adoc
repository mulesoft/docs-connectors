...
+
The default value for *Connect Timeout* is 30000 milliseconds (30 seconds). Any connection requests that take longer than 30 seconds to send and receive throw an exception. If you set the *Connection Timeout* field to 0, MongoDB Connector waits indefinitely until a request is successfully sent and a response successfully received.
+
