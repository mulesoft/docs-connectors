...
* *Database*
+
Database to send and retrieve data from the MongoDB instance
...
However, you can hard code your connection credentials in the Global Element Properties if you are in the development stage and want to speed up the development process.

=== Global Configuration with SSL (Optional)

MongoDB Connector v6.0.0 and later supports the use of SSL, if your instance supports it. To set up the global configuration for SSL, select the `TLS context` property in the *Security* tab* in your MongoDB connection.
