= Invoke Methods With the Java Module - Mule 4
...
[IMPORTANT]
====
The classes used with the Java Module must be exported unless they belong 
to the default package. If not, execution fails with a JAVA:CLASS_NOT_FOUND error. See xref:4.2@mule-runtime::how-to-export-resources.adoc[how to export resources].
====
...
In Studio, the Java Module supports DataSense for the `invoke` operation, which provides metadata
...
This example sets up two flows in a Mule app:
...
+
Optional: You can also set the *Scheduler* frequency to a value other
