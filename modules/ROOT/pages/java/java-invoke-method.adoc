= Invoke Methods Using the Java Module
...
xref:4.2@mule-runtime::target-variables.adoc[target variable].
...
. Use the xref:java/java-create-instance.adoc[`new`
operation] from the Java Module to create an object on which a method is later invoked. This provides an instanced object that can then call one of its methods. 
...
xref:4.2@mule-runtime::target-variables[target parameters].

In the next example, an instance of the class `TaxCalculator` is created and placed
into the target variable `taxCalculator`. Then the `calculateTax(Double, Integer)` method is
...
xref:4.2@mule-runtime::choice-router-concept.adoc[Choice] component:
...
. Create a Java package for your classes by right clicking your Mule project's `src/main/java` directory in
*Package Explorer*.
. Select *New* > *Package*.
