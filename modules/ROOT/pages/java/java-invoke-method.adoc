...
. Use the xref:java/java-create-instance.adoc[new operation] from the Java Module to create an object on which a method is later invoked. This provides an instanced object that can then call one of its methods. 
...
`calculateTax` returns a `Number`, so the output metadata for the `invoke` operation looks like:
