...
https://docs.mulesoft.com/mule-runtime/4.2/target-variables[target variable].

== Invoke Instance Methods

Given the following Java class `TaxCalculator` that belongs to the `com.foo` package:
...
In order to invoke instance methods:

. First an instanced object is needed to then call one of its methods. The xref:java/java-create-instance.adoc[`new`
operation] from the Java module can be used to create an object on which a method is later invoked.
...
In the following example, an instance of the class _TaxCalculator_ will be created and placed
into the target variable `taxCalculator`. Then the _calculateTax(Double, Integer)_ method is
called, which takes _price_ and _percentAdditionalTax_ as arguments, and its return value is
...
for both the input arguments and the output value. In the example, DataSense discovers that
_calculateTax_ returns a Number, so the output metadata for the `invoke` operation will look like this:
...
methods that return `boolean` values. The function takes as arguments the full class name of the
given object, the instance method to execute, the instanced object and the method arguments as a map.

Given the Java class at the <<Invoke Instance Methods,top of this page>>, this examples creates
a new instance of _TaxCalculator_ and, rather than use the Java module `invoke` operation to call
the _isTaxFree(Double)_ method, it embeds the DataWeave function in a
