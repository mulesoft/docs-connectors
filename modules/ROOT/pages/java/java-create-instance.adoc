= Example: Create an Instance of a Class
...
The examples on this page are based on the following Java class `Person` which
...
If not exported, the execution fails with a JAVA:CLASS_NOT_FOUND error.
See xref:4.2@mule-runtime::how-to-export-resources.adoc[how to export resources].
====
...
the xref:7.3@studio::datasense-concept[DataSense] support,
...
image::java/graphical-view.png[30, 30]
...
xref:4.2@mule-runtime::set-payload-transformer-reference.adoc[Set Payload]
component can do), the view looks like this:

image::java/matching-input.png[]

Here the fields can be matched by dragging and dropping from the left to the right. Doing so produces:

image::java/matched-input.png[]
...
image::java/output-metadata.png[]
...
xref:4.2@mule-runtime::target-variables.adoc[target parameters].
...
* xref:java/java-throwable.adoc[Example: Work With Throwables]
