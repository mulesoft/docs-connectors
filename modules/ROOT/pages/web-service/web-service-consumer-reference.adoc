...
Use the Anypoint Connector for Web Service Consumer (Web Service Consumer Connector) to consume SOAP Web services.
...
| Must Understand a| Boolean |  Determines whether the `mustUnderstand` attribute in the `wsse:Security` header is `true` or `false`. <p> Defaults to true. |  true | 
| Actor a| String |  A SOAP message may travel from a sender to a receiver by passing different endpoints along the message path. The SOAP actor attribute is used to address the `wsse:Security` header to a specific endpoint. <p> This parameter's value must be a URI. |  | 
|===
...
| Target Value a| String |  An expression that is evaluated against the operation's output. The outcome of that expression is stored in the target variable. |  #[payload] | 
...
| Add Nonce a| Boolean | Specifies if a cryptographically random nonce should be added to the message. | false | 
...
| Time To Live a| Number a| The time difference between creation and expiry time in the time unit specified in `timeToLiveUnit`.
 ...
 <p>
 This parameter's value must be greater than or equal to 1 second (or similar in in another unit).
...
|===
| Field | Type | Description | Default Value | Required
| Max Idle Time a| Number | A scalar time value to specify the maximum amount of time a dynamic configuration instance can be idle before it's considered eligible for expiration. |  | 
