...
Using connectors in a Mule app provides provides the following advantages:
...
For more information about HTTP Listener and the Scheduler component, see the xref:http/http-connector.adoc[HTTP Connector User Guide] and xref:mule-runtime::scheduler-concept.adoc[Scheduler Endpoint (Trigger)].
...
* Select connectors are available to anyone.
+
However, you must have an Anypoint Platform subscription to access MuleSoft support for these connectors.
+
