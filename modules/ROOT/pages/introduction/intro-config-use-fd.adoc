...
. Follow the procedure described in <<configure-trigger,Configure a Trigger for the Flow>>.
. In the *Search* field, enter *HTTP*.
. Click *HTTP Listener*.
. In the *Path* field, specify the path on which `HTTP Listener` listens for requests.
. Optionally, use the default CloudHub connection, use another existing connection, or configure a new one, as described in the following sections.

==== Use the Default CloudHub Connection

The default CloudHub connection uses host 0.0.0.0 and port 8081. To use this connection, click *Save* after you specify the path.

==== Select Another Existing Connection

To select an existing connection other than the default CloudHub connection:

. Click *Edit* on the HTTP trigger card.
+
...
+
. Optionally configure a reconnection strategy for the new connection:
.. Scroll down and click *Advanced*.
.. In the *Reconnection Strategy* field, select a reconnection strategy.
.. Complete the fields *Reconnection Strategy* section.
+
...
Use `Scheduler` to initiate a flow when a time-based condition is met. You can configure `Scheduler` to trigger a flow at a regular interval or you configure `Scheduler` to use a more flexible cron expression. A cron expression is a standard for specifying date and time information that triggers a specified action.
...
. Optionally click *Advanced* to create a reconnection policy or expiration policy.
+
NOTE: The available options vary by connector.
+
