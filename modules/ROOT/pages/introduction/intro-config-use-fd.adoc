...
. Configure the input source (trigger) for the flow.
. Configure the target connector operation.

[[configure-input-source]]
== Configure the Input Source for the Flow
...
+
.Clicking *Go straight to canvas* takes you to an empty trigger card.
...
If a connector has associated input sources, you can use a connector input source to initiate the flow. For example, you can use Amazon SQS Receive Messages as an input source for Anypoint connector for Amazon SQS (Amazon SQS Connector). This input source initiates a flow each time the configured Amazon SQS queue receives a new message.

To start the configuration for connector-specific input sources, follow these steps:
...
Each connector-specific input source has a unique configuration, but most configurations require the following actions:

* Completing the required fields.
* Select an existing connection configuration or adding a new one.

For more information about configuring a connector-specific input source, see the reference guide for the associated connector.

== Configure the Target Connector Operation

After you configure an input source for a connector, you can configure a connector operation as the target component:

. Click *+* next to the trigger card for the input source.
+
