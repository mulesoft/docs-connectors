...
Use `Scheduler` to trigger a flow when a time-based condition is met. You can configure `Scheduler` to trigger a flow at a regular interval or give it a more flexible cron expression.
...
The following configuration uses a cron expression to log `my message` every second:
+
...
. Click *+* next to the trigger card of the input source.
+
