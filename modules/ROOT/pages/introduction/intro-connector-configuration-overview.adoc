...
Instead of using static values for a connector configuration in Studio, you can use http://ant.apache.org[Ant-style] property placeholders, as shown in this Studio example for the *Access Key* and *Secret Key* fields:
...
.Click the Test button in Studio to test the connection
...
. Use a reconnection strategy in Studio to specify reconnection options.
...
.Use a reconnection strategy in Flow Designer to specify reconnection options.
