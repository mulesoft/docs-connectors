...
* Use property placeholders for property values if you are configuring a connector in Studio or configuring it using XML outside the Studio editor.
...
* The first item is a placeholder for the Amazon SQS access key.
...
To make the values for the placeholders available, create a YAML file (`.yaml`) or properties (`.properties`) file to contain the values, place the file in `src/main/resources` in the Mule project, and then reference the file from the connector configuration.
...
. Use these fields in Studio to specify a reconnection strategy
image::introduction/intro-config-overview-reconnection-strategy-studio.png[Reconnection strategy example using Studio]
...
.Use these fields in Flow Designer to specify a reconnection strategy
...
You can configure a connector using Studio, Flow Designer, or XML.
