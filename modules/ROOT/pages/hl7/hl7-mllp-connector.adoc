...
** Keep connection open after data sending to be ready for next operations: Whether or not to close socket connections after each send operation.

In the Security tab you can configure a new TLS configuration, its presence implies the use of SSLServerSocket instead of plain TCP.

image::hl7/hl7-mllp-tls.png[Studio Global Element Properties - Security Tab]

In the Buffer Configuration tab you can set the send or receive buffer size and set the Validate output parameter in order to perform special character validation.
...
* Access the xref:release-notes::connector/hl7-mllp-connector-release-notes-mule-4.adoc[HL7 MLLP Connector Release Notes].
