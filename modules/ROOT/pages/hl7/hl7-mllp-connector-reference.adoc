...
| Keep connection open after data sending to be ready for next operations | Boolean | Whether or not to close socket connections after each send operation. | true |
| Validate output | Boolean | If a set throws an exception where the payload contains special characters (only ASCII chars greater than 0x1F are valid except 0x0A and 0x0D). | false |
|===
