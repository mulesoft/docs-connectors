...
| Keep connection open after data sending to be ready for next operations | Boolean | Whether to close socket connections after each send operation. | true |
| Validate output | Boolean | If set throws an exception if the payload contains special characters (only ASCII chars greater than 0x1F are valid except 0x0A and 0x0D). | false |
|===
