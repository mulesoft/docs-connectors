...
:page-aliases: servicenow-ex-to-get-incident-table-keys.adoc, servicenow-ex-to-get-record.adoc, servicenow-ex-to-get-records-from-incident-table.adoc, servicenow-XML-reference.adoc

You can try the following examples in Studio or Design Center.

== Example: To Get a ServiceNow Incident Record

This example creates a Mule flow in Design Center that retrieves a record based on an incident number.
...
+
. After the HTTP Listener is configured, add the `Invoke` operation from the ServiceNow connector to your flow.
...
+
These values must be available as menu items in the *Service* and *Operation* fields.
...
+
In your `Invoke` operation in Design Center, click  *Map* for the `Body` field, click the `Script` tab at the bottom of the *Body* window,
and paste a transformation script like this:
+
----
%dw 2.0
...
+
The response should look similar to this (several fields omitted for brevity):
+
----
...
This example creates a Mule flow in Anypoint Studio 7 to get records from the Incident table.
...
NOTE: Use a description that matches records in your ServiceNow incident table.
+
----
%dw 2.0
output application/xml
...
+
If the connection is unsuccessful, correct any invalid connection parameters, and test again.
+
* In the `General` side-tab, set *Service* to `incident` and *Operation* to `getRecords`.
+
* Set the Message Body to `payload`.
. Add the Transform Message component to transform XML to JSON for better readablity:
+
----
%dw 2.0
...
. To test the app, navigate to `+http://localhost:8081/getrecords+`
. The response should look similar to this (several fields are omitted for brevity):
+
----
{
  "headers": {
...
. Add a Transform message component after the HTTP connector, and provide a transformation script something like this, just use description that matches records in your ServiceNow incident table:
+
----
%dw 2.0
output application/xml
...
If the connection is unsuccessful, correct any invalid connection parameters, and test again.
+
* In the General side-tab, set *Service* to `incident` and *Operation* to `getKeys`.
+
. Set the Message Body to `payload`.
. Add the Transform Message component to transform XML to JSON for better readablity:
+
