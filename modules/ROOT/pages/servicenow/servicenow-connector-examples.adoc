...
:page-aliases: servicenow/servicenow-ex-to-get-incident-table-keys.adoc, servicenow/servicenow-ex-to-get-record.adoc, servicenow/servicenow-ex-to-get-records-from-incident-table.adoc, servicenow/servicenow-XML-reference.adoc

You can try the following examples in Studio or Design Center.

== Retrieve a ServiceNow Incident Record

This example creates a Mule flow in Flow Designer that retrieves a record based on an incident number.
...
`getrecords`
====
image::servicenow/servicenow-listener.png[HTTP Listener general settings with path set to getrecords]
+
...
+
image::servicenow/servicenow-studio-connection-6.6.0.png[Servicenow studio connection]
+
image::servicenow/servicenow-studio-config-transport-tab.png[Servicenow studio config transport tab]
+
...
+
IMPORTANT: In Studio version 7.5.0 and later you can choose `service` and `operation` keys without specifying the `Show Reference Values` key in order to resolve the metadata. However in older Studio versions metadata won’t be loaded until you specify all of the metadata keys and moreover it’ll result in a tooling Exception
+
image::servicenow/servicenow-metadata-without-reference-values.png[Servicenow metadata for Studio 7.5.0 without reference values]
+
...
+
image::servicenow/servicenow-flow-incident-getrecords.png[Servicenow flow - incident get records]
+
...
If you choose for `Show Reference Values` the value `ALL` the response should look like this (several fields are omitted for brevity)
...
----
where the response element name for the display value field will be prefixed with `dv`. For more info https://docs.servicenow.com/bundle/orlando-application-development/page/integrate/inbound-soap/concept/return-display-reference-variables.html[Servicenow - Display value].
...
* xref:servicenow/servicenow-connector-studio.adoc[Use Anypoint Studio to Configure ServiceNow Connector]
