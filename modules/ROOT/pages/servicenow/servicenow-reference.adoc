= ServiceNow Connector 6.6 Reference - Mule 4
...
| Show Reference Values a| String | Choose which values are returned for reference fields. Possible options are to return sys_ids or display values or both. This option should be used only for `get` and `getRecords` operations. |  | 
