...
* `WHEN_EXHAUSTED_FAIL` +
Throws a `NoSuchElementException` | Specifies the behavior of the Mule component pool when the pool is exhausted. If a positive `maxWait` value is supplied, it will block for, at most, that many milliseconds, after which a `NoSuchElementException` will be thrown. If maxThreadWait is a negative value, it will block indefinitely. |  |
...
* `INITIALISE_ALL` +
Will load all components in the pool on startup a| Determines how components in a pool should be initialized. |  |
