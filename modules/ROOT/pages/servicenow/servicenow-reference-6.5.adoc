= ServiceNow Connector 6.5 Reference - Mule 4
...
|Name | String | The name by which connectors reference this configuration.  | | x
...
| Sign Security Strategy a| <<WssSignSecurityStrategyAdapter>> |  A secure signing WSS configuration |  | 
| Username Token Security Strategy a| <<WssUsernameTokenSecurityStrategyAdapter>> |  A username token WSS configuration |  | 
...
| Access Token Url a| String |  The service provider's access token endpoint URL |  https://{instance-id}.service-now.com/oauth_token.do | 
| Scopes a| String |  The OAuth scopes to request during the OAuth dance. If not provided, it defaults to the scopes in the annotation. |  | 
| Resource Owner Id a| String |  The resourceOwnerId that each component should use if not otherwise referenced |  | 
| Before a| String |  The name of a flow to execute before starting the OAuth dance |  | 
| After a| String |  The name of a flow to execute right after an access token is received |  | 
...
| External Callback Url a| String |  If the callback endpoint is behind a proxy or will be accessed through a non-direct URL, use this parameter to specify the URL for the OAuth provider to use to access the callback |  | 
| Object Store a| String |  A reference to the object store to use to store each resource owner ID's data. If not specified, Mule automatically provisions the default. |  | 
|===
...
This operation invokes Web Service operations.
...
=== WSS Signing Security Strategy Adapter
