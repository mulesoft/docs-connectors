...
Exchange: https://www.mulesoft.com/exchange/com.mulesoft.modules/mule-oauth2-provider-module/[OAuth Provider Module]
...
| Client Scopes a| Array of String | The scopes that will match this client. If a request is received with this clientId but with different scopes, it will not be processed. See also <<newvaluehandling,New Value Handling>>. |  | 
| Type a| Enumeration, one of:
