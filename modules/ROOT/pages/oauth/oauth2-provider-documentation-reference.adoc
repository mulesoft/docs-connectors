...
The following documentation assumes a basic knowledge of the OAuth2 authorization protocol. If you are not familiar with this protocol, see https://tools.ietf.org/html/rfc6749[RFC-6749].

If the app behaves as the client in the OAuth2 dance, use the xref:oauth/oauth-documentation.adoc[OAuth Module] instead.
...
https://www.mulesoft.com/exchange/com.mulesoft.modules/mule-oauth2-provider-module/[OAuth Provider Module] in 
...
| Client Validation Rate Limiter a| One of:

* <<PeriodRateLimiter>> |  The rate limiter used to control access to certain operations. If none is specified, PeriodRateLimiter is used as the default. |  | 
...
| Resource Owner Security Provider a| String |  The security provider used to authenticate resource owners. Not needed if only the CLIENT_CREDENTIALS grant type is used. |  | 
...
| Supported Grant Types a| String |  The comma-separated grant types this provider will support. If none specified, only the authorization code grant type will be supported. |  AUTHORIZATION_CODE | 
...
| Fail If Present a| Boolean |  Defines what to do if the a client with the same ID is already registered. If `true`, an error is raised. Otherwise, the client is updated.|  `false` | 
|===
...
| Login Page a| String | Relative file path to the web page for the resource owner to provide its credentials. +
  ...
  For example, if the login page HTML references an external `.css` style file, an endpoint that provides that file must exist. See xref:http/http-documentation.adoc#loadStaticResource[HTTP Load Static Resource]. | www-static/auth.html | 
...
| Client Redirect Uris a| Array of String | List of registered redirectUris to use to respond to the client once the request has been processed.

Most request allow the definition of new redirection URIs so these will not always be taken into account. See also <<newvaluehandling,New value Handling>>. 
...
| Client Scopes a| Array of String | The scopes that will match this client. If a request is received with this clientId but different scopes, it will not be processed. See also <<newvaluehandling,New value Handling>>. |  | 
| Type a| Enumeration, one of:

** CONFIDENTIAL
** PUBLIC | The type of the client. Allowed values are PUBLIC  where clients incapable of maintaining the confidentiality of their credentials or CONFIDENTIAL where clients are capable of maintaining the confidentiality of their credentials. The client type defines if the client is able to maintain confidentiality for its credentials. | PUBLIC | 
|===
...
when a refresh token request comes, it always is rejected.
...
|===
| Field | Type | Description | Default Value | Required
| Object Store a| Object Store | A reference to a globally defined object store or a definition of a private object store, which stores the generated refresh tokens. | A persistent object store created from the ObjectStoreManager with an entry TTL of 86400 seconds. | 
|===
...
Because the OAuth dance is done through HTTP, the OAuth2 Provider makes use of the xref:http/http-connector.adoc[Mule HTTP Connector].
...
Apart from that, the provider defines an operation: <<validateToken>> that can check if a token is authorized. That operation can be added anywhere in a flow to control it's execution. If the token is indeed authorized,
...
One way of doing this is using the Spring Framework, defining both security providers and then using the xref:spring/spring-module.adoc[Spring Module] to add the providers to the Mule Security Manager:
