...
| Login Page | String | Relative file path to the web page for the resource owner to provide it's credentials. +
  Keep in mind that in case the default page is not used, if the referenced login page requires external files, they won't be handled by the oauth provider. +
  For example: If the login page html references an external .css style file, that file won't be found unless explicitly exposed by any mean other than the oauth provider. Check  xref:http/http-documentation.adoc#loadStaticResource[HTTP:load-static-resource] | www-static/auth.html | {nbsp}
