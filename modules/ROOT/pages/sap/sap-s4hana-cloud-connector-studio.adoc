= Use Studio to Configure SAP S/4HANA OData Cloud Connector - Mule 4
...
Add SAP S/4HANA Odata Cloud Connector to a Mule project to automatically populate the XML code with the connector's namespace and schema location, as well as to add the required dependencies to the project's pom.xml file.

. In the *Mule Palette* view, click *(X) Search in Exchange*.
. In *Add Dependencies to Project*, type `SAP S/4HANA OData` in the search field.
. Click the SAP S/4HANA OData Cloud connector name in *Available modules*.
. Click *Add*.
. Click *Finish*.
...
To use SAP S/4HANA OData Cloud Connector in a Mule app, you can configure a global SAP S/4HANA configuration element for use by SAP S/4HANA OData Cloud Connector operations in your application.
...
.. In the *Connection* drop-down, select the connection type:
  ...
  ** Token URL +
  Enter Token URL that corresponds to the URL of the endpoint which provides the access tokens.
...
|===
|Studio Field |Description
|Time zone |The timezone with which the Date objects returned from S/4Hana will be converted.
|Batch buffer size |Defines the maximum memory the connector will use to keep batch requests in memory. If more than that is consumed it will start to buffer the content on disk.
|Buffer unit | The unit in which Batch buffer size parameter is expressed.
|===
+
...
After configuring the SAP S/4HANA OData Cloud Connector for use in Studio, see the xref:sap/sap-s4hana-cloud-connector-examples.adoc[Examples] 
