...
== Example: Create, Get, Query, Update and Execute

This example demonstrates the use of SAP S/4 HANA connector to create and get an entity, query for a value, update entity and execute a function.
...
NOTE: Please take note that in case of update entity operation, the entity key *must* be specified as part of the payload.
...
payload]]]></s4hana:entity>
		...
		<s4hana:get-entity doc:name="Get entity by key" config-ref="SAP_S_4HANA_Config" service="API_SALES_ORDER_SRV" entityType="A_SalesOrder">
