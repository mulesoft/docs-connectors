...
The Mule SAP S/4HANA Cloud Connector provides full support for the query, get, create, update, delete entities, and execute functions using the ODATA API exposed by S/4HANA.
...
To use the SAP S/4HANA Cloud connector in a Mule application, you can configure a global SAP S/4HANA configuration element for use by as many SAP S/4HANA connectors as you require for your application.
...
. Search for `HTTP` and drag the *Listener* operation to the canvas. Click the green plus sign next to *Connector Configuration*, and click *OK* in the menu to accept the default values.
