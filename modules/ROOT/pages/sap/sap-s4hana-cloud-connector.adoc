...
SAP S/4HANA OData Cloud Connector v2.0

Anypoint Connector for SAP S/4HANA OData Cloud (SAP S/4HANA OData Cloud Connector) provides full support for the query, get, create, update, delete entities, and execute functions using the OData API exposed by SAP S/4HANA.
...
Add SAP S/4HANA OData Cloud Connector to a Mule project to automatically populate the XML code with the connector's namespace and schema location, as well as add the required dependencies to the project's pom.xml file.
...
To use SAP S/4HANA OData connector in a Mule app, you can configure a global SAP S/4HANA configuration element for use by SAP S/4HANA connector operations in your application.
...
To access the data in the SAP S/4HANA instance, you need to authenticate your application’s requests using a supported authentication method. 
...
. Check the console to see when the application starts. You should see messages like the following if an error does not occur:
...
Replace the `<groupID>` and `<artifactID>` elements with the correct information from Exchange. Verify that the connector `<version>` corresponds to the one you are using. 
...
* SAP S/4HANA Cloud Connector v1.0.0 and later
...
* https://www.mulesoft.com/exchange/com.mulesoft.connectors/mule-sap-s4hana-cloud-connector/[SAP S4HANA OData Cloud Connector]
