...
SAP S/4HANA Cloud Connector v1.2

Anypoint Connector for SAP S/4HANA OData Cloud (S/4HANA OData Cloud Connector) provides full support for 
the query, get, create, update, delete entities, and execute functions using the OData API exposed by SAP S/4HANA.

The connector lets you perform operations to:
...
* Familiarity with S/4HANA OData, Mule, Anypoint Connectors, and the Anypoint Studio essentials.
...
|===
|Studio Field |Description
|Discovery Service Enabled |Feature switch for service discovery. If disabled, Service discovery relative path is not applied.
|Service directory |Common HTTP directory for all SAP services. It is possible to leave this parameter blank and specify the HTTP directory individually for every service in the unlisted services list.
...
* Places placeholder values in the `mule-artifact.content` configuration file in the `src/main/resources` 
