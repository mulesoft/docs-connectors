...
The batch processing feature is available starting in SAP S/4HANA OData Cloud Connector version 2.1. 

You must follow the correct order for structural batch operations:
 
. The batch must be initiated before any requests are added into it.
. Requests cannot be added to the batch or the change set after it is executed.
...
This operation returns a unique change set identifier, which must then be used in the *Batch Create*, *Batch Update*, *Delete Entity*, or *Batch Execute Function* operations in order to add them to the change set.
+
...
+
. When the batch is complete, use the *Execute Batch* operation to send a batch request.
...
* The connector throws an exception if an operation references an invalid batch identifier, either because it was never initiated or if the *Execute Batch* operation was already performed for that particular batch identifier. 
