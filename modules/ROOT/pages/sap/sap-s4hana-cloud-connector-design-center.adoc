= SAP S/4HANA OData Cloud - Design Center Configuration - Mule 4
...
the xref:sap/sap-s4hana-cloud-connector-reference.adoc[SAP S/4HANA OData Cloud Connector Reference].
...
. In *Select a component*, search for the SAP S/4HANA OData Cloud Connector.
. Select the connector name. 
. Select the Query operation for the connector.
. Complete these fields:
+
[%header,cols="30s,70a"]
|===
|Field |Description
|*Service*| The name of the OData service to invoke.
|*Entity type*| Entity type to query the OData service for.
|===
+
. The list of services and operations in the dropdown is automatically populated through auto-discovery of enabled OData APIs. 
. Click the *Test* button to test the connection. 
