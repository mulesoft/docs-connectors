...
* SNC (Secure Network Communications): SNC integrates SAP NetWeaver single sign-on or an external security product with SAP systems. SNC provides security functions not available with SAP systems.
+
NOTE: SNC is not supported when deploying an application containing the SAP Connector in Runtime Manager (CloudHub).
