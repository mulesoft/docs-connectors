...
=== Function Source - Return Values to SAP

The current use case describes how to create a Mule application that receives a function using the *Function source* operation that passes the export parameter back to SAP.

. Create a new *Mule Project* in Anypoint Studio.
. Install the SAP connector from Exchange.
. Drag the *Function source* operation to the canvas.
. Create a new *SAP Inbound* global element configuration and fill with the environment values.
. Add a transform message component to the flow and fill the details based on metadata.
+
For example:
+
image::sap/function-return-response.png[idoc]
+
The flow should look like this:
+
image::sap/flow-response.png[idoc]
+
In the SAP user interface you should see:
+
image::sap/result-sap-gui.png[idoc]

=== Send IDoc to SAP

How to create a Mule application to send an IDoc to SAP.
...
. From a web browser, test the application by entering an employee's internalId, fist name, and last name in the form of the following query parameters:
+
[source]
----
