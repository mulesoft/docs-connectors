...
|Operation timeout |operationTimeout |Timeout for operations.  A positive value in milliseconds. A value of 0 means indefinite waiting. | 0
|===
...
* SNC (Secure Network Communications): SNC integrates SAPNetWeaver Single Sign-On or an external security product with SAP systems. With SNC, you strengthen security by using additional security functions provided by a security product that are not directly available with SAPsystems. NOTE! SNC is not currently supported when deploying an application containing SAP Connector in cloudhub.
