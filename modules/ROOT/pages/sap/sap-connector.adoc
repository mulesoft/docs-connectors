...
== good idea to help customers over a common problem we see in the field
developers working on Windows or Mac, servers running Linux. Maven can seamlessly build for either Windows or *NIX by using the Maven "profile" feature - see https://mulesoft.slack.com/archives/C02KJHQU0/p1591311651311100?thread_ts=1591310985.311000&cid=C02KJHQU0
...
* Power user +
Read the >>>>>this link is either broken or the Git repo is not viewable to me (in "employee" team, so most of the world will also not be able to read it)<<<<< xref:sap/sap-connector-xml-maven.adoc[XML and Maven Support], 
