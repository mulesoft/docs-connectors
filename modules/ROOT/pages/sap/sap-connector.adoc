...
The Anypoint Connector for SAP (SAP Connector) enables the integration of data to and from external systems based on SAP NetWeaver.
...
Mule runtime engine supports SAP integration through the Anypoint Connector for SAP, an SAP-certified Java connector that leverages the SAP Java Connector (JCo) libraries, which allows Mule applications to:

* Execute BAPI functions over the following types of RFC protocol:
** Synchronous Remote Function Call RFC (sRFC)
** Asynchronous Remote Function Call RFC (aRFC)
* Act as a JCo Server to be called as a BAPI over sRFC, aRFC.
...
* A SAP ID to download the required SAP libraries.
* SAP login credentials to test your connection to your target resource.
* Use the RFC protocol to connect to NetWeaver Application Servers (NWAS). http://www.cipherbsc.com/solutions/sap-erp-central-component-erp-ecc/[ECC] and https://help.sap.com/viewer/p/SAP_CUSTOMER_RELATIONSHIP_MANAGEMENT[CRM] run on top of NWAS, as other SAP solutions do, so any customer using the connector can access those systems.
...
* Starting user -- Configure your Mule app in 
...
* Power user - Read the xref:sap/sap-connector-xml-maven.adoc[XML and Maven Support], 
...
Originally, SAP created libraries in C language to allow direct RFC calls to SAP, to manipulate with data. JCo wraps C libraries in Java to provide platform-native access into the SAP system. RFC middleware uses the RFC library through the JNI Layer.
