...
=== Function source returning values to SAP

The current use case describes how to create a Mule application that receives a function using *Function source* which will pass the export parameter back to SAP.

. Create a new *Mule Project* in Anypoint Studio.

. Drag *Function source* to the flow.

. Create a new **SAP Inbound** global element configuration and fill with environment values.

. Add a transform message component to the flow and fill the details based on metadata. Example:
