...
The current use case describes how to create a Mule application that receives a function using *Function source* which passes the export parameter back to SAP.
...
This use case describes how to create a Mule application to send an IDoc to SAP.

. Create a new Mule Project in Anypoint Studio.
. Create a new HTTP Listener global element configuration and leave it with the default values.
. Drag a HTTP Request onto the canvas and configure the following parameters:
+
[%header%autowidth.spread]
|===
|Parameter |Value
