= SAP S/4HANA OData Cloud Connector 2.1 Reference - Mule 4
...
| Object Store a| String |  A reference to the object store used to store each resource owner ID's data. If not specified, Mule automatically provisions the default one. |  | 
|===
...
Adds a create entity request into the batch. This is a dynamic operation, returning whatever the OData specification states that will be returned by the operation. The result will be available only after the Execute batch operation is executed.
...
Adds an execute function request (invoke request) to the batch. The result will be available only after the Execute batch operation is executed.
...
Adds a query operation request with defined key into the batch. This is a dynamic operation, returning whatever the OData specification states that will be returned by the operation. The result will be available only after the Execute batch operation is executed.
...
Adds a query operation request into the batch, filtering the results by the parameters defined. If no filter is added, then all results will be listed. The result will be available only after the Execute batch operation is executed.
...
Adds an update entity request into the batch. This operation works through PATCH request. This means that any fields that are missing will not be updated.
...
Executes the batch and returns a result for all requests within the batch.
