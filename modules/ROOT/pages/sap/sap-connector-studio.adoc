...
. Click the Exchange image::sap/exchange-icon-in-studio.png[] icon in the upper left of the Studio task bar.
...
. In Studio, create a Mule project.
. In the *Mule Palette*, click *image::sap/exchange-icon-in-studio.png[] Search in Exchange*.
. In *Add Modules to Project*, type "SAP" in the search field.
. In *Available modules*, select *SAP Connector*.
. Click *Add*.
. Click *Finish*.
...
For ease of use, the SAP Connector shows only the most common properties as connector parameters. To configure a property that is not listed in the *Properties* panel, see the xref:sap/sap-connector-config-topics.adoc[Define Extended Properties] topic.
