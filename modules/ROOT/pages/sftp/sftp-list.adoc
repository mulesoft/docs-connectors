...
In the above example, a flow lists the contents of a folder once per second. The flow then processes the files one by one and deletes each file after it is processed because there is a `delete` operation in the `<foreach>` component.
...
This is an example of an inner, non reusable, matcher:
