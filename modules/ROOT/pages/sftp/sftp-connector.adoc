...
Poll a directory to look for new files to process. Because Mule 4 no longer has a polling message source (like the old Mule 3 transport did), you can achieve polling by combining a `<scheduler />` element with the `<sftp:list>` element.
...
Another common scenario when listing files is the need to list files which match certain criteria. 
