...
** MINUTES
** HOURS
** DAYS |  A TimeUnit that qualifies the *Time Between Size Check* attribute. |  MILLISECONDS | 
| Expiration Policy a| <<ExpirationPolicy>> |  Configures the minimum amount of time that a dynamic configuration instance can remain idle before the runtime considers it eligible for purging. This does not mean that the platform expires the instance at the exact moment that it becomes eligible. The platform purges expired instances as needed to free resources. |  | 
|===
...
** MINUTES
** HOURS
** DAYS |  A TimeUnit that qualifies the *Connection Timeout* attribute. |  SECONDS | 
...
** MINUTES
** HOURS
** DAYS |  A TimeUnit that qualifies the *Response Timeout* attribute. |  SECONDS | 
...
| Passphrase a| String |  The pass phrase password for the identity file, if required. Notice that this parameter is ignored if *Identity File* is not provided. |  | 
...
| Recursive a| Boolean |  Whether to include the contents of sub-directories. |  false | 
...
The *Source Path* can either be a file or a directory. If a directory, then the files are moved recursively.
...
| Create Parent Directories a| Boolean |  Whether or not to attempt creating any parent directories which don't exist. |  true | 
| Lock a| Boolean |  Whether or not to lock the file. |  false | 
| Write Mode a| Enumeration, one of:

** OVERWRITE
** APPEND
** CREATE_NEW |  A FileWriteMode. |  OVERWRITE | 
| Reconnection Strategy a| * <<reconnect>>
* <<reconnect-forever>> |  A retry strategy in case of connectivity errors. |  | 
|===
...
The key part of this functionality is determining if a file is new: 

* Set the `autoDelete` parameter to `true` - This deletes each file after it has been processed, causing all files obtained in the next poll to be necessarily new.
...
| Watermark Enabled a| Boolean |  Controls whether or not to watermark, and if so, if the watermark should consider the file's modification or creation timestamps. Watermarking is a technique for storing and retrieving the point in a file for its next update.  |  false | 
...
** MINUTES
** HOURS
** DAYS |  A TimeUnit that qualifies the *Time Between Size Check* attribute. |  | 
| Output Mime Type a| String |  The MIME type of the payload that this operation outputs. |  | 
| Output Encoding a| String |  The encoding of the payload that this operation outputs. |  | 
...
| Apply Post Action When Failed a| Boolean |  Whether any of the post actions (*Auto Delete* and *Move To Directory*) should also be applied in case file processing fails. If set to `false`, failed files are not moved or deleted. |  `true` | 
|===
...
|===
| Field | Type | Description | Default Value | Required
| *Max active* a| Number | Controls the maximum number of Mule components that can be borrowed from a session at one time. When set to a negative value, there is no limit to the number of components that can be active at one time. When *Max active* is exceeded, the pool is said to be exhausted. |  | 
...
| *Min eviction (ms)* a| Number | Determines the minimum amount of time an object can remain idle in the pool before it is eligible for eviction. When non-positive, no objects are evicted from the pool due to idle time alone. |  | 
| *Eviction check interval (ms)* a| Number | Specifies the number of milliseconds between runs of the object evictor. When non-positive, no object evictor is executed. |  | 
...
* WHEN_EXHAUSTED_GROW - Create a new Mule instance and return it. This setting negates the *Max active* value. If a positive *Max wait* value is supplied, it blocks for at most that many milliseconds, after which `NoSuchElementException` is thrown. If `maxThreadWait` is a negative value, it blocks indefinitely. |  | 
| *Initialisation policy* a| Enumeration, one of:

** INITIALISE_NONE
** INITIALISE_ONE
** INITIALISE_ALL a| Determines how to initialize components in a pool. The possible values are: 
...
|===
| Field | Type | Description | Default Value | Required
| *Initial buffer size* a| Number | The number of instances to initially keep in memory to consume the stream and provide random access to it. If the stream contains more data than can fit into this buffer, then the buffer expands according to the value in *Buffer size increment*, with an upper limit of the value in *Max in memory size*. Default is 256 KB. | 256 | 
| *Buffer size increment* a| Number | This is by how much the buffer size expands if it exceeds its initial size. Setting a value of zero or lower means that the buffer should not expand, meaning that a STREAM_MAXIMUM_SIZE_EXCEEDED error is raised when the buffer gets full. Default is 256 KB. | 256 | 
| *Max in memory size* a| Number | Maximum amount of instances to keep in memory. If more are required, then a
STREAM_MAXIMUM_SIZE_EXCEEDED error is raised. A value lower or equal to zero means no limit. Default is 256 KB.
| 256  |
...
** KB
** MB
** GB | The unit in which maxInMemorySize is expressed. | KB | 
|===
...
|===
| Field | Type | Description | Default Value | Required
| *In memory size* a| Number | Defines the maximum memory that the stream should use to keep data in memory. If more than that is consumed, then it starts to buffer the content on disk. | 1024 | 
| *Buffer unit* a| Enumeration, one of:
...
|===
| Field | Type | Description | Default Value | Required
| *Initial buffer size* a| Number | The number of instances to initially keep in memory to consume the stream and provide random access to it. If the stream contains more data than can fit into this buffer, then the buffer expands according to the value in *Buffer size increment*, with an upper limit of *Max in memory instances*. Default value is 100 instances. | 100 | 
| *Buffer size increment* a| Number | By how much the buffer size expands if it exceeds its initial size. A value of zero or lower means that the buffer should not expand, and to raise a STREAM_MAXIMUM_SIZE_EXCEEDED error when the buffer is full. Default value is 100 instances. | 100 | 
| *Max in memory instances* a| Number | Maximum number of instances to keep in memory. If more are required, then a
STREAM_MAXIMUM_SIZE_EXCEEDED error is raised. A value lower or equal to zero means no limit. The default is 500 instances.
| 500  | 
|===
...
|===
| Field | Type | Description | Default Value | Required
| *In memory objects* a| Number | The maximum amount of instances to keep in memory. If more than that is required, then the connector starts to buffer the content on disk. The default is 500 objects. | 500 | 
|===
