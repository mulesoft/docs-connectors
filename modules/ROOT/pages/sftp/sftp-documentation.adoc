...
If `targetPath` doesn't exist, and nor does its parent, then an attempt is made to create a parent folder depending on the value of the `createParentFolder` argument. If `createParentFolder` is set to `false`, then an `SFTP:ILLEGAL_PATH` error is thrown. 
...
If `targetPath` doesn't exist, and nor does its parent, then an attempt is made to create a parent folder, depending on the value of the `createParentFolder` argument. 
