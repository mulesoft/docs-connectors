...
For information about Flow Designer field values, see the xref:salesforce/salesforce-connector-reference-100.adoc[Salesforce Connector Reference].
...
+
Use the authorization server's URL for the audience value: https://login.salesforce.com, https://test.salesforce.com, or https://community.force.com/customers if implementing for a community.
...
* *Key Store* (required)
+
Path to the keystore used to sign data during authentication. Format the keystore in JAVA. See xref:salesforce/salesforce-connector-config-topics.adoc#keystoregen[Generate a Keystore File].
+
* *Store Password* (required)
+
Keystore password
