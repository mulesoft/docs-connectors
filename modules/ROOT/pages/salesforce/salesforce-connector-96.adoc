...
https://www.mulesoft.com/exchange/?search=salesforce&type=template[Anypoint Exchange templates] provides starting points for Anypoint Studio projects such as:
...
. Click a trigger to initiate access to your flow. To have Salesforce send content to your application, select Salesforce as the trigger and an operation:
+
...
* Consumer Key: The consumer key for the Salesforce connected app. See https://docs.mulesoft.com/connectors/salesforce/salesforce-connector#create-a-consumer-key[Create a Consumer Key].
...
After setting the *Global Element Properties* screen, return to the main 
connector menu and set the <<Connector Property Values>>.
...
image::salesforce/salesforce-studio-user-pass.png[]
...
image::salesforce/salesforce-studio-oauth-2.png[]
...
image::salesforce/salesforce-studio-oauth-jwt.png[]
...
image::salesforce/salesforce-studio-oauth-saml.png[]
...
Based on the operation that you have dragged to the canvas, configure the following fields:
...
You can set Apex in Design Center and in Anypoint Studio 7.
...
== Example: Create or Update an sObject With Parent Child Relationships

A Salesforce object (sObject) can have standard or custom relationships between objects.
