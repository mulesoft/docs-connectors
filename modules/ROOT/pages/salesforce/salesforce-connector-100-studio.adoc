...
. Add Salesforce Connector to your project.
. Configure an input source for your flow.
. Configure one or more Salesforce Connector Operations.
...
To use a Salesforce Connector input source, drag one of these input sources to the Source area of the flow:
...
+
Subscribes to a streaming channel. This input source provides channel-related notification settings for new events and events stored by Salesforce within the 24-hour retention period. Events that take place in the specified channel initiate the flow.
+
NOTE: A channel or topic must be published to Salesforce before a subscription to the channel can be created.
+
* *Replay topic listener*
+
Subscribes to a topic. This input source provides topic-related notification settings for new events and events stored by Salesforce within the 24-hour retention period. Events that take place on records in the specified topic initiate the flow.
** *Subscribe channel listener*
+
Subscribes to a streaming channel. This subscription provides channel-related notification settings for events that take place after you subscribe.
+
* *Subscribe topic listener*
+
Subscribes to a topic. This subscription provides topic-related notification settings for events that take place after you subscribe.

For the *On New Object* and *On Modified Object* operations, specify a UTC value in the *Since* field. If the server is in a different time zone, you must specify a value for the *Since* field because if the *Since* field is left empty, the connector takes the local timestamp. Because servers are in datacenters located generally elsewhere from one's location, use of the *Since* field is recommended.
+
. Click the operation.
...
Configure authentication for a Salesforce Connector input source.
...
* *Authorization URL*: URL for the endpoint configured to handle SOAP authentication requests.
...
+
The consumer key for the Salesforce-connected app. See <<Create a Consumer Key>>.
* *Key store* (required)
+
