...
When a Mule app starts for the first time, the connector creates an object store that is used to save data related to messages that processed successfully or that failed:

* For each message that processed successfully, the information stored consists of a a replay ID associated with the message.
