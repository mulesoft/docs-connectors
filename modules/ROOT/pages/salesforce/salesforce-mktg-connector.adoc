...
. To create a global element for the connector, enter values for the Username Password or OAuth Username Password authentication types:
+
...
. Drag the *Listener* onto the canvas.
...
. Add a *Transform Message* component after *Salesforce Marketing Cloud*.
. Configure each element. Double-click on the *Listener* component.
+
