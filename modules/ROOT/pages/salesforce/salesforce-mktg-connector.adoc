...
https://www.mulesoft.com/exchange/com.mulesoft.connectors/mule-sfdc-marketing-cloud-connector/[Salesforce Marketing Cloud Connector] in Anypoint Exchange and click *Dependency Snippets*.
...
. Open the pom.xml file and add the following dependency for Mule Salesforce Marketing Connector. Mule translates the `RELEASE` dependency keyword to the current version.
+
[source,xml,linenums]
----
<dependency>
...
+
. Click image:salesforce/salesforce-mktg-as-plus-button.png[plus Button] next to the *Connector configuration* field.
...
=== Retrieve operations limitations
...
Retrieve operation supports filters for querying the data. However this filter does not behave like a SQL filter condition. For example, a clause like WHERE 1=1 will work in SQL, but will result in an error in Marketing Cloud because the API doesn't support this. The operand in the left of the equal (in this case) sign  must be a valid property of the DE.

When executing the request with WHERE 1=1 clause, this will be transformed in the following request:
...
The response will be: `<OverallStatus>Error: The Filter Property '1' is not a retrievable property.</OverallStatus>`.
Therefore, the filter expects a property and not necessarily a valid SQL clause.
