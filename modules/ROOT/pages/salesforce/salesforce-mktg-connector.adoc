...
+
. Open pom.xml and under dependencies section add the following dependency for Mule Salesforce Marketing Connector (Mule converts RELEASE to the latest version):
+
[source,xml,linenums]
----
<dependency>
...
http://www.mulesoft.org/schema/mule/sfdc-marketing-cloud/current/mule-sfdc-marketing-cloud.xsd">
	...
	<sfdc-marketing-cloud:sfdc-marketing-cloud-config 
		name="Salesforce_Marketing_Cloud_Sfdc_marketing_cloud_config" 
		doc:name="Salesforce Marketing Cloud Sfdc marketing cloud config">
...
[{
	ListName: payload.listName
		...
		<sfdc-marketing-cloud:create 
			doc:name="Create" 
			config-ref="Salesforce_Marketing_Cloud_Sfdc_marketing_cloud_config" 
			objectType="List"/>
		<ee:transform doc:name="Transform Message">
			<ee:message >
				<ee:set-payload ><![CDATA[%dw 2.0
