...
** *Interactions* - An array of one or more interactions for the schedule operation
+
...
. In the Mule Palette view, search for *HTTP Listener*.
...
. Add a second *Transform Message* component to the right of the *Salesforce Marketing Cloud* component.
...
. Once deployed, use a REST client to make a POST request to `+x-www-form-urlencoded to localhost:8081/create+`. Include the parameter payload listName. For example:
+
