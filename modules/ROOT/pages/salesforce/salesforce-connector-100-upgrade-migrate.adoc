...
* Keep alive session feature is no longer a configurable option. The connector will automatically perform one API call every
14 minutes (salesforce minimum session ttl = 15 minutes), so the session will no longer expire
...
|===
|Salesforce 8.8.x Operation | Changes in Salesforce 10.0
