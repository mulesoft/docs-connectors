...
* Removed the keep alive feature as a configurable option. The connector now automatically performs one API call every
...
* Added connection pool functionality to improve multi-threading support. With this functionality, each request sent to the API has its own headers and read timeout, which prevents the operation headers from being overridden with the wrong values when operations are executed simultaneously.
* Added a new *Security* tab for configuring TLS.
* Reordered the parameters in a more intuitive way.
...
|===
|Salesforce 9.8.x Operation | Changes in Salesforce 10.0
...
| Get session id | This operation exposed internal session id that is used by the connector internally. This could impact the connection life-cycle regular usage of the connector. We encourage to re-implement using the connector functionality.
|===
...
. Install Salesforce Connector v10.0 to replace the Salesforce operations that were previously included in Salesforce Connector v9.8.x.

== Upgrade Steps

Follow these steps to perform the upgrade from Salesforce Connector v9.8.x to Salesforce Connector v10.0:

. In Anypoint Studio, create a Mule project.
. In the Mule Palette view, click *Search in Exchange*.
. In the *Add Dependencies to Project* window, enter `Salesforce 10` in the search field.
. In the *Available modules* section, select *Salesforce Connector 10.0.0* and click *Add*.
. Click *Finish*.
