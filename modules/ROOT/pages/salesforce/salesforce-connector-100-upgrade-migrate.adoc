...
* Operation now returns only the IDs of the objects that were updated and the latest date covered. To obtain field values for the updated objects, use this operation followed by the Retrieve operation.
+
...
. Click *Finish*.
. Verify that the `salesforce-connector` dependency version is `10.0.0` in the `pom.xml` file.
...
If it is necessary to revert to the previous version of Salesforce Connector, change the `salesforce-connector` dependency version `10.0.0` in the project's `pom.xml` file to the previous version.
