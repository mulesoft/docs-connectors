...
* Removed the keep alive session feature as a configurable option. The connector now automatically performs one API call every 14 minutes, so the session no longer expires. (The Salesforce minimum TTL is 15 minutes.)
