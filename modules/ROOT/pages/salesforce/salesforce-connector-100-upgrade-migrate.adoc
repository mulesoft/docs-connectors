...
* Added the following new tabs in the Salesforce Config:
** *Connection Pool Config* for configuring connection pool functionality
** *Security* for configuring TLS
* Reordered the parameters in a more intuitive way. (*Can we be more specific as to where this was done?*)
* Moved the Read timeout parameter from the provider configuration to the connector configuration.
* Removed the following parameters from the connector configuration: (*George, can we order these alphabetically, or are they ordered this way for a reason?*)
...
| Create multiple bulk | Create Job operation with a `create`
 operation parameter and a Create batch operation
...
| Hard delete bulk | Create Job operation with a `hardDelete` operation parameter and a Create batch operation
