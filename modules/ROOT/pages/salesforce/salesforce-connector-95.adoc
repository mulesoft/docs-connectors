...
Connectors and Mule runtime use Mule's Object Store is used internally by the connector or runtime to provide features such as automatic message replay and message redelivery on input sources.

Mule apps use different versions of object store, depending on how they are deployed:
* If a Mule app runs on-premises, it uses Object Store v1, which has no transaction limits.
* If a mule app is deployed with CloudHub, it uses Object Store v2 to persist data. The free version of Object Store v2 has a limit of 10 transactions per second. For more information, see https://docs.mulesoft.com/object-store/#object-store-notes[Object Store Notes].

=== Replay Topic and Replay Streaming Channel Input Sources

The Replay Topic and Replay Streaming Channel input sources have the option to continue from the last replay ID they received before restarting the application.

To provide this feature, Salesforce Connector uses an object store:

* When a Mule app starts for the first time, the connector creates an object store that saves the replay ID.
* For each message that comes through a topic or streaming channel to which the connector is subscribed, the connector updates the latest processed replay ID in the object store, with the required details attached.
// what are the required details? Do users need to know //this?
* Each time the app restarts, the connector cleans up the expired replay IDs from the object store. A replay ID is expired if it was saved more than 24 hours before the cleanup execution.

// This cleanup task performs three transactions on the // object store for each topic or channel that was used // in that specific app.

// These operations will sum up to 6 transactions.

=== Get Updated Objects operation

This operation retrieves the list of records that were updated between the last time the operation was called and the current server timestamp.

On the first use of this operation, the connector creates an object store and saves the current server timestamp for later use. On subsequent usages, the connector:
* Reads the timestamp from the object store
* Updates the value after it receives the API response
// Each usage of this operation executes 2 transactions // to interact with the Object Store.

=== OAuth 2.0 Authentication Type

When you configure a connection that uses OAuth2.0 for authentication, you can specify an object store to store then ID for each resource owner. If you don't specify an object store, Mule automatically provisions the default object store.

The app interacts with the object store when a new resource owner is authenticated, a token is refreshed, or a token is invalidated.

=== Message Redelivery on Input Sources

You can configure a redelivery policy for input sources so that when processing a message fails, it is redelivered and the processing will be tried again for a specified number of times. This feature is provided by the runtime not the connector itself and the users can use a specific object store for this functionality. If the object store is not specified by the user a non persistent Object Store will be automatically created and used by the runtime. Based on the number of retries configured for the redelivery policy, the number of transactions used to interact with the Object Store will vary. Check https://docs.mulesoft.com/mule-runtime/4.2/redelivery-policy[Redelivery policy] for details.
