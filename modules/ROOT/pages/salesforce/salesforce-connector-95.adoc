...
* A Mule app deployed using CloudHub or RuntimeFabric  uses Object Store v2. The free version of Object Store v2 has a limit of 10 transactions per second.
...
* On subsequent uses of this operation, the connector reads the timestamp from the object store. It updates the value of the object store after it receives the API response from the operation.
...
For more information setting a redelivery policy, see https://docs.mulesoft.com/mule-runtime/4.2/redelivery-policy[Redelivery policy].
