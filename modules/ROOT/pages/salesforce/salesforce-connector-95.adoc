...
Salesforce Connector and Mule use an object store to persist data for features such as automatic message replay and message redelivery:

* A Mule app that runs on-premises uses Object Store v1, which has no transaction limits.

* A Mule app deployed to CloudHub uses Object Store v2. The free version of Object Store v2 has a limit of 10 transactions per second.
// What about an app deployed with Runtime Fabric?

For more information, see https://docs.mulesoft.com/object-store/#object-store-notes[Object Store Notes].
...
////
 What does this mean? "with the required details attached'
These operations will sum up to 6 transactions.
////

* Each time the application is restarted, the connector cleans up the expired replay IDs in the object store. A replay ID is expired if it was saved more than 24 hours before the cleanup execution.

// This cleanup task will perform 3 transactions on the // Object Store for each topic/channel that was used in // that specific application.
...
* On subsequent uses of this operation, the connector reads the timestamp from the object store. Then it updates the value after it receives the API response.

// Each usage of this operation will execute 2 transactions to interact with the Object Store.
...
redelivery policy, the number of transactions used to interact with the Object Store will vary.
////

For more information about object store versions, see https://docs.mulesoft.com/mule-runtime/4.2/redelivery-policy[Redelivery policy].
