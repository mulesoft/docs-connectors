...
NOTE: The Streaming Channel field of the Subscribe Streaming Channel does not display the Change Events available in the Salesforce environment, because there is no way to retrieve this information through the API. However, subscribing to such a channel is supported by the connector. For example, if you want to subscribe to `All Change Events` channel all you have to do is to use `/data/ChangeEvents` as the channel name to subscribe to. More info about Change Events channels can be found https://developer.salesforce.com/docs/atlas.en-us.change_data_capture.meta/change_data_capture/cdc_subscribe_channels.htm[here]
...
* This operation allows users to invoke a method from an Apex class that is exposed as a REST service. Below there is an example of how the payload for this operation can look like:
...
* The `body` contains the `URLParameters` which is a map containing the parameters to be used for replacing the wildcards in the path of the REST resource described in the Apex Class. For example if the rest resource is described as follows: `@RestResource(urlMapping='/myResource/\*/mySubResource/*')`, then the value of Parameter1 will replace the first `\*` and the value of Parameter2 will replace the second `*`. It's important that the keys are named exactly Parameter followed by a number showing the position of the `*` to be replaced. After the `URLParameters` part, there is the actual content of the body to be sent to the REST resource.

* The `headers` and `cookies` fields describe the headers and cookies to be passed along the HTTP request to the desired service.

* The `queryParameters` field describes the query params to be used and the keys and values in this map have to be accepted by the specified Apex Class.
