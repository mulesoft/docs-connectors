...
+
. Subscribe to the channel through the `Subscribe Channel Listener` operation.
+
...
In XML, you use `subscribe-topic-listener` or `replay-topic-listener` as the trigger:
...
In this XML example, the Replay Topic Listener acts like an inbound endpoint for the Logger message:
...
NOTE: Studio automatically converts the connector to Salesforce (Streaming) mode. Technically, this is still the same connector, but it accesses the Salesforce Streaming API. This means that the only operation the converted connector can perform is *Subscribe Topic Listener* (that is, subscribe to PushTopic).
...
The `replay-channel-listener` input source acts like an inbound endpoint and can be used like this:
