...
Anypoint Connector for Salesforce (Salesforce Connector) provides two operations for obtaining custom event notifications. These notifications pertain to general events that are not tied to Salesforce data changes.
...
+
You can also create a streaming channel through Salesforce or Salesforce https://workbench.developerforce.com/about.php[Workbench].
+
. Subscribe to the channel with the `Subscribe Channel Listener` operation.
+
Salesforce Çonnector converts the custom events in your streaming channel to Mule events and dispatches them to your flows.
...
Each event that travels through your flow contains information about the Salesforce data that has changed, how it changed, and when it changed. The connector parses this information and sends you information that a flow can work with.
...
Before you can receive events for Salesforce changes associated with a topic, you must create a topic (a PushTopic). A PushTopic is a special object in Salesforce that binds a name (in this case, the topic's name) and Salesforce Object Query Language (SOQL) query together. After you create a PushTopic, you can subscribe to it by using its name.

You can use either the Create (`create`) or Publish Topic (`publish-topic`) operation to create a topic. Example of the required fields for these operations:
...
Alternatively, in Salesforce you can create a topic by executing code such as this code from an *Enter Apex Code* window, which is accessible through your system logs:
...
You can subscribe to a topic that was not previously published in Salesforce. However, when the topic is later published, you will not receive notifications regarding that topic unless you resubscribe after the topic is created.
...
In Studio, the *Resume from the Last Replay Id* check box lets you specify an automatic replay of stored events, based on the Replay ID of the last event processed by the connector. This functionality is useful in cases when the connector stopped listening for some reason, such as a server shutdown or dropped connection. If the stored Replay ID is outside the 24-hour retention period, your replay option determines what events to replay.

In the following XML example, the Replay Topic Listener acts like an inbound endpoint for the Logger message:
...
You can use Salesforce Connector as an outbound connector in your flow to push data to Salesforce. To use the connector in this capacity, place Salesforce Connector in your flow at any point after an inbound endpoint.

[[receivedata]]
== Receive Data From Salesforce

You can use Salesforce Connector as an inbound connector without wrapping the connector in a poll scope to stream data from Salesforce into your application. To use the connector in this capacity, place Salesforce Connector at the start of your flow.

Studio automatically converts the connector to Salesforce Streaming mode. Technically, this is still the same connector, but it accesses the Salesforce Streaming API. The converted connector can perform only the  *Subscribe Topic Listener* operation (that is, subscribe to PushTopic).
...
Streaming channels provide notifications to subscribers that are not limited to record-based events. You can use Salesforce Connector to work with Salesforce streaming channels.
...
You can use either the *Create* operation or the exclusive `publish-streaming-channel` operation as follows:
...
After you create a streaming channel, you can start receiving events by subscribing to the channel. The `subscribe-channel-listener` acts like an inbound endpoint and is used as follows:
...
Salesforce lets you push custom events to a specific streaming channel through the REST API. To do this, you can use the Salesforce https://workbench.developerforce.com/about.php[Workbench] or this connector.
...
You can retrieve the channel ID from the response map of the `publish-streaming-channel` operation.

Another way of retrieving the ID of the channel is from the Salesforce page:

. Log in to your Developer Edition organization.
. Under *All Tabs (+)*, select *Streaming Channels*.

If the channel ID field is not visible on the channel list:
...
Salesforce Connector provides the Create and Create Bulk operations for working with the Bulk API.
