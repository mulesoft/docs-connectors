...
You can subscribe to a topic that was not previously published in Salesforce. However, when the topic is published, you will not receive notifications regarding that topic unless you resubscribe to it.
...
Although `CreatedDate` field appears as `dateTime`, the query returns a `String` that represents the date. To use the field as a `dateTime` field, configure it using a Transform Message component.
