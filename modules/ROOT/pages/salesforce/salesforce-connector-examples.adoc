...
The Mule flow sets up the following components:
...
* Transform Message
+
Transforms the data structure and format to produce the output that Anypoint Connector for Salesforce (Salesforce Connector) expects
+
...
* Transform Message
+
Transforms a data structure and format to produce the output that the File Connector endpoint expects
+
* File Connector
+
Records data in a file such as a CSV file, and saves it to a user-defined directory or location.
...
This example shows how to use Salesforce Connector to perform an upsert for an object. The relationships shown in the example must exist in Salesforce.

The example uses the following custom object types:

* `MyCustomObject`
* `MyOtherCustomObject`

When upserting `MyCustomObject`, the POJO sent as input to Salesforce Connector looks like this:
...
* `MyCustomObject` has a relationship to `MyOtherCustomObject`.

* `OtherObject` is the name of the `MyCustomObject` field whose value references the `MyOtherCustomObject` object. The `__r` in the field name indicates that the object has a relationship to another object.
* The value of the `OtherObject_r` field is an object with two fields:
** The `CustomField` field identifies the instance of `MyOtherCustomObject` to reference. In this example, `MyCustomObject` references the instance of `MyOtherCustomObject` where CustomField` has a value of `ABC123`.
** The `type` field has the referenced object type name as a value. In this example, the value is a custom `MyOtherCustomObject` type.
