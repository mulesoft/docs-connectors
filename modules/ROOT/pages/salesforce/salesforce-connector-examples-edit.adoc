...
Salesforce objects can have standard or custom relationships between them. The relationships between the objects are usually one-to-many (parent-child) relationships, but can be any link between two objects residing in Salesforce.

This example shows how to perform an upsert for an object using Salesforce Connector. The example assumes that you:

* Use two custom types, `CustomObject1` and `CustomObject2`, which have a one-to-many relationship between them.
* Create a structure in Salesforce for this relationship.

When upserting `CustomObject1`, the POJO sent as input to the Salesforce connector looks like this:

[source,xml,linenums]
----
{
  ...
  // CustomObject1's fields ...
  CustomObject2__r:
  {
...
* `CustomObject2` is the name of `CustomObject1` field whose value references the `CustomObject2` object. The ` _r` extension indicates that the field is related to another object.
* The value of the CustomObject2__r field is an object with two fields.
** The `CustomField_c` field identifies the instance of `CustomObject2` to reference, in this case, the instance identified by `ABC123`.
** The `type` field has the referenced object type name (CustomObject2) as its value.

The following XML code shows how to upsert these objects:
...
http://www.mulesoft.org/schema/mule/ee/tracking/current/mule-tracking-ee.xsd">
    ...
    <flow name="DirectUpsert" initialState="stopped">
        <poll doc:name="Poll">
            ...
            <set-payload
                value="#[ [ ['Name' :'test'+server.dateTime, 'field_1__c' : 'test', 'CustomObject2_r' : ['CustomField__c' : 'customFieldValue', 'type': 'CustomObject2__c'] ] ] ]"
                doc:name="Set Payload"/>
        </poll>

<sfdc:upsert config-ref="Salesforce__Basic_Authentication" externalIdFieldName="Id"
    type="CustomObjectParent__c" doc:name="Salesforce">
...
[{
  ...
  CustomObjectParent__r: {'CustomField__c' : payload.parentRef, 'type': 'CustomObjectChild__c'}
}]]]></dw:set-payload>
        </dw:transform-message>
        <sfdc:upsert config-ref="Salesforce__Basic_Authentication"
            externalIdFieldName="Id" type="CustomObject1__c" doc:name="Salesforce">
            <sfdc:objects ref="#[payload]"/>
        </sfdc:upsert>
