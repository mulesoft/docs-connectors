= Salesforce Connector Reference - Mule 4
...
| Time Object Store a| ObjectStore |  An ObjectStore instance to use in #_get_updated_objects">SalesforceConnector#getUpdatedObjects] |  |
...
| Allow field truncation support a| Boolean |  If true, truncates field values that are too long, which is the behavior in API versions 14.0 and earlier. <p/> Default is false: no change in behavior. If a string or text area value is too large, the operation fails and the fault code STRING_TOO_LONG is returned. |  false |
...
** ZIP_XML
** ZIP_CSV
** ZIP_JSON |  The only value allowed is CSV. If any other value is selected an exception will be thrown |  CSV |
