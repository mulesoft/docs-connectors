...
Salesforce Composite Connector v2.4
...
| Tls configuration a| <<tls-configuration>> |  Protocol to use for communication. Valid values are HTTP and HTTPS. Default value is HTTP. When using HTTPS the HTTP communication is going to be secured using TLS / SSL. If HTTPS was configured as protocol then the user needs to configure at least the keystore in the tls:context child element of this listener-config. |  5 |
|===
...
|===
| Field | Type | Description | Default Value | Required
| Trust Store Configuration a| <<trust-store-configuration>> | Contains the parameters that needs to be configured for trust store configuration. |  |
| Key Store Configuration a| <<key-store-configuration>> | Contains the parameters that needs to be configured for key store configuration. |  |
| Advanced a| <<advanced-tls-configuration>> | Contains advanced parameters to configure Tls. |  |
|===
...
|===
| Field | Type | Description | Default Value | Required
| Path a| String | Defines the location (which will be resolved relative to the current classpath and file system, if possible) where the trust store to use should be looked for, requires password too. |  |
| Password a| String | Defines the password to access the trust store defined in the path. |  |
...
| Defines the type of the trust store (such as jks, jceks or pkcs12). | JKS |
| Algorithm a| String | Defines the trust store algorithm. |  |
| Insecure a| Boolean | Defines whether the trust store should be insecure, meaning no certificate validations should be performed. | False |
...
| Defines the type of the trust store (such as jks, jceks or pkcs12). | JKS |
| Path a| String | Defines the location (which will be resolved relative to the current classpath and file system, if possible) where the key store to use should be looked for. |  |
| Alias a| String | Defines the alias of the key to use when the key store contains many private keys. By default, the first key in the file will be used. |  |
| Key Password a| String| Defines the password used to protect the private key. |  |
| Password a| String| Defines the password to access the key store defined in the path. |  |
| Algorithm a| String | Defines the trust store algorithm. |  |
|===
...
|===
| Field | Type | Description | Default Value | Required
| Enable Protocols a| String | Defines the enabled TLS protocols, which must be a subset of the global enabled ones. | By default, those global ones will be used |
| Enable Chiper Suites a| String | Defines the enabled TLS cipher suites, which must be a subset of the global enabled ones. | By default, those global ones will be used |
|===
