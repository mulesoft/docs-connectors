...
+
Note that you can also create a streaming channel through the Salesforce portal or through Workbench.
+
...
`<salesforce:publish-topic name="AccountUpdates" query="SELECT Id, Name FROM Account"/>`
...
<flow name="accountUpdatesSubscription">
    <!-- INBOUND ENDPOINT -->
    <salesforce:subscribe-topic topic="AccountUpdates"/>
    <!-- REST OF YOUR FLOW -->
    <logger level="INFO" message="Received an event for Salesforce Object ID #[map-payload:Id]"/>
</flow>
----

NOTE: When subscribing to a topic that was not previously published in Salesforce, the subscription is successful. When the topic is later published, the user who is already subscribed to it does not receive notifications regarding that topic. The user has to resubscribe after the topic is created.
...
<flow name="accountUpdatesReplay">
    <!-- INBOUND ENDPOINT -->
    <salesforce:replay-topic topic="AccountUpdates" replayId="1" replayOption="ALL" autoReplay="true"/>
    <!-- REST OF YOUR FLOW -->
...
Salesforce connector: Listens to notifications on a topic or a streaming channel and feeds the data into the flow.
...
<flow name="notificationsChannelSubscription">
  <!-- INBOUND ENDPOINT -->
  <salesforce:subscribe-streaming-channel streamingChannel="/u/TestStreaming"/>
  <!-- REST OF YOUR FLOW -->
...
<flow name="flowStreamingChannelReplay">
    <!-- INBOUND ENDPOINT -->
    <salesforce:replay-streaming-channel streamingChannel="/u/Notifications" replayId="1" replayOption="ALL"/>
    <!-- REST OF YOUR FLOW -->
    <logger level="INFO" message="Replayed events: #[payload]"/>
</flow>
----
...
<flow name="flowPushGenericEvent">
    <!-- INBOUND ENDPOINT -->
    <salesforce:push-generic-event channelId="0M6j0000000KyjBCAS">
      <salesforce:events>
            <salesforce:event payload="Notification message text"/>
        </salesforce:events>
