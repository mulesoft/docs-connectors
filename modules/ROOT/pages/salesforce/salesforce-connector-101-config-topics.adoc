...
When a Mule app starts for the first time, the connector creates an object store that will be used to save data regarding the messages that were processed successfully or that failed. the information stored consists into a number representing the lowest replay id for which the processing failed and a set of replay ids representing the messages that were  processed successfully. These two structures are needed to ensure that no message is processed twice and also that when the application is restarted it can process again the messages that failed.

For each message that comes through a topic or streaming channel to which the connector is subscribed, the connector updates in the object store information using up to four transactions.
