...
== Log Requests and Responses [[log-requests-responses]]

To log requests and responses when using the connector, add the following line to the `Loggers` element of the `log4j2.xml` configuration file for the Mule app:
...
* If you’re using Mule from the command line to run the app, the app log is visible in your OS console.

You can also view the application log in this default location, unless the log file path was customized in the app’s log file (log4j2.xml):

MULE_HOME/logs/<app-name>.log

For more information about the application log, see xref:4.2@:docs-mule-runtime::logging-in-mule.adoc[Configuring logging].
