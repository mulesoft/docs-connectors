...
Participating organizations (POs) can use the APIs to add providers to roster in CAQH ProView. The MuleSoft CAQH connector simplifies access to
...
. Configure the connector's global element fo the connector.
...
. In the connector configuration body, set the connector body in the *General* section to `payload`. Set the *Target Value* in the Advanced section to `#[payload]`.
. Use another Transform Message component to add some wait time because it takes a few minutes to update the provider in CAQH.
+
[source, yaml, linenums]
----
%dw 2.0
