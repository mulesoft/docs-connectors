...
CAQH ProView is a web-based solution that is used by many providers to self-report and share wide-ranging demographic and professional information with healthcare organizations. CAQH DirectAssure ensures the accuracy of provider directories. It works in tandem with CAQH ProView, enabling providers to regularly update their directory information in one place and share it with participating organizations.
...
The participating organisations (POs) can use this APIs to roster providers in CAQH ProView. The MuleSoft CAQH connector facilitates simplified access to
...
. Create a Mule application and set the connector configuration properties in the configuration file in the `src/main/resources`. Here is an example of such a configuration file properties:
+
...
. For the purpose of this demo, use the following query to get contact from Salesforce.
+
[source, sql, linenums]
----
