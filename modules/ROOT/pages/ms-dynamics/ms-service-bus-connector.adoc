...
+
This is an example of an implementation of the SasTokenProvider:
+
----
package sample;
...
----
+
A Spring bean reference implementing the `com.mulesoft.connectors.microsoft.servicebus.extension.api.SharedAccessSignatureProvider` interface (beans.xml).
+
----
...
----
+
In the Mule application's main XML file, a reference to the Spring configuration file where the new bean is defined must be added. Also, the `tokenProvider` attribute of the connector's configuration, must contain the name given to the bean (the one that implements the `SharedAccessSignatureProvider` interface).
+
----
