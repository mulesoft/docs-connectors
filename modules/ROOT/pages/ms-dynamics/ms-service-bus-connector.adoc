...
This use case gets the list of existing queues in a specific namespace. With the list of queues obtained, build a Json to return in response to the request.
...
The following use case contains the flows necessary to send and receive messages for both topics and queues. The functionality of each one is detailed below:

- **load-http-form-flow**: this flow is in charge of offering a web form for entering the parameters, mainly necessary to send the message to the queue and / or topics (the content of the message).
- **topic-endpoint-flow**: this flow is in charge of receiving a message through the load-http-form-flow, the message is stored as the payload, properly converted to a java object and sent to the destination topic.
- **queue-endpoint-flow**: this flow is responsible for receiving a message through the load-http-form-flow, the message is stored as the payload, properly converted to a java object and sent to the destination queue.
- **queue-receive-flow**: This flow is responsible for listening and receiving messages from a specified queue for processing. In this case, the processing consists of receiving it and logging the message.
- **topic-receive-flow**: This flow is responsible for listening and receiving messages from a specified topic for processing. In this case, the processing consists of receiving it and logging the message.
...
The following use case contains the flows necessary to create topics, subscriptions and rules. The functionality of each one is detailed below:

- **service-bus-management-demoFlow**: This flow is in charge of creating topics, for this the following steps are performed: 
  * The topic description is created, with the desired configuration (it is stored as payload). 
  * The topic is created with the stored configuration from the previous step and the name of the topic to be created is obtained from the configured variable ** topic.name **. 
  * A Json is built with part of the information of the topic created successfully in the previous step (the main values are indicated as id, author, etc). 
  * The Json created in the previous step is logged.
- **service-bus-management-demoFlow1**: This flow is in charge of creating subscriptions in a specified topic, for this the following steps are performed: 
  * The subscription description is created, with the desired configuration (it is stored as payload). 
  * The subscription is created with the stored configuration from the previous step. The name of the subscription to create and the existing topic name is obtained from the configured variables: **suscription.name** and **topic.name**. 
  * A Json is built with the subscription information successfully created in the previous step (the main values are indicated such as id, title, etc). 
  * The Json created in the previous step is logged.
- **service-bus-management-demoFlow2**: This flow is in charge of creating rules (for an existing subscription in a topic), for this the following steps are performed: 
  * The rule description is created, with the desired configuration (it is stored as payload). 
  * The rule is created with the stored configuration from the previous step. The name of the rule to be created, the existing subscription and the existing topic are obtained from the configured variables: **rule.name**, **suscription.name** and **topic.name**. 
  * A Json is built with the information of the rule created successfully in the previous step (the main values are indicated as id, link, etc). 
  * The Json created in the previous step is logged.
