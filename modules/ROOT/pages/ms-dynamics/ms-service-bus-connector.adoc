...
+
The following example shows how to configure a SAS token list:
+
[source,xml,linenums]
----
...
+
The following example shows an implementation of the SAS token provider:
+
----
package sample;

import com.mulesoft.connectors.microsoft.servicebus.extension.api.SharedAccessSignatureProvider;

public class SharedAccessSignatureProviderImp implements SharedAccessSignatureProvider{

        @Override
	public String getSASToken(String uri) throws Exception
	{
		//Java code here
	}
...
----
+
This code shows a Spring bean reference that implements the `com.mulesoft.connectors.microsoft.servicebus.extension.api.SharedAccessSignatureProvider` interface (`beans.xml`):
+
----
<beans xmlns="http://www.springframework.org/schema/beans"
    ...
    <bean id="customSharedAccessSignatureProviderImp" scope="singleton" class="sample.SharedAccessSignatureProviderImp"/>

</beans>
----
+
In the Mule application's main XML file, you must add a reference to the Spring configuration file where the new bean is defined. Also, the `tokenProvider` attribute of the connector's configuration must contain the name given to the bean that implements the `SharedAccessSignatureProvider` interface.
+
----
<spring:config name="Spring_Config" doc:name="Spring Config" doc:id="5558e28d-8b2c-4462-a8b5-9deb762af2ae" files="beans.xml"/>

<servicebus:shared-access-signature-connection namespace="mulesbft" tokenProvider="customSharedAccessSignatureProviderImp" ...>
<!-- Other settings -->
</servicebus:shared-access-signature-connection>
----
+
