...
Microsoft Service Bus Connector has one connector-specific source named `Listener`. To configure this source in Flow Designer:

. Click the trigger card for the Listener trigger.
+
image::ms-dynamics/ms-service-listener-source.png[Listener]
+
. Select a value for the *Destination Type* field to indicate whether the trigger listens for new or modified queues, topics, or custom destination types.
...
|Destination | Name of the queue or topic from which messages are retrieved *I think this description is incorrect. Can you tell me the correct one?*
|Subscription | Name of the subscription to use when subscribing to topics. If the destination is a queue, select NONE.
|Topic Path |Name of the topic from which messages are retrieved
|Subscription Path | Name of the subscription to use for retrieving messages from the topic
|===
