...
. Select a value for the *Destination Type* field to indicate whether the trigger listens for new or modified queues, topics, or a custom destination type.
. Enter values for the required parameters:
+
[%header,cols="30s,70a"]
|===
|Field |Description
|Source Type | Indicates whether to retrieve messages from a queue or topic *Is a custom destiniation type also a choice here?*
|Destination | Name of the destination queue or topic from which messages are retrieved *Is a custom destiniation type also a choice here?*
