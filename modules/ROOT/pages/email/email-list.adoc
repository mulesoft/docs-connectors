...
Both the POP3 and IMAP configurations contain a `list` operation. This section covers both and explains their differences. Though the following examples might use `list-pop3` or `list-imap`, all of them apply to both configurations unless stated otherwise.
...
The IMAP and POP3 configurations provide a secured connection type to work over the secured versions of the protocols (POP3S and IMAPS). These connection types allow you to configure a TLS context that enables SSL/TLS encryption.
...
The previous examples return all the emails in the configured mailbox servers that reside in the INBOX folder.
...
When listing emails, you can filter emails that match certain criteria using the `<email:pop3-matcher>` and `<email:imap-matcher>` elements. These elements define the criteria that can be used to process an email.
...
The previous example declares an inner, non-reusable matcher that is proprietary to a particular component. However, matchers can be declared as a reusable element (as a named top-level element) like the following `pop3-matcher`:
...
Email Connector provides a parameter to specify a strategy for how to name attachments for the List and On New Email operations. This parameter is set at the configuration level, but can be overridden at the operation
level. 

See xref:email/email-trigger.adoc[Naming Strategy For Attachments] for more information and examples.

== See Also

* xref:email/email-send.adoc[Sending Emails with the Email Connector]
