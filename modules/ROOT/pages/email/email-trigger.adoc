...
Email Connector provides an Attachment Naming Strategy parameter that enables you to specify a strategy for how to name attachments for the List and On New Email operations. 

This parameter is set at the configuration level, but can be overridden at the operation level. 
...
* `NAME_HEADERS` + 
When the value is set to `NAME_HEADERS`, the connector first tries to obtain the name from the `filename` field in the `Content-Disposition` (the same as when using `NAME`). If that fails, the connector looks for the `name` header in the `Content-Type` line of the attachment. If the `name` header is not present or is empty, then the attachment is named "Unnamed".
* `NAME_HEADERS_SUBJECT` +
When the value is set to `NAME_HEADERS_SUBJECT`, the connector tries the previous strategies (first it looks for the `filename` field and then for the `name` header). If both fail, then the connector checks if the attachment itself is an email (a nested message) and looks for the `Subject` of the email. If the subject is present and is not empty, then the attachment is named after the subject. Otherwise, the attachment is named "Unnamed".
