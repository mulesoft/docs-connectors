...
You can use Transport Layer Security (TLS) and configure email by providing a key store with your certificate. You can also enable 2-way authentication by providing a trust store. For details, see xref:4.2@mule-runtime::tls-configuration.adoc[Configure TLS with Keystores and Truststores].
