...
You can send multiple attachments of different media types, for example, you might want to send a json element,
then a text element and also a file perhaps. To do this simply add them to your attachments element like this.
...
The Send operation will return a message containing an HttpRequestAttributes object in the Message attributes section containing
all the information used to send the email to the SMTP server (Request Path, Headers, etc.). Whatever payload was set
immediately before Send was performed will be the output payload of the operation as well.
