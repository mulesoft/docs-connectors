...
In the following example, a Mule application sends a message to a phone number, and then redacts it. This example uses variables for field values. You can provide the values for each variable in a properties file. To use a properties file, place the file at `/src/main/resources` and use the tag `<configuration-properties>` to point to the file. For more information, see xref:mule-runtime::configuring-properties.adoc#properties-files[Properties Files].
...
---
{
    ...
    From: "${fromNumber}",
    To: "+" ++ attributes.uriParams.toNumber!
}
----
+
