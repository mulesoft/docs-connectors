...
<sns:publish config-ref="Amazon_SNS_configuration"
  doc:name="Publish"
  doc:id="b0c5158d-6f3f-44be-9153-3c6d9dd870f2" >
<sns:publish-details message="Hello World!"
  subject="Testing publish to queue"
...
. In the Mule Palette, search for *SQS*, select the *Receivemessages* operation, and drag it next to the left of the *Flow* component.
...
|Queue Name |Name of the queue. |`${sqs.queueName}`
|Region Endpoint |Set the queue reqion| user can select the Region by using dropdown.
|Default Global Queue URL |Set the queue URL |`${sqs.queueUrl}`. If you entered a value for Queue Name, you can leave this field blank.
|===
+
** Your configuration should look like this:
...
----
+
Make sure SQS-Queue that you mentioned in configuration is subscribed to SNS-Topic.
+
