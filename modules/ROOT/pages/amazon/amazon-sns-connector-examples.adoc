...
image::amazon/amazon-sns-use-case-flow.png[send-messages-to-SQS]
...
+
image::amazon/amazon-sns-select-listener.png[select-listener]
+
...
+
image::amazon/amazon-sns-http-properties.png[http-properties]
+
. In the Mule Palette, search for *SNS* and select the *Publish* operation:
+
image::amazon/amazon-sns-publish-selection.png[publish-selection]
+
...
|*Access Key* |Alphanumeric text string that uniquely identifies the user who owns the account. |`AKIAIOSFODNN7EXAMPLE` or another access key.
|*Secret Key* |Key that plays the role of a password. |`wJalrXUtnFEMI/K7MDENG/bPxRfiCYEXAMPLEKEY` or another secret key.
|*Region Endpoint* |Queue reqion. | Select the queue region.
|===
+
For example:
+
image::amazon/amazon-sns-global-config.png[sns-global-configuration]
+
...
For example:
+
image::amazon/amazon-sns-publish-message.png[sns-publish-message-props]
+
. Select the *Configuration XML* tab to view the corresponding XML:
+
[source,xml]
...
<sns:publish doc:name="Publish"
  ...
  topicArn="arn:aws:sns:us-east-3:123456789022:MyTopic"
...
In the same same example, add a *Logger* component to display the data processed by the *Publish* operation on the Mule console.

. In the Mule Palette, search for *Logger*, and drag the component to the right of the Amazon SNS Connector (*Publish*).
...
|===
|Parameter |Description|Value
|Name |Enter a name for the configuration to reference it. |`Amazon_SNS_Configuration` or another name.
|Access Key |Alphanumeric text string that uniquely identifies the user who owns the account.
|`AKIAIOSFODNN7EXAMPLE` or another access key.
|Secret Key |Key that plays the role of a password. |`wJalrXUtnFEMI/K7MDENG/bPxRfiCYEXAMPLEKEY` or another secret key.
|Region Endpoint |Queue region. | Select the queue region.
|===
+
For example:
+
image::amazon/amazon-sns-sqs-global-config.png[sns-sqs-config]
+
Select the *Configuration XML* tab to view the corresponding XML:
+
[source,xml]
...
|Parameter |Value
2+|Basic Settings
|Display Name |`Amazon SQS (Streaming)`, or another name. |Connector configuration*|`Amazon_SQS_Configuration` (the reference name to the global element you created).
|Queue url | `\https://sqs.us-east-1.amazonaws.com/queue_url` or another URL.
...
<sqs:receivemessages
  ...
  queueUrl="https://sqs.us-east-1.amazonaws.com/queue_url"
...
<mule xmlns:sqs="http://www.mulesoft.org/schema/mule/sqs"
  ...
  http://www.mulesoft.org/schema/mule/sqs/current/mule-sqs.xsd">
	...
	<sns:config name="Amazon_SNS_configuration"
  doc:name="Amazon SNS configuration" >
	<sns:basic-connection accessKey="AKIAIOSFODNN7EXAMPLE"
  secretKey="wJalrXUtnFEMI/K7MDENG/bPxRfiCYEXAMPLEKEY"
  region="us-east-1"/>
	</sns:config>
	<sqs:config name="Amazon_SQS_Configuration"
  doc:name="Amazon SQS Configuration" >
	<sqs:basic-connection region="us_east_1"
  accessKey="AKIAIOSFODNN7EXAMPLE"
  secretKey="wJalrXUtnFEMI/K7MDENG/bPxRfiCYEXAMPLEKEY"/>
	</sqs:config>
  ...
  config-ref="HTTP_Listener_config" path="/"/>
	<sns:publish doc:name="Publish"
  topicArn="arn:aws:sns:us-east-3:123456789022:MyTopic" messageStructure="JSON"
  config-ref="Amazon_SNS_configuration">
	<sns:message ><![CDATA[#[`Hello World`]]]></sns:message>
	</sns:publish>
  ...
  queueUrl="https://sqs.us-east-1.amazonaws.com/queue_url"
  config-ref="Amazon_SQS_Configuration"/>
	<logger level="INFO" doc:name="Display Message"  message="Received Message: #[payload]"/>
  </flow>
</mule>
----
