...
image::amazon/amazon-sns-use-case-flow.png[send-messages-to-SQS,width=40%]
...
+
image::amazon/amazon-sns-publish-selection.png[publish-selection,width=50%]
+
...
+
image::amazon/amazon-sns-global-config.png[sns-global-configuration,width=70%]
+
...
<sns:publish doc:name="Publish"
  topicArn="arn:aws:sqs:us-east-1:123456789012:TopicName"
...
----
+
Please note, the `default` JSON key has a special meaning for SQS and specifies that we want its value to be send as a message to every subscriber regardless of used protocol. User is required to specify at least this key and optionally other top-level keys that define the message we want to send to a specific transport protocol (e.g., "http", "sqs"). 
For more information please see `MessageStructure` in https://docs.aws.amazon.com/sns/latest/api/API_Publish.html#API_Publish_RequestParameters.
...
+
image::amazon/amazon-sns-sqs-global-config.png[sns-sqs-config,width=70%]
+
. Select the *Configuration XML* tab to view the corresponding XML:
+
[source,xml]
