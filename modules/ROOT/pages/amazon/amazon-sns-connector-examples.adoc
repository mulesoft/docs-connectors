...
|*Name* |Name used to reference the configuration. |<Config_Name>
|*Access Key* |Alphanumeric text string that uniquely identifies the user who owns the account. |<Config_Access_Key>
|*Secret Key* |Key that plays the role of a password. |<Config_Secret_Key>
...
<sns:config name="Amazon_SNS_configuration"
  doc:name="Amazon SNS configuration" >
    <sns:basic-connection
      accessKey="ZZZ9kc4luuvCC"
    secretKey="GGGOdVkXe16L" />
</sns:config>
----
+
...
|Access Key |Alphanumeric text string that uniquely identifies the user who owns the account. |Config_Access_Key
|Secret Key |Key that plays the role of a password. |<Config_Secret_Key>
|Default Global Queue URL |Set the queue URL |<Config_Queue_URL>.
...
<sqs:basic-connection accessKey="AAAJSIIXXJLRPC632UV"
  secretKey="ZZZo7kc4luuvsCjDnUjftK" />
</sqs:config>
----
+
...
Paste this code into your XML Editor to quickly load the flow for this example use case into your Mule application. You might need to change some values to reflect your environment.
+
[source,xml]
----
...
<mule xmlns:sqs="http://www.mulesoft.org/schema/mule/sqs"
  ...
  messageStructure="JSON">
   ...
   message="`#[payload]`"/>
	...
	<sqs:receivemessages doc:name="Amazon SQS (Streaming)"
  config-ref="Amazon_SQS_Configuration"
  queueUrl="https://sqs.us-east-1.amazonaws.com/748495737711/sns-test-34c7c15b-c89d-4f46-99fe-b79f19a14e02"/>
	<logger level="INFO" doc:name="Display Message"
  message="Received Message: #[payload]"/>
	</flow>
</mule>
