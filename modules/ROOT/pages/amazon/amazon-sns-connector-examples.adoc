...
This topic contains the following common use cases:
...
. Click the plus sign next to the *Connector Configuration* field to add a Global Element, and configure the Global Element as follows:
+
[%header%autowidth.spread]
|===
...
. Add a *Logger scope* after the Amazon SNS connector to print the data processed by the Publish operation in the Mule Console. Configure the Logger as follows:
+
[%header%autowidth.spread]
|===
|Parameter |Value
...
In the same example, you can add a second flow to receive the messages published by SNS:
