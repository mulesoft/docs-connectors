...
To use the Amazon S3 connector in your Mule application, configure a global Amazon S3 element that can be used by all Amazon S3 connectors in your application.
...
|Name |Name for the configuration to reference the connector instance.
...
. If you are:
+
* Using the default Amazon S3 storage, leave the default entries for *Proxy* and the *Advanced* tabs.
...
An example of S3 compatible storage is the https://github.com/minio/[Minio Project]. For instance, if you configure it locally you might need to set the S3 Compatible Storage URL to `+http://127.0.0.1:9000+`.

== Configure an Input Source

Configure an input source for this connector such as a connector operation, HTTP Listener, Scheduler.
