...
The `pom.xml` is a Maven file that Anypoint Studio generates that contains dependencies for a Mule app. When you code a Mule app manually, include this XML snippet in your `pom.xml` file to enable access to the Amazon S3 connector. Mule translates the `LATEST` dependency keyword to the current version.
