...
. Select the *Recieve messages* operation to use it as a source. (You can also use the HTTP Connector or Scheduler component as a source.)
...
+
For the *Get Queue Attributes* operation, the required fields are as follows. Depending on the trigger you used, these fields might already have values.
+
[%header,cols="30s,70a",width=80%]
|===
|Parameter |Description
...
|===
+
. If you added or changed values for the listed fields, Click *Test* to confirm that the parameters of your configuration are accurate, and that Mule is able to successfully connect to your instance of Amazon SQS.
