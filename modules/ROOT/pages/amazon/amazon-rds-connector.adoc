...
* Anypoint Studio version 7.0 (or higher) or access to Anypoint Design Center.
...
The changes from version *1.2.0* to *1.3.0* include the following:

* Added TLS configuration into the connector
* Upgraded `aws-java-sdk` from 1.11.515 to 1.11.649
* Upgraded `aws-connector-commons` from 1.3.2 to 2.2.0
...
To use the Amazon RDS connector in your Mule application, configure a global Amazon RDS element that all the Amazon RDS connectors in your project can use.
...
|Region Endpoint |Region to use for the Amazon RDS client. When not selected defaults to 'us-east-1'.
|Access Key |Alphanumeric text string that uniquely identifies the user who owns the account.
|Secret Key |Key that plays the role of a password.
|Try Default AWS Credentials Provider Chain |Checkbox that controls whether to use temporary credentials.
|===
+
...
Ensure that you included the Amazon RDS namespaces in your configuration file.
...
To configure an Amazon RDS connector in your application, create a global Amazon RDS configuration outside and above your flows, using the following global configuration code:
...
The Amazon RDS connector supports the following operations:
