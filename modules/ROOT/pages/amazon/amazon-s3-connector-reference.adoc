...
| Socket Timeout a| Number |  +++The amount of time to wait (in milliseconds) for data to be transferred over an established, open connection before the connector gives up and times out. A value of 0 means infinity and is not recommended.+++ |  +++50000+++ | 
...
| Connection Timeout a| Number |  +++The amount of time to wait (in milliseconds) when initially establishing a connection before giving up and timing out. A value of 0 means infinity, and is not recommended.+++ |  +++50000+++ | 
| Socket Timeout a| Number |  +++The amount of time to wait (in milliseconds) for data to be transferred over an established, open connection before the connection is timed out. A value of 0 means infinity,and is not recommended.+++ |  +++50000+++ | 
| Access Key a| String |  +++The access key provided by Amazon.+++ |  | x
| Secret Key a| String |  +++The secret key provided by Amazon.+++ |  | x
| Try Default AWSCredentials Provider Chain a| Boolean |  +++Set this field to true to obtain credentials from AWS environment, See: https://docs.aws.amazon.com/sdk-for-java/v2/developer-guide/credentials.html+++ |  +++false+++ | 
| Max Connections a| Number |  +++Sets the maximum number of allowed open HTTP connections.+++ |  | 
| Region Endpoint a| String |  +++Set the topic region endpoint.+++ |  +++US_EAST_1+++ | 
| Reconnection a| <<Reconnection>> |  +++When the application is deployed, a connectivity test is performed on all connectors. If set to true, deployment fails if the test doesn't pass after exhausting the associated reconnection strategy+++ |  | 
| Host a| String |  +++The optional proxy host+++ |  | 
