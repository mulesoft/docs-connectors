...
If no destination bucket is specified, the same bucket as the source bucket is used (local copy). The estimation object's `Content-Type` HTTP header is set with the specified value, and if it is passed null, then the default MIME type `application/octet-stream` is set.
...
| Destination Bucket Name a| String |  The destination object's bucket. If none is provided, object is copied within the same bucket. |  |
...
| Name | Type | Description | Default Value | Required
| Configuration | String | The name of the configuration to use. | | x
| Bucket Name a| String |  The unique name for the bucket to create. |  | x
...
| Content Language a| String |  The Content-Language HTTP header that describes the natural language(s) of the intended audience for the enclosed entity. |  |
