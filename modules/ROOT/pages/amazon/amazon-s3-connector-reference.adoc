...
Copies a source object to a new destination. To copy an object, the caller's account must have read access to the source object and write access to the destination bucket. By default, all object metadata for the source object is copied to the new destination object, unless new object metadata in the specified is provided. The AccessControlList is not copied to the new object, and, unless another ACL is specified, PRIVATE is assumed. If no destination bucket is specified, the same bucket as the source bucket is used: local copy. The estimation object's Content-Type http header would be set with the specified value and if it is passed null, then the default MIME type 'application/octet-stream' is set.
...
| Destination Bucket Name a| String |  The destination object's bucket. If none provided, object is copied within the same bucket. |  |
...
| Destination User Metadata a| Object |  The new metadata of the destination object. If specified, overrides that copied from the source object |  |
...
| Force a| Boolean |  (Optional) `true` if the bucket must be deleted even if it is not empty, false if the operation fails in this scenario. |  false |
| Reconnection Strategy a| * <<reconnect>>
* <<reconnect-forever>> |  A retry strategy in case of connectivity errors. |  |
|===
...
Deletes a given object. Only the owner of the bucket containing the version can perform this operation. If a version is specified, versioning must be enabled, and once deleted, there is no method to restore such version. Otherwise, once deleted, the object can only be restored if versioning was enabled when the object was deleted. If you attempt to delete an object that does not exist, Amazon S3 returns a success message instead of an error message.
...
Return the notification configuration of a bucket. Currently, only the topic configuration is supported.
...
| Bucket Name a| String |  The object's bucket. |  | x
| Key a| String |  The object's key. |  | x
| Version Id a| String |  The specific version of the object to get its contents, if versioning is enabled. |  |
| Modified Since a| DateTime |  The modified constraint that restricts this request to executing only if the object has been modified after the specified date. |  |
| Unmodified Since a| DateTime |  The unmodified constraint that restricts this request to executing only if the object has not been modified after this date. |  |
...
Lazily lists all object versions for a given bucket that has versioning enabled. Because S3 does not limit in any way the number of objects, the list can retrieve an arbitrary amount of object versions, and might need to perform extra calls to the API while it is iterated.
...
| Configuration | String | The name of the configuration to use. | | x
| Bucket Name a| String |  The target bucket's name. |  | x
| Prefix a| String |  optional parameter restricting the response to keys which begin with the specified prefix. |  |
| Key Marker a| String |  Where in the sorted list of all versions in the specified bucket to begin returning results. Results are always ordered first alphabetically and then from most recent version to least recent version. |  |
| Version Id Marker a| String |  Where in the sorted list of all versions in the specified bucket to begin returning results. Results are always ordered first alphabetically and then from most recent version to least recent version. |  |
...
| Max Results a| Number |  The maximum number of results to return per page. If not specified, S3 will limit the number of returned results per page. |  |
| Encoding Type a| Enumeration, one of:
