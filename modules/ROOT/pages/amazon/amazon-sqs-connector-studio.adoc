...
* Use the Exchange button in the Studio taskbar.

* Use the Exchange button in the Mule Palette.
...
=== Install the Connector from the Mule Palette

. In Studio, create a Mule project.
. In the Mule Palette, click *(X) Search in Exchange*.
. In *Add Modules to Project*, type the `Amazon SQS` in the search field.
