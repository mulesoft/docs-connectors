...
|*Try Default AWS Credentials Provider Chain* |Checkbox that controls whether temporary credentials should be use.
|*Region Endpoint*| Topic region endpoint
|*Globally Defined Default Topic Arn* |Topic ARN to test the connectivity
|===
+
For simpler maintenance and better reuse of your project, instead of providing values for *Name*, *Access Key*, and *Secret Key*, create a configuration that contains these values and place it in the  `src` folder of your project file.
+
. Retain the default entries for the *Proxy* and *Advanced* tabs.
