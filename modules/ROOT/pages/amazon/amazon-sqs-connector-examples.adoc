...
image::amazonb/amazon-sqs-send-message.png[send-message]
...
In the *Get approximate number of receive messages* configuration, click *+* next to the *Connector configuration* field to add a global element.
. Configure the global element as follows:
+
[%header%autowidth.spread]
|===
...
For example:
+
image::amazon/amazon-sqs-get-number-global-config.png
+
. Configure the Following fields on the properties window:
[%header%autowidth.spread]
|===
...
image::amazon/amazon-sqs-demo-get-message-count.png[get-message-count]
