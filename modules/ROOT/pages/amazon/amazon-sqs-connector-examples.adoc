...
Before you try the example, access Anypoint Studio, and check whether the Mule Palette displays entries for Amazon SNS and Amazon SQS. If not, follow the instructions in xref:amazon/amazon-sqs-connector-studio.adoc[Amazon SQS Studio Configuration] to install both connectors.

== Create a Flow to Send a Message

Start the first flow by sending a message to the queue:
...
Create another flow to receive messages and log them before deleting them from the queue.
...
|Field |Value
|Display Name |Name for the connector operation.
|Connector Configuration |Global configuration you created previously for Amazon SQS Connector
