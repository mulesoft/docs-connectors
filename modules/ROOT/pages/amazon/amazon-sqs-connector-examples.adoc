...
== Example
//  I SUGGEST OMITTING THIS SUBTITLE, because the topic set already has a title (Amazon SQS Examples - Mule 4) and because this subtitle is at the same level as the two subtopics that it contains. Also, the title "Example" is not useful, but the two subtitles "Create a Flow..." are. //

This example sends a message with metadata to an Amazon SQS queue and then receives it from the queue. The example is split into the two flows:

image::amazon/amazon-sqs-message-flow.png[sqs-message-flow]

. The first flow sends a message with metadata and then gets the count of the messages in the queue to validate that the message was sent.
//  LIST ITEMS SHOULD BE ONE SENTENCE MAX. Additional statements, if necessary, belong in a paragraph beneath the item.  GLOBAL.  //
. The second flow receives the message and logs the message body.

This example uses variables for some field values. You can either replace the variables with actual values in the code, or you can provide the actual values for each variable in the `src/main/resources/mule-artifact.properties` file.

Before you try the example, access Anypoint Studio and verify that Mule Palette displays entries for Amazon SNS and Amazon SQS. If not, follow the instructions in xref:amazon/amazon-sqs-connector-studio.adoc[Amazon SQS Studio Configuration] to install both connectors before trying this example.
//  BECAUSE THIS IS A PREREQUISTE TO THE ENTIRE EXAMPLE, consider moving this text to the very beginning of this introduction, or creating an actual Prerequisites subheading.  //

== Create a Flow to Send a Message

Start the example by creating a flow that sends a message to the queue. Creating the flow involves creating a new Mule project and configuring the Listener operation, adding a transform message, adding and configuring an SQS Send Message operation, adding a Logger component to display the response, obtaining the number of messages in the queue, and adding a Logger to display that number.
//  I SUGGEST ADDING THIS TEXT TO PREPARE THE AUDIENCE FOR THE SUBTOPICS WITHIN THIS CONTAINER TOPIC. This is one of the GLOBAL requirements for a topic set intro text, along with addressing the audience as "you" and telling "you" exactly WHY they want or need the information in this topic set.  //
//  The following steps really should have its own subtitle, because it is not the entire process identified by the title preceding it.  //
...
// IS THERE ANY SORT OF TRANSITION SNIPPET THAT WOULD ADD VALUE here and serve as a leadin to the first step?  //
...
// DITTO re. useful transition snippet?  //

. In Mule Palette, search for *SQS* and select the *Send message* operation:
+
...
. In Mule Palette, search for *Amazon SQS*.
...
. In the Mule Palette, search for *SQS* and select the *Receive messages* operation:
+
...
// I WONDER IF WE SHOULD NAME THIS "EXAMPLE" CODE RATHER THAN "DEMO" CODE, to avoid confusion. I also wonder if this should not be included BEFORE the steps of the example that uses it? No mention has been made of it as a prerequisite; it could join the other prerequisite identified above.  //
