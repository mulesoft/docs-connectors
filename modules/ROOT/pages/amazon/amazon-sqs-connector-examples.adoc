...
. Drag an *HTTP Connector Listener* operation onto the canvas.
. Double-click the *Listener* to open the properties editor.
+
image::amazon/amazon-sqs-http-config.png[]
+
. Set the *Path* field to `/`.
. Click *+* next to the *Connector configuration* field to add an HTTP Listener Configuration global element.
. Leave the default values for the fields:
+
...
. In the Mule Palette, search for *Transform Message* and drag the component next to the *Listener*.
. Double-click the *Transform Message* component.
. Add this metadata data to the *Output* section:
+
...
. Add values for the *Access Key*, *Secret Key*, and *Default Queue URL*.
/// The original text mentioned Queue name, but I don't see that
/// now.
