...
Before you try the example, access Studio, and check to see if the Mule Palette displays entries for Amazon SNS and Amazon SQS. If not, follow the instructions in xref:amazon/amazon-sqs-connector-studio.adoc[Amazon SQS Studio Configuration] to install both connectors.
...
. In the *Listener* configuration, click *+* to next to the *Connector configuration* field to add a global element.
. Accept the defaults.
...
|Message |`payload`
|Queue url |Amazon SQS queue URL. If provided, the value of this field takes precedence over the value of the *Default Global Queue URL* field on the *Default Global Queue URL* screen.
|===
+
For example:
+
...
|Field |Value
|Display Name |Enter a name for the connector operation.
|Connector Configuration |Select the global configuration you created previously for Amazon SQS Connector
|Number of Messages |10
|Queue url | Amazon SQS queue URL. If provided, the value of this field takes precedence over the value of the *Default Global Queue URL* field on the *Default Global Queue URL* window.
|===
+
For example:
+
