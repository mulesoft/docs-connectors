...
THIS IS A TEST TO SEE IF I CAN EDIT IN MY OWN BRANCH AND THEN COMMIT. ***this is a test of making a comment***This example sends a message with metadata to an Amazon SQS queue and then receives it from the queue. The example is split into the two flows:
