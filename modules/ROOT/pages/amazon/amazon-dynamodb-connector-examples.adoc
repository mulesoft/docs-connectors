...
.Use this sample flow to create an Amazon DynamoDB table in Studio.
image::amazon/amazon-dynamodb-create-table-flow.png[Studio flow for creating an Amazon DynamoDB table]
...
. Drag the *Listener* operation onto the Studio canvaas.
. In the Listener properties window, click the plus sign (+) next to the *Connector configuration* field to access the global element configuration fields for `HTTP Listener`.
...
. Click *OK*.
. In the *Path* field in the Listener properties window, enter `/createtable`:
+
.Enter `/createtable` in the *Path* field to listen for HTTP requests on the `/createtable` path.
image::amazon/amazon-dynamodb-http-prop.png[HTTP Listener properties]

=== Configure the Create Table Operation

Configure the `Create Table` operation to create a table named `Issues`.

. In the *Mule Palette*, click *Search in Exchange*, and search for `amazon dynamodb`.
...
|Region Endpoint |Region for the Amazon DynamoDB client |Select `us-east-1`
|===

For example:

.Configure the credentials needed to access Amazon DynamoDB in the *Connection* section of the global element.
image::amazon/amazon-dynamodb-global-element.png[Amazon DynamoDB global element]
--
+
...
. Click *OK*.
. In the Create table properties window, configure the following fields:
+
...
|Global secondary indexes | Specifies an optional, additional key structure for the table | Enter `#[vars.secondary]` and then add the secondary index in the XML code. For information, see <<secondary-index, Add a Secondary Index to the Issues Table>>.
|===
+
For example:
+
.Enter values for the `Create table` operation properties.
image::amazon/amazon-dynamodb-create-table-props.png[Create table operation properties]
+
In this screenshot:
...
* The fifth items specify the maximum number of strongly consistent reads consumed per second and strongly consistent writes consumed per second before Amazon DynamoDB returns a ThrottlingException.
* The sixth item species a variable for the secondary index.
...
* `CreateDateIndex`, with the partition key `CreateDate` and sort key `IssueId`
* `TitleIndex` with the partition key `Title` and sort key `IssueId`
* `DueDateIndex` with the partition key `DueDate`

You must include these elements in the XML:

* `<Projection>`, which specifies the attributes to copy from the table into the index. In this example, the value `ALL` means that all attributes are copied.
+
...
. Enter `#[payload]` in the *Message* field of the Logger properties window.
+
.Enter `#[payload]` in the *Message* field to print the operation's response to the Mule console.
image::amazon/amazon-dynamodb-logger-props.png[Logger properties]
...
The following screenshot shows the Anypoint Studio flow for deleting an Amazon DynamoDB table:

.Use this sample flow to delete an Amazon DynamoDB table in Studio.
image::amazon/amazon-dynamodb-delete-table-flow.png[Studio flow for deleting an Amazon DynamoDB table]

=== Preliminary Steps

Follow these steps start deleting an Amazon DynamoDB table:
...
. In the *Path* field on the Listener properties window, enter `/deletetable`.
...
* If the connection is unsuccessful, review or correct any incorrect parameters, and then test again.
. Click *OK*.
. In the Delete table properties window, set the value of `Table name` to `Issues`:
...
. In the Logger properties window, enter `#[payload]` in the *Message* field.

=== XML Flow for the Delete Table Example
...
+
You should see the generated response from the `Delete table` operation in the Mule console.
...
* To perform subsequent scans, apply the value of the `LastEvaluatedKey` field in the JSON response to the `Exclusive start keys` field in the Studio configuration of the `Scan` operation. When the operation has returned all pages of data, the `LastEvaluatedKey` value in the JSON response is null.

=== Flow for Scanning a Table

The following screenshot shows the Studio flow for scanning an Amazon DynamoDB table:

.Use this sample flow to scan an Amazon DynamoDB table in Studio.
image::amazon/amazon-dynamodb-scan-flow.png[Studio flow for scanning an Amazon DynamoDB table]
...
. In the Listener properties window, click the plus sign (+) next to the *Connector configuration* field to access the global element configuration fields for `HTTP Listener`.
. In the *Host* field, select `localhost` and click *OK*.
. In the *Path* field in the Listener properties window, enter `/scantable`.
...
* If the connection is unsuccessful, review or correct any incorrect parameters, and then test again.
. Click *OK*.
. In the Scan properties window, set the value of `Table name` to `Issues`.
...
. In the *Mule Palette*, search for `transform`.
...
http://www.mulesoft.org/schema/mule/ee/core http://www.mulesoft.org/schema/mule/ee/core/current/mule-ee.xsd">
	...
	<dynamodb:config name="Amazon_DynamoDB_Configuration1" doc:name="Amazon DynamoDB Configuration" >
		<dynamodb:basic-connection accessKey="${config.accesskey}" secretKey="${config.seccretkey}" />
	</dynamodb:config>
	<flow name="delete-table-flow" >
...
+
You should see the generated response from the `Scan` operation in the Mule console.
