...
. In the properties window, click the plus sign (+) next to the *Connector configuration* field access the global element configuration fields for `HTTP Listener`.
...
. Click *OK*.
. In the *Path* field in the properties window, enter `/createtable`:
+
...
* The first item is the name of the component that represents the `Create table` operation in the flow.
* The second item is the name of the table to create.
...
http://www.mulesoft.org/schema/mule/http/current/mule-http.xsd">
	...
	<http:listener doc:name="Listener" config-ref="HTTP_Listener_config" path="/createtable"/>
		<dynamodb:create-table doc:name="Create table" config-ref="Amazon_DynamoDB_Configuration" tableName="Issues" readCapacityUnits="5" writeCapacityUnits="5">
...
. Enter `#[payload]` in the *Message* field of the properties window.
+
...
. In the *Mule Palette*, search for `http` and select the *Listener* operation.
. Drag the *Listener* operation onto the canvas.
. In the properties window, click the plus sign (+) next to the Connector configuration field to create a global element for `HTTP Listener`.
. In the *Host* field, select `localhost` and click *OK*.
. In the *Path* field on the properties window, enter `/deletetable`.
...
* If the connection is unsuccessful, review or correct any incorrect parameters, and then test again.
. Click *OK*.
. In the properties window, set the value of `Table name` to `Issues`:

[[configure-logger]]
=== Configure a Logger Component

Configure a `Logger` component to print to the Mule console the response generated by the `Delete table`:
...
. In the properties window, click the plus sign (+) next to the Connector configuration field to create a global element for `HTTP Listener`.
. In the *Host* field, select `localhost` and click *OK*.
. In the *Path* field in the properties window, enter `/scantable`.
