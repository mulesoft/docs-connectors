...
|Region Endpoint |Region for the Amazon DynamoDB client |Select the region. By default, `USEAST1`
|===
+
...
The ``<Projection>`` element represents attributes to copy from the table into an index. The primary key attributes and index key attributes are automatically copied:
