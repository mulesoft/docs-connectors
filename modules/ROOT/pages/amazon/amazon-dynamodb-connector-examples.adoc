...
. Click *OK*.
. In the *Path* field on the Listener properties window, enter `/createtable`:
+
...
. Add the properties file you created earlier to the *Configuration properties* field, as described in <<introduction/intro-connector-configuration-overview.adoc#property-placeholders,Use Property Placeholders for Property Values>>.
. Click *Create*.
...
|Global secondary indexes | An optional, additional key structure for the table | Enter `#[vars.secondary]` to create a variable for the secondary index, and then add the secondary index in the XML code. For information, see <<secondary-index, Add a Secondary Index to the Issues Table>>.
|===
+
For example:
+
.Enter values for the `Create table` operation fields.
image::amazon/amazon-dynamodb-create-table-props.png[Create table operation fields]
+
In this screenshot:
...
* The fifth items specify the maximum number of strongly consistent reads consumed per second and strongly consistent writes consumed per second before Amazon DynamoDB returns a throttling exception.
...
* The DynamoDB AWS SDK paginates results by dividing them into specific pages. A one MB limit applies to returned results. When this limit is exceeded, you must perform another scan to receive the rest of the data.
...
. In the Listener properties window, click the plus sign (+) next to the *Connector configuration* field to access the global element configuration fields for `HTTP Listener`.
. In the *Host* field, select `localhost` and click *OK*.
. In the *Path* field on the Listener properties window, enter `/scantable`.
