...
=== Flow for Creating a Table

The following screenshot shows the Anypoint Studio flow for creating a DynamoDB table:

image::amazon/amazon-dynamodb-create-table-flow.png[Creating a table in Amazon DynamoDB]

=== Steps for Creating a Table

To create a DynamoDB table in Studio:
...
. In the properties window, set the *Path* to `/createtable`:
+
image::amazon/amazon-dynamodb-http-props.png[DynamoDB HTTP configuration properties]
+
. Click the green plus icon to the right of the *Connector configuration* field the global element configuration fields.
. In the *Host* field, enter `localhost` and click *OK*.
. Drag the Amazon DynamoDB Connector *Create Table* operation to the right of *Listener* on the Studio canvas.
. Click the green plus icon to the right of the *Connector configuration* field to access the global element configuration fields for the *Create Table* operation.
. Enter the following values to configure the global element:
+
...
|===
|Field |Value
|Display Name |Display name to use for the the Create Table operation.
2+|Basic Settings
|Extension Configuration |Configuration name you used in the global element for the connector to reference.
2+|General
...
|KeySchema | The attributes that make up the primary key for a table or an index.
...
+
. To add *Global Secondary Indexes*. add *variable* before the *Create Table* operation in this format:
+
[source,xml,linenums]
----
<set-variable value='#[[
...
. Configure the *Logger* as follows to print the response that is generated by the Create Table operation in the Mule Console:
+
[%header,cols="30s,70a"]
|===
|Parameter |Value
...
The XML for this example looks like this:
