...
+
image::amazon/amazon-dynamodb-http-prop.png[HTTP properties]
...
|Region Endpoint |Region for the Amazon DynamoDB client |Enter `us-east-1`
|===
...
image::amazon/amazon-dynamodb-scan-flow.png[Creating a table in Amazon DynamoDB]

=== Start to Scan an Amazon DynamoDB Table

Follow these steps to start to scan an Amazon DynamoDB table:
...
. In the Listener properties window, click the plus sign (+) next to the Connector configuration field to create a global element for `HTTP Listener`.
. In the *Host* field, enter `localhost` and click *OK*.
. In the *Path* field, enter `/scantable`.
...
Configure a `Logger` component to print the response generated by the `Scan` operation to the Mule console.

. Drag a *Logger* component to the right of *Transform* on the Studio canvas.
