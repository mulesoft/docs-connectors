...
|Access Key |Alphanumeric text string that uniquely identifies the user who owns the account |`${config.accesskey}` property placeholder
|Secret Key |Key that acts as a password |`${config.secretkey}` property placeholder
...
|Global secondary indexes | An optional, additional key structure for the table | Enter `#[vars.secondary]` to create a variable for the secondary index, and then add the secondary index in the XML code, as described in <<secondary-index, Add a Secondary Index to the Issues Table>>.
|===
+
...
The XML flow for the complete create table example looks like this:
...
The XML flow for the delete table example looks like this:
