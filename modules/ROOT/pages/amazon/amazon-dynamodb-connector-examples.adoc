...
* To perform subsequent scans, apply the value of the `LastEvaluatedKey` field in the JSON reponse to the `Exclusive start keys` field in the Studio configuration of the `Scan` operation. When the operation has returned all pages of data, the `LastEvaluatedKey` value in the JSON response is null.
