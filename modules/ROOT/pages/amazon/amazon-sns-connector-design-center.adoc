...
For information on the connector field values, see
the xref:amazon/amazon-sns-connector-reference.adoc[Amazon SNS Connector Reference].

== Configure the Input Source Trigger

A trigger starts your app. You can use the `HTTP Listener` operation to start your app from a browser, use a command such as `curl`, or use the `Scheduler` to start your app on a timed schedule.
