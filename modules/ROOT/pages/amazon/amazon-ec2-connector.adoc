...
global Amazon EC2 element that can be used by all uses of the 
Amazon EC2 Connector in your project. You can configure using 
Anypoint Studio's visual editor, an XML editor, or a standalone editor.
...
----
+
. Drag the HTTP Listener component to the canvas, and set *Host* to `localhost` 
and *Port* to the `8081` value.
...
|Name |Enter a name for other instances of the connector to reference it
|Session Token | Session token used to validate the temporary security credentials
|Access Key |Alphanumeric text string that uniquely identifies the user who owns the account
|Secret Key |Key that plays acts as a password
|Region Endpoint |Region to select from the drop-down menu for the Amazon EC2 client
|===
+
.. Your configuration should look like this:
+
image::amazon/mule-amazon-ec2-global-config.png[ec2 global config]
+
...
----
+
. Add Set Payload after the Amazon EC2 Create Volume operation to send the response to the 
...
+
. Add a Logger component after Set Payload to display the Volume ID that is being transformed 
by the Set Payload transformer from the Create Volume operation in the Mule Console. 
