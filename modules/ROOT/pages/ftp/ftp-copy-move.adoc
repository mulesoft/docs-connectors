...
While SFTP has a native concept of copying and moving, FTP does not. The connector simulates these operations by downloading the content and writing it to a new location. Although all of this is done with concurrent streams that start writing at the same time as it starts reading, copying and moving does has a significant network overhead and should be used wisely.
