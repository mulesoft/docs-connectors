= WebSocket Connector Concepts - Mule 4
...
* <<full-duplex-advantages,Advantages of Full Duplex Communication>>
* <<websocket-types,WebSocket types>>
* <<websocket-id,WebSocket ID>>
* <<groups,Groups>>

== Advantages of Full-Duplex Communication [[full-duplex-advantages]]
...
When a client connects to a server using WebSockets, a full duplex connection is established and kept open. Both the client and server can then send multiple messages to each other without needing to follow the HTTP request-response message exchange pattern. A WebsSocket is built on top of the existing HTTP infrastructure, making it easy to reuse infrastructure that is already in place for traditional HTTP.
