...
** IMMEDIATE
** AUTO
** MANUAL |  The acknowledgment mode to use for the messages retrieved by the subscribers. This affects only the Subscriber source and is unrelated to the Consume operation. Refer to <<SubscriberAckMode>> for more details about each strategy. |  AUTO |
...
Execute an Acknowledgment (ack) over a given <<AnypointMQMessageContext>> object indicating that the message has been consumed correctly, and then delete the message from the In Flight status.
...
| Output Mime Type a| String |  The MIME type of the payload that this operation output. |  |
| Output Encoding a| String |  The encoding of the payload that this operation output. |  |
