...
While in the buffer, messages are kept as in-flight for the broker, so no redelivery occurs as long as needed for the message to be dispatched to the flow, or until the Subscriber operation is stopped and the buffer cleared.
...
For more information regarding a message ACK, see xref:anypoint-mq/2.x/anypoint-mq-ack.adoc[Anypoint MQ ACK and NACK Operations].
...
Since version 2.1.0, the Anypoint MQ connector Subscriber operation provides out of the box a circuit breaking capability, allowing you to have more control on how the connector deals with errors occurred while processing a consumed message.
...
* onErrorTypes
+
The error types whose occurrence during the flow execution counts as a failure in the circuit. An error occurrence counts only when the flow is completed with an error propagation. By default all errors count as a circuit failure.
* errorsThreshold
+
