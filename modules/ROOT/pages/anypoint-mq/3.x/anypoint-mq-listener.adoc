= MQ Listen for New Messages
ifndef::env-site,env-github[]
include::_attributes.adoc[]
endif::[]

The `Subscriber` source in the Anypoint MQ connector provides the ability to listen for new messages and consume them as they arrive at the destination. You can configure different listening strategies that enable you to tune the consumption for performance, predictability, and schedules.


== Continuous Listening for New Messages

By default, the Anypoint MQ subscriber works in a continuous listening mode, providing the maximum throughput by consuming messages as soon as they arrive in the queue. This is called the `prefetch` `subscriber-type` configuration. When using the `prefetch` mode, the subscriber attempts to keep a local buffer of messages full, making them available to be dispatched to the Mule flow as soon as the application can accept it.
...
IMPORTANT: When a message is taken from the queue and stored in the local buffer, it appears as in-flight for any other consumer and prevents them from consuming it. The message remains in this in-flight state for as long as necessary for the application to begin the processing.
...
* Reduce the buffer size to process messages as they are published to the queue.
+
The smaller buffer restricts application-level throughput and avoids blocking competing consumers.
...
You can customize the subscriber polling strategy using the standard Mule Runtime's scheduling strategies:
...
* *Automatic*
+
By default, the subscriber component uses the `AUTO` `acknowledgementMode`. In this mode, the messages that the subscriber retrieves are acknowledged automatically once the flow processing of the message succeeds. This means that the subscriber receives a message, dispatches it to the flow, and waits to see how the message processing completes. It executes an ACK only when the processing completes without exceptions.
+
If the execution of the processing flow is completed with a propagated exception, the message is automatically NACKed, returning it to the queue for redelivery.
+
...
Using Anypoint MQ in a Mule 4 application means having a Mule flow with an MQ subscriber that consumes messages from a queue and processes them using an external service. When this service isn't available:

. The request fails.
. An error propagates.
. Message processing completes as either a failure or as a custom error for handling the message, such as sending it to a DLQ.

When the external service is not available, every attempt to process a message results in a failure, forcing the application to loop, consuming messages that cannot succeed. You can avoid this behavior by notifying the subscriber of the error in a way that prevents it from consuming more messages for a certain period.

=== Circuit Breaker Processes

The circuit breaker capability that `Subscriber` operation provides is bound to the error handling mechanism provided by Mule. It uses the error notification mechanism to keep a count of errors related to an external service, which is known as circuit failures. You can bind any error to a circuit failure, for example you can bind `HTTP:TIMEOUT`, `FTP:SERVICE_NOT_AVAILABLE`, or even a custom error from your application, such as `ORG:EXTERNAL_ERROR`.

If a Mule flow completes its execution with an error, the subscriber checks if the error is one of the `onErrorTypes` that indicates an external service error, and counts consecutive occurrences until the `errorsThreshold` is reached. When the `errorsThreshold` is reached, the circuit trips and stops polling for new messages the duration specified by `tripTimeout`. Messages are consumed again on the next poll after the tripTimeout elapses.

By default, the circuit breaking feature is disabled.

=== Circuit Breaker States

The circuit breaker has three states: Closed, Open, and Half Open. The behavior of the application changes based on the current state. See the https://docs.microsoft.com/en-us/azure/architecture/patterns/circuit-breaker[Microsoft Circuit Breaker pattern] for more information.
...
+
Messages that were already dispatched to the flow then complete processing, regardless of whether the result is success or failure.
+
Messages kept locally that are in-flight for the broker but haven't been dispatched yet are NACKed and returned to the queue for redelivery to another consumer.
+
* *Open*
+
...
+
When the subscriber successfully fetches a single message, dispatches it to the flow, and processing completes successfully, the subscriber returns to normal and immediately attempts to fetch more messages.
+
...
You can configure a Circuit Breaker as either a Global Circuit Breaker or as a Private Circuit Breaker.

Either way, the configuration parameters are the same:

* `onErrorTypes`
+
The error types that count as a failure during the flow execution. An error occurrence counts only when the flow completes with an error propagation. By default, all errors count as a circuit failure.
* `errorsThreshold`
+
...
Inside this configuration wizard, populate the following fields as needed. Once this configuration is complete, you can reference this `Circuit_breaker` declaration from any Anypoint MQ Subscriber.
...
You declare a Private Circuit Breaker internally on a single subscriber. This "circuit" declaration is only used in the flow where the subscriber is declared, *isolated from all the other circuits*.
...
----
[cols="8,92,options="noheader", frame="none"]
|===
...
* xref:anypoint-mq/3.x/anypoint-mq-ack.adoc[MQ Message Acknowledgment]
