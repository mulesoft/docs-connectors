= Anypoint MQ Subscriber Operation
...
* *Automatic*
+
By default, the Subscriber operation uses the `AUTO` `acknowledgementMode`. In this mode, the messages that the subscriber retrieves are acknowledged automatically once the flow processing of the message succeeds. This means that the subscriber receives a message, dispatches it to the flow, and waits to see how the message processing completes. It executes an ACK only when the processing completes without exceptions.
+
