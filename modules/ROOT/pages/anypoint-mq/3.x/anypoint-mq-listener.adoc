= Anypoint MQ Subscriber Operation - Mule 4
ifndef::env-site,env-github[]
include::_attributes.adoc[]
endif::[]

The Subscriber operation in the Anypoint MQ connector enables the app to listen for new messages and consume them as they arrive at the destination. You can configure different listening strategies that enable you to tune the consumption for performance, predictability, and schedules.


== Maximum Message Throughput

By default, the Subscriber operation works in continuous listening mode, providing maximum throughput by consuming messages as soon as they arrive in the queue. This is called the `prefetch` subscriber-type configuration. When using the `prefetch` mode, the Subscriber operation attempts to keep a local buffer of messages full, making them available to be dispatched to the Mule flow as soon as the app can accept them.

For `subscriber-type`, you configure the `maxBufferSize` parameter to specify the target size for a full buffer, which controls how quickly messages are taken from the queue.
...
If your app requires predictable and controlled message consumption rather than maximum throughput, you can configure `Subscriber` as a polling source. The Subscriber operation checks for new messages in the queue at a fixed scheduling rate. Every time the poll is triggered, the Subscriber operation retrieves up to 10 messages to be dispatched to the flow in order, one by one. To do so, configure `polling` in `subscriber-type`.

Messages aren't consumed immediately. Instead, they remain in the queue until the next poll of the Subscriber operation operation is triggered. The queue contains up to 10 messages to be retrieved. For example, if your queue has 15 messages, one poll consumes the first 10 messages and the remaining 5 messages wait for the next poll.
...
You can customize the Subscriber operation polling strategy using the standard Mule runtime engine scheduling strategies:
...
You can customize the frequency at which the Subscriber operation polls for new messages by declaring an `anypoint-mq:polling` `anypoint-mq:subscriber-type` strategy with a customized `fixed-frequency` scheduling strategy:
...
* *Automatic*
+
By default, the Subscriber operation uses the AUTO acknowledgment mode. With this mode, the messages that the Subscriber operation retrieves are acknowledged automatically after message flow processing succeeds. This means that the Subscriber operation receives a message, dispatches it to the flow, and waits to see how the message processing finishes. It executes an ACK only when the processing finishes without exceptions.
+
...
* *Manual*
+
When you use the MANUAL acknowledgment mode, the app logic decides when to perform the acknowledgment of the message, using the xref:anypoint-mq/3.x/anypoint-mq-ack.adoc[ACK] or xref:anypoint-mq/3.x/anypoint-mq-ack.adoc[NACK] operations.
+
...
When the external service is not available, every attempt to process a message results in a failure, forcing the app to loop, consuming messages that cannot succeed. You can avoid this behavior by notifying the Subscriber operation of the error in a way that prevents it from consuming more messages for a certain period.

=== Circuit Breaker Processes

The circuit breaker capability that the Subscriber operation provides is bound to the error handling mechanism provided by Mule. It uses the error notification mechanism to count errors related to an external service, which is known as a circuit failure. You can bind any error to a circuit failure. For example, you can bind `HTTP:TIMEOUT`, `FTP:SERVICE_NOT_AVAILABLE`, or even a custom error from your app, such as `ORG:EXTERNAL_ERROR`.

If a Mule flow finishes its execution with an error, the Subscriber operation checks if the error is one of `onErrorTypes` that indicates an external service error, and counts consecutive occurrences until `errorsThreshold` is reached. When `errorsThreshold` is reached, the circuit trips and stops polling for new messages the duration specified by `tripTimeout`. Messages are consumed again on the next poll after the tripTimeout elapses.
...
* *Closed*
+
The starting state where the Subscriber operation retrieves messages normally from MQ based on its configuration, effectively working as if the circuit breaker is not present.
+
...
* *Open*
+
the Subscriber operation doesn't attempt to retrieve messages, and skips the iterations silently until `tripTimeout` is reached.
+
* *Half Open*
+
After `tripTimeout` elapses, the Subscriber operation goes to a Half Open state. In the next poll for messages, the Subscriber operation retrieves a single message from the service and uses that message to check if the system has recovered before going back to the normal Closed state.
+
When the Subscriber operation successfully fetches a single message, dispatches it to the flow, and processing finishes successfully, the Subscriber operation returns to normal and immediately attempts to fetch more messages.
+
...
. Select the Subscriber operation in the canvas.
...
You declare a Private Circuit Breaker internally on a single subscriber. This circuit declaration is  used only in the flow where the Subscriber operation is declared, isolated from all the other circuits.
...
In this example, a single subscriber consumes messages from a queue and posts the messages to another service using its REST API. You can stop processing messages after 5 requests to the external service result in a timeout. Once processing stops, the Subscriber operation waits for 30 seconds for the service to recover before retrying with a new message.
...
| *(2)* | Identifies two errors that can affect the processing of messages from the Subscriber operation and passes each as a CSV list.
...
In this scenario, both subscribers stop polling for messages as soon as the error count reaches the `errorsThreshold="10"` value, counting both `FTP:RETRY_EXHAUSTED` and `HTTP:SERVICE_UNAVAILABLE` errors. When `tripTimeout` elapses, one of the Subscriber operations polls for a message and uses it to test the circuit. If the message succeeds, polling for both subscribers is enabled.
