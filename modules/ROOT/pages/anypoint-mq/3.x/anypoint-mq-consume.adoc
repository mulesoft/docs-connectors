...
The simplest declaration to consume a message from a queue is to drag the `Consume` operation from the palette to the canvas and configure the `Queue` parameter with the name of the queue from which to retrieve the message:
...
The example operation consumes the first available message in the queue identified by the `Queue` called `myQueue`, converting it to a `MuleMessage` with the following structure:
...
The first scenario is the most common: a queue has messages waiting to be consumed. When the `Consume` operation is executed, it returns immediately with one of the messages.

If a queue is empty, you can decide how the operation behaves:

* Immediate Response on Empty Queue
+
Set the `pollingTime` parameter to `0`, the `Consume` operation returns immediately *without any error* and producing a `null` message as the result when the queue is empty.
...
By default, the `Consume` operation waits for 10 seconds for a new message to arrive when consuming from an empty queue. It fails if no message arrives in that timeframe.
...
For information about the `attributes` structure, see xref:anypoint-mq/3.x/anypoint-mq-connector-reference.adoc#consume[MQ Consume Operation].
