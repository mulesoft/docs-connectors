...
WARNING: When using the `prefetch` mode in the Anypoint MQ Subscriber source, the time the message remains in the local buffer doesn’t count as part of the timeout. The `acknowledgementTimeout` timer begins from the moment the message is dispatched to the flow, not when it is taken from the queue, *using a default of 2 minutes as TTL*. If a different TTL is needed, it has to be defined *explicitly* in the `acknowledgementTimeout` parameter of the subscriber.
