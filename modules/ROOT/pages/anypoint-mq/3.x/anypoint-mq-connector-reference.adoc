= Anypoint MQ Connector 3.x - Mule 4e — Mule 4
...
| Acknowledgment Timeout a| Number |  Duration that a message is held by a broker waiting for an acknowledgment or not acknowledgment before being made available to any subscriber. Specifies the default `acknowledgmentTimeout` value for all Subscriber operations, unless it is overridden.  |  0 |
| Acknowledgment Timeout Unit a| Enumeration, one of:
...
|===
| Field | Type | Description | Default Value | Required
| Max Buffer Size a| Number | Maximum number of messages to be prefetched from the service and kept locally before being dispatched to the flow | 30 |
|===
