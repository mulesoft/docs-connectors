...
|*(1)* - Acknowledgment
|Global acknowledgment settings for the Anypoint MQ Subscriber operations.
...
|*(5)* - Circuit Breaker
|Circuit Breaker configuration for the Anypoint MQ Subscriber operation.
|Handle each Circuit Breaker instance either as a standalone element referenced by each Anypoint MQ Subscriber or as a private Circuit Breaker for the Subscriber operation that declares it.
|===
...
In version 2.x of this connector, the acknowledgement mode of the Subscriber operation is defined at the configuration level:
...
In veresion 3.x, these parameters are part of the Subscriber operation itself and affect only that single Subscriber instance. The semantics of the acknowledgment parameters has not changed.
Here's the equivalent configuration of the Subscriber operation:
...
|`maxBufferSize`| Declared at the `Subscriber` level, explicitly controls the maximum size that the local message buffer can have. This parameter serves as a target number, attempting to fetch as many messages as needed to fill the buffer completely.
...
In version 3.x of this connector, the polling mode of the Subscriber operation is simplified and normalized. You can use the schedulers provided by Mule runtime engine out of the box, either as `fixed-frequency` or `cron`:
...
In version 2.x of this connector, you declared the circuit breaker as part of the Subscriber configuration and then referenced it across multiple Subscriber operations.
