...
| Frequency a| Number |  Fixed frequency in milliseconds at which the retrieval of messages os attempted when there are no more messages to process locally. In the case were a queue is empty and all the messages retrieved were already processes, this frequency defines how long to wait before attempting to retrieve more messages. By default, this frequency is 5 seconds (5000 milliseconds). |  5000 |
|===
...
Executes an Not Acknowledgement for an <<AnypointMQMessageContext>> and changes the status of the message from In Flight to In Queue to be consumed again for a subscriber.
