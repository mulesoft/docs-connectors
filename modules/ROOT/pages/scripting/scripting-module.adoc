...
The Scripting module is included in Anypoint Studio. To add the component into one of your flows, you need to search for it in the Mule Palette.

image::scripting/scripting-flow.png[Scripting Execute component]

== Basic Usage

To configure the Scripting component in Studio: 

. Drag the *Execute* operation of the Scripting module into your flow. 
. Double-click the *Execute* element to open the Execute configuration tab.
. From the *General* tab, provide your code in the *Code* text window.
...
Note that you can also load code from an external file by setting this and by using a xref:4.1@mule-runtime::configuring-properties.adoc#_file_properties[File configuration property]:
