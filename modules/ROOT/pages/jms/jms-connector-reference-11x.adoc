= JMS Connector Reference 1.1 - Mule 4
...
| Max Redelivery a| Number | *NOTE:* This parameter is not functional. Used to configure the number of redelivers before discarding the message. No redelivery is represented with 0, while -1 means infinite re deliveries accepted. |  0 | 
...
|===
| Field | Type | Description | Default Value | Required
| Max Redelivery Count a| Number | *NOTE:* This parameter is not functional. The maximum number of times a message can be redelivered and processed unsuccessfully before triggering process-failed-message. |  | 
...
| Max Redelivery a| Number | *NOTE:* This parameter is not functional. Used to configure the RedeliveryPolicy#getMaximumRedeliveries().
