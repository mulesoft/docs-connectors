...
JMS Connector includes an `active-mq-connection` configuration, which provides out-of-the-box support for ActiveMQ 5.  With this connection, you can use both JMS 1.1 (default)
or JMS 1.0.2b specs, and configure all the general connection parameters for JMS,
as well as the custom parameters that are present only in ActiveMQ.
...
You can enable the serialization of more classes by setting the `trustAllPackages` parameter to `true`, which reverts to the old behavior of allowing any object to be serialized and deserialized. This is totally disallowed to prevent malicious attacks. 
...
The ActiveMQ client library is the only library that is required to use ActiveMQ connections. It also needed to connect to a broker.
