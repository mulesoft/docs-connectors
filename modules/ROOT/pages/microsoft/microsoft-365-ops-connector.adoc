...
+
image::microsoft/microsoft-365-ops-create-global-element.png[Design Center config name and connection type]
+
...
+
image::microsoft/microsoft-365-ops-as-oauth2.png[Studio OAuth configuration]
+
...
** Token Request Endpoint - The token endpoint that is called to get the access token. Example: `+https://login.windows.net/TENANT_ID/oauth2/token+` where TENANT_ID is the Azure Active Directory ID.
+
image::microsoft/microsoft-365-ops-as-oauth-username-pass.png[Studio OAuth Username Password configuration]
