...
* Resource - Resource URL for accessing the Microsoft Dynamics 365 instance
* Consumer Key - OAuth consumer key registered with the service provider
* Consumer Secret - OAuth consumer secret registered with the service provider
* Listener Config - A reference to the listener that catches the access token callback endpoint 
...
* Token Request Endpoint - URL of the OAuth token request server
...
* Entity set::
Named collections of entities, for example, an account is an entity set containing account entities. An entity's key uniquely identifies the entity within an entity set.
* Entity types::
Named structured types with a key. Entity types define the named properties and relationships of an entity, and may derive by single inheritance from other entity types.
