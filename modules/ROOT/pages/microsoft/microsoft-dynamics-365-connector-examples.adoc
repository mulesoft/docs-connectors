...
* `config-oauth-user-pass.resource` - The App ID URI of the web API (secured resource). It must be root URI, without specifing the CRM version (Example: `https://<your-org>.crm2.dynamics.com/`)
...
* `CREATE_EMPTY_CONTACT_DEMO`: Creates an empty contact entity that is required later in other flows.
+
...
+
* `DISASSOCIATE_ENTITIES_DEMO`: This flow dissasociates entities. You provide the ID of the entity upon which the request will be made and the keys that will be dissasociated.
+
