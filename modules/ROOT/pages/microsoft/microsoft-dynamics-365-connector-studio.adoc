...
Configure an input source for the connector using *On modified object* or *On new object*. When a new or modified 
object occurs in the Microsoft Dynamics 365 operation, an input source is sent to the connector app. 

Alternatively, you can also trigger a connector app by listening for requests from HTTP or at timed intervals using Scheduler.
