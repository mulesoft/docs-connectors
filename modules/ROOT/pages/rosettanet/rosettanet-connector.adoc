...
| Force message signing |Optional override to force the use of signed messages even when not required by the PIP definition Leaving this value empty uses the PIP definition for signing messages, `NEVER` means to never sign any messages, and `ALWAYS` means always sign messages.
