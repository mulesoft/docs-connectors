...
| Object Store a| String |  A reference to the object store to use to store each resource owner ID's data. If an object store is not specified, Mule automatically provision the default object store. |  | 
|======================
...
Use the authorization server's URL for the audience value: `+https://login.salesforce.com+`, `+https://test.salesforce.com+`. If it is a community implementation, use `+https://community.force.com/customers+`. |  | 
| Reconnection a| <<Reconnection>> |  When the application is deployed, a connectivity test is performed on all connectors. If set to `true`, deployment fails if the test doesn't pass after exhausting the associated reconnection strategy. |  | 
|======================
