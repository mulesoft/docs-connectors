= Salesforce Composite Connector 3.1 Reference 
...
* <<composite-config_oauth-client-credentials, OAuth Client Credentials>>
...
* <<composite-config_token-connection, Token Connection>>
...
| Max connections a| Number |  The maximum number of outbound connections to keep open at the same time. Use `-1` for unlimited. |  -1 |
| Use persistent connections a| Boolean |  If set to `false`, each connection is closed after the first request completes. |  true |
| Connection idle timeout a| Number |  How long a connection can remain idle before it is closed. Only applies when persistent connections are enabled. This attribute works in tandem with Connection Idle Timeout Unit. |  30 |
...
** MINUTES
** HOURS
** DAYS |  Time unit to use for the Connection Idle Timeout parameter. |  SECONDS |
...
[[composite-config_oauth-client-credentials]]
...
| Token Url a| String |  The service provider's token endpoint URL. The default is `+https://login.salesforce.com/services/oauth2/token+`.  |   |
| Scopes a| String |  The OAuth scopes to request during the dance. If not provided, it defaults to the scopes provided in the annotation. |  |
| Object Store a| String |  A reference to the object store to use to store each resource owner ID's data. If an object store is not specified, Mule automatically provisions the default object store. |  |
|===
...
| Error Mappings a| Array of <<error-mapping>> |  Set of error mappings. |  |
...
| Trust Store Configuration a| <<trust-store-configuration>> | Contains the configuration parameters for the trust store configuration |  |
| Key Store Configuration a| <<key-store-configuration>> | Contains the configuration parameters for the key store configuration |  |
