...
+++
<p> The purpose of this method is to provide an enhanced way to perform composite requests, which execute a series of REST API requests in a single call. </p>
+++
...
| Graphs a| Array of Object |  +++the list of graphs to be executed+++ |  +++#[payload]+++ | {nbsp}
...
Provide metadata for the result of a getLimits. This operation takes as input one record from the result returned by `executeCompositeBatch(String, java.util.List)`, which belongs to a create subrequest generated by `preCreate(String, java.util.Map)`.
