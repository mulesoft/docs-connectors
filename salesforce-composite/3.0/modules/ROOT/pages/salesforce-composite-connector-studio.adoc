= Using Anypoint Studio to Configure Salesforce Composite Connector 3.0 
...
Enter the following information on the *General* tab of the *Global Element Properties* screen to configure OAuth client credentials authentication:

* Client Id +
The OAuth client ID for the Salesforce connected app
* Client Secret +
The OAuth client secret for the Salesforce connected app
* Token Url +
The token endpoint URL used to request access tokens
* Scopes (Optional) +
The OAuth scopes to request for the connection
+
.OAuth client credentials authentication includes a client ID, client secret, token URL, and optional scopes.
image::salesforce-composite-studio-oauth-client-credentials.png["Global configuration window for OAuth Client Credentials"]
