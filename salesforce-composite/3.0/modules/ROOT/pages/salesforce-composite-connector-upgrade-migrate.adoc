...
:page-aliases: connectors::salesforce/salesforce-composite-connector-upgrade-migrate.adoc
...
. In *Add Dependencies to Project*, enter `salesforce composite` in the search field.
. In *Available modules*, select *Salesforce Composite* and click *Add*.
. Click *Finish*.
. Verify that the Salesforce Composite dependency version is `3.x` in the project's `pom.xml` file.
