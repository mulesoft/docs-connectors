= Use Anypoint Studio to Configure Salesforce Composite Connector - Mule 4
...
|Access Key | Enter the access key provided by Salesforce Composite.
|Secret Key | Enter the secret key provided by Salesforce Composite.
|Try Default AWS Credentials Provider Chain | Set to `true` to obtain credentials from the AWS environment.
|Region Endpoint | Select the region endpoint for the service.
|===
