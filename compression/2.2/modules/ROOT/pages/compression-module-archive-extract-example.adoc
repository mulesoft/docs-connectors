...
The following Compression module example shows how to archive and extract files from a folder. The File Connector *List* operation lists all the files from a folder named `input`. Then, the *Compress* operation creates a compressed archive by generating an object with the key set to the file name, and with a value set to the value of the listed file. 
Subsequently, the *Extract* operation decompresses the archived file. Then, the *For Each* scope iterates over each extracted key/value and writes the file to a new output folder with the key as the new file name, and the value to the corresponding extracted file content.
