= Use Anypoint Studio to Configure Confluent Schema Registry Connector - Mule 4
...
|Access Key | Enter the access key provided by Confluent Schema Registry.
|Secret Key | Enter the secret key provided by Confluent Schema Registry.
|Try Default AWS Credentials Provider Chain | Set to `true` to obtain credentials from the AWS environment.
|Region Endpoint | Select the region endpoint for the service.
|===
...
See xref:confluent-schema-registry-connector-config-topics.adoc[Additional Configuration Information] for more configuration steps.
