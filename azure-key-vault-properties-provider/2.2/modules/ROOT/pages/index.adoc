...
[NOTE]
====
This example shows the legacy secret format `${azure-key-vault-properties-provider::secret::yourSecret}`, which applies to a single configuration that does not set a `name` attribute. If your Mule app defines multiple configurations, use the namespaced format `${configName::secret::yourSecret}` instead.
====

Starting in version 2.2.0, you can define more than one Azure Key Vault Properties Provider configuration in the same Mule app, with each configuration connecting to a different key vault. To retrieve a secret from a specific vault, reference the secret using the configuration name as a namespace, in the format `${configName::secret::yourSecret}`, where `configName` is the value of the `name` attribute of the properties provider configuration. For more information, refer to xref:azure-key-vault-properties-provider-reference.adoc[Azure Key Vault Properties Provider Reference].
