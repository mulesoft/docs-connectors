...
For authenticating to Azure Key Vault using environment variables you must set the following environment variables `AZURE_CLIENT_ID`, `AZURE_CLIENT_SECRET`, and `AZURE_TENANT_ID`. Refer to xref:mule-runtime::setting-environment-variables.adoc[Setting environment variables] documentation for details.
