...
These are stored in secure configuration files with the values encrypted. The values are decrypted by the runtime. After they are decrypted by the runtime, they remain in memory in their decrypted state. For more information, refer to xref:cloudhub-2::ch2-protect-app-props.adoc[].
