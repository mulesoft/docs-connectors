...
[IMPORTANT]
====
As of March 30, 2026 (Sandbox) and April 30, 2026 (Production), the QuickBooks Online Accounting API returns `FullyQualifiedName` in the `AccountRef.name` field for all entities and transactions. Previously, this field returned values inconsistently.

To avoid integration issues, always use the `AccountRef.value` field when querying for account data. This change does not affect write operations.

For more information, see https://medium.com/intuitdev/upcoming-changes-to-accounting-apis-that-may-impact-your-application-023386ca2c52[Intuit's announcement on Accounting API changes^].
====
