...
[NOTE]
====
In all entities and transactions that include the `AccountRef` field in the Accounting API response, the API currently returns values inconsistently in the `AccountRef.name` field. Starting March 30, 2026 (Sandbox) and April 30, 2026 (Production), QuickBooks Online will consistently return the `FullyQualifiedName` in the `AccountRef.name` field.

MuleSoft recommends that you always use the `AccountRef.value` field when querying for account data to avoid being impacted by this change. Write operations are not impacted by this change. Write operations are not impacted by this change.
