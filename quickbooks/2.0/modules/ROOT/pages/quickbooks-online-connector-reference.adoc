...
[NOTE]
====
The *State* parameter is typically standard for OAuth and is optional, however, QuickBooks requires the *State* parameter.

Because QuickBooks requires the *State* parameter, you must pass the *State* parameter or the *Resource Owner Id*
parameter (in case of multi-tenancy) to pass the *State* parameter to the OAuth dance, for example, `/authorize?state=test_state`.
Failing to do so results in a `The state query parameter is missing from the authorization
request` error from QuickBooks.
