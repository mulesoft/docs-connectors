...
This document assumes that you are familiar with Mule, xref:studio::index.adoc[], xref:3.9@mule-runtime::global-elements.adoc[Global Elements], xref:studio::datasense-concept.adoc[], and xref:3.9@mule-runtime::batch-processing.adoc[batch processing] in Mule. Further, it assumes you are familiar with BMC Remedy AR System.
