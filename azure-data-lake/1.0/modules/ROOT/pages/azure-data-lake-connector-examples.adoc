...
To do this, you first set up the app flow, add the File Connector *Read* operation, configure the *Set Variable* component, then configure the *Update Path* operation with an *append* action.
* The second *Update Path* operation is then configured with a *flush* action to flush the contents of an existing file on the file system.
