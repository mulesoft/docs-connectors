...
:page-aliases: 3.6@mule-runtime::concur-connector.adoc
...
* A consumer key assigned by Concur; refer to the detailed documentation on how to xref:3.6@mule-runtime::using-a-connector-to-access-an-oauth-api.adoc[use the Concur connector to access an OAuth API].

*Release Notes*: xref:release-notes::connector/concur-connector-release-notes.adoc[Concur Connector Release Notes]
...
You can "test drive" a connector in Anypoint Studio:
...
Contact the mailto:info@mulesoft.com[MuleSoft Sales Team] to obtain either of these. (Read more about xref:3.6@mule-runtime::installing-an-enterprise-license.adoc[Installing an Enterprise License].)

== Configuring the Concur Connector

To use the Concur connector in your Mule application, you must configure:

* A global Concur element that is used by all the Concur connectors in the application (read more about xref:3.6@mule-runtime::global-elements.adoc[global elements in Mule]).
...
. Back in the connector's properties editor, set the `submitreport` path.
The new flow is then reachable at the `+http://localhost:8081/submitreport+` path.
...
|Value a|
#[['name':'April Expenses','purpose':'All expenses for April','comment':'This is a comment.','orgUnit1':'US','orgUnit2':'NW','orgUnit3':'Redmond','custom1':'Client','custom2':'Local','userDefinedDate':'2014-03-26 15:15:07.0']]
|===
+
image::ex2.png[]
+
...
* http://mulesoft.github.io/mule3-microsoft-dynamics-365-connector/[SAP Concur Technical Reference].

* Access the xref:release-notes::connector/concur-connector-release-notes.adoc[release notes] for the Concur Connector.
