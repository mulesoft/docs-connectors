...
* *Session Attributes*
+
Optional key-value pairs that persist across the entire agent session. Map each value to a string; values support DataWeave expressions. Blank keys or blank values are filtered before the request is built.

* *Prompt Session Attributes*
+
Optional key-value pairs that apply only to the current prompt and turn. Map each value to a string; values support DataWeave expressions. Blank keys or blank values are filtered before the request is built.
...
* `stop`
+
If you specify the `stop_sequences` inference parameter, `stop` contains the stop sequence that signalled the model to stop generating text. For example, `stop` contains `\n\nHuman:` in the following response.
