...
The *CHAT Answer Prompt Streaming* operation is similar to the *CHAT Answer Prompt* operation but streams the response incrementally, allowing for real-time display of the response as it is generated. You can include images or documents with the prompt.
...
* *Attachments*
+
Optional images or documents to include with the prompt. For each attachment, set *Format* to one of `png`, `jpeg`, `gif`, `webp`, `pdf`, `csv`, `doc`, `docx`, `html`, `txt`, `md`, `xls`, or `xlsx`. You can identify the file with *Path* or provide the content in *Bytes*, and you can set *Name*.
+
Size limits are enforced before any bytes are read or decoded: images (`png`, `jpeg`, `gif`, `webp`) are capped at approximately 3.75 MB (3,750,000 bytes) and documents at approximately 4.5 MB (4,500,000 bytes). An attachment that exceeds its cap fails fast with `MS-BEDROCK:VALIDATION_ERROR`.
...
* xref:amazon-bedrock-connector-reference.adoc#ChatAnswerPrompt[CHAT Answer Prompt Reference]
* xref:amazon-bedrock-connector-reference.adoc#ChatAnswerPromptStreaming[CHAT Answer Prompt Streaming Reference]
* xref:amazon-bedrock-connector-reference.adoc#ConverseAttachment[Converse Attachment]
