...
Anypoint Connector for Amazon Bedrock (Amazon Bedrock Connector) provides access to Amazon Bedrock, a fully managed service that offers 
high-performing foundation models (FMs) from leading AI companies. You can experiment with, evaluate, and customize top FMs using techniques such as fine-tuning and Retrieval Augmented Generation (RAG), and build agents that execute multistep tasks with your enterprise data. While interaction with each LLM provider varies, the connector provides a unified way to connect to all supported LLMs.
...
Amazon Bedrock Connector provides access to foundation models from leading AI companies:
...
+
Connect to your preferred LLM provider through an unification layer. Amazon Bedrock Connector provides the ability to connect to all supported LLMs, simplifying how you interact with different model providers.
