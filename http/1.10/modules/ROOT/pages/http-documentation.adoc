= HTTP Connector 1.10 Reference - Mule 4
...
| Enable Cookies a| Boolean |  If true, cookies received in HTTP responses will be stored, and sent in subsequent HTTP requests. 
...
Note that if you have a dynamic configuration, a new `http:request-config` instance is created for each parameterization, for details refer to the xref:mule-sdk::static-dynamic-configs.adoc#dynamic-configuration-lifecycle[dynamic configurations lifecycle] documentation. Therefore, each parameterization has its own `CookieManager` instance.

This feature can be useful, for example, to have a separate `CookieManager` instance for each event. |  true |
