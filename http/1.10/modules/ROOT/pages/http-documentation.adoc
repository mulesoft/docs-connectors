...
| Enable Cookies a| Boolean |  If true, cookies received in HTTP responses are stored and sent in subsequent HTTP requests.

When an `http:request-config` element has cookies enabled, a single `CookieManager` is created for each instance of the `request-config`. If multiple `http:request` elements in a Mule application reference the same `http:request-config`, they all share the same `CookieManager` instance.

If you have a dynamic configuration, a new `http:request-config` instance is created for each parameterization. Therefore, each parameterization has its own `CookieManager` instance. For details, see xref:mule-sdk::static-dynamic-configs.adoc#dynamic-configuration-lifecycle[Dynamic Configuration Lifecycle]. |  true |
...
| Response Buffer Size a| Number |  The space in bytes for the buffer where the HTTP response will be stored. |  1024 |
...
| Enabled Cipher Suites a| String | A comma separated list of cipher suites enabled for this context. Use IANA (Internet Assigned Numbers Authority) naming convention for the cipher suites. Check the https://testssl.sh/openssl-iana.mapping.html[OpenSSL - IANA mapping] for reference. Additionally, review xref:mule-runtime::tls-configuration.adoc#example-adding-additional-cipher-suites-and-protocol-restrictions[TLS Configuration Examples].   |  |
