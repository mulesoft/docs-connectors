...
[source,text,linenums]
----
Registration has been carried out successfully! Wellcome #[payload.'firstname'] #[payload.'lastname']!
----
+
...
When using an HTTP Listener Connector, whenever there are outbound properties included in the Mule Message that arrives to the end of the flow, each outbound property is transformed into an HTTP response header. It's easy to add properties to the Mule Message by using the xref:3.7@mule-runtime::property-transformer-reference.adoc[Property Transformer]. +
