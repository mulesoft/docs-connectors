= Configure HTTP Request Authentication
...
The following Mule app example illustrates how to configure basic authentication for the HTTP *Request* operation by sending a request to the https://docs.github.com/en/rest[GitHub API] for user information. In this example, the GitHub API accepts requests for user information on port 443 to `+https://api.github.com/user+`.
...
The OAuth2 authorization code configures the OAuth 2.0 authorization code grant type. The OAuth authentication server holds the resources protected by OAuth. For example, API calls to the GitHub API can be authenticated through https://docs.github.com/en/rest/oauth/[GitHub server using OAuth].
...
* *Token Manager* +
To access authorization information for client credentials and authorization codes, configure a token manager. If you don't configure a token manager, the OAuth Module re-initializes every time a new request is made.
...
+
. In a browser, enter the local authorization URL `+http://localhost:8082/login+` to initiate the https://datatracker.ietf.org/doc/html/rfc6749[OAuth2 dance].
+
...
NOTE: Although the *Client id* and *Client secret* fields support using variables, the connector doesn't support changing these values dynamically during each flow execution. The configured credentials must remain static.
If you require multiple sets of client credentials, it is recommended to create a separate configuration for each client.
