...
The following Mule application example helps you to set the error status code and reason phrase for a user login validation request. To view the status code and explanation of a an HTTP request, use a browser extension such as Postman (Google Chrome), or the http://curl.haxx.se/[curl] command line utility. +
...
. On the *On Error Propagate* component, add two *Set Variable* transformers and: +
