...
The following example illustrates how to configure the Anypoint Connector for HTTP (HTTP Connector) *Listener* source and *Load static resource* operation to serve up static content for use with HTTP, using the request path to lookup the resource.
