...
There are two easy ways to configure retries on the HTTP Requester:

. Using an *Until Successful* scope:
+
[source,xml,linenums]
----
...
----
You can enclose the requester in ans *Until Successful* scope, which allows you to configure the maximum number of retries, and the time between them.
You can read more about this scope in xref:mule-runtime::until-successful-scope.adoc[Until Successful Scope].

. Using the built-in mechanism in the requester:
By default, the HTTP Requester automatically retries the idempotent methods (PUT, DELETE, GET, HEAD, OPTIONS, and TRACE) three times before fail.
You can configure two system properties about this mechanism:
* `-M-Dmule.http.client.maxRetries=5` allows you to configure the number of retries before fail. You can also set it to zero to avoid this mechanism.
* `-M-Dmule.http.client.retryOnAllMethods=true` extends this mechanism to all the methods, and not only the idempotent ones.

In order to understand why the non-idempotent methods (such as POST) aren't retried by default, refer to xref:https://tools.ietf.org/html/rfc7230#section-6.3.1[RFC 7230].
