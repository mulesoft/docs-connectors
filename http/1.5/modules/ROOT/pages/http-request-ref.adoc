...
. Built-in mechanism +
The HTTP Requester automatically retries by default the idempotent methods `PUT`, `DELETE`, `GET`, `HEAD`, `OPTIONS`, and `TRACE`, three times before it fails. Thereby, you can configure two system properties for this mechanism:

* `-M-Dmule.http.client.maxRetries=5` +
This system property enables to configure the number of retries before the HTTP Requester fails, additionally, you can also set its value to zero to avoid using this mechanism.

* `-M-Dmule.http.client.retryOnAllMethods=true` +
This system property extends this mechanism to all the methods, and not only the idempotent ones.

For additional information on these properties, see xref:mule-runtime::mule-app-properties-system.adoc[System Properties].

In order to understand why nonidempotent methods, such as `POST`, aren't retried by default, refer to xref:https://tools.ietf.org/html/rfc7230#section-6.3.1[RFC 7230].
