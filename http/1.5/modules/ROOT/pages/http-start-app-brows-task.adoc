...
The following example illustrates how to configure Anypoint Connector for HTTP (HTTP Connector) *Listener* source to initiate a flow by starting an app from a browser and listen on a specific port for every network interface on the server. Then, the *Logger* component logs the message `Hello`.
To accomplish this example, you must create the Mule app, configure an HTTP global element, run, and test the app by starting it from a browser with curl commands.
