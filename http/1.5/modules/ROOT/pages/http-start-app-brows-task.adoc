...
The following example illustrates how to configure Anypoint Connector for HTTP (HTTP Connector) *Listener* source to initiate a flow. In this example, HTTP *Listener* listens on a specific port for every network interface on the server. Then, the *Logger* component logs the message `Hello`.
