...
|===
| Field | Type | Description | Default Value | Required
| Connection Timeout a| Number | Time to wait during a connection to a remote server before failing with a timeout. If the requester host configured resolves to multiple IPs, the host tries to connect to every IP.  Therefore, if every connection times out, the number of resolved IPs multiplies the global connection timeout. | 30000 |
| Send Tcp No Delay a| Boolean | Indicates whether or not to collect the transmitted data for greater efficiency and sent the data immediately. | true |
