= HTTP Connector Reference 1.5 - Mule 4
...
only serves as an extension definition. HTTP Connector configurations are divided between a server (`<http:listener-config>`) and a client (`<http:requester-config>`) capabilities.


[[listenerConfig]]
== Listener Configuration

Configuration element for an HttpListener.
...
=== Listener Connection Type

Connection provider for an HTTP Listener, manages the creation of HttpServer instances.
...
Configuration element for HTTP requests.
...
Connection provider for an HTTP request, handles the creation of HttpExtensionClient instances.
