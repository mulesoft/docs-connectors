...
In the following example you configure Anypoint Connector for HTTP (HTTP Connector) to consume a REST service by setting up the HTTP *Listener* and *Request* operations.
...
. Return to the *Request* configuration screen and set the *URL* field to `+https://jsonplaceholder.typicode.com/users+` to use HTTPS protocol.
