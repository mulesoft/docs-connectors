...
The HTTP *Request* operation requires a global request configuration with a connection provider. Since HTTP Connector version 1.8.0, the connection provider supports connectivity testing.

[NOTE]
Currently, the Connectivity Testing feature is supported in XML view mode only. The feature will support Studio UI view mode in the next release of Anypoint Studio.
...
. Click *Test connection...* to trigger a request to the `+http://localhost:80/thePath+` URL.

If a connection is established, the connector sends a request and then receives a response. A response status code in the range 0-399 indicates that the test connection succeeded. Any other response code indicates that the test connection failed.
...
In this example, if you click on the *Test Connection...* button in the global configuration window, HTTP Connector sends a POST request to `+http://localhost:80/testPath?Key1=Value1&Key2=Value2+`. +

The connector waits 30 seconds for a response and follows redirects if needed. The `key` parameters in both headers have the same name, which is allowed. For this example, the test connection is successful only if the response status code is 200.
...
[NOTE]
In this example, the available bindings are the `payload` and the `attributes`. You can also use the response validator configuration in the HTTP *Request* operation. However, in that context, the only available bindings are the `payload` and `attributes` from the response.
