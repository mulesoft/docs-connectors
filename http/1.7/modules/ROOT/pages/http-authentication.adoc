...
* *Token Manager* +
To access authorization information for client credentials and authorization codes, configure a token manager. Note that if you don't configure a token manager then the OAuth Module re-initializes every time a new request is made.
