...
+
Note that the user is not exposed to the token nor does the Mule app see customer credentials, but just an authorization code. 
...
To complete the process, you need to add the `token.url` property. After the Mule app obtains the authorization code (through the authorization callback URL), OAuth requests an access token from `token.url`.   
