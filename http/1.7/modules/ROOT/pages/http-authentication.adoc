...
You can also configure the OAuth2 authorization code grant type authentication using the xref:access-management::connected-apps-developers.adoc[Connected Apps] feature.  

The following example shows how to configure the OAuth2 authorization code grant type authentication for the HTTP *Request* operation by creating a Mule app to access a protected resource and the resource in the resource server. See xref:mule-runtime::authorization-grant-types.adoc[OAuth Authorization Grant Types]. The example covers how to:
...
This example requires that you have an Anypoint Platform account that has the organization administrator permission.
...
. The Mule app redirects the request to the protected resource in the Anypoint organization server.
. The Anypoint organization server rejects the request because the Mule app doesn't have a token.
. The Mule app redirects the user to an URL corresponding to the authorization server, indicating where to call the Mule app back when the authorization code is available.
...
. The Mule app calls the authorization server again and provides the authorization code to obtain an access token (stored in a object store).
. The Mule app calls the authorization server, providing a valid access token.
. The Anypoint organization server validated the token and delivers the protected resource after successful authentication. 
+
Note that the user is not expose to the token nor the Mule app sees customer credentials, but just an authorization code. 
...
The Mule app consists of an HTTP *Listener* source and an HTTP *Request* operation. If the `host`, `port`, and `resource` properties in your YAML file aren't set to an accessible resource, the Anypoint platform fails with a 401 status code because it expects to obtain the access token in the request:
...
The app uses credentials to identify itself to the authentication server. On your XML file, add the redirection to the authorization URL:
...
An OAuth dance mechanism starts when the authentication is added to the HTTP request connection. This creates an endpoint in the `localAuthorizationUrl` that redirects you to the `authorizationUrl` parameter used to configure the authorization server location. In the following example, the location is `https://anypoint.mulesoft.com/accounts/api/v2/oauth2/authorize`.
After the `</http:listener-config>` and before `<flow name="authCodeFlow">`, add the following statement to enable the OAuth authorization code:
...
The Connected Apps feature enables you to delegate access to applications that use APIs to interact with Anypoint Platform. This feature supports OAuth 2.0.  See xref:access-management::connected-apps-developers.adoc[Connected Apps]. To create a Connected App in the Anypoint Platform:
 ...
 . In Anypoint Platform > *Access Management* > select *Connect Apps*.
 . Click *Create App*.
 . Set a name.
 . Set *Type* to `App acts on behalf of a user`.
 . Set *Grant Type* to `Authorization Code`.
 . Set *Website URL* to a website you choose.
 . Set *Redirect URIs* to a website you choose. You are configuring the `external.callback.url`.
 . In *Scopes*, set *General* to `Full Access`.
 . Click *Save*.
 
A new Connected App appears enabling you to copy the *ID* and *Secret*. Use those buttons to update the properties YAML file:
...
To complete the process, you need to add the `token.url` property. After the Mule app obtains the authorization code (through the authorization callback URL), OAuth request an access token from `token.url`.   
...
To run the Mule app and get the get the authorization code, perform the following steps before the access token expires:

. In your browser, enter the local authorization URL `http://localhost:8070/test`.
+
Anypoint Platform prompts you to log in. 
. Log in to Anypoint Platform using your credentials. 
+
Anypoint Platform prompts you to authorize access to your account. Connected App now acts on your behalf.
...
+
`Successfully retrieved access token` appears as body text in the browser you used to initiate the OAuth2 dance.
. For the token to get data, enter the local authorization URL in a browser again `http://localhost:8081/github`.
+ The request is processed with the token retrieved. 
