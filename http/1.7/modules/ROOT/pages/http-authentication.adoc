...
[NOTE]
This example requires you to have access to an Anypoint Platform account with organization administrator permission and be familiar with xref:mule-runtime::configuring-properties.adoc[Configuring Properties].
...
Note that this example uses dynamic configuration represented by the use of an expression in the `resourceOwnerId` parameter. See xref:mule-sdk::static-dynamic-configs.adoc[Dynamic Configurations].
...
A new Connected App appears enabling you to use the *Copy Id* and *Copy Secret* buttons to update the properties YAML file:
...
. In your browser, enter the flow's URL `http://localhost:8070/test`.
+
