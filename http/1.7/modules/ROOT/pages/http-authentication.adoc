...
Thw following example shows how to configure thw OAuth2 authorization code grant type authentication for the HTTP *Request* operation by creating a Mule app to access a protected resource and the resource in the resource server. The example covers how to:
...
. The Anypoint organization server rejects the request the resquest because the Mule app doesn't have a token.
...
. The user authorizes the authentication server to act on their behalf.
. The authorization server receives and sends an authorization code back to the redirect URI.
. The Mule app uses the authorization code to obtain an access token and stores it in a object store.
. The user submits the request again and the Mule app accesses the resource in the Anypoint organization server with a valid token.
. The Anypoint organization server validated the token and serves the protected resource after successful authentication. 
+
Note that the user doesn't see the see token nor the Mule app sees customer credentials, but just an authorization code. 
