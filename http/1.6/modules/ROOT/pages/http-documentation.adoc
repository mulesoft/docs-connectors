...
| Reject Invalid Transfer Encoding a| Boolean |  If `true`, requests with an invalid value for Transfer-Encoding header are rejected with a 400 Bad Request message. The valid values are defined in `RFC-7230` and `RFC-2616`. They are: `chunked`, `deflate`, `compress`, `identity` and `gzip` |  false |
|===
