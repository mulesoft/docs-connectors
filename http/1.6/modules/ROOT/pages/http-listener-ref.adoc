...
The following example shows how to configure  *Response streaming mode* in Studio. The main contact data for an account always returns using `Content-Length` encoding:
...
In the Configuration XML editor, the `allowedMethods` and `responseStreamingMode` configurations looks like these:
...
== Configure Rject Invalid Transfer Encoding
