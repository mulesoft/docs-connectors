...
image::http-listener-read-timeout.png[HTTP Listener configuration window with Reject invalid transfer encoding field deselected]

If `true`, requests with an invalid value for Transfer-Encoding header are rejected with a 400 Bad Request message.
== See Also
