...
To configure the OAuth2 Authorization code grant type for the HTTP *Request* operation review the following example that shows how to create a Mule app to access a protected resource, GitHub user data, on the GitHub OAuth authentication server. The example covers how to:
...
To set up the authorization, refer to the steps on the following diagram:
...
The following diagram describes the relationship between both CA and OAS:
