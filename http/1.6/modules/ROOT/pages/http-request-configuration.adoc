...
To use an HTTP *Request* operation, you must set, at a minimum, a host URI, which can include a path. You can additionally configure a receiving port and a communication protocol.
...
Two ASCII `..` full stop characters define the range of failure status codes. Any value from 500 through 599 is considered a failure and raises an error. If the HTTP response has any other status code value, it's considered a success.
...
In the *Configuration XML* editor, the `until-successful`, `maxRetries`, and `rmillisBetweenRetries` configurations look like this example:
