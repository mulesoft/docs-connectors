...
| Response Size | `org.glassfish.grizzly.nio.transport.TCPNIOTransport.max-send-buffer-size` | Maximum size of the send buffer that controls the outgoing packages until there is an acknowledgement from the receiving systems. | `1048576` (1MB)
|===
...
There is no such Max Client Response Header system property as there is no maximum limit for max client HTTP response headers.
