...
To change the default behavior, configure *Request streaming mode* to any of these options:

* *ALWAYS* +
Always enable streaming regardless of the payload type.
* *AUTO* (default) +
The behavior depends on the payload type. If the payload is stream, streaming is enabled, otherwise it is disabled.
* *NEVER* +
...
image::http-request-6.png[HTTP Request configuration window with Response streaming mode field set to ALWAYS]
...
=== Configure Request Streaming System Property

The previous *Request streaming mode* configuration causes the Mule application to send HTTP requests in chunks, but the configuration doesn't imply that the Mule app streams the payload internally. The HTTP *Request* operation needs all the data in memory before sending HTTP requests. Therefore, if the Mule app needs to send a huge request body, the Mule app can generate a high memory consumption, and eventually cause an out-of-memory error. To prevent such an issue, Mule provides the `mule.http.requestStreaming.enable` system property to enable internal request streaming.
If you configure the system property, the HTTP *Request* operation won't wait until having the whole request body send the request, and then the Mule app won't need that memory.

To configure the system property, set it when starting Mule by adding `-M-Dmule.http.requestStreaming.enable=true`. The system property defaults to `false`.

You can also configure the internal buffer `mule.http.requestStreaming.bufferSize` system property for the streaming mechanism. To configure the system property, set it when starting Mule by adding `-M-Dmule.http.requestStreaming.bufferSize=value`. The system property is measured in bytes and defaults to `8192`.
