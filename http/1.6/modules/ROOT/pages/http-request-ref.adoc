...
Anypoint Connector for HTTP (HTTP Connector) *Request* operation enables you to consume an external HTTP service using the default `GET` method. You can choose what methods the operation accepts. The `GET` and `OPTIONS` methods do not send the payload in the request, the body of the HTTP request is empty. The remaining methods `CONNECT`, `DELETE`, `PATCH`, `POST`, and `PUT` send the message payload as the body of the HTTP request.
...
By default, the operation sends the Mule message payload as the HTTP request body but you can customize it using a DataWeave script or expression.
