...
image::http-request-3.png[Authentication field set to Basic authentication and User field set to testuser and Password field to Mypasswordd]
...
//Need to clarify this section with developers, as it is not a full DW expression
...
In the *Configuration XML* editor, the `target` and `targetValue` configurations look like these:
...
By default, the *Stream response* field is deselected (`false`). When you select this field (`true`), you enable Mule to manage the HTTP response as a stream by saving chunks to an in-memory buffer. You can also configure the *Response buffer size* field attribute. Defaults to `1024`. To avoid issues, it's important to consume responses when streaming.
...
[WARNING]
The HTTP *Request* operation does not use the reconnection strategy for retries. The reconnection strategy is a mechanism that Mule SDK uses to re-establish connections when a `ConnectionException` is caught. This mechanism does not affect connections to an HTTP service created by the HTTP *Request* operation.
To learn about connections, refer to the xref:mule-sdk::connections.adoc[Mule SDK Connections] documentation.

To configure how many times the HTTP *Request* operation can try consuming an external HTTP service (_retries_), you can either use:
...
The following example shows how to configure the retry mechanism with *Until Successful* scope in Studio:

. In the *Mule Palette* view, select *Core > Until Successful*.
. Drag *Until Successful* scope component to the Studio canvas.
...
In the *Configuration XML* editor, the `until-successful`, `maxRetries`, and `rmillisBetweenRetries` configurations look like these:
...
To learn about the scope, refer to the xref:mule-runtime::until-successful-scope.adoc[Until Successful Scope] documentation.

=== Configure Built-In Retry Mechanism

When the HTTP *Request* operation receives an error of type `Remotely Closed`, the operation automatically retries the idempotent methods `PUT`, `DELETE`, `GET`, `HEAD`, `OPTIONS`, and `TRACE`, three times before the retry fails. Thereby, you can configure two system properties for this mechanism:

* `mule.http.client.maxRetries=5` +
This system property enables you to configure the number of retries before the HTTP *Request* operation fails. Additionally, you can also set the system property value to zero to avoid using the built-in mechanism. +
