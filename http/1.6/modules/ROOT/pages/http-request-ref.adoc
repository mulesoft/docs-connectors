...
This configuration tells the requester to send the request in a chunked manner. By default, if the type of the payload is a stream, streaming is used to send the request. You can change this default behavior. Select *General > Request > Request Streaming* with one of the following values:
...
=== Internal Request Streaming

The above configuration only means that the request will be sent in a chunked manner, but it doesn't imply that it will stream the payload internally: the requester operation needs all the data in memory before start sending the request. Therefore, if the application needs to send a huge request body, it could generate a high memory consumption, and eventually an out-of-memory error. In order to prevent that issue, Mule Runtime provides a system property to enable the internal request streaming. If you configure that property, the requester won't wait until having the whole request body to start sending it, and then the application won't need that memory.
To configure the system property, run the mule runtime adding `-M-Dmule.http.requestStreaming.enable=true`. The default value for this property is `false` until mule runtime 4.4, and will be `true` since 4.5.
It's also configurable the internal buffer utilised for this streaming mechanism. It's name is `mule.http.requestStreaming.bufferSize`, it's measured in Bytes and its default value is 8192.
