...
You can also configure the internal buffer `mule.http.requestStreaming.bufferSize` system property for the streaming mechanism. To configure this property, set it when starting Mule by adding `-M-Dmule.http.requestStreaming.bufferSize=value`. This system property is measured in bytes and defaults to `8192`.
