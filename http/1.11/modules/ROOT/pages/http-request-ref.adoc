...
| Request Header Size |  `mule.http.headerSectionSize` | Maximum size in bytes accepted for the HTTP request header section (request line + headers) | `8192`
| Response Header Size |  `mule.http.client.headerSectionSize` | Maximum size in bytes accepted for the HTTP response header section (response line + headers) | `8192`
...
You can also configure Global Element Properties for the response validator:

. In Studio, open the *HTTP Request configuration* window.
. Select *Settings*.
. Set *Response validator* to *Success status code validator*.
. In *Values*, add the list of acceptable status codes, separated by commas, such as `200,201`. +
 If the HTTP response returned has any other status value, the operation fails and raises an error.
...
Configure this system property when starting Mule by adding: +
 ...
 The default value of this property depends on the Mule runtime version. For Mule 4.4 and earlier, the property defaults to `false`. For Mule 4.5 and later, the property defaults to `true`.
...
By default, the *Stream response* box is deselected (`false`). When you select this box (`true`), you enable Mule to manage the HTTP response as a stream by saving chunks to an in-memory buffer. The *Response buffer size* value defaults to `-1`, which means that the buffer size isn't limited and that the connector uses a system-defined default. You can set a positive value (in bytes) to limit the buffer, however, to avoid buffering issues, it's important to consume responses when streaming.
...
To learn how to set system properties, refer to the xref:mule-runtime::mule-app-properties-system.adoc[System Properties] documentation. To understand why nonidempotent methods, such as `POST`, aren't retried by default, refer to https://datatracker.ietf.org/doc/html/rfc7230[RFC 7230].
