...
By default, the *Stream response* box is deselected (`false`). When you select this box (`true`), you enable Mule to manage the HTTP response as a stream by saving chunks to an in-memory buffer. The *Response buffer size* defaults to `-1` (buffer size not limited; system-defined). You can set a positive value (in bytes) to limit the buffer; however, to avoid buffering issues, it's important to consume responses when streaming.
