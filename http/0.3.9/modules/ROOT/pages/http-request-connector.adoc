...
|Enable Datasense |The *Enable DataSense* checkbox allows Studio to use xref:studio::datasense-concept.adoc[DataSense] to expose metadata from the RAML file and interact with it on other elements in Studio.
|===
...
Whenever the message contains outbound attachments, the connector automatically generates an HTTP Request with the header `Content-Type:multipart/form-data`. The payload of the Mule message is ignored, and instead each attachment in the message is converted into a part of the HTTP Request body. If you want to create a different multipart request, then the Content-Type header can be set with a different value by adding custom parameters with <<adding-custom-parameters-visual,the Studio Visual Editor>> or with <<adding-custom-parameters-xml,the Studio XML Editor>>.
...
[[adding-custom-parameters-visual]]
...
image::http-request-connector-a7f0c.png[params set dynamically]
...
[TIP]
`mule.http.disableRoundRobin` system property is available since Mule 3.8.4. Make sure you are up to date with the latest update site.
