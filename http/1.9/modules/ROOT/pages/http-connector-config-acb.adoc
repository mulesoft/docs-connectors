...
//This include won't work for me idk why
include::anypoint-code-builder::partial$acb-connectors-create-integrations.adoc[Create Integrations with Anypoint Code Builder]
...
. In the Explorer view, select the XML file for your project, such as `get-flights.xml`:
+
...
----
. Add the `<http:listener-config/>` component from a snippet.
+
For more information about snippets, see xref:code-builder::int-work-with-code-snippets.adoc[].
+
