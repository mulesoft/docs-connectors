= HTTP Connector
:page-aliases: 3.5@mule-runtime::http-connector.adoc
...
To learn more about endpoints, refer to xref:3.5@mule-runtime::anypoint-connectors.adoc[Anypoint Connectors].
...
|*Enable HTTPS* |Check this box to send HTTPS over SSL protocol. If you enable HTTPS, ensure that you configure, then reference a global HTTPS connector on your endpoint.
...
|*Encoding* |Choose from a drop-down to identify the string encoding for a message (i.e. UTF-8).
...
|*MIME Type* |Select from the dropdown list one of the formats this endpoint supports. For example, set the MIME type to `text/HTML` on HTTP endpoints to handle plain text messages.
...
|*Keep Alive* |Check the *Keep Alive*] box to maintain an open socket connection when a small interruption occurs.
|===

== Reference Documentation

See the xref:3.5@mule-runtime::http-transport-reference.adoc[HTTP Transport Reference] for details on setting the properties for a HTTP endpoint using an XML editor.
