...
| Name | Type | Description | Default Value | Required
| Configuration | String | The name of the configuration to use. | | *x*
| filename a| String |  Name of the file being uploaded.|   | *x*
| length a| Integer |  Size in bytes of the file being uploaded.|   | *x*
| Alt txt a| String |  Description of image for screen-reader.|   |
| Snippet type a| String | Syntax type of the snippet being uploaded.|   |
...
**NOTE : ** This method is deprecated and will stop functioning on March 11, 2025. For more details, visit: https://api.slack.com/methods/files.upload
...
Finishes an upload started with `files.getUploadURLExternal`. This operation makes an HTTP POST request to the `files.completeUploadExternal` endpoint.
...
| Channel id | String | Channel ID where the file will be shared. If not specified the file will be private.| |
| Initial comment | String | The message text introducing the file in specified `channels`.| |
| Thread ts | String | Provide another message's `ts` value to upload this file as a reply. Never use a reply's `ts` value; use its parent instead.| |
...
| Custom Headers a| Object | Custom headers to include in the request. The specified custom headers are merged with the default headers that are specified in the configuration. |  |
| Response Timeout a| Number |  Timeout for requests to the remote service. |  |
| Response Timeout Unit a| Enumeration, one of:44166921
...
receive when the workflow step starts. **NOTE**: The embedded variable format is set and replaced by the workflow system. You cannot create custom variables that will be replaced at runtime. [Read more about variables in workflow steps here](/workflows/steps#variables). |  |
