...
This example shows how to upload a file or file content to a public or private channel.
...
+
This operation makes a GET request to Slack. It requires filename and length as mandatory fields, which are provided as `attributes.fileName` and `attributes.size`, obtained from the Read File operation.
+
+
* Set Variable Second
+
...
+
This operation makes a POST request to Slack using the file content retrieved from the Get Upload URL operation response. The URL is set to `#[payload.upload_url]`, and the content is set to `#[vars.fileContent]`.
...
+
This operation makes a POST request to Slack to complete the file upload process. It requires a files array as a mandatory parameter.
+
**Note**: If `Channel id` is not provided, the file will be uploaded to the Slack server and remain private
...
http://www.mulesoft.org/schema/mule/file http://www.mulesoft.org/schema/mule/file/current/mule-file.xsd">
	<slack:config name="Slack_Connector_Config" doc:name="Slack Connector Config" doc:id="633042e0-f79b-448c-b59f-cab049c61875" >
		<slack:slack-auth-connection >
			<slack:oauth-authorization-code consumerKey="7592115463188.8029816604519" consumerSecret="8ffac51a4da607ccc162238f7f62f5d5" scopes="channels:write groups:write im:write mpim:write files:write files:write:user"/>
			<slack:oauth-callback-config listenerConfig="HTTP_Listener_config1" callbackPath="/callback" authorizePath="/authorize" externalCallbackUrl="http://localhost:8081/callback"/>
		</slack:slack-auth-connection>
	</slack:config>
	...
	<file:config name="File_Config" doc:name="File Config" doc:id="d1713841-299b-4778-9f04-567c174701aa" >
		<file:connection workingDir="/Users/susanta.kora/Downloads" />
	</file:config>
	<flow name="upload-file-to-slack" doc:id="bcca99f9-6fd8-4b02-b9d6-83683f9d2f02" >
	<http:listener doc:name="Listener" doc:id="330f48d3-cec8-4fda-833c-2feb0cdc7081" config-ref="HTTP_Listener_config1" path="/file-upload"/>
		<file:read doc:name="Read" doc:id="cb378cad-3ed2-4385-babc-6a373ec124e7" config-ref="File_Config" path="/Users/susanta.kora/Downloads/VSCode-darwin-universal (1).zip"/>
            <set-variable doc:name="Set Variable" doc:id="04f2e56a-ea51-43ec-93cd-301aeaf90809" variableName="fileContent" value="#[payload]"/>
            <slack:get-filesget-upload-urlexternal doc:name="Get upload url" doc:id="c937d6ed-7c3e-4254-80c3-5a172d9e34ba" config-ref="Slack_Connector_Config" length="#[attributes.size]" filename="#[attributes.fileName]"/>
            <set-variable value="#[payload.file_id]" doc:name="Set Variable" doc:id="a70bb92f-f7ee-4e62-b4ca-16b8668f3147" variableName="fileId"/>
            <http:request method="POST" doc:name="Request" doc:id="aa2b115a-548e-4d3d-b468-efb656b80333" url="#[payload.upload_url]" responseTimeout="#[900000]">
                <http:body ><![CDATA[#[vars.fileContent]]]></http:body>
            </http:request>
            <ee:transform doc:name="Transform Message" doc:id="5c134af4-8ca7-41a2-a9db-cbaa5a18a163" >
                <ee:message >
                    <ee:set-payload ><![CDATA[%dw 2.0
output application/json
---
write(
    ...
    "application/json"
            ...
            <slack:create-filescomplete-upload-external doc:name="Complete Upload External" doc:id="abd96288-d944-4497-9970-cd694d321146" config-ref="Slack_Connector_Config" channelId="${channel.id}" initialComment="Initial Comment" target="completeUploadExt">
                <slack:files >
                    <slack:file value="#[payload]" />
                </slack:files>
            </slack:create-filescomplete-upload-external>
		<logger level="INFO" doc:name="Logger" doc:id="7d9e2166-0c59-4059-b7ce-e4903b545b44" />
            </flow>
</mule>
...
image::slack-connector-file-content-upload-flow.png["App flow that shows the components used uploading a file to slack"]
