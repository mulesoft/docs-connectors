...
The following examples show how to create public Slack channel and how to post a message to a channel.
...
.App flow for the Creating Channel example
image::slack-connector-create-channel-flow.png[This app flow shows the components used in the Creating a Channel example.]

Implementing this example involves creating a new Mule project and configuring HTTP Listener, the Slack Connector Create Channel operation, two Transform Message components, and one Logger component.

=== Prerequisites

This example requires the `channels:write` scope on both the connection configuration and Slack application.
...
. Start the Mule app.
. Send as POST request to `\http://localhost:8081/account?name="your channel name"``.
...
This example requires the following scopes in both the connection configuration and Slack app:
...
* HTTP Listener
+
initiate a Mule flow when a call is made to the `/sned/message` path on localhost port 8081:
+
