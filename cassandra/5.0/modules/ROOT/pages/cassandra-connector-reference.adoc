= Apache Cassandra Connector 5.0 Reference 
:page-aliases: connectors::cassandra/cassandra-connector-reference.adoc



Anypoint Connector for Cassandra (Cassandra Connector) implements the DataStax Java Driver for Apache Cassandra 4.17.0.
...
** HTTPS
** LDAPS
** DISABLED | Algorithm that clients use to validate the server host name against the server certificate. The default is `DISABLED`, which turns hostname validation off and matches version 4.1 behavior, where the setting wasn't enforced. Set `HTTPS` or `LDAPS` to require the certificate Subject Alternative Name or CN to match the host. Enabling validation can reject servers whose certificates don't include a matching host name. |  DISABLED |
...
** V2
** V3
** V4 | Native protocol version that the driver uses. Don't select `V1` or `V2`. Those values fail at application initialization. Leave this field unset to let the driver negotiate, including native protocol V5 on Apache Cassandra 5.x. You can also select `V3` or `V4`. |  |
...
| Request Timeout (ms) a| Number | Per-request timeout for CQL operations, in milliseconds. The default of `12000` preserves the 12-second deadline from version 4.1. Raise this value for high-latency clusters or heavy DDL. Lower it to fail faster. This timeout is distinct from *Max Schema Agreement Wait Seconds*, which only bounds the post-DDL schema-agreement poll. |  12000 |
| Compression a| Enumeration, one of:

** LZ4
** NONE
** SNAPPY|  The compression to use for the transport. |  |
| Prepared Statements Cache Size a| Number |  Cache size for prepared statements. Set a value with at least the number of different statements that you handle in your Mule application. |  10 |
| Local Datacenter a| String | Local datacenter for the driver's default load-balancing policy. Required when contact points don't share one datacenter, such as in a multi-DC cluster. You can omit this field for single-DC clusters. If your cluster doesn't use a single inferred datacenter, set this field or the connection fails. |  |
| TLS Configuration a| <<Tls>> | Configures TLS for the Cassandra connection. |  |
...
Changes the type of a column. To check compatibility, see https://docs.datastax.com/en/archived/cql/3.1/cql/cql_reference/cql_data_types_c.html[CQL type compatibility].

On Apache Cassandra 5.x, *Change Column Type* fails because Cassandra no longer supports altering column types. Add a new column, backfill the data, then drop the old column.
