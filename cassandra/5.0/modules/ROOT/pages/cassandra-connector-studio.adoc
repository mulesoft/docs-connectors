...
|===
|Field |Description
|Local Datacenter | Local datacenter for the driver's load-balancing policy. Set this field when contact points don't share one datacenter, such as in a multi-DC cluster. You can omit it for single-DC clusters.
|Request Timeout (ms) | Per-request timeout for CQL operations, in milliseconds. The default is `12000`.
|Protocol Version | Native protocol version that the driver uses. Don't select `V1` or `V2`. Leave this field unset to let the driver negotiate, including native protocol V5 on Apache Cassandra 5.x.
|===
+
