= Using Anypoint Studio to Configure Apache Cassandra Connector 5.0 
...
|===
|Field a|Description
|*Cassandra Keyspace* | Apache Cassandra keyspace
...
+
. On the *Advanced Settings* tab, optionally specify reconnection information, including a reconnection strategy, and complete these fields when required:
+
[%header,cols="30s,70a"]
|===
|Field |Description
|*Cassandra Cluster Nodes* | Contact points that select the cluster. Use this field instead of the deprecated *Cassandra Cluster Name* field.
|*Local Datacenter* | Local datacenter for the driver's load-balancing policy. Set this field when contact points don't share one datacenter, such as in a multi-DC cluster. You can omit it for single-DC clusters. See xref:cassandra-connector-config-topics.adoc#configure-local-datacenter[Configure the Local Datacenter].
|*Request Timeout (ms)* | Per-request timeout for CQL operations, in milliseconds. The default is `12000`. See xref:cassandra-connector-config-topics.adoc#configure-request-timeout[Configure the Request Timeout].
|*Protocol Version* | Native protocol version that the driver uses. Don't select `V1` or `V2`. Leave this field unset to let the driver negotiate, including native protocol V5 on Apache Cassandra 5.x. See xref:cassandra-connector-config-topics.adoc#configure-protocol-version[Configure the Protocol Version].
|===
+
For the remaining Advanced Settings fields, see xref:cassandra-connector-reference.adoc#config_connection[Connection].
+
. Optionally, configure a connection that uses TLS by completing these fields on the *Security* tab:
+
[%header,cols="30s,70a"]
|===
|Field |Description
|*Endpoint identification algorithm* | Algorithm that clients use to validate the server host name against the server certificate. The default is `DISABLED`. Set `HTTPS` or `LDAPS` to require the certificate to match the host.
|*TLS Configuration* | TLS context for the Cassandra connection. Version 5.0 applies the keystore, truststore, and hostname validation settings to the driver.
|===
. Click *Test Connection* to confirm that Mule can connect with the specified server.
. Click *OK*.
