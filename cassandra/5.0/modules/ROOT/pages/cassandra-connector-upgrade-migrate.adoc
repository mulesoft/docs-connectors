...
* This connector now supports Apache Cassandra 5.x. The connector uses DataStax Java Driver 4.17.0.
* This connector requires Mule 4.6.0 or later. Applications that run on Mule 4.1 through 4.5 must stay on Apache Cassandra Connector 4.1.x.
* The *Cassandra Cluster Name* field is removed. Applications that still set `clusterName` on `cassandra-db:connection` fail to deploy. Delete the attribute. Contact points and xref:cassandra-connector-reference.adoc#config_connection[Cassandra Cluster Nodes] still select the cluster.
...
** The DataStax Java Driver is upgraded to version 4.17.0.
** The `jackson-core` and `jackson-databind` libraries are upgraded to version 2.22.2.
** The `netty-codec` and `netty-handler` libraries are upgraded to version `4.1.137.Final`.
