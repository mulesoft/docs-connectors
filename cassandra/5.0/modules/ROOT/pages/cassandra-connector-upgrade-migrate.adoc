...
* The xref:cassandra-connector-reference.adoc#config_connection[*Cassandra Cluster Name*] field is removed from Studio. It no longer appears on the *General* or *Advanced Settings* tab. Applications that still set `clusterName` on `cassandra-db:connection` fail to deploy. Delete the attribute. Contact points and xref:cassandra-connector-reference.adoc#config_connection[*Cassandra Cluster Nodes*] still select the cluster.
* The new xref:cassandra-connector-reference.adoc#config_connection[*Local Datacenter*] field is on the *Advanced Settings* tab. It sets the local datacenter for the driver's load-balancing policy. Set this field when contact points don't share one datacenter, such as in a multi-DC cluster. You can omit it for single-DC clusters.
* The new xref:cassandra-connector-reference.adoc#config_connection[*Request Timeout (ms)*] field is on the *Advanced Settings* tab. It sets the per-request timeout for CQL operations. The default is `12000` milliseconds, which preserves the 12-second deadline from version 4.1.
* On Apache Cassandra 5.x, the xref:cassandra-connector-reference.adoc#changeColumnType[*Change Column Type*] operation fails because Cassandra no longer supports altering column types. Add a new column, backfill the data, then drop the old column.
