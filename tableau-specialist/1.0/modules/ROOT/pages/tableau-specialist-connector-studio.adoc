= Use Studio to Configure Tableau Specialist Connector 1.0 - Mule 4
...
Add Anypoint Connector for Tableau Specialist (Tableau Specialist Connector) to your Mule project to automatically populate the XML code with the connector's namespace and schema location and add the required dependencies to the project's `pom.xml` file:
...
You can configure one of these sources to use with Tableau Specialist Connector:
...
To add an operation for Tableau Specialist Connector, follow these steps:
...
When you configure a connector, configure a global element that all instances of that connector in the app can use. Configuring a global element requires you to provide the authentication credentials that the connector requires to access the target Tableau Specialist system. Tableau Specialist Connector supports Hyper connection for Hyper configuration, and Basic authentication and PAT authentication for Rest configuration.

To configure the global element for Tableau Specialist Connector, follow these steps:
...
After you configure a global element for Tableau Specialist Connector, configure the other required fields for the connector. The required fields vary depending on which connector operation you use.
