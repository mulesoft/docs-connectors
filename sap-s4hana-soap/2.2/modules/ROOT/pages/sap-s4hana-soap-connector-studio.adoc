...
. Click *Addressing* tab to configure the following values:
+
[%header,cols="30s,70a"]
|===
|Studio Field |Description
|Version | WS-Addressing version. If nothing is specified, the connector will use the value from configuration.
|From| Reference of the endpoint where the message originated from. If nothing is specified, the connector will use the value from configuration.
|===
