= Microsoft Dynamics 365 Connector 2.6 Examples 
...
== Link Records Between Entities

Establish relationships between entities using the OData bind annotation with the syntax: `"<navigationproperty>@odata.bind": "/<entitysetname>(<recordid>)"`

=== Example: Link Contact to Customer

This example creates a Contact record and links it to an existing Customer (Account).
...
---
{
    ...
    emailaddress1: payload.EmailAddress,
    "parentcustomerid@odata.bind": "/accounts(" ++ payload.CustomerId ++ ")"
}
----

Common navigation properties:

* `parentcustomerid@odata.bind` - Links a Contact to an Account (Customer)
* `primarycontactid@odata.bind` - Links an Account to a primary Contact
* `customerid@odata.bind` - Links an entity to a Customer (Account or Contact)

NOTE: For polymorphic lookups that accept multiple entity types, append the entity type suffix: `parentcustomerid_account@odata.bind` for Accounts or `parentcustomerid_contact@odata.bind` for Contacts.

You can link multiple relationships by including multiple `@odata.bind` annotations in the same payload.
