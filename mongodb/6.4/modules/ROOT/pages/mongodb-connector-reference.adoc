...
OAuth 2.0 (OIDC) Client Credentials connection provider for MongoDB. The provider performs the OAuth client_credentials token POST via HttpService so that the optional audience parameter can be placed in the form-encoded request body (Auth0, Atlassian, and similar identity providers require this). The acquired token is fed into the MongoDB driver via the OIDC callback.
...
| Audience a| String |  Optional OAuth 2.0 audience parameter required by some identity providers (for example, Auth0). Sent in the form-encoded request body to the token endpoint. |  |
| Custom Token Request Parameters a| Object |  Additional form-encoded parameters appended to the OAuth token POST body. Use for identity-provider-specific fields beyond the RFC standards (for example, Microsoft Entra v1 `resource`, Auth0 `organization`, Okta `claims`). |  |
| Include Accept Header a| Boolean |  When `true`, sends `Accept: application/json` on the OAuth token request. Disable for identity providers that reject the header. |  `true` |
| Client Authentication a| Enumeration, one of:

** BASIC_AUTHENTICATION_HEADER
** BODY |  How client credentials are presented to the token endpoint, per RFC 6749 §2.3.1. `BASIC_AUTHENTICATION_HEADER` (client_secret_basic, the RFC-recommended default) sends them in an HTTP Basic Authorization header; `BODY` (client_secret_post) sends them as form fields in the request body. Other parameters (audience, scope, custom parameters) are always sent in the body. |  `BASIC_AUTHENTICATION_HEADER` |
| Allow Invalid Hostnames a| Boolean |  Disables TLS hostname verification on the MongoDB connection only. Required when MongoDB Atlas traffic is fronted by an NLB or proxy whose certificate CN/SAN does not match the connection hostname. The OAuth token endpoint always verifies hostnames; this setting does not affect identity provider TLS. Not for production or FIPS environments: disabling hostname verification removes a control mandated by FIPS 140-3, so the provider fails fast when this is `true` and Mule is in FIPS mode. |  `false` |
| Direct Connection a| Boolean |  Connect directly to the configured server address without performing replica-set or topology discovery. Required when MongoDB Atlas is fronted by a single NLB, proxy, or HAProxy endpoint, where discovery would otherwise return real cluster member hostnames that are not reachable. Only honored when a single server address is configured. |  `false` |
| Token Endpoint Proxy a| <<ProxyConfig>> |  Optional HTTP proxy through which the OAuth token requests to the identity provider are routed. Does not affect the MongoDB connection itself. |  |
| Tls Context a| <<Tls>> | Configures TLS. If using the HTTPS protocol, you must configure TLS. |  |
| Client ID a| String |  OAuth 2.0 client identifier issued by the identity provider. |  | x
| Client Secret a| String |  OAuth 2.0 client secret issued by the identity provider. |  | x
...
** HOURS
** DAYS |  Time unit for the *Max Connection Idle Time* field. |  `SECONDS` |
| Token Refresh Skew a| Number |  Time subtracted from the OAuth `expires_in` value when determining how long to keep the cached bearer token. Larger values trigger earlier re-fetches and absorb clock skew between the Mule worker and the identity provider. |  `60` |
...
| Token Request Timeout a| Number |  Maximum time the OAuth token POST is allowed to take. Sent as the connection-level timeout to the underlying HTTP client. If the identity provider does not respond within this window, the request is aborted and connection establishment fails. |  `30` |
