= MongoDB Connector 6.4 Reference 
...
* <<config_oauth-client-credentials-connection, OAuth 2.0 - Client Credentials>>
...
[[config_oauth-client-credentials-connection]]
=== OAuth 2.0 - Client Credentials Connection Type

Uses the OAuth 2.0 (OIDC) client credentials grant to connect to MongoDB. The connector performs the OAuth client_credentials token POST itself so that you can place the optional audience parameter in the form-encoded request body, which Auth0, Atlassian, and similar identity providers require. The connector then feeds the acquired token into the MongoDB driver through the OIDC callback.
...
| Servers (host:port) a| Array of <<ServerAddress>> |  Server in which your MongoDB instance is located. You can specify multiple servers. |  | x
| Username a| String |  Username to use for authentication. |  |
| Database a| String |  Database name to which you want to connect. |  `admin` |
| Audience a| String |  Optional OAuth 2.0 audience parameter that some identity providers, such as Auth0, require. The connector sends this value in the form-encoded request body to the token endpoint. |  |
| Custom Token Request Parameters a| Object |  Additional form-encoded parameters that the connector appends to the OAuth token POST body. Use this field for identity-provider-specific fields beyond the RFC standards (for example, Microsoft Entra v1 `resource`, Auth0 `organization`, Okta `claims`). |  |
| Include Accept Header a| Boolean |  When `true`, the connector sends `Accept: application/json` on the OAuth token request. Set this field to `false` for identity providers that reject the header. |  `true` |
| Client Authentication a| Enumeration, one of:

** BASIC_AUTHENTICATION_HEADER
** BODY |  Determines how the connector presents client credentials to the token endpoint, per RFC 6749 §2.3.1. `BASIC_AUTHENTICATION_HEADER` (client_secret_basic, the RFC-recommended default) sends the credentials in an HTTP Basic Authorization header. `BODY` (client_secret_post) sends the credentials as form fields in the request body. The connector always sends other parameters, such as audience, scope, and custom parameters, in the body. |  `BASIC_AUTHENTICATION_HEADER` |
| Allow Invalid Hostnames a| Boolean |  Disables TLS hostname verification on the MongoDB connection only. Set this field to `true` when an NLB or proxy fronts your MongoDB Atlas traffic and its certificate CN/SAN does not match the connection hostname. The OAuth token endpoint always verifies hostnames, so this field does not affect identity provider TLS. Do not use this field in production or FIPS environments: disabling hostname verification removes a control that FIPS 140-3 requires, so the connector fails fast when you set this field to `true` and Mule is in FIPS mode. |  `false` |
| Direct Connection a| Boolean |  Connects directly to the configured server address without performing replica-set or topology discovery. Set this field to `true` when a single NLB, proxy, or HAProxy endpoint fronts your MongoDB Atlas deployment, where discovery would otherwise return real cluster member hostnames that you cannot reach. The connector honors this field only when you configure a single server address. |  `false` |
| Token Endpoint Proxy a| <<ProxyConfig>> |  Optional HTTP proxy through which the connector routes the OAuth token requests to the identity provider. This proxy does not affect the MongoDB connection itself. |  |
| Tls Context a| <<Tls>> | Configures TLS. If using the HTTPS protocol, you must configure TLS. |  |
| Client ID a| String |  OAuth 2.0 client identifier that the identity provider issues. |  | x
| Client Secret a| String |  OAuth 2.0 client secret that the identity provider issues. |  | x
| Token URL a| String |  Token endpoint URL of the identity provider. |  | x
| Scopes a| String |  Space-separated OAuth 2.0 scopes to request from the identity provider. |  |
| Connection Timeout a| Number |  How long the connector waits before timing out when establishing the socket connection. `0` is infinite. |  `30` |
| Connection Timeout Timeunit a| Enumeration, one of:
...
** HOURS
** DAYS |  Time unit for the *Connection Timeout* field. |  `SECONDS` |
| Socket Timeout a| Number |  Socket timeout that the connector uses for I/O socket read and write operations. Set this field to `0` if you do not want socket read and write operations to time out. |  |
| Socket Timeout Unit a| Enumeration, one of:
...
** HOURS
** DAYS |  Time unit for the *Max Connection Idle Time* field. |  `SECONDS` |
| Token Refresh Skew a| Number |  Time that the connector subtracts from the OAuth `expires_in` value when determining how long to keep the cached bearer token. Larger values trigger earlier refetches and absorb clock skew between the Mule worker and the identity provider. |  `60` |
| Token Refresh Skew Unit a| Enumeration, one of:
...
** MINUTES
** HOURS
** DAYS |  Time unit for the *Token Refresh Skew* field. |  `SECONDS` |
| Token Request Timeout a| Number |  Maximum time that the connector allows the OAuth token POST to take. The connector sends this value as the connection-level timeout to the underlying HTTP client. If the identity provider does not respond within this window, the connector aborts the request and connection establishment fails. |  `30` |
| Token Request Timeout Unit a| Enumeration, one of:
...
** MINUTES
** HOURS
** DAYS |  Time unit for the *Token Request Timeout* field. |  `SECONDS` |
|===
...
Configures an HTTP proxy for the OAuth token requests to the identity provider.
...
| Non Proxy Hosts a| String | Comma-separated list of hosts that the connector must not route through the proxy. |  |
|===
