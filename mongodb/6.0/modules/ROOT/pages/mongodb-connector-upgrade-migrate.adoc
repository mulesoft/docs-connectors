...
* **ISO 8601 Format:**
  Dates in JSON documents inserted or retrieved from MongoDB are transformed into the ISO 8601 format. Additionally, any String values in the JSON that match the ISO 8601 format will be automatically transformed and stored as Date types in MongoDB. This behavior is consistent whether the data is being sent to or retrieved from MongoDB.
...
This behavior applies in both directions: when dates are sent from the connector to MongoDB, they are converted to UTC, and when dates are retrieved from MongoDB via the connector, they are also presented in UTC. Additionally, any String values that match the ISO 8601 format will be stored as Date types in MongoDB. This ensures consistency in how dates are handled, regardless of whether data is being stored or retrieved, and standardizes the date format across different systems using MongoDB.
