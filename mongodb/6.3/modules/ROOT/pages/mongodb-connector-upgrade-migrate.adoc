= Upgrading and Migrating MongoDB Connector to version 6.x - Mule 4
...
== Removal of `numToSkip` in Version 6.x

In version 6.x of the MongoDB connector, the `numToSkip` parameter, previously used in the "Find documents" operation to control the offset of results, has been removed. This change is in line with MongoDB’s recommended best practices for performance optimization.

=== Reasons for the Removal

The `skip()` method in MongoDB, which was used internally by the `numToSkip` parameter, can lead to performance issues, especially as the offset increases. This is because the server must scan through the results from the beginning to the specified offset before returning any documents, which adds unnecessary load and slows down the operation.

=== Recommendations for Handling Pagination in v6.x

To optimize pagination and improve performance when migrating to MongoDB connector version 6.x, the following approaches are recommended:

. **Avoid using `skip()` for large offsets**: Instead of relying on the `numToSkip` parameter, it is more efficient to refine query conditions to exclude unwanted documents directly.
. **Use Range Queries**: Take advantage of existing indexes to perform range queries. For example, if your documents have a timestamp or unique ID field (indexed), you can use these fields to filter results directly, avoiding the need to scan through unwanted documents.
...
. **Apply `limit()` for Pagination**: Use the `limit()` method in combination with range queries to control the number of documents returned, effectively implementing pagination without the performance issues associated with `skip()`.
...
. **Combine Filters and Sorting**: Consider ordering documents by an indexed field, which allows for more efficient queries when paginating through results.
