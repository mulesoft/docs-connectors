...
+
Instead of relying on the *Num To Skip* field, it is more efficient to refine query conditions to exclude unwanted documents directly.

* Use range queries.
+
Use existing indexes to perform range queries. For example, if your documents have a timestamp or a unique ID field (indexed), you can use these fields to filter results directly, avoiding h scan through unwanted documents.
