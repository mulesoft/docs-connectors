...
This is not a paginated operation, it only returns the results of a single invocation to the API executeCommand.
In the case that the execution returns a cursor reference, meaning it has additional elements in the results, the transversal of the results of the execution of the command must be done by invoking additional executeCommand(s) with the getMore command.
See the examples documentation for this connector for more details
