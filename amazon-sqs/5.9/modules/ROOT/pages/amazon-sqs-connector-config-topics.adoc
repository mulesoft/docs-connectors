= Amazon SQS Connector 5.9 Additional Configuration Information - Mule 4
:page-aliases: connectors::amazon/amazon-sqs-connector-config-topics.adoc

== Long and short polling

Since version 5.9, the default behaviour of the Receive Messages source and the Read operation is to use long polling.

This behaviour can be controlled at individual source and operation level or it can be globally configured in your SQS Configuration using the "Default Wait Time" parameter under the *Advanced* tab.

image::amazon-sqs-config-wait-time.png[Default Wait Time Configuration]

The default value is 20 seconds. Setting this parameter to any value higher than 0 will mean that when querying for messages, SQS
will wait *at most* that amount of time before returning an empty response, however if any messages are present the calls will still return as soon as possible.

If you want short polling to be the default behaviour you can set this value to 0 and SQS will always return as soon as possible even if it means returning an empty response.
