...
Set this field to an integer value between `1` and `20` (the default) to specify long polling as the default behavior. The integer specifies how long to wait, in time units specified in the Time unit field, before returning an empty response.

Set this field to `0` to specify short polling as the default behavior.
...
*Question for Reviewers asked in the Slack channel. Why are the polling fields missing for the Read operation in Studio? Only the Wait Time field is showing up there.*
