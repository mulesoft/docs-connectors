...
With version 5.9, it's now possible to take advantage of long polling, which can reduce the number of network requests without affecting the throughput. The polling frequency is now also configurable instead of the default 1 second.

Additionally, you can now configure the number of threads for your sources, allowing for a higher throughput in some cases.
