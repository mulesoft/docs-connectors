...
| Queue Url a| String |  Permissions are added to the queue represented by this URL. This parameter is optional and if you do not specify queueUrl you must set it in the configuration level Default Global Queue URL. |  |
| Target Variable a| String |  The name of a variable to store the operation's output. |  |
| Target Value a| String |  An expression to evaluate against the operation's output. The expression outcome is stored in the target variable. |  `#[payload]` |
...
This operation changes the visibility timeout of a specified message in a queue to a new value. The maximum allowed timeout value is 12 hours. This means that you can't extend the timeout of a message in an existing queue to more than a total visibility timeout of 12 hours.
