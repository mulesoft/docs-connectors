...
Since the upgrade of AWS SDK from v1 to v2, the endpoint that the STS client of the AssumeRoleConnectionProvider uses to get session tokens now is Regionalized instead of Global. Refer to the https://docs.aws.amazon.com/sdkref/latest/guide/feature-sts-regionalized-endpoints.html[AWS STS Regionalized endpoints] documentation for details.
Additionally, as explained in the https://docs.aws.amazon.com/general/latest/gr/sts.html[AWS Security Token Service endpoints and quotas] documentation, regionalized endpoints shouldn't represent any change in the connector's functionality.
AWS recommends using Regional STS endpoints to reduce latency, build in redundancy, and increase session token validation.

If you encounter any issues about the endpoints change in the regions, you can force Amazon SQS Connector to use the global endpoint by specifying the URL endpoint https://sts.amazonaws.com in the Custom STS Endpoint field located in the Advanced tab of the Global Elements properties configuration window of the connector.
