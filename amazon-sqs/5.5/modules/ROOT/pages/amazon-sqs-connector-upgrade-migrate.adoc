...
Version 5.5.0 was deprecated because cluster mode support was accidentally removed. The behaviour used for running the application on single or all cluster nodes was added back to the connector, so be aware of this behaviour - now the source runs on all cluster nodes by default. It can be changed using the `Primary node only` parameter which is placed in the `Advanced tab`.
...
. When the application contains *Receive Message* source, choose whether the application should run on single or all cluster nodes by checking the `Primary node only` option which is placed in the `Advanced tab`.
