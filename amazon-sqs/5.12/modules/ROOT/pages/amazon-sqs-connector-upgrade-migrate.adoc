= Upgrading and Migrating Amazon SQS Connector to 5.12 
:page-aliases: connectors::amazon/amazon-sqs-connector-upgrade-migrate.adoc

Upgrade Anypoint Connector for Amazon SQS to 5.12 to improve and optimize the performance of your projects using long polling, multithreading, and more control over the polling parameters.
...
* Amazon SQS Connector now supports passing an external ID in Role-based authentication.
* In the *Receive messages* source, the *Time unit* field is renamed to *Start delay time unit*, and its default value is changed from `MILLISECONDS` to `SECONDS`.
+
[NOTE]
This was changed back in v5.12.2. The field in the *Receive messages* source is now called *Time unit* again, and its default value is set back to `MILLISECONDS`.

=== 5.11

All of the Amazon SQS Connector error types are now exported. You can see specific error messages instead of MULE:UNKNOWN.
