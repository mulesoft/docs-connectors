...
Amazon SQS Connector supports listening for messages on the primary node only or on all nodes when running in cluster mode. For more information, see <<xref:amazon-sqs-connector-config-topics.adoc#node-behavior,Receiving Messages in Cluster Mode>>.
