...
. In this version, the `Receive Messages` input source receives messages on the primary node only by default. If you're upgrading from version 5.5.0 and you want the `Receive Messages` input source to receive messages on all nodes, access the `Advanced` tab in the `Receive Messages` configuration and select the `Primary node only` field.
+
