...
Follow these steps to use variables for field values:

. Create a new Mule project in Studio.
. Add the following field values to the `mule-artifact.properties` file:
...
|fromNumber |The phone number from which the SMS is sent. This phone number is configured inside the Twilio instance.
|===
+
...
Follow these steps to configure the Mule app to send a message to a phone number and then redact the message:
...
** Set *Message Sid* to `#[messageSid]`, which is the variable stored in step 5.
+
