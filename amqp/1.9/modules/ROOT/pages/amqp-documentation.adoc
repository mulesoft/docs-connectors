...
| Max Inbound Message Body Size a| Number |  The maximum inbound message body size to set to the underlying AMQP Connector in bytes. |  67108864 (64 MB) | 
...
| Routing Keys a| Array of String |  List of routing keys to use when publishing messages. |  | 
| Delivery Mode a| Enumeration, one of:
...
| Content Type a| String |  The content type of the message body |  | 
| Encoding a| String |  The encoding of the message body |  | 
| Routing Key a| String |  The routing key to publish to in publish-consume operations. |  | 
| Delivery Mode a| Enumeration, one of:
...
| Message Digest Algorithm a| String | Secure hashing algorithm to use. If not set, the default is SHA-256. If the payload of the message is a Java object, Mule ignores this value and returns the value that the payload's `hashCode()` returned. |  | 
...
Configures the message identifier strategy for the redelivery policy.
