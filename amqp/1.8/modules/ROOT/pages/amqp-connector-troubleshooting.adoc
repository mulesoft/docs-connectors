= Troubleshooting AMQP Connector 
...
== Troubleshoot XML Parameters Order in AMQP Publish Operation

When upgrading from AMQP Connector version 1.7.7 to 1.8.0, you can encounter build failures such as the following: 

[source,xml,linenums]
----
[ERROR] org.xml.sax.SAXParseException; lineNumber: X; columnNumber: Y; cvc-complex-type.2.4.a: Invalid content was found starting with element '{"http://www.mulesoft.org/schema/mule/amqp":routing-keys}'. One of '{"http://www.mulesoft.org/schema/mule/amqp":message}' is expected.
----

This occurs because your XML configuration includes both `fallback-exchange-definition` and `routing-keys` parameters, and there is a change in the XML schema validation requirements for AMQP Connector 1.8.x:

XML parameters order in AMQP Connector version 1.7.7:
...
XML parameters order in AMQP Connector version 1.8.x:
...
To resolve this issue, update your XML configuration with the correct XML parameter order:

. Move the `amqp:routing-keys` parameter before the `amqp:fallback-exchange-definition` parameter.
. Ensure all other XML parameters maintain their relative order.
. Update any XML configurations that use both parameters to follow the correct XML parameter order.

Consider these points:

* Update your XML configurations only when you use both `routing-keys` and `fallback-exchange-definition` parameters together.
* XML configurations using only one of these parameters either `routing-keys` or `fallback-exchange-definition` are not affected.
* The functionality of AMQP Connector in version 1.8.x remains the same; only the XML structure changed.
