...
== Empty payload on callRestletGet operation

There is a design problem on the callRestletGet operation. Even if it is a GET operation, you are asked to provide a payload body called `data`.
If not specified, it will throw an error. This could be easily override by putting an empty payload like `{}`.
