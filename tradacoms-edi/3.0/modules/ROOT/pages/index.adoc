...
Convert TRADACOMS transmissions to and from
DataWeave-compatible representations by using lists and maps.
...
a directory in `src/main/app` and refer to the location by using `${app.home}`.
...
. To save the global connector configurations, click *OK*.
...
. In the properties view, select the connector configuration that you created, and then select the *Read* operation. This operation reads any byte stream into the structure described by your TRADACOMS schemas.
