...
* TRADACOMS EDI connector 3.x is to support newer version of Mule Runtimes and do not include any functional changes.

* Mule 4 TRADACOMS EDI connector 2.x supports messages from multiple versions of the standards. The version information in the message header (UNH segment) data is used to identify the standard version, and the message identifier determines the particular schema.
...
You must change the default Anypoint configuration to use the TRADACOMS EDI connector 3.0.0.
