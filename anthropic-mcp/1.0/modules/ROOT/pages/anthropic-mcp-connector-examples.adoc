...
You can use existing APIs to create a Mule app, which you can convert to an MCP server using MCP Connector and then connect to AI clients.

== Create an MCP Server

This is an example of how to write an Agent that creates purchase orders in SAP Concur. For that, the Agent must select from a list of approved vendors. In this custom implementation, each vendor is assigned with a “product category” mapped in a custom field called “Custom19”.
...
.. This is a simple tool that doesn’t take any input arguments, but according to the MCP standard it still requires a JSON schema to define that.
...
The implementation uses the off-the-shelf SAP Concur Connector to get the vendors and a DataWeave (DW) transformation to filter by approved status and map the custom field to a `productCategories` field.
...
The tool in this example takes input arguments defined in the JSON schema and each of those arguments includes a natural language description. This allows the LLM to map and consume the values. 
