...
:page-aliases: 3.6@mule-runtime::pop3-connector.adoc

The Anypoint Connector for POP3 (POP3 Connector) is configurable only as an inbound endpoint, that is, as a message source with a one-way exchange pattern. This connector implements a transport channel that enables your Mule application to retrieve email from a POP3 email server.
...
* See the xref:3.6@mule-runtime::pop3-transport-reference.adoc[POP3 Transport Reference] for details on setting the properties for a POP3 endpoint using an XML editor.
