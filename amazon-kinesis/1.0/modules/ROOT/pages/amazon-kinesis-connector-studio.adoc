...
+
Records include binary data, a partitionKey, and an explicitHashKey. The explicitHashKey value is normally computed with the MD5 function of the record partition key, but you can override the value by using  the explicitHashKey field. The operation uses KPL which increases throughput.
