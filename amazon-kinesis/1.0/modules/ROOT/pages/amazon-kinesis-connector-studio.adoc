...
. Click *OK*.
. In the *Listener Params* section of the *Kinesis Stream Listener* properties window, optionally enter values for the fields.
...
+
If the *Aggregation Enabled* field is `true` (the default), then multiple records are combined and stored in one Kinesis Data Streams record.
+
