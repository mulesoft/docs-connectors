...
*Note:* Sometimes, due to varying batch sizes, checkpoints may not occur in the correct sequence, potentially leading to transaction loss in certain cases. It is recommended to set *maxConcurrency=1* to ensure transactions are processed sequentially.
