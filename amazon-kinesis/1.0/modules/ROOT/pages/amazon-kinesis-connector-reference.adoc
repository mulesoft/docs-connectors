...
The operation checkpoint current position of a listener into DynamoDB table. Upon failover, the connector will start fetching records after the checkpointed position.
...
| Application Name a| String |  Represents DynamoDB table which holds data about current stream positions. |  | {nbsp}
| Shard ID a| String |  The shard ID. |  | {nbsp}
| Sequence Number a| String |  A sequence number at which to checkpoint in the shard. |  | {nbsp}
| Subsequence Number a| Number |  A subsequence number at which to checkpoint within the shard. |  | {nbsp}
...
The operation puts a record into a Kinesis Data Stream.
...
| Configuration | String | The name of the configuration to use. | | *x*{nbsp}
| Stream Name a| String |  The stream name. |  | {nbsp}
| Partition Key a| String |  The partition key, if not provided the connector generates UUID for its value. |  | {nbsp}
| Explicit Hash Key a| String |  The value to override the Explicit Hash Key which is normally computed with the MD5 function of the record partition key. |  | {nbsp}
| Data a| Any |  The content. Up to 1MB in size. |  #[payload] | {nbsp}
| Config Ref a| ConfigurationProvider |  The name of the configuration to be used to execute this component |  | *x*{nbsp}
...
| Shard Configs a| Array of <<ShardConfig>> |  The shard configuration. If specified the source will listen only from selected shards. |  | {nbsp}
...
| Failover Time a| Number |  Duration of a lease for a worker. |  10 | {nbsp}
...
** MINUTES
** HOURS
** DAYS |  Time unit for failover time field. |  SECONDS | {nbsp}
