...
| Record TTL a| Number |  Use this setting if your app uses late data stream records. When setting this value, take into consideration the fact that data records still incur network latency after they leave the Amazon Kinesis Producer Library (KPL). |  30 |
...
The operation checkpoint current position of a listener into DynamoDB table. Upon failover, the connector starts fetching records after the checkpointed position.
...
| Consumer ID a| String |  The unique identifier of the consumer. This identifier is used for shard lease management. |  | x
...
| Partition Key a| String |  The partition key |  |
...
| Data a| Any |  The content to put in the Amazon Kinesis data stream. This can be 1MB. |  #[payload] |
| Config Ref a| ConfigurationProvider |  The name of the configuration to use to execute this component |  | x
...
| Stream Name a| String |  The stream name. |  |
| Application Name a| String |  The DynamoDB table that holds data about current stream positions. If the table doesn't exist, then the connector creates a new one at the initial position. |  |
| Consumer ID a| String |  The unique ID of the consumer used for shard lease management. This value must be unique, or an `InvalidArgumentException` message is thrown. This value is used for the *Seek* operation and for Amazon DynamoDB lease management. The KCL LeaseManager object uses the ID to identify which shard is reserved for the consumer identified by the consumer ID. The Consumer ID must be unique for the application name. |  | x
| Shard Configs a| Array of <<ShardConfig>> |  The subscription configuration. If it is not N/A, the listener listens from selected shards only. |  |
...
[%autowidth.spread]
|===
|Type |Array of Message of [Binary] payload and [<<RecordAttributes>>] attributes
|===
