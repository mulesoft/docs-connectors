...
NOTE: Due to varying batch sizes, checkpoints might sometimes not occur in the correct sequence, potentially leading to transaction loss in certain cases. Set `maxConcurrency=1` to ensure transactions are processed sequentially.
