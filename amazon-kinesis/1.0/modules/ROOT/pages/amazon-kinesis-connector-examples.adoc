...
The following example shows how to use Amazon Kinesis Data Streams Connector to put data records and listen from an Amazon Kinesis data stream.
...
* This flow uses the *Put* operation to put data into the Amazon Kinesis data stream. In this flow, *HTTP > Listener* receives a query parameter named *data* that sets the payload for the *Put* operation. It uses a query parameter named *partitionKey* to set the partition key value for the Put operation.
+
image::amazon-kinesis-example-put-record.png[Put Records flow]
+
* This flow uses the *Kinesis Listener* to listen for new data records:
+
image::amazon-kinesis-example-listener.png[Listener flow]
+
* This flow uses the *Kinesis Listener* to listen for new data records. After receiving the new records, it calls the *Checkpoint* operation to set a  manual checkpoint:
+
