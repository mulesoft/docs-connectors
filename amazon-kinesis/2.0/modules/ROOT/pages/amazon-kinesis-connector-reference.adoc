= Amazon Kinesis Data Streams Connector 2.0 Reference 
...
| Record Max Buffered Time a| Number |  Maximum amount of time that an Amazon Kinesis data record spends buffering before it is sent. Records can be sent earlier than this value, depending on other buffering limits. |  100 |
...
** MINUTES
** HOURS
** DAYS |  A time unit that qualifies the *Record Max Buffered Time* value|  MILLISECONDS |
...
|===
| Name | Type | Description | Default Value | Required
| Custom Kinesis Endpoint a| String |  Specifies a custom Kinesis endpoint, often used for FIPS compliance. |  | 
| Custom STS endpoint a| String |  Sets a custom STS endpoint. This is useful when a non-standard service endpoint is required, such as a VPC endpoint or a FIPS-compliant endpoint. |  | 
...
| Custom Dynamo DB Endpoint a| String |  Sets a Dynamo DB endpoint to track and maintain state information. |  | 
| Custom Cloud Watch Endpoint a| String |  Sets a Cloud Watch endpoint for monitoring and observability. |  | 
| Connection Timeout a| Number |  The amount of time to wait when initially establishing the TCP connection between the connector and Amazon Kinesis server before throwing an exception if the connection fails |  30 |
...
| Primary Node Only a| Boolean |  Set to true to restrict the execution of this source to the primary node when operating in a cluster environment. |  | 
