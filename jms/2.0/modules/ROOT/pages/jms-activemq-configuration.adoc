...
== Configure an ActiveMQ XA Connection

Use *ActiveMQ XA Connection* to configure a connection that participates in Extended Architecture Transactions (XA transactions). This connection type is dedicated to XA scenarios and requires Mule Runtime 4.10.0 or later.

If you previously enabled XA on an *ActiveMQ Connection*, migrate to *ActiveMQ XA Connection* as described in xref:jms-connector-upgrade-migrate.adoc[Upgrading and Migrating JMS Connector].
...
For a complete example of how to use this connection type with XA transactions, see xref:jms-xa-transactions.adoc[Configuring XA Transactions in JMS Connector].

== Configure an ActiveMQ XA Connection - No Connectivity Test (DEPRECATED)

*ActiveMQ XA Connection - No Connectivity Test* is identical to *ActiveMQ XA Connection*, except that it does not perform a connectivity test prior to deployment. Use this connection type in the same scenarios described for *ActiveMQ Connection - No Connectivity Test* earlier in this topic, when your application also requires XA transactions.

In the following example, you configure an *ActiveMQ XA Connection - No Connectivity Test*:

. In Studio, navigate to the *Global Elements* tab.
. Click *Create*.
. In the filter box, type `jms` and select *JMS Config*.
. Click *OK*.
. In the *Connection* field, select *ActiveMQ XA Connection - No Connectivity Test*.
. Set the optional *Client id* field.
. In the *Factory configuration* field, select *Edit Inline*.
. Set the *Broker url* field value to the address of the broker to connect to, for example, `tcp://localhost:61616`.
. In the *Advanced* tab, optionally configure the *XA connection pool* fields for *Min pool size*, *Max pool size*, and *Max idle seconds*.
...
In the XML editor, the `<jms:active-mq-xa-nct-connection>` and `<jms:factory-configuration>` configuration looks like this:

[source,xml,linenums]
----
<jms:config name="JMS_Config">
  <jms:active-mq-xa-nct-connection clientId="my-mule-cli">
    <jms:factory-configuration brokerUrl="tcp://localhost:61616" />
    <jms:xa-connection-pool minPoolSize="1"
                            maxPoolSize="10"
                            maxIdleSeconds="45"/>
  </jms:active-mq-xa-nct-connection>
</jms:config>
----

== Configure the Required Libraries

When you configure any connection type, you must always configure a library containing the JMS client implementation, because JMS Connector is not bound to any particular implementation. Configure ActiveMQ external libraries in the global elements view of Studio or manually add the library dependency in your `pom.xml` file. These libraries are required for all ActiveMQ connection types, including *ActiveMQ XA Connection* and *ActiveMQ XA Connection - No Connectivity Test*.
