...
| Max Redelivery a| Number a| Do not configure this parameter here, as it has no effect. See [ActiveMQConnectionFactoryConfiguration] | 0 |
...
|===
| Field | Type | Description | Default Value | Required
| Max Redelivery Count a| Number a| Do not configure this parameter here, as it has no effect. See [ActiveMQConnectionFactoryConfiguration] | 0 |
...
| Initial Redelivery Delay a| Number | Used to configure the RedeliveryPolicy#getInitialRedeliveryDelay(). This feature is not available in an XA context. | 1000 |
| Redelivery Delay a| Number | Used to configure the RedeliveryPolicy#getRedeliveryDelay(). This feature is not available in an XA context. | 1000 |
| Max Redelivery a| Number a| Used to configure the RedeliveryPolicy#getMaximumRedeliveries(). Sets the maximum number of times a message will be redelivered before it is sent to the Dead Letter Queue. 
...
| Verify Host Name a| Boolean | If true, the broker's host name is verified against the host name in the broker's SSL certificate during the SSL handshake. | false |
|===

[[topic-consumer]]
=== Topic Consumer

Marks the consumed Destination as a Topic and provides a way to configure topic-specific consumer parameters.
...
Enables caching and provides default values for all the configurable parameters.
...
A ConnectionFactory that wraps a ConnectionFactory delegate discovered using a JndiNameResolver.
...
Maintains a Context instance that remains open at all times and is always used to perform lookups.
...
Uses a cache to store already resolved names. The cache has no automated mechanism for cleaning up data; if the cache becomes corrupted, stop and restart the instance to clear it.
...
| `mule.jms.generic.additionalCertificateFileName`| JKS file name that contains SSL certificates to add.
| `mule.jms.generic.additionalCertificatePassword`| Truststore password that you set for the `mule.jms.generic.additionalCertificatePassword` property.
| `mule.jms.operation.artemis.dispose.delay`| Sets a delay during the undeployment stage to prevent thread leaks and allow Artemis time to close the channels it opened with the JMS broker. Set this property when Artemis is the chosen JMS driver.
|===
