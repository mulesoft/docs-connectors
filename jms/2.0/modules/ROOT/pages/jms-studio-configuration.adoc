...
To configure the connection in the global element, define the connection by selecting *ActiveMQ Connection*, *ActiveMQ XA Connection*, *Generic Connection*, or *Generic XA Connection*, and set up the connection libraries and the JMS specification. +
...
. In the *Factory configuration* field, select *Edit Inline*.
. Set the *Broker url* field value to the address of the broker to connect to, for example, `tcp://localhost:61616`.
. In the *Advanced* tab, optionally configure the *XA connection pool* fields for *Max pool size* and *Max idle seconds*.
. Click *OK*.
...
For either ActiveMQ connection type (*ActiveMQ Connection* or *ActiveMQ XA Connection*), you can configure three libraries:
