...
Using prefetch can result in the loss of predictability of API calls. If prefetch is used, JMS Connector aggressively tries to fetch messages, which results in increased API calls. If predictability and control on API requests is important, check your third-party JMS vendor provider documentation to disable prefetch.

As an alternative, use listeners to consume messages:
* The *Consume* operation does not support prefetch enabled when ACK MANUAL option is configured. Otherwise, an unexpected behavior on the order in which messages are consumed can arise.
* The *On new message* source supports prefetch enabled only for some third-party vendors that requires it to use async APIs.
