...
However, when you work with durable subscriptions, only one node initialize a connection while the other node fails to do it. In these scenarios, the nodes are not clones because one node has a connection while the other one doesn't. The current design of Mule running in HA clusters aims that all nodes initialize connections to ensure that they are capable of facing a failure of the primary node.
