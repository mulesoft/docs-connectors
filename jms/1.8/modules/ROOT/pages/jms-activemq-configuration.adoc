...
== ActiveMQ Listener with primaryNodeOnly=true on Cloud

There may be certain scenarios in which the credentials of a broker connection cannot be used by more than one node simultaneously. In these cases, mule applications use the primaryNodeOnly=true property to ensure listener execution on only one of the nodes. However, the connection test that is performed prior to deployment may result in a deployment error on the primary node if another node used the credentials previously. For this particular case, the *ActiveMQ Connection - No Connectivity Test* has been created. 
This connection is identical to the ActiveMQ Connection, except that it does not perform a connectivity test prior to deployment.
...
. Set the *Client id* field (Optional).
. In the *Factory configuration* field, select *Edit Inline*.
. Set the *Broker url* field value to the address of the broker to connect to, for example, `tcp://localhost:61616`.
. Click *OK*.

.ActiveMQ Connection - (No Connectivity Test) Configuration
image::jms-activemq-nct-configuration.png[ActiveMQ Connection - (No Connectivity Test) Configuration.]
