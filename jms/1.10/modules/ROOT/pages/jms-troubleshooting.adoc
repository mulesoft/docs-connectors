...
== Troubleshoot ActiveMQ with JMS Connector

When using ActiveMQ version 5.16.x and later, attempting to use an XAConnection (implemented by ActiveMQXAConnection) and an XASession (implemented by ActiveMQXASession) outside of a transaction results in a JMS exception message:

`javax.jms.JMSException: Session's XAResource has not been enlisted in a distributed transaction.`

To resolve this issue, choose one of the these options:

* Perform the XA enable operation within a transaction context.
* Pass the `jms.xaAckMode=1` query parameter in the broker URL to execute an XA operation outside a transaction context. For example, for this broker URL: `vm://localhost?jms.xaAckMode=1`

[NOTE]
If you plan to use ActiveMQ version 5.16.x, use the ActiveMQ driver version 5.16.4 or later.
