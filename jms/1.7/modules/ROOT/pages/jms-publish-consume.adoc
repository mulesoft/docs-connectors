= Publish Messages and Consume Replies Examples
...
destination, and setting the *Destination* field to `targetDestination`.
...
In the following example, you configure the *Publish consume* operation to publish a message to the `targetDestination` with the current `payload` as its body and the `JMSReplyTo` header configured to the known queue, and setting the *Reply To* field to `replyToDestination`.
