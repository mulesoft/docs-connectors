...
If you want the *On New Message* source to use a transactional session when dispatching the messages, configure the *Transactional action* field to `ALWAYS_BEGIN`. With this configuration, each new message is processed in a transaction that propagates to all the components in the flow and occurs once the flow execution completes successfully. The transaction rolls back if the flow execution returns an error.

By default, other components in the flow do not join the transaction created by the *On New Message* source. In order to execute other operations using the source transaction, configure in those components the *Transactional action* field either with `ALWAYS_JOIN` or `JOIN_IF_POSSIBLE`.
...
. Drag a JMS *Publish* operation inside the *Try* scope component.
