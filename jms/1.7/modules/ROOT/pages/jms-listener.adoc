...
In the following example, you configure the *On New Message* source to listen for new messages in the queue specified in the *Destination* field. Which returns a Mule Message each time a JMS message is available in the queue. The Mule Message has:
...
For more information regarding a Message ACK, see the xref:jms-ack.adoc[Manage Message Acknowledgement] documentation.
...
For more information on topic subscriptions, see the xref:jms-topic-subscription.adoc[Configure Topic Subscriptions] documentation.
