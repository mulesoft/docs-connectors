...
[Note]
If you use JMS Connector with Oracle JMS, the *On New Message* source receives new messages with a delay of 1 to 10 seconds, based on the frequency of the messages. Control this behavior by using `-Doracle.jms.maxSleepTime=xxxx`. For further details, see https://forums.oracle.com/ords/apexds/post/jms-client-receives-aq-messages-asynchronously-with-a-delay-1125[JMS client receives AQ messages asynchronously with a delay of 15 seconds^]. 
