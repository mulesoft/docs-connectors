...
== Using ActiveMQ 5.16.x with connector

With ActiveMQ 5.16.x and above, attempting to use an XAConnection (implemented by ActiveMQXAConnection) and an XASession (implemented by ActiveMQXASession) outside of a transaction will result in a JMSException. The exception appears as follows:

    javax.jms.JMSException: Session's XAResource has not been enlisted in a distributed transaction.

More details regarding the issue can be found link:https://issues.apache.org/jira/browse/AMQ-2659[here].

* To resolve the issue, you can choose one of the following options:

. Perform the XA enable operation within a transaction context.
. Pass the jms.xaAckMode=1 query parameter in the broker URL if you need to execute an XA operation outside a transaction context.

    vm://localhost?jms.xaAckMode=1

[NOTE]
If you wish to use version 5.16.x, it is recommended to use the ActiveMQ driver version 5.16.4 or later.
