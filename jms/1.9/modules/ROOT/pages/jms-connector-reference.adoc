...
| Max Redelivery a| Number a| Do not configure this parameter in the connector, as it has no effect. This must be configured on the broker side.
...
|===
| Field | Type | Description | Default Value | Required
| Max Redelivery Count a| Number a| Do not configure this parameter in the connector, as it has no effect. This must be configured on the broker side.
