= Adobe Marketo Connector Reference 
...
| Name | Type | Description | Default Value | Required
| Configuration | String | The name of the configuration to use. | |x
| Add To List Request Data a| Any | JSON object that contains an `input` array. Each array item contains a numeric lead ID, for example, `{"input":[{"id":101},{"id":102}]}`. |  `#[payload]` |
| Id a| Array of Number | Array of numeric lead IDs to add to the list. |  |
| List Id a| Number | ID of the target static list. |  |x
| Target Variable a| String |  The name of a variable that stores the output of the operation. |  |
| Target Value a| String |  An expression to evaluate against the operation's output and the outcome of that expression is stored in the target variable. |  `#[payload]` |
|===
