...
| Name | Type | Description | Default Value | Required
| Configuration | String | Name of the configuration to use. | | x
| Prompt a| String | Plain text prompt to send to the LLM. | | x
| Input Stream a| Any | Document content to ingest into the embedding store. | | x
...
| File Type a| Enumeration, one of:

** PDF
** TEXT
** CSV | Type of the document provided as input. | PDF | 
| Option Type a| Enumeration, one of:

** FULL
** PARAGRAPH | How to split the loaded document before embedding. | FULL | 
| Read Timeout a| Number |  Read timeout value. |  | 
| Read Timeout Unit a| Enumeration, one of:
