...
. In the *Create volume* *General* tab, enter the following values:
.. Enter the *Display Name*. +
This example uses `Create volume`.
.. In *Basic Settings*, select the *Amazon EC2 Configuration* configuration you created in <<configure-global-elements,Configure Global Elements for the Connector>>.
