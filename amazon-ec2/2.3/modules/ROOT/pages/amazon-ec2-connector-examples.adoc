...
This example creates an EBS volume that you can attach to an Amazon EC2 instance in the same availability zone:

image::amazon-ec2-create-volume-usecase-flow.png[Creating an EBS volume]

. Create a new Mule project in Anypoint Studio.
. Add the following properties to the `mule-artifact.properties` file to store your Amazon EC2 credentials and place it in the project's `src/main/resources` directory:
+
...
. Configure the Amazon EC2 connector by adding a new global element.
