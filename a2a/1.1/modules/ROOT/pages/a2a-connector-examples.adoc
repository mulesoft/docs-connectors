...
[NOTE]
====
To close the SSE stream gracefully, set `final: true` on the last status update. If `final` isn't emitted, clients can remain subscribed waiting for additional events.
====
