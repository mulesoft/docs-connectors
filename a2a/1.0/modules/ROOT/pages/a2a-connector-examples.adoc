...
The A2A server configuration uses an agent card JSON file that contains metadata about the agent. Client A2A agents use this information to determine:
...
<a2a:server-config name="A2A_Server" doc:name="A2A Server" doc:id="3e1c7c42-133d-4853-87f0-90617c8ad700" 
	cardPath="agent-card.json"> #(1)
	<a2a:connection listenerConfig="HTTP_Listener_config" agentPath="/financial-summary-agent" /> #(2)
</a2a:server-config>
----

[calloutlist]
.. `cardPath` specifies the location of the agent card JSON file. The file can point to a resource on the classpath or on disk.
