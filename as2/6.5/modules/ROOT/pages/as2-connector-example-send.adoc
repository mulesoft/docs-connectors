...
. Configure the connection by setting the *Partner URL* field to `+http://localhost:8081/as2-receive+` on the *General* tab.
+
...
You can now send AS2 requests to the endpoint configured in the xref:as2-connector-example-receive-send.adoc#receiving-messages-example[Receiving AS2 Messages and Sending Back MDNs] example. With the example running, you can observe the message sent by the *Send with Async MDN* operation.
