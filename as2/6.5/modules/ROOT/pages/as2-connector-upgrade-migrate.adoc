...
AS2 Connector 6.5 contains new functionality that enables you to check for duplicate incoming messages when using the *As 2 listener* source. This new functionality includes the use of a MuleSoft Object Store to store the received messages.

To support his new functionality, the release contains the following new global configuration elements:

* *Advanced* tab, on which you can enable checking for duplicate messages and reference the Object Store configuration
* *Object store* dialog, on which you can configure the persistence, maximum number of entries, time-to-life, and expiration interval for the Object Store
* *ObjectStore Config*, on which you can create a reconnection strategy for...

To see descriptions of the newly-added fields, see the xref:as2-connector-reference.adoc[AS2 Connector Reference].
