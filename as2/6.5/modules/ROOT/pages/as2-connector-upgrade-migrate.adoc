...
* The *Check duplicated messages* field enables the connector to check if a message was already processed.
* The *Store time-to-live* field enables you to define the number of days for which message information is stored.

Both of these fields are on the *As 2 listener*, *AS 2 mdn Listener*, and *Non repudiation listener* *General* tab.
