...
If selected, you must reference a global object store in the Object Store field to store the existing messages.
--
+
* If deselected, the connector accepts duplicate messages.
| false |
| Object Store |<<object-store,Object Store>> | Object store where the connector stores new messages. This field is mandatory if the *Check duplicate messages* field is selected.| |
...
Configures the MuleSoft Object Store that stores incoming messages for the  *As 2 listener* source.

[[object-store]]
[%header,cols="20s,20a,35a,20a,5a"]
|===
| Name | Type | Description | Default Value | Required
| Name | String | Name of the object store that stores the received messages.  | |
| Persistent | Boolean | Indicates whether the object store saves received messages in persistent storage.| true |
| Max entries| Number | Maximum number of messages that the object store can hold. When this number is reached, the connector expunges the cached responses, trimming the first entries (first in, first out) and those that have exceeded their time-to-live. If you don't specify a value, the maximum number of entries is unlimited.|  |
| Entry ttl | Number| How long a message is stored in the object store. The maximum length of time for storing a message is 30 days. A value of `0` or less means that the entry is stored for 30 days by default. | 30 days|
...
* NANOSECONDS
* SECONDS
| Time unit for the *Entry ttl* field.| DAYS | 
| Expiration interval | Number a| Frequency with which the Object Store checks for cached messages that it should expunge. For example, if the value of *Expiration interval unit* is `MILLISECONDS` and you enter `5000` for the expiration interval, the object store reviews all cached response events every 5000 milliseconds to see which ones exceeded their time-to-live and should be expunged.

Set this value to a positive value that is less than the value of the *Entry ttl* field.

This field requires that the *Max entries* or *Entry ttl* fields also be set. If neither is set, then setting this field has no effect because the expiration thread doesn't run. If the *Expiration interval* field is set to a value lower than or equal to `0`, the object store never expires.| 1 minute|
...
* MINUTES
* NANOSECONDS
* SECONDS  |Time unit for the *Expiration interval* field. | MINUTES |
| Configuration Reference | String | Reference to the object store configuration. | | 
|===

==== Object Store Config Parameters

Configures a reconnection strategy for ...

[[object-store]]
[%header,cols="20s,20a,35a,20a,5a"]
|===
| Name | Type | Description | Default Value | Required
| a| Name | Name for this configuration.   |  true |
| Connection| | ???| |
...
| Fails deployment when test connection fails | Boolean | FIX What to do if, when an app is deployed, a connectivity test does not pass after exhausting the associated reconnection strategy:

* true: Allow the deployment to fail.
* false: Ignore the results of the connectivity test. | |
| Reconnection strategy a| Enumeration, one of:

* None
* Standard
* Forever
a| Reconnection to use:
