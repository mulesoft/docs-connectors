...
* If `true`, the connector checks to see if a new message is duplicated, as determined by comparing these values in the new messsage to those in existing messages: 
+
...
| false |
| Store time-to-live a| Number |  Minimum number of days to store the sender, receiver, and message ID for checking message uniqueness. This value is used only when the *Check duplicated messages* field is set to `true`. The connector uses the default persistent object store from the Mule configuration for storing this information. This field sets the entry time-to-live for the stored information. | 30 |
