...
| Check duplicated messages a| Boolean |  If set to `true`, it will store each request and validate the duplicity, rejecting duplicated messages with the same message id and from the same sender.| false |
| Store time-to-live a| Number |  If the property `Check duplicated messages` is enabled, the number of days can be configured. | 30 |
