...
* If selected, the connector checks to see if a new message is duplicated, as determined by comparing the following values in the new message to those in existing messages:
+
...
Configures the MuleSoft Object Store that store incoming messages for the  *As 2 listener* source.
...
| Persistent | Boolean | Whether the object store is persistent or transient.| true |
| Max entries| Number | Maximum number of messages that the object store can hold. When this number is reached, the connector stops storing new messages. If you don't specify a value, the maximum number of entries is unlimited.|  |
...
| Expiration interval | Number a| How frequently the expiration thread runs. 
Set this value to a positive value that is less than the value of the *Entry ttl* field.

This field requires that the *Max entries* or *Entry ttl* fields also be set. If neither is set, then setting this field has no effect and the expiration thread does not run. If the *Expiration interval* field is set to a value lower than or equal to `0`, the object store never expires.| 1 minute|
