...
a| <<ExpirationPolicy>> |  Configures the minimum amount of time that a dynamic configuration instance can remain idle before Mule considers it eligible for expiration. This does not mean that the platform expires the instance at the exact moment that it becomes eligible. Mule purges the instances as appropriate. |  |
...
|===
| Name | Type | Description | Default Value | Required
| Check duplicate messages | | Checks for duplicate messages:

* If selected, the connector checks to see if a new message is duplicated, as determined by comparing these values in the new message to those in existing messages:
+
...
+
If selected, you must reference a global <<object-store, object store>> configuration that stores the existing messages.
+
...
| Max entries| Number | Maximum number of messages that the object store can hold. When this number is reached and a new message is rececived, the connector doesn't store that message. IS THERE A DEFAULT?| |
| Entry ttl | Number| How long a message is stored in the object store. | |
