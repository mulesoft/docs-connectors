...
// TODO : This field is not a boolean anymore, it is now a complex type, that currently has only one field inside it, the Object Store reference. The class is 'CheckDuplicateMessage', how should we document it? The behavior is the same as below. We only made the time-to-live configurable with Object Store.
| Check duplicated messages a| Boolean a|  Check for duplicate messages:

* If `true`, the connector checks to see if a new message is duplicated, as determined by comparing these values in the new message to those in existing messages:
+
...
* If `false`, the connector accepts the duplicate message. 
| false |
| Object Store a| Object Store |  Object Store config reference used to store the messages when the *Check duplicated messages* is enabled. |  | x
