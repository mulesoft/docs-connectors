...
| Use different Alias for MDN Signature| Boolean| (Applies to the *AS2 Listener*) Use the alias specified in *Alias For MDN Signature* to use a different keystore entry for the MDN signature.| true|
...
| Private Key Password a| String |  If the private key to be used for signing the MDN is password protected, provide that value here. |  |
