...
When using the *Send with Sync Mdn* operation, you can now use different a alias to validate the MDN signature than the alias used for encryption. To support this functionality, the release contains two new fields: *Use different Alias for MDN Signature Validation* and *x509 Alias For MDN Signature*.
