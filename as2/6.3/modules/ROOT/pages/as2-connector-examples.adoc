...
image::as2-connector-mdn1-example.png[Flow for receiving and forwarding AS2 messages]

The second flow receives message payloads and AS2 sender and receiver information via HTTP. It then sends outbound AS2 messages to a target AS2 endpoint using the AS2 *Send with Sync MDN* operation:

image::as2-connector-mdn2-example.png[Flow for receiving and forwarding AS2 messages]

=== XML for the First Flow
