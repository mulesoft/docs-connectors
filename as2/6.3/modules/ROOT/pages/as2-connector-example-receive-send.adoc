...
Creating this example involves creating a new Mule project and configuring the *HTTP>Listener* source *Logger* component.
...
=== Create a Flow to Send Synchronous MDNs
...
|===
|Field |Value
|Partner URL | `+http://localhost:8081/as2-receive+`
|Port | `8081` (default)
|Keystore Password |`test`
|Keystore Path| `+as2/partnera.p12+`
|Private Key Password | `test`
...
. Configure the keystore that contains the certificates and keys:
+
[%header%autowidth.spread]
|===
|Field |Value
...
+
. You can now send AS2 requests to the endpoint configured in the <<receiving-messages,Receiving Messages>> example.
+
...
This example is similar to the <<synchronous-mdns,Sending Synchronous MDNs example>> except that in this example:
...
+
You can configure this field globally, in the global element for the *Send with Async MDN* operation, or in the properties window for that operation.

We will use the endpoint of the xref:as2-connector-receiving-receipts.adoc[AS2 MDN Listener example]: +
`http://localhost:8082/receiveMDN`

== XML for Sending Asynchronous MDNs
