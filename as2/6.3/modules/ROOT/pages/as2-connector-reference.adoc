...
+
If you select *Shared mode*, uncheck *Partner config* (mandatory) and configure each partner using the Shared mode fields in this table. |  | x
...
| Name | Type | Description | Default Value | Required
| Self Config - AS2 Partner Name a| String |  AS2 system identifier used for AS2-From and AS2-To headers. |  | x
| Self Config - x509 Alias a| String |  Certificate, or private key alias, or both in the keystore. Required if using signing or encryption. |  |
| Self Config - email a| String |  Email address used as the HTTP 'From' header when sending. |  | x
