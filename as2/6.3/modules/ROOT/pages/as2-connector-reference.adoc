...
Listens for AS2 requests and sends an MDN back to the sender when a request is received

[NOTE]
====
Upon receiving an AS2 message, AS2 Connector automatically sends an asynchronous MDN to the receipt URL provided in the inbound transmission. If the receipt URL is inaccessible, the inbound AS2 message is not processed.
====
