...
* You can now configure the *As 2 listener* to send the MDN at the end of the flow by using the *Mdn mode* option in `AUTO`.
* You can now configure retries to ensure the delivery of the MDN by specifying it in the *Connection's General* tab from the *As 2 listener*.
* A new field named *Validate certificate date* allows you to enable or disable the certificate date validation.
* A warning message will be logged by the console when a certificate is near its expiration date or if it is expired. The days before its expiration can be configured with the new property *Warning before (days)* in the *KeyStore Config*.
