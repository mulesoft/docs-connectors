...
| Validate certificate expiry date a| Boolean |  If enabled, it won't be possible to receive or send messages if the certificate is expired. | false |
| Warning before (days) a| Number | The number of days before receiving a warning message when the certificate configured is about to expire. It only applies if `Validate certificate expiry date` is enabled and this field is greater than 0. |  |
|===
