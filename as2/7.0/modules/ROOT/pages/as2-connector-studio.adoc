...
. <<configure-external-libraries,Configure a global element to add the required external libraries>>.
...
== Configure a global element to add the required external libraries

When you use as2 connector in the app, there should only be one global connector configuration to add the external libraries needed for the specific security mode. 

To configure the global element for AS2 Connector and add external libraries, follow these steps:

. Select the *Package Explorer* or *Canvas*, open your Mule flow or configuration.
