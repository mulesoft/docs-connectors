= Sending AS2 Messages and Receiving Back MDNs 

The following examples show how to use Anypoint Connector for AS2 (AS2 Connector) to send AS2 messages to a trading partner and receive MDNs (Message Disposition Notifications) from that partner in response:

* <<send-synchronous-msg,Send with Sync MDN>>
+
Send a synchronous AS2 message and receive a MDN in response to the AS2 message.
+
* <<send-asynchronous-msg,Send with Async MDN>>
+
Send an asynchronous AS2 message and receive an MDN in response to each received AS2 message.

[[send-synchronous-msg]]
== Send with Sync MDN
...
=== Create a Flow to Send Synchronous AS2 Message
...
[[send-asynchronous-msg]]
== Send with Async MDN

This example is similar to the <<send-synchronous-msg,Send with Sync MDN>> example, except that in this example:
