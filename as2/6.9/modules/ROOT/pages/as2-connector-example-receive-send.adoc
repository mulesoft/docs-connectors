...
When your trading partner is using an AS2 listener (for example, the <<receiving-messages,Receiving AS2 Messages>> example in this page), the listener’s MDN mode controls when a synchronous MDN is returned:

- IMMEDIATE: The receiver sends the synchronous MDN back on the same HTTP connection as soon as the incoming AS2 message is parsed and validated. The receiver’s business flow continues after the MDN is already sent.
- AUTO: The receiver sends the synchronous MDN back on the same HTTP connection only after its business flow completes. If the flow fails, a negative MDN is returned.

[[asynchronous-mdns]]
== Sending Asynchronous AS2 MDNs

When receiving an AS2 message that requests an asynchronous MDN, the as2-listener validates for presence of `Disposition-Notification-To` and the `Receipt-Delivery-Option` headers. 
The value in the Receipt-Delivery-Option field provides the URL where the receiver must post the MDN via a new connection. 

- The receiver immediately returns HTTP 200 OK to acknowledge the inbound AS2 message.
- IMMEDIATE: The receiver posts the asynchronous MDN to the `Receipt-Delivery-Option` URL that is sent in header as soon as the inbound message is parsed and validated.
- AUTO: The receiver posts the asynchronous MDN to the `Receipt-Delivery-Option` URL that is sent in header only after its business flow completes. If the flow fails, a negative MDN is posted.
----
== See Also
