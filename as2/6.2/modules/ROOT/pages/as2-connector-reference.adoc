...
* Chunked transfer mode
+
Sends the headers without the Content-Length property and modifies the message body to trasnfer it as a series of chunks.
+
* Identity transfer mode
+
...
+
** If the message body is less than the threshold, the connector uses the Identity transfer mode option.
** If the message body is greather than the threshold, the connector uses the Chunked transfer mode option.
| Chunked transfer mode
|
| Threshold
| Number
