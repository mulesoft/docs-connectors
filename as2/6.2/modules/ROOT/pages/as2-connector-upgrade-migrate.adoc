...
* The *Propagate Content-Type* field enables you to define whether to propagate the content type on the AS2 message sent to the partner.

Both of these fields are on the *General* tab.

=== 6.1

The *Transfer Mode Configuration* Field has been added to the `Send Config` class so that you can enable and disable chunked transfer of data when using the *Send* operation.
