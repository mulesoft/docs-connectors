...
Use the *Write strategy* field to configure the strategy to write the file. You can either select `STANDARD` or a `CUSTOM` strategy. Additionally, you can configure the buffer size for writing into the file via the *Buffer size for write strategy* field. Note that the concurrency is not maintained for the servers that do not follow the SFTP RFC. 

The custom *Write strategy* enables you to manually specify the offset for the server to write the file in append mode, as older SFTP servers do not directly support write modes. Therefore, in cases where the SFTP server is outdated and does not support write modes, use the custom write mode.
