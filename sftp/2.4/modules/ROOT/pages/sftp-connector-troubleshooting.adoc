...
== Troubleshoot SFTP Connector Issues with Identity File in .ppk Format

When using the SFTP Connector version 2.0.0 and later with the Preferred Authentication Method set to `PUBLIC_KEY`, you might face issues if the Identity File is in `.ppk` format. This is due to a limitation with the `Apache Mina library`, which does not support the `.ppk` file format, causing the application to fail.

To resolve this issue:

. Convert the Identity File format from `.ppk` to `.pem` using PuTTYgen.
