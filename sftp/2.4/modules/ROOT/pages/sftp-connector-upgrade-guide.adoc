...
If you use a custom write strategy and experience consecutive write call failures during the `APPEND` write mode, set the *Max Idle* field in the Advanced tab of the SFTP Connection Config to `0`. This change forces the SFTP connection to be recreated for each connection, ensuring that the `APPEND` write mode executes successfully. This workaround incurs a performance impact due to the overhead of frequent connection reestablishment.
