...
If your file write operation overrides instead of appending when using the `APPEND` mode, use the `CUSTOM` write strategy as it is not applicable to the `OVERWRITE` or `CREATE_NEW` modes. This strategy enables you to manually specify the offset for the server to write the file in `APPEND` mode, particularly only for older SFTP servers that do not directly support write modes. Use this strategy only if the `STANDARD` strategy results in errors during file writing in `APPEND` mode and does not offer concurrency. Additionally, you can adjust the buffer size in the *Buffer Size for Write Strategy* field. 
