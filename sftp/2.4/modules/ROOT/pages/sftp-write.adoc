...
[NOTE]
Use the `CUSTOM` write strategy only in `CUSTOM_APPEND` mode as it is not applicable to the `OVERWRITE` or `CREATE_NEW` modes. Use this strategy only if the `STANDARD` strategy results in errors during file writing in `APPEND` mode. Concurrency is not maintained for servers that do not follow SFTP guidelines.
