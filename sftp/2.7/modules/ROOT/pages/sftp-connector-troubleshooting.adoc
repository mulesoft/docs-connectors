...
[NOTE]
====
The `CUSTOM_APPEND` strategy manually calculates the file offset to ensure data is added to the end of the file. Use this strategy only for legacy servers that don't support the `STANDARD` strategy, as it doesn't support concurrent writes to the same file.
====
