= SFTP Connector 2.7 Reference 
:page-aliases: connectors::sftp/sftp-documentation.adoc


Anypoint Connector for SFTP (SFTP Connector) enables secure and reliable access to files and directories on SFTP servers. It facilitates secure file transfers between your Mule applications and remote SFTP locations, supporting a variety of file operations such as reading, writing, and managing files and folders.
...
| Response Timeout a| Number |  A scalar value representing the amount of time to wait before an SFTP request times out. This corresponds to the internal `sftp-client-cmd-timeout` property on the `org.apache.sshd.sftp.client.impl.AbstractSftpClient` AbstractSftpClient, which is applied by `DefaultSftpClient` when sending SFTP commands. Defaults to 30 seconds (the default of the SFTP client), but can be overridden here. This attribute works in tandem with *Response Timeout Unit*.|  `30` |
...
* MINUTES
* HOURS
* DAYS |  A TimeUnit that qualifies the *Response Timeout Unit* attribute.  |  `SECONDS` |
| Idle Timeout Unit a| Enumeration is one of:
...
* MINUTES
* HOURS
* DAYS |  A time unit that qualifies the *Idle Timeout* attribute. |  `SECONDS` |
| Idle Timeout a| Number |  Maximum length of time a connection can remain inactive before the SSH client library closes it. This attribute works in tandem with *Idle Timeout Unit*. |  |
...
| Strict Kex a| Boolean |  When `true`, strict key-exchange handling is enabled by default for the SSH session and is disabled automatically when the peer does not support it. |  `true` |
...
* APPEND
* CUSTOM_APPEND
* CREATE_NEW |  A `FileWriteMode`.

* OVERWRITE
+
If the file to be written exists, then overwrite it.
* APPEND
+
If the file to be written exists, then append the content to that file.
* CUSTOM_APPEND
+
If the file to be written using the `CUSTOM` write strategy exists, then append the content to that file.
* CREATE_NEW 
+
Create a file and raise an error if the file exists.

 |  `OVERWRITE` |
| Write Strategy a| Enumeration, one of:

* STANDARD
* CUSTOM | Strategy to write the file. 

* STANDARD
+
The standard write function is used where the file write mode is honored.
* CUSTOM
+
A custom write function is used to write to files where offset is manually calculated.
...
[NOTE]
====
If your SFTP server is a legacy system that doesn't support the `SSH_FXF_APPEND` flag, set *Write Strategy* to `CUSTOM` and *Write Mode* to `CUSTOM_APPEND` to prevent files from being overwritten. For more information, refer to xref:sftp-connector-troubleshooting.adoc#standard-append-mode-overwrites-or-fails[Standard Append Mode Overwrites or Fails].
====
...
| Reconnection Strategy a| * <<reconnect>>
* <<reconnect-forever>> |  A retry strategy in case of connectivity errors. The source stops polling if the <<reconnect>> strategy fails. Use the <<reconnect-forever>> strategy to run the source indefinitely. |  |
...
| Disabled a| Boolean | Whether connection pooling is disabled for this configuration. |  |
|===
...
* MINUTES
* HOURS
* DAYS | A *Not updated in the last* attributes. | MILLISECONDS |
...
| Message Identifier a| <<RedeliveryPolicyMessageIdentifier>> | Defines how to identify a redelivered message for this policy. |  |
...
| Id Expression a| String | Defines one or more expressions to use to determine when a message has been redelivered. Set this property only when *Use Secure Hash* is `false`. |  |
|===
