= Troubleshooting SFTP Connector 
...
* Get better and more detailed logs on the Mule app side by enabling verbose logging for SFTP Connector:
+
...
== Enable Apache SSHD Library Logging

[NOTE]
====
Enabling TRACE level logging for `org.apache.sshd` generates a high volume of debug logs that can cause severe performance degradation, including high CPU usage, thread contention, and system-wide delays. 
====
...
. Open the `src/main/resources` path folder.
. Open the `log4j2.xml` file inside the folder.
. Add this line to enable Apache SSHD library logging:
+
[source,xml,linenums]
----
...
== Enable SSL/TLS Handshake Debugging 

For diagnosing SSL/TLS handshake issues, add this argument to your run configuration:

`-M-Djavax.net.debug=ssl,handshake`

This argument enables detailed SSL/TLS debugging output. Use the argument only when troubleshooting encryption-related connection issues.
...
When using the SFTP Connector version 2.0.0 and later with the preferred authentication method set to `PUBLIC_KEY`, you can face issues if the `Identity` file is PPK. The `Apache Mina library` doesn’t support the PPK file format, so the application fails.

To resolve this issue:

. Convert the PPK `Identity` file to PEM by using `PuTTYgen`.
...
== Standard Append Mode Overwrites or Fails

When using the `APPEND` write mode, the SFTP connector overwrites the existing file instead of appending data to it, or the operation fails with a generic error.
This behavior occurs when the target SFTP server is a legacy system that doesn't support the `SSH_FXF_APPEND` flag. Without this flag, the server cannot natively process a request to append data to an existing file.

To resolve this issue:

. In the *Write* operation configuration, set the *Write Strategy* field to `CUSTOM`.

. Set the *Write Mode* field to `CUSTOM_APPEND`.

. Adjust the *Buffer Size for Write Strategy* field. When using `CUSTOM_APPEND`, this field defines the size of the data chunks sent to the server. For legacy servers with high latency or restricted packet sizes, adjusting this value can significantly improve performance and stability.

[NOTE]
====
The `CUSTOM_APPEND` strategy manually calculates the file offset to ensure data is added to the end of the file. Use this strategy only for legacy servers that do not support the `STANDARD` strategy, as it does not support concurrent writes to the same file.
====
