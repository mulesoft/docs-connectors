...
This behavior occurs when the target SFTP server is a legacy system that doesn't support the `SSH_FXF_APPEND` flag. Without this flag, the server can't natively process a request to append data to an existing file.
