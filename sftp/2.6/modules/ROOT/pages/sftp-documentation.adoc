...
[NOTE]
====
If your SFTP server is a legacy system that does not support the `SSH_FXF_APPEND` flag, set *Write Strategy* to `CUSTOM` and *Write Mode* to `CUSTOM_APPEND` to prevent files from being overwritten. For more information, refer to xref:sftp-connector-troubleshooting.adoc[Troubleshooting SFTP Connector].
====
