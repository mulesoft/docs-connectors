...
* SFTP Connector 2.0.0 no longer supports the `jcraft library`. The connector now uses `apache mina sshd library`.
* Changed the *List* operation, from now on the operation does not fetch the content of each of the files, instead, the payload of each list item contains the complete path to the found file or directory. 
* The SFTP File Attribute named *Regular Size* was renamed to *Regular File* since its behavior is not related to size. The truth value is related to whether it is a file (true) or a directory (false).
* The SFTP Connector 2.0.0 does not support ciphers 3des-ctr and arcfour (See the *Ciphers Supported* section in this guide).
* The SFTP Proxy protocol only works with HTTP and SOCKS5. The support for SOCKS4 would be added in the feature.
...
* Internally, the connector have change the classes to manage the date from LocalDateTime to ZonedDateTime.
...
To make it easier to review the changes, you can refer to the following table:
