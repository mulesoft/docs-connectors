...
* The *List* operation does not fetch the content of each file, instead, the payload of each list item contains the complete path to the found file or directory. 
...
* SFTP Connector 2.0.0 does not support ciphers `3des-ctr` and `arcfour`. See <<supported-ciphers,Supported Ciphers project>>.
* The SFTP proxy protocol works only with HTTP and SOCKS5.
* SFTP Connector no longer uses `javax.activation` to resolve the media types of the files. The connector now utilizes `Apache Tika`, which can result in different media file interpretations.
