...
* As the `jcraft library` no longer has support, it was replaced by `apache mina library`.
* Changed the `List` operation name to `List and get`. 
** In the XML, changes from `<sftp:list/>` to `<sftp:list-and-get/>`.
...
. In *Add Dependencies to Project*, enter `SFTP Connector` in the search field.
