= Upgrade and Migrate SFTP Connector to Version 3.x  

Upgrade Anypoint Connector for SFTP (SFTP Connector) to version 3.x.x.
...
* The connector has a new custom xref:connectors::sftp/sftp-write.adoc#write-strategy[*write strategy*] field that enables you to configure the strategy to write a file.
* The system property `mule.sftp.useHomeDirectory` was removed. SFTP Connector now uses the default home directory.
* SFTP Connector 3.x.x no longer supports the `jcraft library`. The connector now uses the `apache mina sshd` library.
...
* SFTP Connector 3.x.x does not support ciphers `3des-ctr` and `arcfour`. For more information, refer to <<supported-ciphers,Supported Ciphers project>>.
...
[%header,cols="50a,50a"]
|===
|JSCH - SFTP Connector 1.6 | https://github.com/apache/mina-sshd/blob/sshd-2.9.2/sshd-common/src/main/java/org/apache/sshd/common/cipher/BuiltinCiphers.java[MINA SSHD] - SFTP Connector 2.0
...
=== Override Standard Write Strategy

If your *File Write* operation overrides instead of appending when using the `APPEND` mode, use the `CUSTOM` write strategy along with the `CUSTOM_APPEND` mode only as it isn't applicable to the other modes. With this strategy, you can manually specify the offset for the server to write the file in `APPEND` mode, particularly only for older SFTP servers that don't directly support write modes. Use this strategy only if the `STANDARD` strategy results in errors during file writing in `APPEND` mode and doesn’t offer concurrency. Additionally, you can adjust the buffer size in the *Buffer Size for Write Strategy* field. 
