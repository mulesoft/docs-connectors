...
| Reconnection Strategy a| * <<reconnect>>
* <<reconnect-forever>> | The reconnection strategy to use. It overrides the `Fails Deployment` field |  |
|===
...
* `WHEN_EXHAUSTED_FAIL`
+
Throws a `NoSuchElementException` error.

* `WHEN_EXHAUSTED_WAIT`
+
Blocks by invoking `Object.wait(long)` until a new or idle object is available. If *Max Wait* is a positive value, it blocks for, at most, the specified number of milliseconds, after which the connector throws a `NoSuchElementException` error. If *Max Wait* is a negative value, it blocks indefinitely.

* `WHEN_EXHAUSTED_GROW`
+
Creates a new Mule instance and returns it. In this case, the value for *Max Active* does not apply. | `WHEN_EXHAUSTED_GROW` |
...
* INITIALISE_ALL, which will load all components in the pool at startup. | INITIALISE_ONE  |
