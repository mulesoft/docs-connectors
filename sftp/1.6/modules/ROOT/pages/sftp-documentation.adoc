...
* `WHEN_EXHAUSTED_FAIL`, which will throw a `NoSuchElementException`. 
* `WHEN_EXHAUSTED_WAIT`, which will block by invoking `Object.wait(long)` until a new or idle object is available. If a positive `maxWait` value is supplied, it will block for, at most, the specified number of milliseconds, after which a `NoSuchElementException` will be thrown. If `maxWait` is a negative value, it will block indefinitely.
* `WHEN_EXHAUSTED_GROW`, which will create a new Mule instance and return it, essentially making `maxActive` meaningless.  | WHEN_EXHAUSTED_GROW |
