...
| Watermark Enabled a| Boolean |  Controls whether or not to do watermarking, and if so, if the watermark should consider the file's modification or creation timestamps. Watermark enabled picks only files that have been created or updated after the last poll was executed. |  false |
