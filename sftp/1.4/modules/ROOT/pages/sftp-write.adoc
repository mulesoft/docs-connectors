...
Because the Write operation fails by default if you set nonexistent directories in the *Path* parameter, set the *Create parent directories* parameter to `True (Default)` to automatically create any missing directories.
