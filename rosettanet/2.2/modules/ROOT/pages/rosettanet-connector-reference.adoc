...
| Require secure transport a| Boolean | Records the `isSecureTransportRequired` field of `DeliveryHeader` as `Yes` when HTTPS is active by verifying usage of HTTPS. Generates errors for insecure channels of messaging between partners. | false |
...
** SHA256
** SHA1 |  Algorithm to use for signing of S/MIME messages. | SHA1 |
