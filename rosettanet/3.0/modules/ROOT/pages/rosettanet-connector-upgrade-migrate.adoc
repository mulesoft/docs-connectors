...
* For non-FIPS mode, Bouncy Castle Java Cryptography Extension (JCE) provider designed for JDK 1.8 or higher. This configuration requires the `bcprov-jdk18on`,`bcpkix-jdk18on`, and `bcjmail-jdk18on` libraries. Version 1.82 is recommended.
* For Signing MIME Parts, the `jakarta.activation` and `jakarta.mail` libraries are required.
* For FIPS modes, bcjmail-fips library is required. Other Bouncy Castle FIPS (BCFIPS) libraries as required by your Mule runtime and security model.
...
* *non-FIPS (default)*
+
This is the default security mode for Mule applications that don't require FIPS compliance. RosettaNet Connector uses the standard cryptographic libraries `bcprov-jdk18on`,`bcpkix-jdk18on`, `bcjmail-jdk18on`, `jakarta.activation` and `jakarta.mail` which aren't FIPS compliant. Don't set the system property as the default mode.
...
|non-FIPS | `RosettaNet External Libs - Non FIPS` | `RosettaNet__External_Libs___Non_FIPS` | org.bouncycastle:bcpkix-jdk18on:1.82, org.bouncycastle:bcprov-jdk18on:1.82, org.bouncycastle:bcjmail-jdk18on:1.82, com.sun.activation:jakarta.activation:2.0.1, com.sun.mail:jakarta.mail:2.0.2
|FIPS 140-2 | `RosettaNet External Libs - FIPS 140-2` | `RosettaNet__External_Libs___FIPS_140_2` | org.bouncycastle:bcjmail-fips:1.0.4, com.sun.activation:jakarta.activation:2.0.1, com.sun.mail:jakarta.mail:2.0.2
|FIPS 140-3 | `RosettaNet External Libs - FIPS 140-3` | `RosettaNet__External_Libs___FIPS_140_3` | org.bouncycastle:bcjmail-fips:2.0.5, com.sun.activation:jakarta.activation:2.0.1, com.sun.mail:jakarta.mail:2.0.2
|===
...
** For FIPS 140-3, set `mule.security.model` to `fips140-3`. If the property is configured incorrectly, a message is logged to correct the setting to `-Dmule.security.model=fips140-3`.

* Make sure the implementation library for the selected security mode is  on the same `classpath`. If the library is missing, startup fails. To resolve the issue, review the error log to find the conflicting library and remove it from the `classpath`.
+
