...
* Bouncy Castle Java Cryptography Extension (JCE) provider designed for JDK 1.8 or higher for non-FIPS mode. This configuration requires the `bcprov-jdk18on`, `bcpkix-jdk18on`, and `bcjmail-jdk18on` libraries. Version 1.86 is recommended.
...
+ 
Set this security mode to use FIPS 140-2 approved cryptographic providers.
* *FIPS 140-3 Compliant Mode*
+ 
Set this security mode to use FIPS 140-3 approved cryptographic providers.
...
Refer to the 
xref:release-notes::connector/rosettanet-connector-release-notes-mule-4.adoc[RosettaNet Connector Release Notes] to include specific required library versions that are compliant with the corresponding version of the connector.
...
|non-FIPS | `RosettaNet External Libs - Non FIPS` | `RosettaNet__External_Libs___Non_FIPS` | org.bouncycastle:bcpkix-jdk18on:1.86, org.bouncycastle:bcprov-jdk18on:1.86, org.bouncycastle:bcjmail-jdk18on:1.86, com.sun.activation:jakarta.activation:2.0.1, com.sun.mail:jakarta.mail:2.0.2
...
|FIPS 140-3 | `RosettaNet External Libs - FIPS 140-3` | `RosettaNet__External_Libs___FIPS_140_3` | org.bouncycastle:bcjmail-fips:2.1.7, com.sun.activation:jakarta.activation:2.0.1, com.sun.mail:jakarta.mail:2.0.2
|===

image::rosettanet-connector-upgrade-global-element-config.png["Connector configurations that are used for creating Global Elements",width=50%]
...
. Add the external library global configuration and don't set the runtime property. Make sure the BouncyCastle jdk18on library version 1.86 is available to add as an external library. 
...
+
For FIPS 140-2 compliance, follow the xref:mule-runtime::fips-140-2-compliance-support.adoc[FIPS 140-2 Compliance Support] documentation.
...
.. Name the configuration.
.. Click *Configure* > *Add recommended libraries*.
.. Click *OK* to include the libraries as dependencies and shared libraries in the Mule app `pom.xml` file.  
. Verify that your project has the required dependencies for your selected security mode. Add the dependencies via Maven if they aren't added automatically.
