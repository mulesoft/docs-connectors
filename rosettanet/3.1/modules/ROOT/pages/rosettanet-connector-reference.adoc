= RosettaNet Connector 3.1 Reference 
...
| Require secure transport a| Boolean | When HTTPS is active, the system verifies its usage and records the *isSecureTransportRequired* field in the `DeliveryHeader` as *Yes*. An error is generated for any insecure channels detected during messaging between partners. | false |
...
| Signing algorithm| Enumeration, one of:

** SHA256
** SHA1 |  Algorithm used to sign S/MIME messages. | SHA1 |
| Content transfer encoding| Enumeration, one of:

** BASE64
** BINARY
** QUOTED_PRINTABLE | Content transfer encoding for S/MIME messages. | QUOTED_PRINTABLE |
| Service-Content Compression| Enumeration, one of:

** GZIP
** ZLIB
** NONE | Algorithm to use to compress service-content part of S/MIME messages. | NONE |
