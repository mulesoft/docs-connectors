...
* Configure external libraries for security modes (non-FIPS, FIPS 140-2, and FIPS 140-3).
* Supports PIPs that have only XSD definitions (without DTDs).
...
| Signing algorithm |Algorithm to sign S/MIME messages. Select one:
...
* BINARY
* QUOTED_PRINTABLE
| Service-Content Compression |Algorithm to compress the service-content part of an S/MIME message. Select one:
