...
|3A4 |3A4MG1 v02.02, 3A4MG2 v02.02, V11.10  |Request Purchase Order
|3A6 |3A6MG1 v02.00, v02.02, v02.03, v11.00 |Distribute Order Status
|3B2 |3B2MG1 v01.00, v11.00 |Notify of Advance Shipment
|3C3 |3C3MG1 v01.00, v01.11, V11.00 |Notify of Invoice
|===
...
RosettaNet connector 3.x for Mule 4 makes several changes to the configuration and handling over what was implemented by RosettaNet connector 2.x:

* Configuring External libraries to Support Security Modes (non-fips,fips140-2 and fips140-3).
* Support of PIP's that only have XSD definitions (without DTDs).
* Allow compression of Service-Content part of S/MIME messages.
* Allow configuration of content transfer encoding for S/MIME messages. 
...
| Require secure transport| When HTTPS is active, the system verifies its usage and records the *isSecureTransportRequired* field in the `DeliveryHeader` as *Yes*. An error is generated for any insecure channels detected during messaging between partners.
...
| Signing algorithm |Algorithm to sign S/MIME Message, select one of:

* SHA1
* SHA256 
| Content transfer encoding |Encoding format to use when parts are transmitted, one of:

* BASE64
* BINARY
* QUOTED_PRINTABLE
| Service-Content Compression |Algorithm to compress the Service Content part of S/MIME Message, select connector, one of:
...
|isSecureTransportRequired |Require secure transport (HTTPS)
|contentTransferEncoding |Content transfer encoding
|serviceContentCompression |Service-Content compression
|===
...
Inside the PIP configuration directory you'll find an XML file giving the parameters for a particular version of the PIP (such as `V02_02.xml`). This XML file gives all the details of retry counts, times to acknowledge, and signing requirements for the action(s) defined by the PIP. This file also references DTD and/or XSD definitions for the actual action messages (for PIP's that only have DTD, a XSD file is also needed; the XSD is used to provide DataSense information inside Mule).
...
* xref:release-notes::connector/rosettanet-connector-release-notes-mule-4.adoc[RosettaNet Connector Release Notes]
