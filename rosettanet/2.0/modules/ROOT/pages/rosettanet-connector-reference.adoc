...
| Name | Type | Description | Default Value | Required
| Configuration | String | Name of the configuration to use. | | x
| Initiating action a| <<ReplyAttributes>> | Lists the <<ReplyAttributes>> to send inside the Service header in the MIME message.|  |
...
Completion status and details for the *Completion* source.
...
Output of the *Action* source, *Completion* source, or *Send Action* operation.
