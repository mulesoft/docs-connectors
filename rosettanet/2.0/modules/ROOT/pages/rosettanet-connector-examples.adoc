...
. Receives an input purchase order request document via `HTTP POST`.
. Uses a *Send Action* operation to send a purchase order request message to the Seller app.
. Transforms the message ID from the `HTTP POST` to plain text for use in responding to the `HTTP POST`.
. Receives an ACK signal from the Seller app and logs information about the ACK signal.
. Receives a purchase order completion message from the Seller app and logs information about the purchase order completion message.
...
Add the *Send Action* operation to send the purchase order request message to the seller by using the configuration defined for the `PO_InitiatorConfig_Buyer` global element:
...
. Uses a *Send Action* operation to send an ACK signal to the Buyer app.
. Transforms the message ID from the ACK signal to plain text for use in responding to the `HTTP POST`.
. Receives the purchase order request from the Buyer app and logs information about the request.
. Sends a purchase order completion message to the Buyer app and logs information about the completion message. 
