...
There was an error when the Validate schema operation validated if both fields, Schema and Schema Content, are used at the same time at the execution time. Note that you can only use one of the fields to validate a schema XSD content.
