...
This document presents two usage examples of the database connector. This document assumes you are familiar with the xref:index.adoc[database connector] as well as with xref:5.x@studio::datasense.adoc[DataSense]; to understand the second example, the reader should also be familiar with the
xref:5.x@studio::datamapper-visual-reference.adoc[Anypoint DataMapper Visual Reference] and Mule xref:3.7@mule-runtime::batch-processing.adoc[batch processing]. You should also see xref:3.7@mule-runtime::dataweave.adoc[DataWeave], which replaces DataMapper.
...
In this example, a database connector retrieves data from two tables. A xref:5.x@studio::datamapper-visual-reference.adoc[DataMapper transformer] maps this data to the CSV format. A xref:3.7@mule-runtime::file-connector.adoc[file connector] writes the resulting CSV file to disk, and a xref:3.7@mule-runtime::logger-component-reference.adoc[logger component] logs processing details to the console.
...
* Learn more about xref:5.x@studio::datasense.adoc[DataSense] and how you can use it to retrieve metadata on remote applications.
