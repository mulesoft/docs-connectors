...
Database Connector performs type coercion to construct proper JDBC parameter types. The failure point depends on Java type conversion strictness—strict conversions (String to BigDecimal) fail at the connector level, while lenient conversions (String to Integer) may allow invalid values to reach the database. 

Database drivers handle validation differently, which affects when errors occur and whether partial data might be committed. These sections describe the two validation types:

* Connector-Level Validation

When the connector performs strict type conversion, validation failures occur before data is sent to the database:

* DECIMAL/NUMERIC: Non-numeric strings fail BigDecimal conversion
* DATE/TIMESTAMP: Invalid date formats fail date parsing

Result: Zero records are committed because the error occurs before the database operation runs.
...
Result: Records before the failure may be committed depending on how the database driver handles errors. Some drivers continue processing after an error (partial success), while others stop on the first error (all records rejected).

* Preventing Partial Commits

To ensure all-or-nothing behavior and prevent partial commits across all database platforms, wrap bulk operations in a transactional scope. This approach manages transactions at the Mule flow level and handles both connector-side failures (catches exceptions before database interaction) and database-side failures (rolls back partial commits). For example, use a Try scope with `transactionalAction="ALWAYS_BEGIN"` or `"BEGIN_OR_JOIN"`.
