...
Since the Stored Procedure operation closes the connection it receives once the flow it contains is completed, using a Pooling Profile which provides fewer connections than the ones the flow uses will make the whole flow hang.

A plausible solution would be to increase the size of the pool. But sometimes this is not posible so another workaround can be applied by using the VM Connector: we can narrow the life of the connection to be just like the one of a VM's Message. This way every call to the Store Procedure is executed, its result consumed, and the connection associated to it closed and returned back to the pool.
