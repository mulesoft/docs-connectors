...
The following example shows how to use pooling profiles with the *Store procedure* operation and VM Connector. When the foreach element reaches the eleventh iteration the *Stored procedure* operation won't hang since the pool is able to provide a connection:
