...
Anypoint Connector for Database (Database Connector) examples help you to:

* <<configure-database-connection,Configure a Database Connection>> +
Connect to the most popular databases such as Derby, Microsoft SQL Server, MySQL, Oracle, and a generic database. Additionally, configure the connector to connect to a global data source, set a JDBC driver, configure connection pooling, and connection to an Oracle database with TNS.
* <<query-database,Query a Database>> +
Configure the *Select* operation to query data from a database.
* <<insert-update-delete,Insert, Update, and Delete Data with the Database Connector>> +
...
* xref:database-connector-udt-stored-procedure.adoc[Call Oracle Stored Procedure with UDTs] +
Configure the *Stored procedure* operation to call an Oracle database stored procedure that uses UDTs for its input and output parameters. Additionally, the example shows how to use `createStruct` and `createArray` DataWeave functions to map application data to your custom types.
...
Configure the *Bulk insert*, *Bulk update* and *Bulk delete* operations to use them in cases where each input parameter can take only one value.
* <<execute-script,Execute Script Reference>> +
Configure the *Execute script* operation to execute a script of random length as a single statement.
* <<data-types,Define Custom Data Types>> +
Define custom data types that you can use when connected to a particular connection provider.
