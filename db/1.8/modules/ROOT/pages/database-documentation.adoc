...
The *Stored procedure* operation closes the connections it receives when the flow that contains the operation completes. This behavior occurs because the resultset the operation generates can have a stream or be part of an ongoing transaction. Thereby, Mule runtime engine never knows when the flow is going to consume the resultset in its entirety, nor when the connection will be needed.
...
When executing the *Stored procedure* operation with a pooling profile, consider that because connections do not release until the flow completes, the number of available connections must be equal to, or greater than, the connections that will be used.
