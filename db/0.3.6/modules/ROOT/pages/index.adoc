...
:page-aliases: 3.6@mule-runtime::database-connector.adoc

The Anypoint Connector for Database (Database Connector) enables you to connect with almost any Java Database Connectivity (JDBC) relational database using a single interface for every case. The Database connector allows you to run diverse SQL operations on your database, including Select, Insert, Update, Delete, and even Stored Procedures.

*Note*: The Database connector replaces the JDBC connector. Starting with Mule 3.5.0, the JDBC connector is deprecated.
...
. Check that your database engine is <<Configuring Database Engines Supported Out of the Box>>; if it is not, then add the database driver for your database engine.
...
== Configuring Database Engines Supported Out of the Box
...
The following database engines are supported out of the box:
...
|*Name* |Use to define a unique identifier for the global Database connector element in your application. .6+|image:oracle-global-elem.png[oracle_global_elem]
...
|===
|Attribute, Required |Use |Properties Editor
|*Name* |Use to define a unique identifier for the global Database connector element in your application. .6+| image:mysql-global-elem.png[mysql_global_elem]
|*Database Configuration Parameters* +
...
|===
|Attribute, Required |Use |Properties Editor
|*Name* |Use to define a unique identifier for the global Database connector element in your application. .6+|image:derby-global-elem.png[derby_global_elem]
|*Database Configuration Parameters* +
...
3+|*General tab*
|*Enable DataSense* |Use to "turn on" DataSense, which enables Mule to make use of message metadata during design time. |image:config-enable-ds.png[config_enable_DS]
3+|*Advanced tab*
...
== Configuring Database Engines Not Supported Out of the Box (Generic Installation)

All databases that are not configured <<Configuring Database Engines Supported Out of the Box>> must be added through the generic database installation, and then configured accordingly.
...
==== Installing the Database Driver in the Studio Visual Editor
...
==== Installing the Database Driver in the XML Editor or Standalone
...
=== Configuring the Global Database Connector for Generic DB Configuration with the Studio Visual Editor
...
|===
|Attribute, required |Use |Properties Editor
|*Name* |Use to define unique identifier for the global Database Connector element in your application. .6+|image:global-elem-generic-db-gral-tab.png[global_elem-generic_DB-gral_tab]
...
*OPTIONAL:* The following table describes the attributes of the element that you can _optionally_ configure to customize some functionality of the Global Database Connector.
...
=== Configuring the Global Database Connector for Generic DB Configuration with the XML Editor
...
|===
|Attribute, Required |Use |Properties Editor
|*Display Name* |Use to define a unique identifier for the Database Connector element in your flow. .5+a|image:config-db-connector.png[config_db_connector]
...
4+|*General tab*
|*Parameter Name* |Parameterized Stored Procedure |Use to identify a named parameter in your SQL statement for which you wish to use the value at runtime, when your application submits your query which calls upon stored in the database instance. .4+|image:stored-procedure.png[stored+procedure]
|*Parameter Type* |Parameterized Stored Procedure |Use to identify the type of data the stored procedure can expect to receive from your query statement.
|*IN/OUT* |Parameterized Stored Procedure a|Defines the behavior of your stored procedure:
...
==== Configuring a From-Template Query with the Studio Visual Editor
...
==== Configuring a From-Template Query with the XML Editor or Standalone
...
== Execute DDL Examples: Studio Visual Editor
...
== Execute DDL Examples: XML Editor
...
=== Bulk Updates Examples: Studio Visual Editor
...
=== Bulk Updates Examples: XML Editor
...
* Study several xref:database-connector-examples.adoc[example applications] which utilize the Database connector.
* Access plain xref:database-connector-reference.adoc[Reference material] for the Database connector.
* Learn more about xref:3.6@mule-runtime::datasense.adoc[DataSense].
* Learn more about xref:6.x@studio::datamapper-user-guide-and-reference.adoc[DataMapper].
