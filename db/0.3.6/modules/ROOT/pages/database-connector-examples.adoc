...
In this example, a database connector retrieves data from two tables. A xref:6@studio::datamapper-user-guide-and-reference.adoc[DataMapper transformer] maps this data to the CSV format. A xref:3.6@mule-runtime::file-connector.adoc[file connector] writes the resulting CSV file to disk, and a xref:3.6@mule-runtime::logger-component-reference.adoc[logger component] logs processing details to the console.
