= Database Connector Reference 1.13 
...
For Database Connector 1.13, even though the *Fetch size* field relies on streaming, you can use it for this source because the connector ignores the *Max rows* field values. In the next Database Connector version, the *Fetch size* field will be removed for this source and the source will use the correct *Max rows* field instead.
