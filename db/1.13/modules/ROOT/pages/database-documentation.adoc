...
Even though the *Fetch size* field relies on streaming, you can use it for this source. The *Max rows* field values are ignored by the connector.  In the next Database Connector version, the *Fetch size* field will be removed for this source to use the correct *Max rows* field instead.
