...
The `prepareArray` function instructs Mule to create JDBC array objects based on the `Array` type when a database operartion is performed. In this example, types `PHONE_NUMBER_ARRAY`, `PEOPLE`, and `PHONE_BOOK` are array objects. The function expects the following parameters in order:
...
== Migrating away from createStruct and createArray

Previous Mule versions only provided `createStruct` and `createArray`. These now deprecated functions create JDBC values right away. However, these JDBC values must be created with the same connection that's used for the database operation. That's why `prepareStruct` and `prepareArray` were added so these values' creation is deferred until the right SQL connection is available.

Existing code using the deprecated functions can be updated by replacing `createStruct` with `prepareStruct` and + `createArray` with `prepareArray`, and by removing the first parameter (a database configuration).
