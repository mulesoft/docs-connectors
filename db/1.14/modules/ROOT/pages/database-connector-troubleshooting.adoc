...
When using the Stored Procedure Operation and the Named Parameters binding feature the next exceptions are expected for the following scenarios.

Suppose the following Stored Procedure definition for them:
...
=== Mixing Named and Ordinal Parameter Bindings

Suppose the following Stored Procedure execution, using both Named and Ordinal parameter bindings:
...
=== Using a JDBC Driver that does not support Named Parameters Binding

Suppose the JDBC Driver that's being used does not support the Named Paramters binding feature, the following exception is expected:
...
=== Misspelling the Named Parameter

Suppose the following Stored Procedure execution, referencing incorrectly the PARAMETERB as PARAMETERW:
