...
== Troubleshoot the Stored Procedure Operation With Named Parameters
...
Suppose the following stored procedure execution uses both named and ordinal parameter bindings:
...
=== JDBC Driver Does Not Support Named Parameters Binding

If the JDBC driver used does not support the named parameters binding feature, the following exception is expected:
