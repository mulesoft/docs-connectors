...
If a query or stored procedure takes more than 15 or 30 minutes to execute, the connection might get closed by the firewall or network devices due to inactivity.

To prevent this, set the TCP Keep Alive for the Oracle provider by adding the following flag as a JVM parameter:
