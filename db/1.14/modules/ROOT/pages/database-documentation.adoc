...
Starting on 1.14, the *Stored procedure* operation allows binding parameters by their actual named defined in the database. Your JDBC driver must support named parameters, refer to `java.sql.DatabaseMetaData` supports named parameters method for details, otherwise Mule throws `UnsupportedOperationException` at runtime.

For further information about named parameters, refer to xref:database-stored-procedure.adoc#parameters-name[Passing Parameters to Stored Procedures by Parameters Name] documentation.
