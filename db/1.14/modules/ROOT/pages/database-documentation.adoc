= Database Connector Reference 1.14 
...
| Pooling Profile a| <<pooling-profile>> |  Configures the database connection pooling. |  |
| Column Types a| Array of <<ColumnType>> |  Specifies non-standard column types |  |
| Cache Query Template Size a| Number | Sets the cache query template size for the connection. |  50 |
...
NOTE: To incorporate an SQL function within an operation's query, use the `{fn function()}` syntax. This constitutes a JDBC escape call. For instance, `CURRENT_TIMESTAMP` must be written as `{fn CURRENT_TIMESTAMP()}`.
...
| Test connection on checkout a| Boolean | Disables connection testing on checkout to improve performance. If set to `true`, an operation is performed at every connection checkout to verify that the connection is valid. | `true` |
|===
...
| `mule.db.commons.defaultParamTypeResolver.enable.fallback` | This system property handles special characters in stored procedure names, providing an alternative configuration method when these characters are present. | `false` | `-M-Dmule.db.commons.defaultParamTypeResolver.enable.fallback=true`
|===
