= Database Connector Reference 1.14 - Mule 4
...
NOTE: To specify an SQL function in an SQL query in an operation, specify the SQL function in the `{fn function()}` format. Note that this is a JDBC escape function call. For example, the SQL function `CURRENT_TIMESTAMP` is specified as `{fn CURRENT_TIMESTAMP()}`.
