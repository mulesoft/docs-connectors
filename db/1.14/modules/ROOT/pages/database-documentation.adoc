...
=== Using Named Parameters Feature

Starting on 1.14, the Stored Procedure operation allows binding parameters by their actual named defined in the database. In order for this feature to work your JDBC Driver must support such feature (see java.sql.DatabaseMetaData' supportsNamedParameters method), otherwise `UnsupportedOperationException` will be thrown at runtime.

To use this feature, visit xref:database-stored-procedure.adoc[Stored Procedures Example page] (section Passing Parameters to Stored Procedures by Parameters Name).
