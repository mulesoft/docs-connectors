...
| `mule.db.commons.defaultParamTypeResolver.enable.fallback` | This system property handles special characters in stored procedure names, providing an alternative configuration method when these characters are present. | `false` | `-M-Dmule.db.client.defaultParamTypeResolver.enable.fallback=true`
|===
