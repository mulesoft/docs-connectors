...
This document presents two usage examples of the database connector. This document assumes you are familiar with the xref:0.3.6@index.adoc[database connector] as well as with xref:3.6@mule-runtime::datasense.adoc[DataSense]; to understand the second example, the reader should also be familiar with the xref:6@studio::datamapper-user-guide-and-reference.adoc[DataMapper transformer] and Mule xref:3.6@mule-runtime::batch-processing.adoc[batch processing].
...
In this example, a database connector retrieves data from two tables. A xref:6@studio::datamapper-user-guide-and-reference.adoc[DataMapper transformer] maps this data to the CSV format. A xref:3.7@mule-runtime::file-connector.adoc[file connector] writes the resulting CSV file to disk, and a link:/mule-user-guide/v/3.3/logger-component-reference[logger component] logs processing details to the console.
