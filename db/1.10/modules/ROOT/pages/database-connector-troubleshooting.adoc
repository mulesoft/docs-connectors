...
== Troubleshoot Select and Stored Procedure operations when an exception is warned instead of thrown
When DB Connector operations' Select and Stored Procedure execute correctly but results are not present, an error is warned by Mule.

* For Select operation, when a subquery returns more than one value and that result is used for filtering or as an expression then an error is warned stating this situation and why it happened. The warning is produced as soon as the operation's result is being paginated and not when it's executed.
...
* For Stored Procedure operation, when a stored procedure contains multiple statements and one or more of them fail an error is warned stating this situation. The warning is produced as soon as the operation's result is being consumed and not when it's executed.
...
=== Cause
Executing an invalid SQL statement or expecting that an error within a set of statements will collapse the whole procedure when in fact what JDBC returns are a series of ResultSets, UpdateCounts, and Exceptions.

=== Why an Exception is not thrown so the Flow can catch it?
This scenario has its root cause in the behavior of the Driver. Some drivers comply strictly with JDBC and throw the exception properly (at execution time), but for Drivers that do not, adding this behavior into the connector code would break backward compatibility for Mule applications that do not expect an exception under this scenario. Because of this is that we opted to log a warning message informing the user of this situation.
