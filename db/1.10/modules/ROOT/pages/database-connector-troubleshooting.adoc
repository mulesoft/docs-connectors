...
== Troubleshoot CloudHub with Static IP failing on deployment

Deploying a CloudHub application with Static IP throws a DB test connection error making the process fail. This is bacause the test is performed before static IP gets assigned.
