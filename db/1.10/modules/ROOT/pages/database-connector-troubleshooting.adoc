...
When the Database Connector *Select* and *Stored procedure* operations execute correctly but results are not present, Mule runtime engine (Mule) returns a `WARN` exception message instead of throwing the exception.

This occurs when executing an invalid SQL statement or expecting that an error within a set of statements will collapse the procedure when in fact what JDBC returns are a series of `ResultSets`, `UpdateCounts`, and `Exceptions`.

Mule does not throw an exception for the flow to catch it because the root cause is in the driver's behavior. Some drivers comply strictly with JDBC and throw the exception properly, at execution time, but other drivers do not. Therefore, adding into the connector code the behavior of throwing an exception, would break backward compatibility for Mule apps that do not expect an exception. 
...
For the *Store procedure* operation, Mule logs a `WARN` exception when stored procedure contains multiple statements and one or more of them fail. Mule returns the warning as soon as the operation's result is consumed and not when it's executed. For example, for the following SQL Stored Procedure:
