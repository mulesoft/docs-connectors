...
If your Mule app uses Database Connector operations such as *Insert*, *Select* or others that run with an XA transaction, and connection pooling is enabled via the *Pooling profile* fields of the connector configuration, the default connection pool implementation for the Data Source Reference Connection is Enhydra XAPool.
