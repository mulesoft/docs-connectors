...
| Test connection on checkout a| Boolean | If set to `true`, an operation is performed at every connection checkout to verify a valid connection. A better choice is to verify connections periodically using c3p0.idleConnectionTestPeriod. In order to improve performance, set this property to `false`. | `true` |
|===
