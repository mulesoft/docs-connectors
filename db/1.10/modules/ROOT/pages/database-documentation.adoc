...
*Note*: When working with any LOB, since they are treated as a stream, the connection remains open until the flow execution ends or, if the content is consumed before the flow ends, a best-effort approach is taken in order to close the associated connection.
