= Database Connector Reference 1.10 - Mule 4
...
| Max Statement a| Number | Defines the total number PreparedStatements a DataSource will cache. The pool destroys the least-recently-used PreparedStatement when it hits this limit. When set to 0 statement caching is turned off |  |
| Test connection on checkout a| Boolean | Disables connection testing on checkout in order to improve performance. If set to `true`, an operation is performed at every connection checkout to verify that the connection is valid. A better choice is to verify connections periodically using c3p0.idleConnectionTestPeriod. In order to improve performance, set this property to `false`. | `true` |
|===
