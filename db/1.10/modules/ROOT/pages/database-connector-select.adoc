...
Since the expression values may not be available at design time, using this feature might throw DataWeave evaluation errors or affect the Output Metadata (DataSense) of the Select Operation.
To default to a value (and avoid the aforementioned inconvenience) the default modifier may be used as follows:
