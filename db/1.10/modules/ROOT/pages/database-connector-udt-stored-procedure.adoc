...
Adding a pooling profile to your application will help you increase performance by creating a group of already established connections that the connector can draw upon to perform the required operations, without having to establish new connections each time, which would be costly.
When adding a pooling profile, we strongly recommend that you set 'testConnectionOnCheckout=false', will greatly improve the connector's performance with connection pooling. This is because if you leave the default value (true), the connection pool will perform connectivity checks against the database every time a connection is checked out of the pool, and this will inevitably increase the time required for each operation to complete.

Here's an example of how you would set this property to false in a pooling profile.

[source,xml,linenums]
----
	<db:config name="dbConfig">
		<db:oracle-connection host="localhost" user="myuser" password="mypass" serviceName="orclpdb1">
			<db:pooling-profile minPoolSize="4" maxPoolSize="30" testConnectionOnCheckout="false"/>
			<db:column-types>
				<db:column-type id="2003" typeName="PHONE_NUMBER" />
...
Or go to Database Config > General > General > Advanced in Studio and uncheck the property 'test connection on checkout'.
