...
This operation introduces the ability to execute a select statement that returns a single record (row) and access the result without any additional transformations, just by accessing `payload`.

This operation does not apply streaming for any of the returned fields of the record. This means that when the query is executed, all the values of the returned row will be read into memory and returned in the payload. You must be extremely careful, because if your row is too large you could run out of memory.
...
You can invoke the query single mostly in the same way you would a regular select, only you will always get a single result (or less) regardless of the amount of records returned by the actual sql query.

The following SQL statement creates a table called 'BOOK' and inserts 7 records:
...
Then create a mule app and add the following configuration for your database connection.
...
You will then need to add the following flow to your application.
...
----
	<flow name="querysingleForeachFlow">
		...
		<set-payload variableName="bookCollection" value="#[vars.bookCollection]" />
	</flow>
----

Trigger the flow by hitting the endpoint using: `curl localhost:8081/select/book/7`. You should be able to insert as many records as you like and increment the maxId parameter in the http request without issues, regardless of the number of maximum connections you have configured in your pool.
