...
This operation selects a single data record from a database. If you provide an sql query that returns more than one row, then only the first record will be processed and returned. Streaming is not used in this operation, which means that the complete contents of the selected record will be loaded into memory immediately after performing the single query operation.
...
=== Working with Pooling Profiles
When working with pooling profiles and the *Single Query* operation, the connection is returned to the pool immediately after the operation is performed.
