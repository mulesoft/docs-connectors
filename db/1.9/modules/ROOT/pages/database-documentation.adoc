...
=== Working with Pooling Profiles
When working with pooling profiles and the *Single Query* operation, the connection returns to the pool immediately after the operation is performed.
...
This operation does not support streaming, meaning that access to the result will be done without any additional transformations to the payload. This behavior is identical to the one present in the newly released *Query Single* operation (starting on version 1.9.0).
