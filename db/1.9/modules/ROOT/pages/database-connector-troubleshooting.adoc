...
* DB:BAD_SQL_SYNTAX
 ...
 The provided SQL query has a bad syntax.

* DB:CANNOT_LOAD_DRIVER

 Database Connector can not load the JDBC driver.

* DB:CANNOT_REACH

 Database Connector can not establish a connection with the RDBMS.

* DB:CONNECTIVITY

 A connectivity issue with the database that occurs on badly configured connections, no database response while waiting results or fetching rows, lost of database connection, and similars.
