...
== Configure the Stored Procedure with the Input Parameters Field
...
----
== Configure the Stored Procedure with the In Out Parameters Field
...
== Configure the Stored Procedure with the Input and Output Parameters Fields
...
. Repeat the previous three steps to add new values, set the *Key* field to `result2` and the *Type* field to `INTEGER`.
. Create another value again by setting the *Key* field to `myInt` and the *Type* field to `INTEGER`.
