...
Additionally, some JDBC drivers (such as jTDS 1.3.1) do not fully implement the JDBC 4.0 `free()` method, which can cause a `MULE:FATAL_JVM_ERROR` and abort the flow entirely.
...
| `NONE` | Does not call `free()` at all. LOB resources are released passively by the JDBC driver when the ResultSet, Statement, or Connection is closed. | Use when `free()` causes errors (incompatible drivers) or when performance is critical and passive release is acceptable.
|===
...
* `MULE:FATAL_JVM_ERROR` caused by `AbstractMethodError` on `Clob.free()` or `Blob.free()` (consider `NONE`)
