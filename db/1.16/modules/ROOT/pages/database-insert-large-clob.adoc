= Inserting Large File Content into a Table Examples 
...
NOTE: If you experience performance degradation when consuming queries that return many LOB columns, refer to xref:database-connector-troubleshooting.adoc#troubleshoot-lob-resource-release-performance[Troubleshoot LOB Resource Release Performance] for tuning options.
