...
Use streaming with queries that return many records, such as in integration use cases. Streaming is transparent and always enabled.
...
With streaming, Database Connector fetches and processes only part of the query at one time, reducing the load on the network and memory. This means that the connector does not fetch the 10K rows at once; instead, it fetches a smaller chunk, and after that chunk is consumed, it fetches the rest.
