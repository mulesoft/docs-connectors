...
If you don’t know how to use a properties file, see xref:mule-runtime::mule-app-properties-to-configure.adoc[Configure Property Placeholders].
...
.This flow uses the *Parse Template* component to create an HTML input form.
image::salesforce-parse-template.png[Flow for creating an HTML input form]

=== Create a New Salesforce Account

This flow creates a new Salesforce account based on information in the HTML input form:

.This flow uses the *Create* operation to create an HTML input form.
image::salesforce-create-.png[Flow for creating an HTML input form]
...
Add the first *Transform Message* component to convert the field values to Java format so the values can be used as input to the *Create* operation:
...
* A field named `type`, which has the referenced object type name as its value.
+
...
The following XML shows how to upsert the `Component_c` object using Salesforce Connector:
