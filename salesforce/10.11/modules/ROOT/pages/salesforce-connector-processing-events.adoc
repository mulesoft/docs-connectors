= Processing Events With Salesforce Connector 10.11
...
To receive events for Salesforce changes associated with a topic, create a new topic or use an existing one.
...
The way that the object store stores the necessary data for the *Resume from the Last Replay Id* varies depending on the environment:
