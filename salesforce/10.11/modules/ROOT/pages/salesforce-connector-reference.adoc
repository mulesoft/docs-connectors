...
Returns a list of result ids for a specific batch. Salesforce API documentation points out that if the batch required retries, there will be more than one <result> element in the output. This operation should be used along with <<queryResultStreamById>> in order to obtain the results of the query.
