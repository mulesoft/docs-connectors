...
Returns a list of result IDs for a specific batch. The Salesforce API documentation states that if the batch requires retries, there will be more than one <result> element in the output. This operation should be used with <<queryResultStreamById>> to obtain the results of the query.
...
** ZIP_XML
** ZIP_CSV
** ZIP_JSON |  Content type used at job creation. If it is not provided, the default value is used. |ContentType.XML  |
...
Returns an InputStream with the query results for a specific result ID of a submitted BatchInfo. You will need to obtain the result IDs first by using the <<queryResultList>> operation.
