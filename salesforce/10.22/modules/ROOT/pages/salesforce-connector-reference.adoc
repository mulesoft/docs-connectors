...
[NOTE]
If you set a `LocalDate` for a field in the payload, the input time gets automatically converted to `GMT` or the server’s time zone based on the `GregorianCalendar`. This conversion impacts date calculations or comparisons in Salesforce connector operations, as the `LocalDate` is adjusted to align with the server’s time zone or defaults to GMT.
...
| Apex Class Method Name a| String | Apex class method name. The method name format is like, 
