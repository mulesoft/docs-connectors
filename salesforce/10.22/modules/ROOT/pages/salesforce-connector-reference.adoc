...
[NOTE]
If you set a `LocalDate` for a field in the payload, the input time gets automatically converted to `GMT` or the server’s timezone based on the `GregorianCalendar`. This conversion impacts date calculations or comparisons in Salesforce connector operations, as the `LocalDate` is adjusted to align with the server’s timezone or defaults to GMT.
