...
* The free version of Object Store for CloudHub 1.0 and CloudHub 2.0 deployments of Mule apps has a limit of 10 transactions per second (TPS), however other Anypoint Connectors using the object store internally have a limit of 100 TPS.
...
If you have multiple workers deployed in CloudHub 1.0, the *Replay topic listener* and *Replay channel listener* sources process each message on all Mule instances. To avoid this situation, you can deploy your application on a single worker and then use a shared queue to pass the messages to a different worker to process the message.
...
* If you use multiple workers when running the app in CloudHub 1.0, all of the workers receive and process the events.
