= Troubleshoot Salesforce Connector 10.12 - Mule 4
...
== Upserting picklist values

Using the Upsert Metadata operation for a picklist field in any object will only update the picklist mapping for that specific object. If you need to update the actual picklist values(add new values to the picklist or remove some value), you will have to use the Deploy Metadata operation. Based on whether the picklist is tied to custom field or is a global picklist, the users would have to perform a metadata deploy of Custom Field or Global Picklist.
