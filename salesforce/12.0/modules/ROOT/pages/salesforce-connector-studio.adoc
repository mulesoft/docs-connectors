= Using Anypoint Studio to Configure Salesforce Connector 12.0  
...
+
. Optionally specify a value in the *Query Time Buffer (Seconds)* field to subtract that number of seconds from the current time when querying for changes. This prevents data loss caused by records committed after the query timestamp but before the next poll. The value must be between `0` and `120` seconds and defaults to `0`.
+
...
* <<oauth-client-credentials,OAuth Client Credentials authentication>>
...
To set up a mutual TLS certificate in your Salesforce environment, see https://help.salesforce.com/s/articleView?language=en_US&id=security_keys_uploading_mutual_auth_cert.htm&type=5[Set Up a Mutual Authentication Certificate].
...
| Authorization URL | Web service URL responsible for user authentication. This is the URL for the endpoint configured to handle SOAP authentication requests. It defaults to the URL that contains the latest API version supported by the connector, for example, `+https://login.salesforce.com/services/Soap/u/67.0+`. 
|===
...
|===
|Field a|Description
|Consumer key | Consumer key for the Salesforce-connected app. For information about generating a consumer key, refer to the https://help.salesforce.com/s/articleView?id=xcloud.connected_app_rotate_consumer_details.htm&type=5[Salesforce documentation].
...
|===
|Field a|Description
|Api version | API version used. Defaults to the latest API version supported by the connector.
|===
...
OAuth Client Credentials uses a client ID and client secret for authentication.
...
|===
|Field a|Description
|Client id |  The OAuth client ID as registered with the service provider.
|Client secret| The OAuth client secret that corresponds to the client ID.
|===
