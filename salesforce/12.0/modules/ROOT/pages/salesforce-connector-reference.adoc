= Salesforce Connector 12.0 Reference 
:page-aliases: connectors::salesforce/salesforce-connector-reference.adoc, connectors::salesforce-connector-tech-ref.adoc, connectors::salesforce-connector-reference-v910.adoc

include::reuse::partial$non-inclusive-banner.adoc[]

Anypoint Connector for Salesforce (Salesforce Connector) simplifies and accelerates integration across Sales Cloud, Service Cloud, Salesforce Platform, and Force.com. With comprehensive access to Salesforce entities, this connector streamlines automation of business processes—helping you unlock more value from your Salesforce investments, empower your sales teams, increase revenue, and provide better customer service.
...
| Connection a| * <<sfdc-config_basic, Basic Authentication (Supported by Salesforce API v64.0 and below)>>
* <<sfdc-config_config-with-oauth, OAuth v2.0>>
* <<sfdc-config_jwt, OAuth JWT>>
* <<sfdc-config_oauth-client-credentials, OAuth Client Credentials>>
...
[[sfdc-config_basic]]
=== Basic Authentication Connection Type (Supported by Salesforce API v64.0 and below)
...
|===
| Name | Type | Description | Default Value | Required
| Connection Timeout a| Number | How long the connector waits before timing out when establishing a connection to the remote service. Values less than one millisecond are converted to `0`. A value of `0` means wait indefinitely. |  `0` |
| Time unit a| Enumeration, one of:
...
| Login Request Timeout a| Number | How long the consumer waits to receive the login request before the request times out. Values less than one millisecond are converted to `0`.  A default value of `0` means an indefinite wait, so set a non-zero value to avoid prolonged waits based on your requirements. | |
...
| Authorization Url a| String |  URL of the service provider's authorization endpoint. Always use lowercase when specifying the protocol (e.g., http, https). |  `\https://login.salesforce.com/services/oauth2/authorize` |
| Access Token Url a| String |  URL of the service provider's access token endpoint. Always use lowercase when specifying the protocol (e.g., http, https). |  `\https://login.salesforce.com/services/oauth2/token`|
...
| Auth Server Proxy Config a| Any |  Proxy configuration for HTTP requests to the authorization server during the OAuth dance. |  |
...
NOTE: Salesforce Connector supports providing a signed certificate only if you use the `SHA256withRSA` algorithm for the OAuth JWT connection type.

You can only enable *JSON Web Token-based access tokens* option on Salesforce server for REST API calls. This option isn't compatible with Salesforce Connector.
...
| Token Endpoint a| String |  URL pointing to the server that provides the authentication token. |  `\https://login.salesforce.com/services/oauth2/token` |
...
[[sfdc-config_oauth-client-credentials]]
...
| Token URL a| String |  Service provider’s token endpoint URL. Always use lowercase when specifying the protocol (e.g., http, https). | `\https://{domain}.my.salesforce.com/services/oauth2/token` |
| Scopes a| String |  The OAuth scopes to request during the OAuth dance. |  |
| Auth Server Tls Context a| <<Tls>> |  TLS context for HTTP requests to the authorization server during the OAuth dance. |  |
| Auth Server Proxy Config a| Any |  Proxy configuration for HTTP requests to the authorization server during the OAuth dance. |  |
| Object Store a| String |  Configures the object store that stores data for each resource owner. If not configured, Mule uses the default object store. |  |
|===
...
[NOTE]
OAuth SAML currently uses only Salesforce as an identity provider. Refer to https://help.salesforce.com/s/articleView?id=xcloud.sso_sfdc_idp_saml_parent.htm&type=5[Salesforce as a SAML Identity Provider^] to enable Salesforce as an identity provider.
...
For more information, see https://help.salesforce.com/s/articleView?id=001123185&type=1[The Different Types of Object Stores Explained].
...
| Query Time Buffer (Seconds) a| Number |  Number of seconds to subtract from the current time when querying for changes. This prevents data loss caused by records committed after the query timestamp but before the next poll. The value must be between `0` and `120` seconds. |  `0` |
| Primary Node Only a| Boolean |  Determines whether to execute this source on only the primary node when running Mule instances in a cluster. This parameter doesn't have any impact on non-cluster mode with multiple workers. |  |
...
| The size (in bytes) of the event queue (DEPRECATED) a| Number | In previous versions, this field was used to specify the size of the event queue which can lead to memory issues. Now, the messages are streamed and no buffering is needed. | |
| Primary Node Only a| Boolean |  Determines whether to execute this source on only the primary node when running Mule instances in a cluster. This parameter doesn't have any impact on non-cluster mode with multiple workers. |  `true` |
| Config Ref a| ConfigurationProvider |  Name of the configuration to use to execute this component. |  | x
...
[NOTE]
If you set `LocalDate` for a field in the payload, the input time is automatically converted to `GMT` or the server’s time zone based on the Gregorian calendar. This conversion impacts date calculations or comparisons in Salesforce Connector operations, as `LocalDate` is adjusted to align with the server’s time zone or defaults to GMT.
...
* <<repeatable-file-store-stream>>
* <<non-repeatable-stream>> | Configures how Mule processes streams. Repeatable streams are the default behavior. |  |
| Headers a| Object | Map of HTTP headers allowed by Salesforce SOAP API to pass with the request. |  |
| Read Timeout a| Number |  How long the consumer waits for a response before it times out. |  |
| Time unit a| Enumeration, one of:
...
| Stream a| Any |  Stream containing the data. The stream can have a CSV, XML, ZIP_CSV, or ZIP_XML format. |  `#[payload]` |
| Config Ref a| ConfigurationProvider |  Name of the configuration to use to execute this component. |  | x
| Headers a| Object | Map of HTTP headers allowed by Salesforce SOAP API to pass with the request. |  |
| Read Timeout a| Number |  How long the consumer waits for a response before it times out. |  |
| Time unit a| Enumeration, one of:
...
If you input the value in literal mode, an example value is:
...
If you input the value in expression mode, then the input can be dynamic using `output application/json` or `output application/java`, and can be transformed into the required input.
...
| Config Ref a| ConfigurationProvider |  Name of the configuration to use to execute this component. |  | x
| Headers a| Object | Salesforce headers. For information, see https://developer.salesforce.com/docs/atlas.en-us.api_asynch.meta/api_asynch/create_job.htm[Create a Job] in the Salesforce documentation. |  |
| Read Timeout a| Number |  How long the consumer waits for a response before it times out. |  |
| Time unit a| Enumeration, one of:
...
| Config Ref a| ConfigurationProvider |  Name of the configuration to use to execute this component. |  | x
| Headers a| Object | Salesforce headers. For information, see https://developer.salesforce.com/docs/atlas.en-us.api_asynch.meta/api_asynch/query_create_job.htm[Create a Query Job] in the Salesforce documentation. |  |
| Read Timeout a| Number |  How long the consumer waits for a response before it times out. |  |
| Time unit a| Enumeration, one of:
...
| Name | Type | Description | Default Value | Required
| Configuration | String | Name of the configuration to use. | | x
| Type a| String |  Object. The specified value must be a valid object for your organization. For a complete list of objects, see https://developer.salesforce.com/docs/atlas.en-us.object_reference.meta/object_reference/sforce_api_objects_list.htm[Standard Objects]. |  | x
| Config Ref a| ConfigurationProvider |  Name of the configuration to use to execute this component. |  | x
| Headers a| Object | Map of HTTP headers allowed by Salesforce SOAP API to pass with the request. |  |
| Read Timeout a| Number |  How long the consumer waits for a response before it times out. |  |
| Time unit a| Enumeration, one of:
...
| Configuration | String | Name of the configuration to use. | | x
| Id a| String |  ID of the query job. |  | x
| Max Records Per Page a| Number a|  Number of records that are on each page retrieved from the API. This number influences the number of API requests and the memory used by the connector to handle the results. 

Salesforce Connector automatically handles pagination and retrieves all pages before the next processing job starts.
|  `2000` |
| Config Ref a| ConfigurationProvider |  Name of the configuration to use to execute this component. |  | x
...
| Apex Class Method Name a| String | Apex class method name. The method name format is:

`ApexMethodName&#94;RestResourceURL&#94;HttpMethod&#94;OutputType&#94;InputParameterName1=ParameterType1,InputParameterName2=ParameterType2`

For example: 

`getAccountInfo&#94;/accountInfo/*&#94;HttpGet&#94;void`

[NOTE]
Special characters aren't allowed in `ApexMethodName`.|  | x
| Read Timeout a| Number |  How long the consumer waits for a response before it times out. |  |
| Time unit a| Enumeration, one of:
...
| Field Type a| Enumeration, one of:
...
Configures the redelivery policy message identifier type.

[%header,cols="20s,25a,30a,15a,10a"]
|===
| Field | Type | Description | Default Value | Required
| Use Secure Hash a| Boolean | Whether to use a secure hash algorithm to identify a redelivered message. |  | 
| Id Expression a| String | Defines one or more expressions to use to determine when a message has been redelivered. This property may only be set if the *Use Secure Hash* field is false. |  | 
|===
