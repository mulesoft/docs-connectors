...
This document assumes that you are familiar with Mule, the xref:5.x@studio::index.adoc#the-visual-editor[Anypoint Studio interface], xref:3.6@mule-runtime::global-elements.adoc[Global Elements], xref:3.6@mule-runtime::datasense.adoc[DataSense], and the xref:6.x@studio::datamapper-user-guide-and-reference.adoc[DataMapper transformer]. Further, it is assumed that you are familiar with Salesforce and have a http://developer.force.com/[developerforce] account. Learn more about xref:3.6@mule-runtime::flows-and-subflows.adoc[flows and subflows] to gain more insight into the design of this example.
...
Next, a xref:6.x@studio::datamapper-user-guide-and-reference.adoc[DataMapper transformer] maps the Java object to a CSV file which the flow then transforms to a string. The flow passes the message, now a string of duplicated email addresses back to the mainFlow. The mainFlow proceeds to send the message to the *outboundFlow* (configured in a separate mflow file in the application) to email the results to a pre-defined address.
