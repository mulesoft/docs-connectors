= Salesforce Connector 10.9 Reference 
...
|===
| Name | Type | Description | Default Value | Required
| Connection Timeout a| Number |  Specifies the amount of time that the consumer attempts to establish a connection before it times out. Works in tandem with Time unit. Values less than one millisecond are converted to 0. A value of 0 means wait indefinitely. |  0 |
| Time unit a| Enumeration, one of:
...
| Assignment Rule Id a| String | The ID of a specific assignment rule to run for the case or lead. |  |
| Concurrency Mode a| Enumeration, one of:
