...
:page-aliases: 3.8@mule-runtime::salesforce-connector.adoc
...
* *Consumer key and Secret* - If you are using the Salesforce connector to access an *OAuth API*, you also need a consumer key and secret. Refer to the detailed documentation on how to xref:3.8@mule-runtime::using-a-connector-to-access-an-oauth-api.adoc[use the Salesforce connector to access an OAuth API].
...
See the xref:release-notes::connector/salesforce-connector-release-notes.adoc[Salesforce Connector Release Notes] for the latest compatibility information.

*Note:*

* Starting with v.7.0.0, the Salesforce connector is licensed commercially with Anypoint Platform as are other xref:3.8@mule-runtime::anypoint-connectors.adoc#connector-categories[_Select_] connectors.  Prior versions will remain freely available to the community.

== Installing and Configuring in Anypoint Studio
...
[NOTE]
====
You can also use a Salesforce connector in a xref:3.8@mule-runtime::batch-processing.adoc[batch process] to push data to Salesforce in batches (see image below, bottom).
====
...
Use the connector in conjunction with a xref:3.8@mule-runtime::poll-reference.adoc[Poll Scope] in a flow to pull data from Salesforce into your application. To use the connector in this capacity, you must first place a *Poll scope* element at the beginning of your flow, then place a Salesforce connector within the poll scope (see image directly below).
...
+
[NOTE]
You can also use a poll-wrapped Salesforce connector at the beginning of a xref:3.8@mule-runtime::batch-processing.adoc[Batch Process] to extract data from Salesforce, then batch process the content in Mule.
...
.. Learn more about using the xref:3.8@mule-runtime::poll-reference.adoc[Poll Scope] to regularly poll Salesforce for new data for your application to process.
...
+
. After you create the flows, right-click the project name in the image:package-explorer-studio.png[] and click *Run As* > *Mule Application*.
...
* Read more about the xref:3.8@mule-runtime::poll-reference.adoc[Poll Scope].
