...
| Headers a| Object |  |  |
| Read Timeout a| Number |  +++Specifies the amount of time, in the unit defined in #readTimeoutUnit, that the consumer will wait for a response before it times out.+++ |  |
| Time unit a| Enumeration, one of:
