...
This document assumes that you are familiar with Mule, the xref:5.x@studio::index.adoc[Anypoint Studio interface], xref:3.7@mule-runtime::global-elements.adoc[Global Elements], xref:5.x@studio::datasense.adoc[DataSense], and the xref:5.x@studio::datamapper-user-guide-and-reference.adoc[DataMapper transformer]. Further, it is assumed that you are familiar with Salesforce and have a http://developer.force.com/[developerforce] account. Learn more about xref:3.7@mule-runtime::flows-and-subflows.adoc[flows and subflows] to gain more insight into the design of this example.
