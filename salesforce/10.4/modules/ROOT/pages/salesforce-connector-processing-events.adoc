...
You can use Anypoint Connector for Salesforce (Salesforce Connector) as an inbound connector to stream data from Salesforce into your application. To use the connector in this capacity, use one of these sources:
...
To enable the *Resume from the Last Replay Id* functionality for the Replay Topic Listener and Replay Channel Listener source operations, the connector must store some information about the events that were successfully processed by the flow in which the source operation is used. The connector uses Object Store to store that information. For more details about how Object Store is used, see <<objectstoreusage, Object Store Usage>>.

[NOTE]
When deploying apps on a distributed environment (such as a runtime Cluster), the source must run on the primary node. If the same source runs on multiple nodes, it causes duplicate event consumption.

Because the Object Store is used to store the necessary data for the *Resume from the Last Replay Id* feature, the behaviour in different environments will be as follows:
* When a Mule application is running locally the conenctor creates a persistent store in the file system. This way, if the application it is stopped and restarted, or if there was a connection issue that triggered reconnection, the data that was saved in the object store it is not lost and the connector can continue processing messages from where it stopped;
* When running the application in a cluster(either with the nodes on the same machine, or with the nodes on different machines - check this https://docs.mulesoft.com/mule-runtime/4.3/creating-and-managing-a-cluster-manually[page] and this https://docs.mulesoft.com/runtime-manager/cluster-create[example] of how you can configure a cluster), the the object store used is a distributed in-memory one. This way, if the primary node(the one consuming the messages) gets shut down, the node that takes over will not consume the same messages again, but will continue from where the previous node stopped processing. If the cluster it is closed entirely, the data from the object store is lost, so when restarted the messages might be reprocessed depending on the configuration at the source level. If you need data to be persisted so that the application can continue processing events when restated, the object store can be configured to use a database as the persistence layer(see https://docs.mulesoft.com/mule-runtime/4.3/creating-and-managing-a-cluster-manually[this example]).
* When running the application in Cloudhub, if you are using multiple workers, all of them are going to receive the events and process them.
