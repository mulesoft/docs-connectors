...
To enable the *Resume from the Last Replay Id* functionality for the *Replay Topic Listener* and *Replay Channel Listener* source operations, the connector must store some information about the events that were successfully processed by the flow in which the source operation is used. The connector uses Object Store to store that information. For more information about how Object Store is used, see xref:salesforce-connector-config-topics.adoc#objectstoreusage[Object Store Usage].
