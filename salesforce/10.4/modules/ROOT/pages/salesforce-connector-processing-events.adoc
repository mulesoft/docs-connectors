...
[NOTE]
When deploying apps on a distributed environment (such as CloudHub), the source must run on the primary node. If the same source runs on multiple nodes, it causes duplicate event consumption.
