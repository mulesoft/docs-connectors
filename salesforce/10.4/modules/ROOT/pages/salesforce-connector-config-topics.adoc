...
In order to provide the *Resume from Last Replay Id* feature, the connector is using a persistent object store to keep different details regarding the processed messages. This feature has been enhanced in 10.x version to reduce message loss probability and to avoid processing duplicate messages. To find out more details regarding how the Object Store is used check out the xref:salesforce-connector-config-topics.adoc#objectstoreusage[Object Store Usage] section.
