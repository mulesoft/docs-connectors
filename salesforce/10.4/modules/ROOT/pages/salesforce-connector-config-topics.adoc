= Salesforce Additional Configuration 10.4 - Mule 4
:page-aliases: connectors::salesforce/salesforce-connector-config-topics.adoc

In addition to the basic configurations, you can configure Anypoint Connector for Salesforce (Salesforce Connector) to take advantage of more of the connector's features. 

[[eventshandling]]
== Processing Events with Salesforce Connector

You can use Salesforce Connector as an inbound connector to stream data from Salesforce into your application. To use the connector in this capacity, use one of these sources:
...
You can subscribe to a topic that was not previously published in Salesforce. However, after the topic is published, you do not receive notifications regarding that topic unless you resubscribe to it.
...
* Object Store for on-premises deployments of Mule apps has no transaction limits.
...
Although you can't change the `contentType` value for a bulk upsert, you can use the Create job operation to set the content type to either CSV (or zipped CSV if you're close to the character limit) and then follow up with the Create batch operation.
