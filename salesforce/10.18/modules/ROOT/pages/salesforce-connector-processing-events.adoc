...
Learn how the priority in replay sources field setup is handled with different configurations:
...
+
If there are no failed event IDs stored, then processing starts with the highest known replay ID. If there is nothing saved in the ObjectStore, then the *Replay option* field is used.
