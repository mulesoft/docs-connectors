...
Fields of replay sources behave differently depending on priority in setup.
...
+
If enabled, this field overwrites anything that is selected in the *Replay option* field and anything that is entered in the *Replay id* field. 
+
The *Replay failed events if any or resume from last replay id* field checks the ObjectStore for failed event IDs or last known successfully processed event IDs. If there are failed events, processing starts with the lowest failed event, so it will grab all events from `lowestFailedEventId` till now. For example, if `FailedEventIdInObjectStore` is 10 and `LastSuccessfullyProcessedEventIdInObjectStore` is 15, processing starts with 10 and the StreamingClient receives all the events up until 15 and consumes quota. However, these events are not processed again by the flow and are filtered by the connector.
+
If there are no failed event IDs stored, then processing starts with the highest known replay ID. If there is nothing saved in ObjectStore, then the *Replay option* field is used.
...
+
Subscribes with -1. The user receives the events that are created after the connector subscribes.
...
+
Checks the ObjectStore for the highest received event ID. It doesn't matter if the event is processed successfully or not. This option avoids quota consumption compared to the *Replay failed events if any or resume from last replay id* field that starts the source with a failed event ID that is lower than the highest processed event ID.

* *Cache events in memory*
+
If enabled, when the application starts, the connector subscribes from a replay ID. The Salesforce API pushes all available events starting with the ID that is used to subscribe, thus consuming quota.
+
If there are many available events and the Mule app processes them slowly, it might take a while to process all of them.
+
...
+
If there is a connection issue or the token expires and the connector must resubscribe, the events would have been lost without this configuration and quota would have already been consumed.
