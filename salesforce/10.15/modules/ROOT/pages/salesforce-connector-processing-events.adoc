= Process Events with Salesforce Connector 10.15
...
* The free version of Object Store for CloudHub deployments of Mule apps has a limit of 10 transactions per second(TPS), however other Mulesoft Connectors using the Object Store internally have a limit of 100 TPS.
...
The connector has an in memory structure where it stores the replay ids of the events(which events were processed successfully, which events have failed to be processed).
This structure is persisted into the Object Store using the username in the connector configuration to build a specific key. There is also a redundancy mechanism to avoid processing duplicate events, the connector also stores the same data under a backup key.
Each persist operation consists of deleting the existing key and trying to store the new value(if storing the new value fails, the connector will retry), thus consuming 2 or more TPS per key.
