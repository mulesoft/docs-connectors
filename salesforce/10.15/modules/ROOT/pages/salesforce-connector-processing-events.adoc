= Processing Events with Salesforce Connector 10.15
...
This in-memory structure is persisted into the Object Store using the username in the connector configuration to build a specific key. There is also a redundancy mechanism to avoid processing duplicate events by storing this data under a backup key.
