...
[NOTE]
---
In the same mule application there should not be two or more flows using a source which listens to events on the same channel or topic. If you have an use case that requires processing of the same events in two or more different ways consider implementing this logic in a single flow.
