...
[NOTE]
---
We do not recommend having two or more flows using a source which listens to events on the same channel or topic because the events will be consumed twice from your user quota. If you have an use case that requires processing of the same events in two or more different ways consider implementing this logic in a single flow.

Having multiple sources listening to the same events and having the *Resume form Last Replay ID* option enabled will lead to data corruption and potentially losing events.
---
