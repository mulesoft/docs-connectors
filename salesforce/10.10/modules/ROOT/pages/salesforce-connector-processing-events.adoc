...
When a Mule app starts for the first time, the connector creates an object store that saves data related to messages that processed successfully or that failed:

* For each message that processes successfully, the information stored consists of a replay ID associated with the message.
* For each message that fails, the information stored consists of a number that represents the lowest replay ID for which the processing failed.
...
NOTE: If you have multiple workers deployed in CloudHub, all Mule instances process each message when using the *Replay Topic* operations.
...
To enable the *Resume from the Last Replay Id* functionality for the *Replay Topic Listener* and *Replay Channel Listener* sources, the connector must store some information about the events that were successfully processed by the flow in which the source is used. The connector uses Object Store to store that information.
...
** If the primary node (the one consuming the messages) shuts down, the node that takes over for the primary node does not reconsume the same messages. Instead, the node continues from where the previous node stopped processing the messages.
...
NOTE: Having multiple sources listening to the same events and having the *Resume from the Last Replay Id* option enabled can lead to data corruption and potential loss of events.
