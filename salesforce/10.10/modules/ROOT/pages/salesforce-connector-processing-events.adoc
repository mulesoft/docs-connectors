...
Before you can receive events for Salesforce changes associated with a topic, create a topic using Salesforce Connector, if the topic doesn't already exist.
...
To enable the *Resume from the Last Replay Id* functionality for the *Replay Topic Listener* and *Replay Channel Listener* sources, the connector must store information about the events that were successfully processed by the flow in which the source is used. The connector uses Object Store *(Reviewers, is this always the Mule object store or can it be another object store?)* to store that information.
...
A streaming channel can replay notifications. The *Replay channel listener* input source acts as an inbound endpoint, for example:
...
To support the *Resume from Last Replay ID* feature, the connector uses a persistent object store to keep different details regarding the processed messages. This feature reduce messages loss probability and avoids processing duplicate messages. For more details about how Object Store is used, check the <<objectstoreusage,Object Store Usage>> section.
