...
You can configure a redelivery policy for sources by setting the number of redelivery attempts to try after an initial failure. You can specify an object store for this policy. If you don't specify an object store, Mule creates a non-persistent object store.
...
+
. In the *Apex* tab, optionally configure Apex settings, as described in .
...
| Apex Class Names | List of Apex class names to use for limiting the set of classes you fetch, along with the methods they expose. This setting can accelerate the fetch process if you do not need to fetch many classes. Valid values:
...
|===
+
The *Fetch All Apex SOAP Metadata* and *Fetch All Apex REST Metadata* check boxes take precedence over the *Apex Class Names* check box. If either of the first two check boxes are selected, they fetch all of the Apex SOAP metadata or Apex REST metadata, regardless of your selection in the Apex Class Names section.
