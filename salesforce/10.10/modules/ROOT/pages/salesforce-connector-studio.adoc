...
[NOTE]
The connector is using Bouncy Castle cryptographic library to load the certificate from the keystore and sign the payload for the authentication requests.
Bouncy Castle library presents the following vulnerability: https://nvd.nist.gov/vuln/detail/CVE-2018-5382[CVE-2018-5382] which is related to the BKS-V1 keystore file type.
By default the Salesforce Connector is using JKS or PKCS12 keystores. We advise against using BKS-V1 keystore files as per https://www.kb.cert.org/vuls/id/306792[Bouncy Castle proposed solution].
