...
+
*Kim. Ideally the next two paragraphs would be in a section called Authentication, but that would make the current H3's for the authentication types into H4s. I would still like to do that unless you have a better idea. I also am having trouble getting these paragraphs to format correctly within the list.* 
...
You can <<mutual-tls,configure mutual TLS>> for all authentication methods.
