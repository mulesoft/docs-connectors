...
image::salesforce-outbound.png[Flow for creating a new Salesforce account]
* Query the result stream and write the query result to a file
+
. Flow for querying the new account and writing the result to a file
...
Add the first *Transform Message* component to input field values in Java format to the Salesforce Connector *Create* operation:
...
Add the *Create* operation to create a new Salesforce account:
...
Add the second *Transform Message* component to transform the field values JSON format, which the Anypoint Connector for File (File Connector) endpoint expects.
