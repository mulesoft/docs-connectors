...
| Throw Exception When Organization Daily Limit Exceeded a| Boolean |  +++Parameter used to specify if the connector should throw an exception when the appliaction is deployed the organization daily limit for events processing was exceeded+++ |  +++false+++ |
