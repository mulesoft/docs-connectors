...
* `FROM_LAST_REPLAY_ID`
+
Subscriber uses the highest replay ID stored in the object store regardless of whether it's processed successfully or not. If you deploy the Mule app to CloudHub, then use theObject Store V2 option to retain the last processed ReplayId.
