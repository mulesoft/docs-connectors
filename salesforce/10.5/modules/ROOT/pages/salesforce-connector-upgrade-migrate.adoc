= Upgrade Salesforce Connector to 10.5 - Mule 4
:page-aliases: connectors::salesforce/salesforce-connector-100-upgrade-migrate.adoc

Upgrade Anypoint Connector for Salesforce (Salesforce Connector) from version 9.x to version 10.x.
...
* Create Job Bulk Api V2 and Create Query Job Bulk Api V2 operations now support the Sforce-Call-Options header to specify a default namespace.
* Create Job Bulk Api V2 now supports the HardDelete operation parameter.
* Batch Result Stream operation now supports specifying an output mime type based on the content type provided when creating the batch.

=== 10.4

This release contains the following changes:

* Retrieve Job Unprocessed Results BulkV2 operation added
* Retrieve Job Failed Results BulkV2 and Retrieve Job Successful Results BulkV2 operations now return the original fields of the records
* Logging improvements over all operations and connection types
* Subscribe Streaming Channel and Replay Streaming Channel sources now display all types of events available in the configured organization:
** Generic Events
** Platform Events
...
NOTE: The ReplayStreamingChannel and ReplayTopic operations have a functionality to *Resume from the last replay Id* which will not work when upgrading to the new connector version because of some changes in the underlying implementation. The first time when you run the migrated application it  will fetch the events based on the selected *Replay Option* and only on subsequent restarts it will make use of the *Resume from the last replay id* functionality and will continue as expected.
