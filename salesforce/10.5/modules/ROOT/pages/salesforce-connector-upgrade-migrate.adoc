...
* The Create Job Bulk Api V2 operation now supports the hardDelete value in the Operation.
...
NOTE: The Resume from the last Replay Id option for the Replay Channel Listener and Replay Topic Listener operations does not work the first time you upgrade to the new connector version because of changes in the underlying implementation. The first time you run the migrated app, it fetches events based on the value of the Replay Option field. On subsequent restarts, the app uses the Resume from the Last Replay Id functionality and continues as expected.
