= Process Events with Salesforce Connector 10.14
...
When deploying apps in a distributed environment such as a runtime cluster, the source must run on the primary node. Running the source on multiple nodes causes duplicate event consumption and Object Store data corruption for the data stored by the Salesforce Connector.
...
* Modify your app to filter out duplicate messages(this will still consume quota for each worker).
...
== Multiple Flows with Different Listeners

Having multiple sources listening with the *Resume from the Last Replay Id* option enabled, under certain conditions, may have performance implications.

To mitigate it, using different Salesforce Connector Configurations with different Salesforce usernames is suggested.

WARNING: Changing the username for a configuration will lead to the inability of the connector to load the last processed and failed replayId's. Processing will resume from the very first event available in Salesforce.

WARNING: We advise against using personal user accounts to access Salesforce
