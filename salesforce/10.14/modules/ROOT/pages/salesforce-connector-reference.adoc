...
| Cache Events In Memory a| Boolean | When processing flow is slow cache events in memory to reduce quota consumption |  false |
...
** FROM_LAST_REPLAY_ID: Subscriber uses the highest replay id stored in the Object Store(it does not matter if it was processed successfully or not).
...
| Replay failed events if any or resume from last replay id a| Boolean |  If true, when the source is started it will start with the failed events if any or from the last replay id that was processed successfully |  false |
...
| Name | Type | Description | Default Value | Required
| Configuration | String | Name of the configuration to use | | x
| List of Merge Records a| Array of <<MergeRecord>> |  A list of MergeRecord. Each merge record containing the primaryRecord, the recordToMergeIds and an optional additionalInformationMap. |  #[payload] |
...
| Record To Merge Ids a| Array of String | The other record or records to be merged into the primary record. Minimum of one, maximum of two. |  | x
