= Salesforce Connector 10.14 Reference - Mule 4
...
| Expiration Policy a| <<ExpirationPolicy>> |  Configures the minimum amount of time that a dynamic configuration instance can remain idle before Mule considers it eligible for expiration. This does not mean that the instance expires at the exact moment that it becomes eligible.  |  |
...
| Login Request Timeout a| Number | Amount of time, in the unit defined in the Login Request Time unit field, for which the consumer waits for the login request to be received before the request times out. Values less than one millisecond are converted to 0. | |
...
** MINUTES
** HOURS
** DAYS |  Time unit for the Login Request Timeout field value |  |
...
| Authorization URL a| String |  Web service URL responsible for user authentication. The URL for the endpoint that is configured to handle SOAP authentication requests. Defaults to the URL containing the latest api version supported by the connector. For example: `+https://login.salesforce.com/services/Soap/u/48.0+`|  |
...
| Cache Events In Memory a| Boolean | When processing flow is slow, you can cache events in memory to reduce quota consumption. |  false |
...
** FROM_LAST_REPLAY_ID: Subscriber uses the highest replay ID stored in the Object Store regardless of whether it was processed successfully or not.
...
| Replay Failed Events If Any or Resume from Last Replay Id a| Boolean |  If true, when the source is started it starts to consume data with any failed events and then from the last replay ID that was processed successfully. |  false |
| The size (in bytes) of the event queue (deprecated) a| Number | Default value is 26 MB. If the messages are not consumed fast enough and the buffer fills up, the operation returns IllegalStateException. Maximum value for the event size is 2147483647 |  |
...
** hardDelete
** query
** queryAll |  The OperationEnum that will be executed by the job. |  | x
...
| Assignment Rule Id a| String | The ID of an assignment rule to run for a case or lead. The assignment rule can be active or inactive.  |  |
...
| Master Record a| Object |  Must provide the ID of the object that other records will be merged into. Optionally, provide the fields to update and their values. |  | x
| Headers a| Object | Map of HTTP headers in the message. |  |
| Read Timeout a| Number |  Specifies the amount of time, in the unit defined in Time unit, for which the consumer waits for a response before it times out. |  |
| Time unit a| Enumeration, one of:
...
| Name | Type | Description | Default Value | Required
| Configuration | String | Name of the configuration to use | | x
| List of Merge Records a| Array of <<MergeRecord>> |  A list of merge records. Each merge record contains the primaryRecord, the recordToMergeIds, and an optional additionalInformationMap. |  #[payload] |
...
Returns a list of result IDs for a specific batch. If the batch requires retries, there will be more than one <result> element in the output. Use this operation with <<queryResultStreamById>> to obtain the results of the query.
...
** ZIP_XML
** ZIP_CSV
** ZIP_JSON |  Content type used at job creation. |ContentType.XML  |
...
| Name | Type | Description | Default Value | Required
| Configuration | String | Name of the configuration to use. | | x
| Batch info a| <<BatchInfo>> |  The BatchInfo that returned the result ID. |  `#[payload]` | x
| Result ID | String | The result ID obtained with <<queryResultList>>. | | x
| Output Mime Type a| String | MIME type of the payload output of this operation.  |  |
| Output Encoding a| String | Encoding of the payload output of this operation. |  |
| Streaming Strategy a| * <<repeatable-in-memory-stream>>
* <<repeatable-file-store-stream>>
...
| Name | Type | Description | Default Value | Required
| Configuration | String | Name of the configuration to use. | | x
| Package Names a| Array of String |  A list of package names to retrieve. If you are retrieving only unpackaged components, do not specify a name. You can retrieve packaged and unpackaged components in the same retrieval operation. |  |
| Specific Files a| Array of String |  A list of file names to retrieve. If a value is specified for this property, packageNames must be set to null. |  |
...
| Assignment Rule Id a| String | The ID of an assignment rule for a case or lead. The assignment rule can be active or inactive. |  |
| Concurrency Mode a| Enumeration, one of:
