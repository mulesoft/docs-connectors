...
:page-aliases: connectors::salesforce/salesforce-connector-studio.adoc
...
[[keepalive]]
== Keep Alive feature (Deprecated - was removed in Salesforce 10.x)

Through this mechanism a scheduler can be set to make one simple API request (get server timestamp), with a specified time frame, in order to keep the session alive.
To activate this feature, the Advanced reconnection params needs to be configured. This group contains two parameters:

* Maintain session request frequency (default value: 60 seconds)
* Initial delay (default value: 480 seconds)

If there are no value passed by the user, the default ones are used.
These parameters can be configurable because every salesforce instance can specify a session timeout value which can vary from 15 minutes to 24 hours.

Regarding that, for example, if the session timeout on the instance is set to 15 minutes and the application does not make any request to salesforce for 15 minutes or more, the session will expire and if the reconnection is not enabled, further API requests will fail. In order to keep the session alive, the user can configure Advanced reconnection params like this:

.	Initial delay: 13 minutes(a value below 15). This can be set to be higher that session timeout value if it is known that the application will make requests to salesforce and will not let the session to expire, but it is recommended to keep it below that value.
.	Maintain session request frequency: somewhere between 8 and 15 minutes. This can be set to be lower than 8 minutes, but it will consume more than one API request in 15 minutes. It is recommended to keep this value between the session timeout value and the half of it.

This feature can be used to maintain a session valid for up to 24 hours without triggering the reconnection.
