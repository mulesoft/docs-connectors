...
For the Mule 4 Salesforce Connector, you have the option to keep the session valid until it expires by setting the *Disable session invalidation* field to `true` in the *Global Element Properties > General > Advanced* tab, or by setting `disableSessionInvalidation="true"` in the XML flow.
...
To activate this feature, you must configure the reconnection parameters in the *Advanced* tab, which include:
...
In order to keep the session alive, you can configure the *Advanced* reconnection parameters like this:

.	Initial delay +
It is best to set this value at lower than 15, for example, 13 minutes. +
This value can also be set at a higher value than the session timeout value if it is known that the application will make requests to Salesforce and will not let the session expire, but it is best to set it for a lower value.
.	Maintain session request frequency +
Set this value to be between 8 and 15 minutes. +
This value can be lower than 8 minutes, but then it will consume more than one API request in 15 minutes. +
It is best to keep this value halfway between the session timeout value and request frequency.

You can use this feature to maintain a session's validity for up to 24 hours without triggering the reconnection.
