...
This example uses two custom Salesforce object types: `Component__c` and `Plane__c`.
...
This means that the `Component__c` that you are upserting has the entity `Plane__c` with the name `Cobra` as its parent. The `Plane__c` entity must already exist in your environment.
