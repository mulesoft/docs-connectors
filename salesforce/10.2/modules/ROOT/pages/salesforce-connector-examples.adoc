= Salesforce Examples v10.2 - Mule 4
...
Creating or altering objects with relationships is challenging. This section shows how to perform an upsert for an object using Salesforce Connector.

Create a structure in Salesforce for this relationship.

This example assumes two custom types: `Component__c` and `Plane__c`.

Component__c must hold a relationship to a Plane__c(`Plane__r` indicates the name of the relationship between the two objects).
When upserting a `Component__c`, the POJO sent as input to Salesforce Connector looks like this:
...
Apart of the fields of the Component__c object that you want to create, you have to specify the relationship with the parent Plane__c object through the Plane__r field.
The value of this field must be an object with two fields.

A field named type with the referenced object type name as value. In this case a custom `Plane__c` type.

A field with value and name appropriate to identify the right instance of `Plane__c` to reference. In this case it is the one with a value of `Cobra` for the field named `Name`.
This means that the Component__c that you are currently upserting will have as parent the entity Plane__c with the name `Cobra`(this Plane__c entity must already exist in your environment).
