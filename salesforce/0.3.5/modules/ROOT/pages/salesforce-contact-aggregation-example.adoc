...
Next, a xref:5.x@studio::datamapper-user-guide-and-reference.adoc[DataMapper transformer] maps the Java object to a CSV file which the flow then transforms to a string. The flow passes the message, now a string of duplicated email addresses back to the mainFlow. The mainFlow proceeds to send the message to the *outboundFlow* (configured in a separate mflow file in the application) to email the results to a pre-defined address.
...
* Access the introductory material for the xref:0.3.7@index.adoc[Salesforce Connector].
