= Salesforce Connector 11.0 Reference - Mule 4
...
| TLS configuration a| <<Tls>> |  Configures TLS. If using the HTTPS protocol, you must configure TLS.   |  |
| Reconnection a| <<Reconnection>> |  Configures a reconnection strategy to use when a connector operation fails to connect to an external server. |  |
| Client ID a| String |  The OAuth public client ID as registered with the service provider. |  |
| Client Secret a| String |  The OAuth client secret that corresponds to the client ID. |  |
