...
| Service directory a| String |  Common HTTP directory for all unlisted services. This parameter allows user to specify only service names in unlisted services list instead of the service URLs and let the connector construct the service URL using the following format: {base URL parameter}/{this parameter}/{item from unlisted services list} |   |
| Unlisted services a| Array of String |  List of services which will be added to the list of discovered services. Each item in this list must be either valid service URL, or it must be possible to construct a valid service URL with the following format: {base url parameter}/{service directory parameter}/{item from this list}. |  |
