= Using Anypoint Studio to Configure SAP S/4HANA OData Connector 2.8 - Mule 4
...
+
  * *OAuth 2 Application Connection Provider* +
  In OAuth 2.0 authentication, a TLS context that comes with a truststore can't be added because the SDK does not expose the TLS context afterward. For OAuth 2.0 authentication, enter the following required information:
  ** *Service base URL* +
