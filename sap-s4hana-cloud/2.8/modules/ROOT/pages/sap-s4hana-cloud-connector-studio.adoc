...
+
  * *OAuth 2 Application Connection Provider* +
  In OAuth 2.0 authentication, a TLS context that comes with a truststore can't be added because the SDK doesn't expose the TLS context afterward. For OAuth 2.0 authentication, enter this required information:
  ** *Service base URL* +
