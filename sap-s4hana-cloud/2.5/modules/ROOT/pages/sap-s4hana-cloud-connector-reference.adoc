= SAP S/4HANA OData Connector 2.5 Reference 
...
| Unlisted services directory a| String |  Common HTTP directory for all unlisted services. This field enables users to specify service names instead of service URLs in the unlisted services list. It also enables the connector to construct the service URL by using the following format: `{base URL parameter}/{this parameter}/{item from unlisted services list}`. |   |
