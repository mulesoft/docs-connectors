...
| Service Name a| String | Service name, which is usually the title in the service catalog. If the URL contains a version suffix, the service name is the part of the URL after the last `/`. For example, if the URL is `{base URL parameter}/sap/opu/odata/sap/SERVICE_NAME;v=0002`, the service name is `SERVICE_NAME;v=0002`. |  | x
