...
* The *Batch update entity* operation now uses the correct syntax for the OData v4 URI for fields that have the EDM type as GUID. 
* The connector now throws the correct error types:

** When there is an invalid filter provided in the operation, the connector now throws `S4HANA:INVALID_FILTER` instead of `S4HANA:INVALID_ENTITY`. 
** When there are authentication issues (such as an incorrect username, password, or URL), the connector now throws `S4HANA:UNAUTHORIZED` instead of `S4HANA:NO_SUCH_SERVICE`. 

* The connector now proactively re-triggers service discovery before each operation if the service list is empty. 

=== 2.8

* SAP S/4HANA OData Connector now provides support for old pagination logic (version 2.6.2 and earlier) in the *Query* operation.
To support this change, a Legacy Pagination checkbox is added in the Advanced tab of the *Query* operation.

=== 2.7

* SAP S/4HANA OData Connector now provides support for OData API v4 in addition to OData API v2. +
To support OData API v4, the connector now includes the new *OData v4 service discovery relative path* field in the connection types. For information about the *OData v4 service discovery relative path* field, see xref:sap-s4hana-cloud-connector-reference.adoc[SAP S/4HANA OData Connector Reference].
