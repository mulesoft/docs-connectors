= SAP S/4HANA OData Connector 2.9 Reference - Mule 4
...
| Cache Entry TTL a| Number |  The time to live. The time a cache entry is kept in the cache before being evicted. A value of 0 disables the cache. |  0 |
| Cache Entry TTL Time Unit a| Enumeration, one of:
...
** MINUTES
** HOURS
** DAYS |  Time unit for the *Cache Entry TTL* field. |  MINUTES |
| EDM Expiration Interval a| Number |  The interval to check for expired EDM objects in the ObjectStore. |  30 |
| EDM Expiration Interval Time Unit a| Enumeration, one of:
...
** MINUTES
** HOURS
** DAYS |  Time unit for the *EDM Expiration Interval* field. |  MINUTES |
...
| OData v4 service discovery relative path a| String |  Relative path from the base URL to the endpoint used for automatic OData v4 service discovery. |  `sap/opu/odata4/iwfnd/config/default/iwfnd/catalog/0002/ServiceGroups` |
| Scoped service discovery relative path a| String | Relative path from the base URL to the endpoint used for automatic services discovery (with custom namespaces) on cloud instances. |  `sap/opu/odata/IWFND/CATALOGSERVICE;v=0002/ScopedServiceCollection` | 
| Unlisted services directory a| String |  Common HTTP directory for unlisted services and services declared in *Service Attribute* (if the service URL is not specified). This field enables users to specify service names instead of service URLs in the unlisted services list or enables users to not specify a service URL in *Services Attribute*. It also enables the connector to construct the service URL by using the following format: `{base URL parameter}/{this parameter}/{item from unlisted services list or service name from *Service Attribute*}`. |   |
...
| Services attributes a| Array of <<ServiceAttribute>> |  List of services that are added to the list of discovered services. With *Service attributes*, you can add new services or override existing services. |  | 
...
** DAYS |  Time unit for the *EDM Expiration Interval* field. |  MINUTES |
| Discovery Service Enabled a| Boolean |  Feature switch for automatic service discovery. |  `true` |
| Service discovery relative path a| String |  Relative path from the base URL to the endpoint which will be used for automatic services discovery. |  `sap/opu/odata/IWFND/CATALOGSERVICE;v=0002/ServiceCollection` | 
| OData v4 service discovery relative path a| String |  Relative path from the base URL to the endpoint used for automatic OData v4 service discovery. |  `sap/opu/odata4/iwfnd/config/default/iwfnd/catalog/0002/ServiceGroups` |
| Scoped service discovery relative path a| String | Relative path from the base URL to the endpoint used for automatic services discovery with custom namespaces on cloud instances. |  `sap/opu/odata/IWFND/CATALOGSERVICE;v=0002/ScopedServiceCollection` | 
| Unlisted services directory a| String |  Common HTTP directory for unlisted services and services declared in *Service Attribute* (if the service URL is not specified). This field enables users to specify service names instead of service URLs in the unlisted services list or enables users to not specify a service URL in *Services Attribute*. It also enables the connector to construct the service URL by using the following format: `{base URL parameter}/{this parameter}/{item from unlisted services list or service name from *Service Attribute*}`. |   |	
| Unlisted services a| Array of String |  List of services to add to the list of discovered services. Each item in this list must be a valid service URL, or it must be possible to construct a valid service URL with the following format: `{base url parameter}/{service directory parameter}/{item from this list}`. |  |
...
* <<BatchQuery>>
* <<BatchUpdateEntity>>
* <<BatchUpsertCustomBusinessObject>>
* <<CreateChangeSet>>
* <<CreateEntity>>
* <<DeleteEntity>>
* <<ExecuteBatch>>
...
NOTE: Users cannot specify custom namespaces for custom business objects.
...
* S4HANA:NO_SUCH_ENTITY_TYPE
* S4HANA:NO_SUCH_SERVICE
* S4HANA:RESOURCE_NOT_FOUND
* S4HANA:RETRY_EXHAUSTED

[[BatchUpsertCustomBusinessObject]]
== Batch Upsert Custom Business Object
`<s4hana:batch-upsert-custom-business-object>`

This operation supports upsert for custom business objects.
...
| Batch ID a| String |  Batch identifier. |  | x
| Change set ID a| String |  Change set identifier. |  | x
| Service a| String |  Type of service. This is defined from the services available in the SAP S/4HANA instance. |  | x
| Entity type a| String |  Name of the entity type to execute. |  | x
| Entity a| Object |  Custom business object entity of the entity type wrapped into a single object. |  `#[payload]` |
| Custom headers a| Object |  Custom headers that the request includes. The custom headers specified here overwrite the default headers specified in the configuration. |  |
| Custom query parameters a| Object |  Custom query parameters that the request includes. The custom query parameters specified here overwrite the default query parameters specified in the configuration. |  |
...
| Legacy Pagination a| Boolean | Enable users to use the legacy pagination of version 2.6.2 or earlier.|  false |
| Custom headers a| Object |  Custom headers to include in the request. The custom headers specified here will overwrite the default headers specified in the configuration. |  |
| Custom query parameters a| Object |  Custom query parameters that the request will include. The ones specified here will overwrite the default ones specified in the config. |  |
| Response timeout a| Number |  Response timeout for the request. The timeout specified here overwrites the default timeout that is specified in the configuration. |  |
| Response Timeout Unit a| Enumeration, one of:
...
Specify additional details about services with namespaces as a separate configuration.

[%header%autowidth.spread]
|===
| Field | Type | Description | Default Value | Required
| Service Name a| String | Service name, which is usually the title in the service catalog. If the URL contains a version suffix, the service name is the part of the URL after the last `/`. For example, if the URL is `{base URL parameter}/sap/opu/odata/sap/SERVICE_NAME;v=0002`, the service name is `SERVICE_NAME;v=0002`. The service name for OData API v4 is different. |  | x
| Service Url a| String | URL to the service. |  | 
| Namespace a| String | Custom namespace for the service, if applicable. |  | 
|===
