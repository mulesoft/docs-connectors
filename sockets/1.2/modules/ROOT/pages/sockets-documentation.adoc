...
The client reads data through this protocol and shuts down its reading connection at the output stream, this is also known as TPC Half-closed feature where the connection is closed in one direction and still open in the other direction.
Use this protocol with SSL sockets that use TLS 1.3 version and later, (Half Close feature was introduced in TLS 1.3).
