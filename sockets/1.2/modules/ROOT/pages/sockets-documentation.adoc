...
<<<<<<< HEAD
This protocol transfers data. The stream end is indicated by closing the client connection. Reading through this protocol is terminated by the client by shutting down the output stream, also known as TCP Half Close feature. Use this protocol with SSL sockets that use TLS 1.3 version and later, (Half Close feature was introduced in TLS 1.3).
...
Also, `EofProtocol` is not compatible with the pooling profile.
>>>>>>> 1e08225460579e64cc6378b4e0e6b2409807a26b
