...
Reading is terminated by the client by shutting down its output stream (also known as TCP Half Close feature). This protocol should not be used withs SSL sockets using
a TLS version prior to 1.3 (Half Close feature was introduced in TLS 1.3).
Also, PoolingProfile is incompatible with Eof protocol.
