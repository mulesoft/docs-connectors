...
[[common-throws]]
== Handling FTPS Session Reuse Errors
When using the FTPS connector, you may encounter connection issues during session reuse such as a `SocketTimeoutException` during data connection.
To resolve this, set the following JVM system property in your runtime configuration:

`-Dorg.apache.commons.net.ftp.ipAddressFromPasvResponse=true`
[NOTE]
====
This setting ensures the correct IP address from the PASV response is used during data transfer.
**However, be aware that enabling this flag may expose your application to potential security vulnerabilities** if the PASV response is spoofed or manipulated. Use with caution, especially in untrusted network environments.
====
