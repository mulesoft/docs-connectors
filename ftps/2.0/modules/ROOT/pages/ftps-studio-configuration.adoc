...
. Configure *Bouncy Castle* libraries in the *Required Libraries*.
...
To reuse sessions using FTPS Connector, you must configure `Bouncy Castle` libraries in the `Required Libraries` section while configuring the global element for the connector. You can choose to `add recommended libraries` or one of the three available options.

image::ftps-bc-providers.png[JVM argument to disable Extended Main Secret in Run Configurations window]

You can also deliver these libraries to the `JAVA_HOME/jre/lib/ext` folder and set up the Bouncy Castle security provider:
