...
You can also add these libraries to the `JAVA_HOME/jre/lib/ext` folder and set up the Bouncy Castle security provider:

. Open the `AnypointStudio.ini` file and identify where Java VM is stored. The path to VM is set up by the `-vm` command. Consider the following path `JAVA_VM_FOLDER`.
