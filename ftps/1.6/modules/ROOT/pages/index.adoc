...
To reuse sessions using FTPS you must disable the Extended Master Secret extension for compatibility (JDK-8192045), which by default is set to true. The extension protects users from vulnerabilities by using the same session on different groups of clients and servers, which contradicts the demand to use the same session for control and data connections. Extended Master Secret extension activated, enables session reuse only with an endpoint verification, which is implemented for HTTPS and LDAP only.
