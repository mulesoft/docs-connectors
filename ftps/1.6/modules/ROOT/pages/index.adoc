...
To reuse sessions using FTPS you must disable the Extended Master Secret extension for compatibility (JDK-8192045), which by default is set to true. The Extended Master Secret extension protects users from vulnerabilities based on the ability to use the same session on different sets of clients and servers. Therefore, it contradicts the demand to use the same session for control and data connections. If the Extended Master Secret extension is enabled, session reuse is allowed only with an endpoint verification, which is implemented for HTTPS and LDAP only.

To disable the Extended Master Secret extension:

If you run your Mule application in standalone, add the following argument to the JVM execution:

`./bin/mule -Djdk.tls.useExtendedMasterSecret=false`

If you run your Mule application in Anypoint Studio:
...
.JVM argument to disable Extended Master Secret in Anypoint Studio
image::ftps-jvm-configuration.png[alt=``]
