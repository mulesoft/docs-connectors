...
=== Email Connector Debug Logging for CloudHub 2 Mule Apps

Notice that for Mule apps deployed to CloudHub 2, in addition to the SMTP debug setting (`<email:property key="mail.debug" value="true" />`), you must set this property to ensure the SMTP debug logs are captured:

`forwardConsoleLogToAnypointMonitoring.enable=true`

This is necessary because SMTP debug logging uses the `java.util.logging` API to send logs to the console, which differs from the `log4j2` logging used for other purposes, such as HTTP verbose logging.
...
* https://datatracker.ietf.org/doc/html/rfc1064[RFC-1064 - IMAP2 - INTERACTIVE MAIL ACCESS PROTOCOL - VERSION 2]
* https://datatracker.ietf.org/doc/html/rfc1939[RFC-1939 - POP3 - Post Office Protocol - Version 3]
* https://datatracker.ietf.org/doc/html/rfc5321[RFC-5321 - SMTP - Simple Mail Transfer Protocol]
* https://datatracker.ietf.org/doc/html/rfc2045[RFC-2045 - MIME - Multipurpose Internet Mail Extensions Part One: Format of Internet Message Bodies]
