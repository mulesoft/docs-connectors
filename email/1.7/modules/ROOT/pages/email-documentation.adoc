= Email Connector Reference 1.7 

Support Category: https://www.salesforce.com/en-us/wp-content/uploads/sites/4/documents/legal/Agreements/versioning-back-support-policy.pdf[Select]
...
| Host a| String |  Host name of the mail server. | If the Host name value is defined with a property name and the value is empty, then it defaults to the localhost value. | x
...
| Mailbox Folder a| String | Mailbox folder where the emails are. |  INBOX |
| Count Filter a| Enumeration, one of:

** ALL
** DELETED
** NEW
** UNREAD |  Count only messages specified with this option. |  ALL |
...
| Pagination Offset a| Number |  Size of the pagination offset used by the paging provider implementation for fetching the emails from the IMAP server. |  0 |
...
Moves the email with ID `emailId` from the source folder to the target folder.
...
| Name | Type | Description | Default Value | Required
| Configuration | String | Name of the configuration to use. | | x
| Source Folder a| String |  Source mailbox folder where emails to move are located. |  INBOX |
| Target Folder a| String |  Target mailbox folder where emails are moved. |  | x
| Email ID a| Number |  Email ID number of the email to move. |  | x
| Create Target Folder a| Boolean |  Creates the target folder if the folder does not exist. |  false |
| Reconnection Strategy a| * <<reconnect>>
* <<reconnect-forever>> |  Configures a forever reconnection strategy by which the connector source or operation attempts to reconnect at a specified frequency for as long as the Mule app runs. |  |
|===
...
| Pagination Offset a| Number |  Size of the pagination offset used by the paging provider implementation for fetching the emails from the POP3 server. |  0 |
...
Note that Email Connector currently doesn't support multipart/signed (digitally signed) emails, and they are not processed by *On New Email* listeners.
