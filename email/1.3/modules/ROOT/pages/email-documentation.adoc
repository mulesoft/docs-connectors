...
| Enable Remote Search a| Boolean |  If search filters should be resolved on the server side the value is `true`, or on the client side the value is `false`. Default to `false` because some email servers are not fully compliant with `rfc-3501` search terms. Activating this feature diminishes traffic by reducing the amount of emails brought to the client side for processing. |  `false` |
