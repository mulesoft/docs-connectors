...
Email Connector can watermark emails over the IMAP protocol by filtering emails based on their received date. The *On New Email - IMAP* source supports the *Enable Watermark* parameter that enables tp dispatch only messages containing emails received after the last poll executed.
...
Email Connector *On New Email - IMAP* and *On New Email - POP3* sources support *Imap matcher* and *Pop3 matcher* parameters that enables you to custom matcher configurations and provide another way of filtering dispatched messages. Each source matchers can filter emails by dates, addresses, subject, flags and so on.
...
. In the *Answered* field, select the value *EXCLUDED*.
