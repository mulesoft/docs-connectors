= Initiate a Flow When Receiving a New Email Examples - Mule 4
:page-aliases: connectors::email/email-trigger.adoc

Anypoint Connector for Email (Email Connector) provides the *On New Email - IMAP* and *On New Email - POP3* source listeners that initiates a flow by retrieving an email folder from a mailbox for all of the unread emails. A new message is then generated for each email found. The following examples illustrate how to configure these source parameters to identify a new email when initiating a flow.
