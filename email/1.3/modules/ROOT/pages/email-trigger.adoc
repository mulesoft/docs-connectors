...
The following screenshot shows the *Delete after retrieve* configuration for the *On New Email - POP* source:
...
== Configure Enable Watermark Parameter

Email Connector can watermark emails over the IMAP protocol by filtering emails based on their received date. *On New Email - IMAP* source supports *Enable Watermark* parameter that enables dispatching messages only containing emails received after the last poll executed.
...
The following screenshot shows the *Enable Watermark* configuration for the *On New Email - IMAP* source:
...
Email Connector *On New Email - IMAP* and *On New Email - POP3* sources support *Imap matcher* and *Pop3 matcher* parameters that enables to custom matcher configurations and provide another way of filtering dispatched messages. Each source matchers can filter emails by dates, addresses, subject, flags and so on.
...
[start=3]
. For *Subject regex* set the value `IMPORTANT`.

The following screenshot shows the *Imap matcher* configuration for the *On New Email - IMAP* source:
...
Email Connector *On New Email - IMAP*, *On New Email - POP3*, *List - IMAP*, and *List - POP3* operations support the *Attachment naming strategy* parameter that enables you to specify a strategy for naming attachments. You can set the parameter at the global element or override it at the operation level. The parameter has the following options to choose from:

* *NAME* (default) +
Email Connector names the attachment after the value of the `filename` field in the `Content-Disposition` line of the attachment (this is the standard and recommended way of setting the name for an attachment). If the `filename` field is not present or is empty, then the attachment is named `"Unnamed"`.
...
When using *On New Email - IMAP* source and *Imap matcher* parameter, you can specify whether you want the filters to be applied server-side by enabling the *Enable remote search filter* parameter.

This parameter is disabled by default because some email servers are not fully compliant with `rfc-3501` search terms. Enabling the parameter decreases traffic by reducing the amount of emails brought to client-side for processing.

For this feature to be fully functional, the email server should at least
...
. Select the *On New Email - IMAP* source in the Studio canvas.
. On the source configuration screen, enable the *Enable remote search filter* parameter.

The following screenshot shows the *Enable remote search filter* configuration for the *On New Email - IMAP* source:

.Enable remote search filter with On New Email - IMAP source
image::email-enableremote-1.png[Enable Watermark with On New Email - IMAP]

In the XML editor, the configuration `emoteSearchFilterEnabled="true"` is under the `listener-imap` tag:
