...
== Enable Remote Search for IMAP

When using both, On New Email and matchers in IMAP, you can specify whether you want the filters to be applied server-side. By setting remoteSearchFilterEnabled to `true` you are indicating that you want the filters to be applied on the server side.

This featured is disabled by default because some email servers might not be fully compliant with rfc-3501's search terms. Activating it will diminish traffic by reducing the amount of emails brought to client side for processing.
...
.Email Listener (On New Email) enabling Remote Search

This example enables server side filtering to exclude all answered emails.
