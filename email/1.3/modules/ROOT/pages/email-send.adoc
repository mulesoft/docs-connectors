...
The following example illustrates how to send emails and multiple attachments over the SMTP server. The flow initiates with a *Scheduler* component. Then, *Set Variable* components define each attachment media type JSON, text and file. Subsequently, the Email Connector *Send* operation sends the attachments in the email. Then log the result message using a *Logger* component.
