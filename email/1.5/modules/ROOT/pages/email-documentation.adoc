= Email Connector Reference 1.5 - Mule 4
...
=== Allow parsing text file attachment as body message

There is a system property ('parsing.text.attachment.as.body') that allows changing the behavior over the text file attachments parsing, when this file is the only content on the message. The possible values are:  

* true: (default) Preserve the previous behavior and convert the text attachment file on the body message.
* false: The text file attachment is added to the attachment list into the payload and the body is left empty.

To change this behavior you must set the system property parsing.text.attachment.as.body to false. To do this, you can use the different methods described in the following article: https://docs.mulesoft.com/mule-runtime/4.3/mule-app-properties-system[System properties]

=== Jakarta Mail System Properties

Email Connector internally uses the Jakarta Mail library, so you can affect the behavior of some aspects by changing some specific system properties. 
In the Troubleshooting section we have compiled some of the most consulted properties, but for more precise and updated information, we suggest you consult the documentation of that project.
