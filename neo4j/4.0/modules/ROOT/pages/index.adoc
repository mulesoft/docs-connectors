...
Replace `4.0.0` with the version that corresponds to the connector you are using.
...
*Version 4.0.0 Enhancement*: The Neo4j connector now provides comprehensive metadata resolution for improved Anypoint Studio experience.

Neo4j connector metadata is based on the following premises:

* **Dynamic Label Discovery**: Labels are automatically discovered from the database using native Cypher queries
* **Sample-Based Field Resolution**: Metadata fields are generated from actual node properties in the database
* **Comprehensive Type Mapping**: All node properties are included in metadata, not just constraint fields
* **BOLT-Only Operation**: All metadata resolution uses the BOLT connection (no REST API required)

For optimal metadata experience:
* Ensure sample data exists in your Neo4j database for the labels you plan to use
* The connector will display all properties found in sample nodes for comprehensive field suggestions
* Metadata is generated dynamically and reflects the actual data structure in your database
...
+
image::neo4j-operations-studio.png["List of available operations in Studio for Neo4j connector"]
+
[%header,cols="30s,70a"]
|===
|Name | Description
...
+
image::neo4j-config-studio1.png["Global elements properties window"]
+
[%header,cols="30s,70a"]
|===
|Field |Description
...
|BOLT URL |Bolt endpoint URL. *Version 4.0.0*: This is the only required connection parameter.
|===


== Use Case: Node Basic CRUD

image::neo4j-example-flow.png["Studio flow with Listener, Transform Message, Create node, Select nodes, Delete node, and Transform Message"]
...
|===
+
image::neo4j-create-node.png["Create node properties configuration window"]
+
...
image::neo4j-create-node-with-multiple-labels.png["Studio flow with Listener, Transform Message, Create node, Select node, and Transform Message"]
...
|===
+
image::neo4j-execute-create.png["Create node properties configuration window"]
+
...
*Important*: Version 4.0.0 contains breaking changes. If you are upgrading from version 3.x, see xref:neo4j-connector-upgrade-migrate.adoc[Neo4j Connector Upgrade and Migration Guide] for detailed migration instructions.

**Key Changes in Version 4.0.0:**
* **REST URL parameter removed**: Only BOLT URL is required for configuration
* **Enhanced metadata resolution**: Comprehensive field suggestions in Anypoint Studio
* **Minimum requirements updated**: Neo4j 4.4+, JDK 17+, Mule Runtime 4.9.0+
* **Modern Neo4j driver**: Upgraded to Neo4j Java Driver 5.28.8
* **Code quality enhancements**: Improved error handling, modern Java 17+ features, and better internal architecture

== See Also

* xref:neo4j-connector-upgrade-migrate.adoc[Neo4j Connector Upgrade and Migration Guide]
