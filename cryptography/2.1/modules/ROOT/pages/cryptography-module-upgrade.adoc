...
Use *JCE Encrypt* as an alternative to *PGP Encrypt* when operating in a FIPS environment. *PGP Encrypt* is not supported in FIPS mode due to protocol limitations. For more information, refer to xref:cryptography-module-upgrade.adoc#scope-and-limitations-fips[Scope and Limitations (FIPS)] 
...
The following example generates a BCFKS keystore using the symmetric AES algorithm:
+
[source,xml,linenums]
----
