...
. In the project’s `pom.xml file`, change the `mule-cryptography-module` dependency version 2.1.0 to the previous version, and delete the dependencies used from the <dependencies> section.
...
In GPG 2.1 and later, `gpg` isn't directly responsible for encrypting secret keys at rest, and options such as `--s2k-cipher-algo` have no effect. Instead, `gpg-agent` is solely responsible and it provides no interface to change the cipher or digest algorithms in use.
...
Cryptography Module version 2.1.0 supports dynamic algorithm definitions, offering great flexibility by removing the dependency of a predefined list of available values shown in the value provider.
...
* As a custom algorithm format introduced in version 2.1.0, which is detailed in the next section.
...
To enable better support for modern cryptographic configurations, especially in FIPS environments, Cryptography Module version 2.1.0 introduces a custom algorithm format that separates key derivation and encryption components.
...
In Cryptography Module version 2.1, password-based encryption operations now enforce the configuration of both SALT  and `iterationCount` parameters for enhanced security.
...
== JCE Keystore Generation Example

Use *JCE Encrypt* as an alternative to *PGP Encrypt* when operating in a FIPS environment. PGP Encrypt is not supported in FIPS environments because the OpenPGP standard (RFC 4880) mandates the use of RSAES-PKCS1-v1_5, which is strictly prohibited in FIPS mode. For more information, refer to xref:cryptography-module-upgrade.adoc#scope-and-limitations-fips[Scope and Limitations (FIPS)].

To use JCE Encrypt in a FIPS environment, you must first generate a JCE keystore:
...
+
In this example, you generate a BCFKS using the AES symmetric algorithm:
+
[source,xml,linenums]
----
keytool -genseckey -alias <alias-name> -keyalg AES -keysize 256 \
  -keystore keystore.bcfks -storetype BCFKS \
  -providerclass org.bouncycastle.jcajce.provider.BouncyCastleFipsProvider \
  -providerpath /absolute/path/to/bc-fips-1.0.2.5.jar
----
