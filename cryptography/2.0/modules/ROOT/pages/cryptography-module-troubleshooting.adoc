...
[NOTE]
====
**Error: "RSA PKCS#1 v1.5 encryption disallowed"**

**Applies to:** Cryptography Module 2.1.0 and later, FIPS-enabled environments.

This error indicates that a *PGP Encrypt* operation attempted to use RSA PKCS#1 v1.5 for session key encryption.
This mechanism is mandated by the OpenPGP standard (RFC 4880) and is explicitly disallowed in FIPS approved mode.

This is not a configuration issue.
There is no supported workaround to enable PGP Encrypt in FIPS environments.
PGP *Decrypt* and *Validate* operations can still be used in FIPS environments for processing legacy data, as long as the underlying key material and algorithms are FIPS-approved.

If PGP encryption is mandatory, the encryption step must be performed outside the FIPS boundary.
====
