...
Use the *Validate Type* operation to validate that the instance is an instance of (`instanceof`) the expected class.
...
After setting up your debug conditions, xref:anypoint-code-builder::int-debug-mule-apps.adoc#run-debug-mode[run a debug session] and evaluate your Mule application at each breakpoint.
