= Using Studio to Configure LDAP Connector 3.7
...
connection fields.
+
For information about configuring the SSL (Deprecated), Secure SSL, and TLS connection types, see <<ssl-connection-type,Configure an SSL Connection Type (Deprecated)>>, <<secure-ssl-connection-type,Configure a Secure SSL Connection Type>>, and <<tls-connection-type,Configure a TLS Connection Type>>.
+
...
[[ssl-connection-type]]
=== Configure an SSL Connection Type (Deprecated)

IMPORTANT: The SSL connection type is deprecated. Use the <<secure-ssl-connection-type,Secure SSL connection type>> instead, which accepts a Mule TLS context factory and supports FIPS 140-3 compliant deployments.
...
[[secure-ssl-connection-type]]
=== Configure a Secure SSL Connection Type

The Secure SSL connection type uses a Mule TLS context factory for trust store configuration. Use this connection type for FIPS 140-3 compliant deployments. It is also supported in non-FIPS environments with any trust store type provided via *TLS Context* field.

To configure a Secure SSL connection type, use Studio to set up a Secure SSL connection in the LDAP global element:

. In the *Connection* field of the global element, select `Secure SSL Configuration`.
. In the *TLS Context* field, click the plus sign (*+*) to configure a new TLS context, or select an existing one.
. In the TLS context, configure your truststore settings, including the truststore path, password, and type.
. Complete the remaining required fields, including *Principal DN* and *URL*.
. Click *Test Connection* to confirm that Mule can connect with the specified server.
. Click *OK*.

Connect to Secure SSL using port 636.
