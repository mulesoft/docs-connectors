...
The Secure SSL connection type uses a Mule TLS context factory for trust store configuration. Use this connection type for FIPS 140-3 compliant deployments. It is also supported in non-FIPS environments with any trust store type provided via `<tls:context>`.
