...
LDAP Connector supports LDAP over SSL (Deprecated), LDAP over TLS, and a Secure SSL connection type for FIPS 140-3 compliant deployments.
...
* SSL (Deprecated)
+
Communications are encrypted using Secure Sockets Layer (SSL). Accepts a custom truststore path for certificate validation. The SSL connection type is deprecated. Use the Secure SSL connection type instead.
+
* Secure SSL
+
