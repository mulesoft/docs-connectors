...
| Custom TrustStore Path a| String |  Path to the truststore that contains the certificates for secure authentication. To be FIPS 140-2 compliant, the keystore, truststore, and any custom truststore paths you are using,  must be in PKCS#12 format. |  |
...
| Custom TrustStore Path a| String |  Path to the truststore that contains the certificates needed for secure authentication. The custom truststore feature is not FIPS-compliant. |  |
| Custom TrustStore Password a| String |  Password for the custom truststore. |  |
|===
