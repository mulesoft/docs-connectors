...
* <<SSL Configuration Type (Deprecated)>>
...
IMPORTANT: The *SSL Configuration* type is deprecated. Use the <<Secure SSL Configuration Type>> instead, which accepts a Mule TLS context factory and supports FIPS 140-3 compliant deployments.
...
SSL connection provider that accepts a Mule TLS context factory for trust store configuration. This connection type is required for FIPS 140-3 compliant deployments, and is also usable in non-FIPS environments with any trust store type supplied via *TLS Context* field.
