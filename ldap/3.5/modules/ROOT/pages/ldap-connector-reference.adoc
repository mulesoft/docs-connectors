...
| Custom TrustStore Path a| String |  Path to the truststore that contains the certificates needed for secure authentication. Under FIPS 140-2 compliance, KeyStore and TrustStore must be in PKCS#12 format. To ensure FIPS compliance, when using a custom TrustStore path, provide your certificates in PKCS#12 format.  |  |
