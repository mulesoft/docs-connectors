...
+
. Choose *Basic Configuration*, *SSL Configuration*, or *TLS Configuration* from the drop-down list and configure the associated properties:
+
...
| Custom TrustStore Path| Path to the truststore that contains the certificates needed for secure authentication. This feature is not FIPS-compliant.
| Custom TrustStore Password| Password for the custom truststore.
|===
+
In the image above, the placeholder values refer to a configuration file `mule-artifact.properties` placed in the `src/main/resources` folder of your project. You can either enter your credentials into the global configuration properties, or reference a configuration file that contains these values.
+
...
If you configure SSL or TLS for LDAP connector, you can set up a custom truststore that specifies which servers the connector can communicate with. The custom trustore feature is not FIPS-compliant.

This use case shows how to set up a custom truststore for a TLS configuration. The XML for this use case is similar to that of the previous use case. To implement this use case, change the XML for the previous use case as follows:

. Locate the following element in the XML code for Use case 1:
+
[source,xml,linenums]
----
<ldap:config name=:LDAP_configuration" doc:name="LDAP Configuration"
...
----
. Overlay the located element with the following XML code:
+
[source,xml,linenums]
----
