...
This example shows how to use the LDAP *Add entry* and *Delete entry* to add and delete a person from an organizational unit.
...
This example shows how to use extended configuration parameters to set up a custom trust store to tell which servers are allowed to communicate to.
...
To set up a custom trustore, follow the steps in the previous example, except select `TLS Configuration` as the connection type.

Past this code into the XML in the previous example.
