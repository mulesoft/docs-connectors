= LDAP Connector 3.4 Reference - Mule 4
...
** JNDI |  The implementation of the connection to use. |  JNDI |
| Schema Enabled a| Boolean |  If set to `true`, the LDAP connector will use the LDAP schema (works only for LDAP v3) to define the structure of the LDAP entry or map. Must be set to `true` to use DataSense as it will affect the implementing class of `org.mule.module.ldap.api.LDAPEntry` attributes. |  false |
| Referral a| Enumeration, one of:
