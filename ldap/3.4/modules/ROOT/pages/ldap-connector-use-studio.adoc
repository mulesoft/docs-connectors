...
You can configure one of these sources to use with LDAP Connector:
...
. Click the plus sign (*+*) next to the *Connector configuration* field to configure a global element that can be used by all instances of the On Modified Objects source in the app.
...
. In *Mule Palette*, select *LDAP* and then select the desired operation.
...
After configuring Studio, see the xref:ldap-connector-examples-topics.adoc[LDAP Connector Examples] topic for more configuration ideas.
