...
result, the rest of the flow executes. Each execution returns a result that aggregates into a List of results.
...
* OBJECT: Use to search only the entry at the base DN so that only that entry will be returned. This must meet the search filter criteria.
* ONE_LEVEL: This value is used for searching all entries one level under the base DN and does not include entries in the base DN or any entries one level under the base DN.
* SUB_TREE: This value is used for searching all entries at all levels under and including the specified base DN. |  ONE_LEVEL |
