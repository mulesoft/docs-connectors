...
| Resource Owner Security Provider a| String |  The security provider used to authenticate resource owners. Not needed if only the CLIENT_CREDENTIALS grant type is used. See an example with Spring module in xref:mule-runtime::migration-oauth2-provider.adoc[migrating the OAuth2 provider] documentation. |  |
