...
|WHERE Clause |String | WHERE clause that is appended to the VQL query build dynamically. | |
|Fetch Size |Integer |Provides the number of records per page. |`100` |
|Batch Size |Integer |Provides the number of pages per batch.  |`10` |
|===
...
Retrieves object records details using object metadata fields to build a VQL query dynamically and execute on vault to get object details. Place a For-Each/Splitter element after this operation to fetch each object records (page) sequentially.
