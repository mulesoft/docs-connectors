...
. In the *General* tab, in *Connection*, select the <<basic-auth, Basic authentication>> method to configure.
+
...
|Client Id | Client ID to send with each request header to the Veeva Vault instance for tracking purposes.
|===
