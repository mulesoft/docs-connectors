...
== Example: MCP Tools
 ...
 This Mule application showcases the use of the *[MCP] Tooling* operation to interact with OpenAI via the MCP connector. This operation is used to create autonomous agents that can use external tools whenever a prompt can't be answered directly by the AI model. This operation is configured to execute the tool defined in the payload and includes the tool's response in the final output. In this example, it's configured to use a tool to get the current stock price for a given ticker.

[source,xml,linenums]
----
<flow name="TestMcpToolExecutionFlow">
	<http:listener path="/mcp-tool-execution" doc:name="Listener" doc:id="zecmdx" config-ref="HTTP_Listener_config" />
	<set-variable doc:id="7edeecec-4d47-4bfe-a699-23987b60ac14" doc:name="Set Variable" variableName="testPayload">
		<value><![CDATA[#[%dw 2.0
			output application/java
			---
			{
				'template': 'You are a helpful financial assistant',
				'instructions':'Use the available tools to answer the request. If the tool is called, you must include the tool execution result in your final answer.',
				'dataset': 'What is the current stock price for GOOG?',
				'tools': [
                    {
                        'type': 'function',
                        'function': {
                            'name': 'get_stock_price',
                            'description': 'Gets the latest market price for a given stock ticker.',
                            'parameters': {
                                'type': 'object',
                                'properties': {
                                    'ticker': {
                    ...
                    },
                    {
                        ...
                        'function': {
                            ...
                            'description': 'Retrieves a summary of a company\'s annual financial report.',
                            'parameters': {
                                'type': 'object',
                                'properties': {
                                    'ticker': {
