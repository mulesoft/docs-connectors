...
This Mule application showcases the use of the *[MCP] Tooling* operation to interact with OpenAI via the MCP Connector. This operation enables you to execute tools hosted on an MCP server directly through user-defined instructions. In this example, the operation uses the tools to check the inventory status for a specific part number, `MULETEST0`.
...
This feature showcases the ability to control the formatting of LLM outputs directly from the connector using the *Additional Request Attributes* parameter. With this parameter, you can now specify a JSON object with attributes that are flattened into the root level of the request payload. In this example, you can directly pass a `response_format` with type `json_schema` followed by the corresponding JSON schema. This is included in the request sent to the Inference API, enabling consistent and structured output in the provided JSON schema. 
