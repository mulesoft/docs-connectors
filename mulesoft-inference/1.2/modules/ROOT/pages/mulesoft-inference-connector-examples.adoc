...
This feature showcases the ability to control the formatting of LLM outputs directly from the connector. Previously, it wasn't straightforward to include a `response_format` in requests. With this additional request attribute, the `response_format` can now be added to the request. In this example, you can directly pass a `response_format` JSON object for any operation, and it will be included in the request sent to the Inference API, enabling consistent and structured outputs.
