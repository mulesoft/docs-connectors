...
This feature showcases the ability to control the formatting of LLM outputs directly from the connector. With this additional request attribute, you can now include a JSON object containing parameters that will be flattened into the root level of the request payload. In this example, you can directly pass a `response_format` JSON object for any operation, and it will be included in the request sent to the Inference API, enabling consistent and structured outputs. 
