...
Before configuring the MCP tooling operation, ensure that you download the MCP connector from Anypoint Platform Exchange.
...
NOTE:
. The XML DSL tag used to add these attributes varies by operation, so please refer to the specific operation's documentation to identify the supported tag.
. For Gemini Inference, additional parameters can be included within the `GenerationConfig` property of the request payload.
