...
. In the configuration XML, place your cursor before the `<flow/>` element and type `mac-inference`.
+
Ensure that the cursor is not inside the `<flow/>` element.
. Select the `mac-inference:config` snippet:
+
The snippet adds the following code:
+
[source,xml]
----
<mac-inference:config provider="#[]" configType="#[]" filePath="#[]" modelName="#[]" name="name" />
----
. Add the attributes to complete the configuration, for example:
+
[source,xml]
----
<mac-inference:config configType="Environment Variables" filePath="-" provider="OPENAI" modelName="gpt-4o" name="MuleSoft_Inference_Configuration">
</mac-inference:config>
----
+
This example shows a flow named "Chat_Prompt" with two global elements defined (`http:listener-config` and `mac-inference:config`)
+
[source,xml]
----
 <http:listener-config basePath="/path" name="HTTP-Listener-config">
  ...
  <mac-inference:config configType="Environment Variables" filePath="-" provider="OPENAI" modelName="gpt-4o" name="MuleSoft_Inference_Configuration"></mac-inference:config>
