= Google Pub/Sub Connector 1.2 Reference
...
| Default Snapshot Name a| String |  Default snapshot for your project. Used in operations. It can be overridden at the operation config level. |  |
...
|===
| Name | Type | Description | Default Value | Required
| Authentication Method a| String |  Authentication method to use for Google Cloud authentication. Supports *Workload Identity Federation with MuleSoft IdP* and *Service Account Credentials* authentication methods. |  |
| MuleSoft IdP Configuration a| <<WifMuleSoftIdpConfig>> |  MuleSoft Identity Provider configuration for WIF authentication. Used when *Authentication Method* is *Workload Identity Federation with MuleSoft IdP*. Supports both manual configuration and WIF config file input with enhanced stream-based processing. |  |
| Private key ID a| String |  ID of the private key for the service account. It is a part of the service-credentials created in PubSub. Used when *Authentication Method* is *Service Account Credentials*. |  | x
...
| Override Host a| String |  Override host for the Google Pub/Sub API endpoint. If specified along with port, all API calls are directed to this host instead of the default public endpoint. |  | 
| Override Port a| Number |  Override port for the Google Pub/Sub API endpoint. If specified along with host, all API calls are directed to this port instead of the default public endpoint. |  | 
...
[[AcknowledgeMessage]]
== Acknowledge Message
`<pubsub:acknowledge-message>`

Acknowledges a message using the provided acknowledgment ID. Use this operation in manual acknowledgment mode to explicitly acknowledge messages.
...
| Acknowledgment ID a| String | The unique acknowledgment ID received in the message attributes. | | x
...
Negatively acknowledges (NACKs) a message using the provided acknowledgment ID. This causes the message to redeliver.
...
** AUTO
** MANUAL a|  Acknowledgment mode for message processing:

* `AUTO`
+
Messages are automatically acknowledged on success and NACKed on error. 

* `MANUAL`
+
Messages must be explicitly acknowledged using the *Acknowledge Message* operation. |  `AUTO` | 
| Config Ref a| ConfigurationProvider |  Name of the configuration used to execute this component. |  | x
...
** WAIT
** DROP
** FAIL |  Strategy that the runtime applies when the flow receives more messages than it has the capacity to handle. |  WAIT | 
...
[[WifMuleSoftIdpConfig]]
=== Wif MuleSoft Idp Config

Configuration for *Workload Identity Federation with MuleSoft IdP* authentication.

[%header,cols="20s,25a,30a,15a,10a"]
|===
| Field | Type | Description | Default Value | Required
| Wif Config File Path a| String | Path to the WIF configuration file. If provided, other fields are populated from this file. | | x
| Client Id a| String | Client ID of the MuleSoft Connected App. This identifies the application requesting authentication. | | x
| Client Secret a| String | Client secret of the MuleSoft Connected App. This is used to authenticate the application with MuleSoft. | | x
|===
