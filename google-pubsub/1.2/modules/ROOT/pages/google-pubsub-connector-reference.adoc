...
|===
| Name | Type | Description | Default Value | Required
| Authentication Method a| String |  Authentication method to use for Google Cloud authentication. Supports Service Account, WIF Config File, and MuleSoft IdP authentication methods. |  |
| MuleSoft IdP Configuration a| <<WifMuleSoftIdpConfig>> |  MuleSoft Identity Provider configuration for WIF authentication. Used when *Authentication Method* is `wif_mulesoft_idp`. Supports both manual configuration and WIF config file input with enhanced stream-based processing. |  |
| Private key ID a| String |  ID of the private key for the service account. It is a part of the service-credentials created in PubSub. Used for `service_account` authentication method. |  | x
...
Configuration for MuleSoft Identity Provider-based Workload Identity Federation authentication.
