...
* `BLOCK`
+
Specify limits for the memory size or number of outstanding elements (messages
in which Ack() or Nack() was not performed) to limit the number of receiving messages.
If the memory size or number of outstanding elements exceeds the specified limit, the receiver
does not receive more messages.

* `IGNORE`
+
Specify limits for *Max Outstanding Request Size* and *Max Outstanding Element Count*
to control the number of receiving messages. |  BLOCK |
...
Snapshot resource. You may use snapshots in seek operations, which manage message acknowledgments in bulk.
...
Time that the snapshot is guaranteed to exist up until. A newly-created snapshot
expires no later than seven days (the age of the oldest unacked message in the
subscription) from the time of its creation. Its exact lifetime is determined
