...
This operation has the `void` return type. However, if you use *CompletionCallback*,
the output is derived from the ApiFuture result, so the return type can be `null`, or
`messageId` in which the unique identifier of the sent message within the topic
is returned.
