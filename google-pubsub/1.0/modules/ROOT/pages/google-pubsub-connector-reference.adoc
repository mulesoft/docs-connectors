...
Creates a snapshot from the requested subscription. Use snapshots in seek operations, which manages message acknowledgments in bulk. This enables you to set the acknowledgment state of messages in an existing subscription to the state captured by a snapshot.
