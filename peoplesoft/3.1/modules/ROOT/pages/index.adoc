= Oracle PeopleSoft Connector 3.1 
...
[NOTE]
====
To run the connector on PeopleTools 8.61, you must add the `-DTM_ALLOW_NOTLS=Y` flag to both of these locations:

* In Studio, *Run* > *Run Configurations* window > *Arguments* tab > *VM arguments*
* In the `AnypointStudio.ini` file, such as after the `-Dmule.deployment. forceParseConfigXmls=true` flag
====
...
* Visit Oracle's https://docs.oracle.com/en/applications/peoplesoft/peopletools/index.html[PeopleSoft Component Interface API site]
