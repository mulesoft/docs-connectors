...
Engine names display differently than in previous Scripting Module versions. When you see different engine names in the Studio configuration tab and Configuration XML tab, this occurs because the engine company defines and provides an identifier for both the engine and language name, for example:
...
When you see Oracle Nashorn (ECMASCript) engine as an option to select in the Studio configuration tab without the necessity to install or configure it, this indicates that some versions of JVM (Java Virtual Machine) previously installed and use the engine.
