...
Upgrade Anypoint Connector for Scripting module (Scripting module) to version 2.1.0.
...
. Click *Finish*.
. Verify that the `mule-scripting-module` dependency version is `2.1.0` in the Mule project file.
...
. In the project’s `pom.xml` file, change the `mule-scripting-module` dependency version `2.1.0` to the previous version, and delete the engine dependency used from the `<dependencies>` section.
