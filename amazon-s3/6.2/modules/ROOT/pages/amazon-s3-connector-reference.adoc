= Amazon S3 Connector 6.2 Reference - Mule 4
...
* When you upload large file content in parts (from, for example, a FTP Connector *Read* operation or SFTP Connector *On New or Updated File* source of a Mule app), you must configure the *non repeatable stream* strategy in these operations. Additionally, in the *Upload part* operation specify in the *Content length* field the number of bytes to consume from the content.
* When you upload many small file contents, you can either configure *repeatable stream* or *non repeatable stream* strategy in the FTP or SFTP Connector operations. In the *Upload part* operation, you can either specify or not in the *Content length* field the entire file size you want to upload.
