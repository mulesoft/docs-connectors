= Using Anypoint Studio to Configure Amazon S3 Connector 7.0 - Mule 4
...
If omitted, a default queue name with prefix `mule-s3-trigger-<event-type>-queue-<bucket-name>-folder-` is created and used. Where even-type is either `new` or `deleted`.
Folder name specified in the `General` section of configuration on which event is triggered, will be used as the queue name. If any special character other than `a-z` , `A-Z` , `0-9` , `\_` , `-` is used in folder name, then from default queue name these characters will be replaced by `_` .
Also if length of the default queue name goes beyond `80 in size`, rest will be trimmed off.
