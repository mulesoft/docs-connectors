= Upgrading and Migrating Amazon S3 Connector to Version 8.x 
:page-aliases: connectors::amazon/amazon-s3-connector-upgrade-migrate.adoc

Upgrade Anypoint Connector for Amazon S3 (Amazon S3 Connector) to version 8.x.
...
* As per AWS updates, the *Display Name* field has been deprecated and is no longer supported in the *Owner* and *Grantee* object types.
* The AWS SDK version is upgraded to 2.35.3.
* The `netty-common` library is upgraded to version 4.2.5-Final.
...
Follow these steps to perform the upgrade to version 8.x:
...
. Click *Finish*.
. Verify that the Amazon S3 dependency version is 8.x in the `pom.xml` file in the Mule project.
