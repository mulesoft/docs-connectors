= Amazon S3 Connector 8.0 Reference 
...
| Try Default AWSCredentials Provider Chain a| Boolean |  If `true`, obtains credentials from the AWS environment. For more information, refer to https://docs.aws.amazon.com/sdk-for-java/latest/developer-guide/credentials.html[Provide Temporary Credentials to the SDK. If you enable this option, the SDK manages the token renewal because the connector doesn't manage the credentials.
]. |  `false` | 
| Max Connections a| Number |  Defines the maximum number of outbound connections that are kept open at the same time. Unlimited by default. |  `-1` | 
...
Upon receiving this request, Amazon S3 concatenates all the parts in ascending order by part number to create a new object. In the *Complete Multipart Upload* request, you must provide the parts list and ensure that it's complete.
...
You can store individual objects of up to 5 TB in Amazon S3. You create a copy of your object up to 5 GB in size in a single atomic action using this API. However, to copy an object greater than 5 GB, you must use the multipart upload *Upload Part - Copy API*. For more information, refer to https://docs.aws.amazon.com/AmazonS3/latest/userguide/CopyingObjectsMPUapi.html[Copy Object Using the REST Multipart Upload API].

All copy requests must be authenticated. Additionally, you must have read access to the source object and write access to the destination bucket. For more information, refer to https://docs.aws.amazon.com/AmazonS3/latest/API/RESTAuthentication.html[REST Authentication].
...
| Part Number a| Number |  Required if *Method* is `UPLOAD_PART`. If the content size is equal to or less than 5 MB, then *Part Number* is `1`. If not, use a loop to increase the number (1, 2, 3, and so on).  |  | 
| Upload Id a| String |  Required if *Method* is `ABORT_MULTIPART_UPLOAD`, `COMPLETE_MULTIPART_UPLOAD`, or `UPLOAD_PART`.  |  | 
...
|===
|Type |Any
|Attributes Type | <<GetObjectOperationAttributes>>
|===
...
| Configuration | String | Name of the configuration to use. | | x
| Bucket a| String |  Name of the bucket. |  | x
| Bucket Tags a| Object | Bucket tags. |  |
| Config Ref a| ConfigurationProvider |  Name of the configuration to use to execute this component. |  | x
| Reconnection Strategy a| * <<Reconnect>>
* <<ReconnectForever>> |  Retry strategy in case of connectivity errors. |  |
|===
...
** BUCKET_OWNER_FULL_CONTROL |  Canned ACL to apply to the object. |  |
| Config Ref a| ConfigurationProvider |  Name of the configuration to use to execute this component. |  | x
| User Metadata a| Object |  Custom user metadata. Use only US-ASCII characters for user metadata. |  |
...
| Sse Kms Key Id a| String |  AWS Key Management System key ID used for Server Side Encryption of the Amazon S3 object. You must specify the *Server Side Encryption* field when you set the *Sse Kms Key Id* field.  |  |
| Sse Customer Key a| String | Represents a customer-provided key to use with Amazon S3 server-side encryption. |  |
| Sse Kms Key Id a| String | AWS Key Management System key ID to use for server-side encryption of the Amazon S3 object. The *Server Side Encryption* field must be specified when this field is used. |  | 
| Sse Customer Key MD5 a| String |  Specifies the base64-encoded 128-bit MD5 digest of the encryption key according to RFC 1321. Amazon S3 uses this header for a message integrity check to ensure that the encryption key is transmitted without error. |  |
...
| Checksum SHA256 a| String | Checksum SHA256. |  |
| E Tag a| String | ETag. |  | 
| Last Modified a| String | Last modified. |  | 
...
|===
| Field | Type | Description | Default Value | Required
| Error Document Key a| String | Specifies the key of the error document that Amazon S3 returns when a request results in an error. |  |
| Index Document Suffix a| String | Defines the suffix that Amazon S3 adds to requests for the bucket root or any subdirectories. |  |
| Redirect All Requests To a| <<RedirectRule>> | Specifies a rule to redirect all requests. |  |
| Routing Rules a| Array of <<RoutingRule>> | Defines an array of rules that specify redirects based on request criteria. |  |
|===
...
[[GetObjectOperationAttributes]]
=== Get Object Operation Attributes

Configures the get object operation attributes response type. 
...
| Content Range a| String | Content range. |  | 
...
[NOTE]
The Role connection type is supported only for standalone, CloudHub 2.0, or Runtime Fabric deployments. The Role connection type is not supported for CloudHub deployments unless access key and secret key are provided.
...
| Tags a| Object | Tags. |  |
| Custom Sts Endpoint a| String | Custom STS endpoint. The custom STS endpoint might not work if `DefaultAWSCredentialsProviderChain` is used. Instead, use AWS environment variables such as `AWS_STS_REGIONAL_ENDPOINTS`, `AWS_REGION`, and `AWS_ENDPOINT_URL`. |  |
|===
