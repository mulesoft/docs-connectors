= Amazon S3 Connector 6.4 Reference - Mule 4
...
| Test Connection Bucket Name a| String |Validates the AWS S3 client connection using the provided bucket name.This field is mandatory when a FIPS compliant endpoints is specified in the Custom Service Endpoint. |  |
...
[IMPORTANT]
Current Limitations: The ListBuckets API does not support FIPS-compliant endpoints (e.g., https://aws.amazon.com/compliance/fips/[AWS FIPS endpoints]). This is a known https://github.com/aws/aws-sdk/issues/521[issue] and has been logged for future enhancement.
