...
+
NOTE: When specifying FIPS-compliant endpoints in the *Custom Service Endpoint* field, users must also provide the bucket name in the *Test Connection Bucket Name* field. For non-FIPS compliant endpoints, users have the option to provide the bucket name or leave it unspecified.
