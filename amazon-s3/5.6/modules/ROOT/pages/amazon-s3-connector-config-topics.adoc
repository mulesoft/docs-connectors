...
Amazon S3 source operations use Amazon SQS for notification of when a new object is created in an S3 bucket. 
...
Additionally, to enable the queue to receive messages when a new object is created in an S3 bucket, you must create the policy for the queue in this format:
...
In this scenario, the user must have permission to create a queue and to also set the queue attributes:
...
When the queue is created by the source operation, an SQS policy is added to the queue when the S3 object is created in the bucket, in the following format:
