...
There are two scenarios for the `On New Object` and `On Deleted Object` source operations:

. The <<queue-is-created,queue is created>> and specified
. The <<queue-created-by-source,queue will be created>> by the source

[[queue-is-created]]
== Queue is Already Created

To correctly use a source operation that already exists, you must enable these actions for the Amazon SQS queue:
...
Additionally, to enable the queue to receive messages when a new object is created in an S3 bucket, you must create permission for the action in this format:
...
== Queue Created by Source Operation

If the queue is created by the source operation, the queue is created with the prefix `MULE-S3-TRIGGER`. 

In this scenario, you must have permission to create a queue and to also set the queue attributes:
...
When the queue is created by the source operation, an S3 policy is added to the queue when the S3 object is created in the bucket, in the following format:
...
In this example, the *SPECIFIC_ACTION* is `s3:PutObject` or `s3:DeleteObject`, which is based on the specified source operation.
