...
You must also add the following permissions for this source:
...
| Scheduling Strategy a| scheduling-strategy |  Configures the scheduler that triggers the polling. |  | x
| Redelivery Policy a| <<RedeliveryPolicy>> |  Defines a policy for processing the redelivery of the same message. |  |
| Notification Queue Name a| String |  Amazon SQS queue name used to notify users when a new object is created in an S3 bucket. If omitted, `MULE-S3-TRIGGER-NEW-QUEUE-<'bucket-name'>` is used. |  |
