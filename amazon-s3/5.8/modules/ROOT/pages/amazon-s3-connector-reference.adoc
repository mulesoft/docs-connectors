...
| Notification Queue Name a| String |  +++Amazon SQS queue name used to notify users when a new object is created or deleted in an S3 bucket. If omitted, a default queue name with prefix MULE-S3-TRIGGER-<'event type'>-QUEUE-<'bucket-name'> is created and used. NOTE: As the default queue is created (if not specified) and the source uses SQS:Received to poll for the event notifications connector needs following SQS permissions: CreateQueue, sqs: GetQueueAttributes, sqs: SetQueueAttributes, sqs: GetQueueUrl, sqs: ReceiveMessage, sqs: DeleteMessage+++ |  | {nbsp}
| Notification Configuration Name a| String |  +++Specifies the name of the notification configuration that will be used in the trigger. Notification configuration must exist and must be of Event Type 's3:ObjectCreated:*' in case of On New Object trigger and of Event Type 's3:ObjectRemoved:*' in case of On Deleted Object trigger. Configurations with event of same type can not have overlapping prefix or suffix configuration. If omitted, a default notification configuration with prefix MULE-S3-TRIGGER-<'event-type'>-OBJECT-CONF-<'bucket-name'> is created and used.+++ |  | {nbsp}
| Reconnection Strategy a| * <<reconnect>>
* <<reconnect-forever>> |  A retry strategy in case of connectivity errors. |  |
|===
