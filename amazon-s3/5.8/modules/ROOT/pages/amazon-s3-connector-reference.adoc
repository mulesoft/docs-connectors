...
| Source Key a| String |  Source bucket key under which the source object to copy is stored. |  | x
...
| Destination Key a| String |  Destination bucket key under which the new, copied object is stored. |  | x
| Upload Id a| String |  ID of the existing, initiated multipart upload with which this new part is associated with. |  | x
| Part Number a| Number |  Part number describing this part's position relative to the other parts in the multipart upload. |  | x
...
| Last Modified a| DateTime | Date, according to Amazon S3, when this object is last modified. |  |
...
| Replication Status a| String | Replication status of the object if it is from a bucket that is the source or destination in a cross-region replication. |  |
| Requester Charged a| Boolean | If a bucket is enabled for Requester Pays, then any attempt of operation from it without Requester Pays enabled results in a 403 error and the bucket owner is charged for the request. Enabling Requester Pays disables the ability to have anonymous access to bucket. |  |
| Restore Expiration Time a| DateTime | New restore expiration time for the object. |  |
| Sse Algorithm a| String | Specifies the server-side encryption algorithm to use when Amazon S3 creates an object. |  |
...
** ONEZONE_IA
** REDUCED_REDUNDANCY
** GLACIER | Storage class of the object. |  |
| User Metadata a| Object | User metadata. |  |
| Version Id a| String | Version ID of the associated Amazon S3 object if available. |  |
|===
...
| Last Modified Date a| DateTime | Date, according to Amazon S3, when this object result is last modified. |  |
