= Using Anypoint Studio to Configure DocuSign Connector 1.1 - Mule 4
...
* <<jwt-connection, JWT Connection Provider (JWT Grant)>>
...
[[jwt-connection]]
=== JWT Connection Provider (JWT Grant)

Use JWT Grant for server-to-server or machine-to-machine communication for authentication without user involvement. The client authenticates using a signed JWT, which the authorization server or resource server verifies.

Enter the following information on the *General* tab of the *Global Element Properties* screen to configure JWT Connection Provider (JWT Grant):

[%header,cols="30s,70a"]
|===
|Field |User Action
| Base Uri  | Parameter base URI for each instance or tenant. In the DocuSign app, go to *Settings > Apps and Keys > My Account Information* to find the account base URI and then add `/restapi` to the end, for example, `\https://demo.docusign.net/restapi`.
| Integration Key  |  Integration key that identifies the integration and links to its configuration values with the service provider.
| Impersonated user id  |  The user ID is a unique identifier associated with the user in the DocuSign system. 
| Requested Scopes |  A space-delimited list of the scopes the app needs. For JWT Grant, this must include the `impersonation` scope. 
| Private Key |  The RSA private key.
|===

This screenshot shows an example of configuring the *General* tab for JWT Connection Provider (JWT Grant):

image::jwt-general-tab.png["General tab for JWT Grant authentication"]
