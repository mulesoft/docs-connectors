= Google BigQuery Connector 1.1 Reference - Mule 4
...
|  offline |
| Prompt a| String | Prompt. |  consent |
| Refresh Token Condition a| String |  Refresh condition. |  |
| Protocol a| Enumeration, one of:
...
Get results of a query. This operation makes an HTTP GET request to the /projects/{projectId}/queries/{jobId} endpoint.
...
| Page Token a| String |  Token for continuing a previous list request on the next page. Set this parameter to the value of `nextPageToken` from the previous response. This parameter takes precedence over Start Index and Max Results.|  |
