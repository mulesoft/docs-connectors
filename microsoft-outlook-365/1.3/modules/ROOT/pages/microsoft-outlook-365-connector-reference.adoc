= Microsoft Outlook 365 Connector 1.3 Reference - Mule 4
include::reuse::partial$non-inclusive-banner.adoc[]

NOTE: All permissions for this connector must be assigned under the Microsoft Graph API. 
...
Sends the message specified in the request body. The message is saved in the Sent Items folder by default. This operation limits the size of the attachment you can add to 3 MB.
...
While using the *On New Email* source, Microsoft Outlook 365 Connector handles the watermark value internally by updating the latest email's `receivedDateTime` to prevent reprocessing of the same emails. To reprocess older emails, redeploy the application to delete the watermark value stored in the object store. Depending on your use case, deleting the watermark value must be done with caution as this might reprocess the emails more than once.
