...
Gets a list of event objects in the user's mailbox. The list contains single instance meetings and series masters. Currently, this operation returns event bodies in only HTML. If not specified by the *Top* parameter, the default page size is 300.
...
Gets the messages in the signed-in user's mailbox, including the Deleted Items and Clutter folders. If not specified by the *Top* parameter, the default page size is 300.
